...
usage() {
  echo "usage: $0 [--operator-only --launch] --mednafen PATH --bios PATH --bios-sha256 HEX --disc PATH --disc-sha256 HEX --trace PATH --validator PATH --manifest PATH [--skip-frames DEC] [--frame-limit DEC] [--press-start-frame DEC] [--press-start-length DEC]" >&2
}
...
while (($#)); do
  case "$1" in
    ...
    --mednafen|--bios|--bios-sha256|--disc|--disc-sha256|--trace|--validator|--manifest|--skip-frames|--frame-limit|--press-start-frame|--press-start-length)
      (($# >= 2)) || { usage; exit 2; }
...
[[ "$skip_frames" =~ ^[0-9]+$ && "$frame_limit" =~ ^[1-9][0-9]*$ &&
   "$press_start_frame" =~ ^[0-9]+$ && "$press_start_length" =~ ^[1-9][0-9]*$ ]] || exit 1
...
umask 077
{
  ...
  printf 'bios_sha256=%s\ndisc_sha256=%s\nskip_frames=%s\nframe_limit=%s\npress_start_frame=%s\npress_start_length=%s\n' \
    "$(lower "$bios_sha256")" "$(lower "$disc_sha256")" "$skip_frames" "$frame_limit" "$press_start_frame" "$press_start_length"
