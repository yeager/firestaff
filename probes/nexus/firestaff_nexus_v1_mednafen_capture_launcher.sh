...
while (($#)); do
  case "$1" in
    ...
    --mednafen|--bios|--bios-sha256|--bios-region|--disc|--disc-sha256|--trace|--validator|--menu-bpk|--menu-bpk-sha256|--dm-bin|--dm-bin-sha256|--dgn|--dgn-sha256|--manifest|--replay-trace-fnv|--replay-dgn-fnv|--replay-bitmap-fnv|--replay-epoch)
      (($# >= 2)) || { usage; exit 2; }
...
: "${mednafen:?}" "${bios:?}" "${bios_sha256:?}" "${bios_region:?}" "${disc:?}" "${disc_sha256:?}" \
...
umask 077
{
  ...
  printf 'bios_sha256=%s\nbios_region=%s\ndisc_sha256=%s\nmenu_bpk_sha256=%s\ndm_bin_sha256=%s\ndgn_sha256=%s\n' "$bios_sha256" "$bios_region" "$disc_sha256" "$menu_bpk_sha256" "$dm_bin_sha256" "$dgn_sha256"
...
FIRESTAFF_NEXUS_TRACE_OUTPUT="$trace" "$mednafen" "$bios_option" "$bios" "$disc"
[[ -s "$trace" ]] || exit 1
