...
static int try_candidate(
    ...
    struct MemoryGraphicsDatInputCommandQueueResult_Compat* inputResult,
    struct MemoryGraphicsDatHeader_Compat* header) {
    struct MemoryGraphicsDatState_Compat fileState = {0};
    enum MemoryGraphicsDatInputSignal_Compat inputs[3] = {
        MEMORY_GRAPHICS_DAT_INPUT_SIGNAL_NONE,
        MEMORY_GRAPHICS_DAT_INPUT_SIGNAL_ADVANCE,
        MEMORY_GRAPHICS_DAT_INPUT_SIGNAL_ADVANCE
    };
    ...
    ok = F0479_MEMORY_RunInputCommandQueueMini_Compat(
        graphicsDatPath,
        &fileState,
        dialogGraphicIndex,
...
int main(int argc, char** argv) {
    ...
    for (dialogIndex = dialogStart; dialogIndex < dialogLimit; ++dialogIndex) {
        for (viewportIndex = 0; viewportIndex < viewportLimit; ++viewportIndex) {
            memset(&inputResult, 0, sizeof(inputResult));
            attempts++;
            if (!try_candidate(graphicsDatPath, dialogIndex, viewportIndex, &inputResult, &header)) {
                continue;
            }
            printf("ok\n");
            ...
            printf("requestedInputCount=%u\n", inputResult.requestedInputCount);
            printf("mappedInputCount=%u\n", inputResult.mappedInputCount);
            printf("advanceInputCount=%u\n", inputResult.advanceInputCount);
            printf("idleInputCount=%u\n", inputResult.idleInputCount);
            printf("completedQueueLength=%u\n", inputResult.typedQueue.queue.completedQueueLength);
            printf("handledTickCommandCount=%u\n", inputResult.typedQueue.handledTickCommandCount);
            F0479_MEMORY_FreeInputCommandQueueMini_Compat(&inputResult);
            F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
            return 0;
        }
    }

    fprintf(stderr, "failed: no input-command queue candidate succeeded in %u attempts\n", attempts);
    F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
    return 1;
}
