...
/* The corpus audit identifies raw Structure1B selector values but does not
 * itself exercise a runtime pose. This proves that a genuine Track 1 level
 * reaches the package gate before any unproved selector can borrow decoded
 * MNS pixels. It does not guess which byte maps to which material. */
static int prove_unbound_structure1b_selector_blocks(
    Nexus_V1_Engine *engine, int *out_level, int *out_x, int *out_y)
{
    int level_index;
    ...
    for (level_index = 0; level_index < 16; ++level_index) {
        ...
        for (y = 0; y < NEXUS_MAX_MAP_SIZE; ++y) {
            ...
            for (x = 0; x < NEXUS_MAX_MAP_SIZE; ++x) {
                ...
                if (engine->current_level.squares[y][x] == 0 ||
                ...
                if (!plan && receipt->status ==
                        NEXUS_V1_DGN_RENDERER_HANDOFF_BLOCKED_STRUCTURE1B_SELECTOR &&
                    receipt->static_mns_source_pair_bound &&
                    !receipt->structure1b_selector_binding_proven &&
                    !receipt->uses_static_mns_material_route &&
                    !receipt->uses_bpk_material_route &&
...
static int prove_structure1f_direct_entries_reach_selector_gate(
    Nexus_V1_Engine *engine, int *out_level, int *out_x, int *out_y,
    int *out_entry_count)
{
    int level_index;
        ...
        for (entry_index = 0;
            ...
            for (direction = 0; direction < 4; ++direction) {
                ...
                if (!plan && receipt->status ==
                    ...
                    if (out_entry_count) {
                        /* The selector gate fires before a render plan is
                         * constructed, so retain the selected original
                         * Structure1F record rather than claiming it became
                         * a drawable plan command. */
                        *out_entry_count = 1;
                    }
                    return 1;
                }
...
int main(int argc, char **argv) {
    ...
    printf("Structure3: declared=%d valid=%d bytes=%d nonzero=%d transitions=%d "
           ...
           "complete-wall-sensor-control-destination-tuples=%d "
           ...
           "complete-alcove-payload-rotation-pairs=%d "
    ...
    if (prove_unbound_structure1b_selector_blocks(
            &engine, &missing_level, &missing_x, &missing_y)) {
        printf("Fail-closed Structure1B selector: level=%d cell=%d,%d\n",
               missing_level, missing_x, missing_y);
    } else {
        fprintf(stderr, "FAIL: no real unbound Structure1B selector reached the package no-draw gate\n");
        nexus_v1_shutdown(&engine);
        return 1;
    }
    if (prove_structure1f_direct_entries_reach_selector_gate(
            &engine, &structure1f_level, &structure1f_x, &structure1f_y,
            &structure1f_entry_count)) {
        printf("Structure1F selector gate: level=%d cell=%d,%d entries=%d\n",
               structure1f_level, structure1f_x, structure1f_y,
               structure1f_entry_count);
    } else {
        fprintf(stderr, "FAIL: no direct Structure1F entry reached the real selector no-draw gate\n");
        nexus_v1_shutdown(&engine);
        return 1;
    }
    ...
    nexus_v1_shutdown(&engine);
    return receipt.readable_level_count == receipt.expected_level_count &&
