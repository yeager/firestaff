...
static void p_reset_unpopulated(void) {
    unsigned char squares[3][3] = { 0 };
    dm1_v2_presentation_mode_reset();
    dm1_v2_presentation_mode_set_modern_pack_available(1);
    dm1_v2_presentation_mode_set(DM1_V2_PM_V22_MODERN);
    m11_v22_shape_cache_update(0, squares);
    check(m11_v22_shape_cache_populated() == 1, "reset: populated after update");
    m11_v22_shape_cache_reset();
    check(m11_v22_shape_cache_populated() == 0, "reset: unpopulated after reset");
    check(m11_v22_shape_cache_get(1, 0) == NULL, "reset: D1C NULL after reset");
}
