...
int main(int argc, char** argv) {
    ...
    unsigned int viewportGraphicIndex = 694;
    unsigned int firstFrameNumber = 1;
    unsigned int frameCount = 3;
    struct BootSequenceRuntimeResult_Compat result;
    ...
    if (!F9006_RUNTIME_RunBootSequence_Compat(
            graphicsDatPath,
            outputPrefix,
            dialogGraphicIndex,
            ...
            frameCount,
            &result)) {
        fprintf(stderr, "failed: boot sequence did not complete\n");
        return 1;
    }
    ...
    printf("firstFrameNumber=%u\n", result.runtime.sequence.firstFrameNumber);
    printf("requestedFrameCount=%u\n", result.runtime.sequence.requestedFrameCount);
    printf("completedFrameCount=%u\n", result.runtime.sequence.completedFrameCount);
    ...
    printf("bootSequenceCompleted=%d\n", result.bootSequenceCompleted);
    return 0;
}
