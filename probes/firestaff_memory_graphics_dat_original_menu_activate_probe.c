...
static int try_candidate(
    ...
    struct MemoryGraphicsDatMenuActivateResult_Compat* activateResult,
    ...
    ok = F0479_MEMORY_RunMenuActivateMini_Compat(
        graphicsDatPath,
        &fileState,
        dialogGraphicIndex,
...
int main(int argc, char** argv) {
    ...
    for (dialogIndex = dialogStart; dialogIndex < dialogLimit; ++dialogIndex) {
        for (viewportIndex = 0; viewportIndex < viewportLimit; ++viewportIndex) {
            memset(&activateResult, 0, sizeof(activateResult));
            attempts++;
            if (!try_candidate(graphicsDatPath, dialogIndex, viewportIndex, &activateResult, &header)) {
                continue;
            }
            printf("ok\n");
            ...
            printf("finalSelectionIndex=%u\n", activateResult.render.menuState.finalSelectionIndex);
            printf("selectedRenderVariant=%u\n", activateResult.render.selectedRenderVariant);
            printf("activateEventCount=%u\n", activateResult.render.menuState.dispatch.activateEventCount);
            printf("activationTriggered=%d\n", activateResult.activationTriggered);
            printf("activatedSelectionIndex=%u\n", activateResult.activatedSelectionIndex);
            printf("activatedGraphicIndex=%u\n", activateResult.activatedGraphicIndex);
            F0479_MEMORY_FreeMenuActivateMini_Compat(&activateResult);
            F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
            return 0;
        }
    }

    fprintf(stderr, "failed: no menu-activate candidate succeeded in %u attempts\n", attempts);
    F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
    return 1;
}
