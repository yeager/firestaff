/*
 * firestaff_dm1_v1_hoc_champion_portrait_01_input_focus_restore_portrait_rect_position_145_gate_probe.c
 *
 * Source-locked verification gate for one narrow DM1 V1 Hall of
 * Champions slice:
 *
 *   ordinal  1     (HALK / "THE BARBARIAN", C026 col 1 row 0 -> source
 *                  rect (32, 0, 32, 29) inside the 256x87 atlas. The
 *                  real DM1 V1 PC 3.4 DUNGEON.DAT places a C127 sensor
 *                  with sensorData=1 on cell (1, 1) at the NORTH wall,
 *                  so the front_mirror route is reached by parking at
 *                  (1, 2) facing NORTH (dir=0).)
 ...
 *
 * This probe is intentionally disjoint from the existing ordinal-1
 * gates:
 *
 *   * firestaff_dm1_v1_hoc_champion_portrait_01_redraw_after_candidate
 *     _portrait_rect_position_097_gate_probe.c
 *     - Drives SelectFrontMirrorCandidate + CancelMirrorCandidate
 *       through the public helpers, framebuffer state across the
 *       panel open/close transition, and 4 append/clear cycles. Does
 *       NOT exercise HandleInput across the panel open/close
 *       transition, so it cannot prove the input focus was actually
 *       returned to the world after the modal panel closed.
 *
 *   * firestaff_dm1_v1_champion_mirror_ordinal_1_halk_pose_probe.c
 *     - Entry pose + atlas math + resurrect round-trip via the public
 *       helpers, no panel/HandleInput. Catches the panel-on redraw
 *       regression only via a single redraw after ConfirmMirror-
 *       Candidate, not through the input dispatcher's BACK/ACTION
 *       route.
 *
 *   * firestaff_dm1_v1_hall_of_champions_portrait_01_cancel_reopen_*
 ...
 *
 * Run: firestaff_dm1_v1_hoc_champion_portrait_01_input_focus_restore_
 *      portrait_rect_position_145_gate_probe DATA_DIR
 *
...
enum {
    ...
    ORDINAL_1_COL = 1 & 7,
    ORDINAL_1_ROW = 1 >> 3,
    ORDINAL_1_SRC_X = ORDINAL_1_COL << 5,
    ORDINAL_1_SRC_Y = ORDINAL_1_ROW * 29,
    TARGET_ORDINAL = 1,
    HALL_MAP_INDEX = 0,
    HALL_1_POSE_X = 1,
    HALL_1_POSE_Y = 2,
...
static void park_h1_front_route(M11_GameViewState* state) {
    state->world.party.mapIndex = HALL_MAP_INDEX;
    state->world.party.mapX = HALL_1_POSE_X;
    state->world.party.mapY = HALL_1_POSE_Y;
    state->world.party.direction = HALL_1_POSE_DIR;
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 HoC portrait-01 / input_focus_restore / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    printf("\n[Group A] ordinal 1 atlas/catalog/front-route identity\n");
    {
        char msg[200];
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 atlas source rect = (%d,%d,%d,%d)",
                 ORDINAL_1_SRC_X, ORDINAL_1_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ATLAS_COLS == 8 && ATLAS_ROWS == 3 &&
              ORDINAL_1_COL == 1 && ORDINAL_1_ROW == 0 &&
              ORDINAL_1_SRC_X == 32 && ORDINAL_1_SRC_Y == 0, msg);
    }
    atlasOpaque = atlas_cell_opaque_count(portraits, TARGET_ORDINAL);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 C026 cell has defined opaque pixels "
        ...
        &state, TARGET_ORDINAL, titleBuf, (int)sizeof(titleBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 1 to %s",
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog title for HALK is non-empty (%s)",
                 titleBuf[0] ? titleBuf : "<empty>");
        CHECK(titleRc > 0 && titleBuf[0] != '\0', msg);
    }

    clear_party(&state);
    park_h1_front_route(&state);
    frontOrdinal = M11_GameView_GetFrontMirrorOrdinal(&state);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "front mirror at (1,2) NORTH reports ordinal 1 (got %d)",
                 frontOrdinal);
                                      ...
                                      PORTRAIT_BAND_Y0, PORTRAIT_BAND_H);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "panel-off D1C rect matches ordinal 1 at >=90%% "
                 "(got %d%%)", matchBefore);
                                        ...
                                        TARGET_ORDINAL);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "panel-on redraw suppresses stale full HALK sprite "
                                           ...
                                           PORTRAIT_BAND_H);
    {
        char msg[240];
        snprintf(msg, sizeof(msg),
                 "after cancel D1C rect repaints HALK at the same rect "
    ...
    }
    {
        char msg[220];
        snprintf(msg, sizeof(msg),
                 "after cancel side walls still have no floating HALK "
                                            ...
                                            PORTRAIT_BAND_H);
    {
        char msg[220];
        snprintf(msg, sizeof(msg),
                 "after confirm D1C rect is wall-only for ordinal 1 "
    ...
    }
    {
        char msg[220];
        snprintf(msg, sizeof(msg),
                 "after confirm side walls still have no floating HALK "
