/*
 * firestaff_dm1_v1_hoc_champion_portrait_22_wall_ornament_no_float_runtime_probe.c
 *
 * Source-locked verification gate for one narrow Hall of Champions
 * slice that the existing portrait_22 cancel_reopen / portrait_22
 * redraw_after_candidate / portrait_22 front_north_entry probes do
 * not cover:
 *
 *   ordinal 22               (mirror catalog record GOTHMOG, untitled)
 ...
 *           (viewport-relative)
 *
 *       Ordinal 22 sits at row 2 / column 6 of the atlas
 *       (srcX=192, srcY=58).  Row 2 is the BOTTOM row of the 8x3
 *       atlas (the last 8 portraits of the catalog, ordinals 16..23),
 *       so this slice also implicitly proves that the C346 frame
 *       backing fires correctly for a sprite whose source Y is the
 *       last row of the C026 strip (a different atlas cell than
 *       ordinal 19, which is row 2 / col 3).
 *
 ...
 *
 *   - the C026 portrait cell is row 2 / col 6 of the atlas
 *     (NOT row 2 / col 3 like ordinal 19, and NOT row 0 / col 5
 *     like ordinal 5), so the C346 backing fires for a third
 *     distinct C026 sprite - proving the backing is independent
 *     of the ordinal content (the wall_ornament_no_float invariant
 *     holds for any champion-portrait ordinal that the C127 sensor
 *     can surface, not just for ordinals 5 and 19).
 *
 * This is disjoint from:
 *   - firestaff_dm1_v1_hoc_champion_portrait_19_wall_ornament_no_float
 *     (which covers ordinal 19 / HAWK, row 2 / col 3 of the
 *     atlas; this probe is row 2 / col 6);
 *   - firestaff_dm1_v1_hoc_champion_portrait_05_wall_ornament_no_float
 *     (which covers ordinal 5 / ELIJA, row 0 / col 5 of the
 *     atlas; this probe is row 2 / col 6 - the last column of
 *     the bottom row, which is the boundary cell of the atlas);
 *   - firestaff_dm1_v1_hall_of_champions_portrait_22_cancel_reopen
 *     (which covers select->cancel->select on the (1,2,0) NORTH
 *     route with seeded ordinal 22, panel-guard, and the C040
 *     panel redraw stability, but does NOT verify the C346 frame
 *     backing or the C346/C026 sprite isolation);
 *   - firestaff_dm1_v1_hall_of_champions_portrait_22_redraw_after_candidate
 *     (which covers the C040 panel active redraw on the (1,2,0)
 *     NORTH route with seeded ordinal 22, but does NOT verify
 *     the C346 frame backing at the destination);
 *   - firestaff_dm1_v1_hall_champion_portrait_22_front_north_entry
 *     (which covers the source-locked ordinal 22 -> C026 source
 *     rect math (192, 58, 32, 29), the catalog identity (GOTHMOG,
 *     untitled), and the no-floating route contract via
 *     M11_GameView_GetFrontMirrorOrdinal, but does NOT drive
 *     the M11_GameView_Draw framebuffer and does NOT pixel-verify
...
enum {
    ...
    /* Atlas math for ordinal 22 (row 2 / col 6).  Distinct from
     * ordinal 19 (row 2 / col 3) and ordinal 5 (row 0 / col 5)
     * which are the focus of the existing wall_ornament_no_float
     * probes.  Row 2 is the bottom row of the 8x3 atlas; col 6 is
     * the second-to-last column (last column is col 7 = ordinal 23
     * ATALANTA, the catalog tail).  This is the boundary cell of
     * the C026 strip: srcX=192 + 32 = 224 = PORTRAIT_STRIP_W - 32,
     * srcY=58 + 29 = 87 = PORTRAIT_STRIP_H. */
    ATLAS_W = 256,
    ATLAS_H = 87,
     ...
     * has sensorData=1 (HALK).  We seed that sensor to ordinal 22
     * so we can lock the ordinal-22 edge case without changing
     * the map layout. */
 ...
 * Same palette set as the existing portrait_22 / portrait_19
 * wall_ornament_no_float probes:
 ...
 * existing portrait_19 wall_ornament_no_float probe. */
...
int main(int argc, char** argv) {
     ...
     * (1,2,0)=22, for the redraw-stability check.  The C346
    ...
    printf("=== DM1 V1 Hall of Champions portrait-22 / wall_ornament_no_float ===\n");
    printf("dataDir=%s\n", dataDir);
     ...
     * col 6 and that the math matches COORD.C / DEFS.H:821-826.
     * The runtime drive is what actually links the atlas cell to
     * the D1C cutout in the next groups. */
    printf("\n[Group A] C026 atlas math for ordinal 22 (row 2 / col 6)\n");
    ...
    }
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 ORDINAL_22_SRC_X, ORDINAL_22_SRC_Y, ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_22_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_22_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }
    /* Row 2 / col 6 is the second-to-last atlas cell on the
     * bottom row; it must end exactly at the C026 strip bottom
     * edge so a regression that uses only 2 rows or a wrong
     * stride (e.g. 30 instead of 29) will fail this gate. */
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 22 source rect bottom edge equals C026 strip "
                 "height: srcY (%d) + portraitH (%d) == stripH (%d)",
                 ORDINAL_22_SRC_Y, D1C_PORTRAIT_H, ATLAS_H);
        CHECK(ORDINAL_22_SRC_Y + D1C_PORTRAIT_H == ATLAS_H, msg);
    }
    opaquePortraitPixels = atlas_cell_opaque_count(portraits, 22);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 22 atlas cell has >= 200 opaque pixels (got %d) "
    ...
    /* ----------------------------------------------------------------
     * Group B - park the party on the (1,2,0) NORTH route, seed the
     * C127 sensor to ordinal 22, and lock the D1C wall ornament
     * zone to coordSet 5 / index 12. */
    printf("\n[Group B] Park on (1,2,0) and seed C127 sensor to ordinal 22\n");
     ...
     * ----------------------------------------------------------------
     * The C026 sprite for ordinal 22 (GOTHMOG) must match the
                                   ...
                                   D1C_PORTRAIT_W, D1C_PORTRAIT_H);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "D1C portrait rect has >= 30 warm-color pixels (got %d) "
                 "- portrait sprite, not just frame fill "
                 "(ordinal 22 / GOTHMOG is a sparse portrait with "
                 "only ~200 opaque atlas pixels, so the warm-palette "
                 "count is naturally lower than HAWK / ordinal 19)",
                 warmPortrait);
        CHECK(warmPortrait >= 30, msg);
    }
     ...
     * byte-stable across redraws of the same (1,2,0)=22 pose.
    ...
    matchB = match_portrait_at_rect(portraits, fbB, TARGET_ORDINAL);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "second redraw: D1C portrait rect matches ordinal 22 "
                 "at >= 90%% (got %d%%)",
                 matchB);
        CHECK(matchB >= 90, msg);
    }
