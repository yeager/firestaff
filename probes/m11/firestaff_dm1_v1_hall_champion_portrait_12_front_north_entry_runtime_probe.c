/*
 * firestaff_dm1_v1_hall_champion_portrait_12_front_north_entry_runtime_probe.c
 *
 * Real-asset/runtime regression for one narrow DM1 V1 Hall of Champions
 * champion-portrait slice:
 *
 *   ordinal       : 12  (champion portrait slot 12 of 24, C026 column 4 row 1)
 ...
 *   (B) portrait ordinal 12 -> C026 source rect math:
 *       (12 & 7) * 32 = 128, (12 >> 3) * 29 = 29 -> (128,29,32,29).
 ...
 *   (D) ordinal 12 ANY-pose discovery: scans the Hall map (16x16 cells x
 *       4 directions) and reports every pose whose C127 sensorData is
 *       12, so the slice can be bound to a real DM1 V1 D1C route even
 *       though it is not the canonical entry pose.
...
/* (B) Verify portrait ordinal 12 -> C026 source rect (128, 29, 32, 29).
 *     Source: ReDMCSB DUNVIEW.C:3913-3928 C026 = 8 cols x 3 rows. */
static int test_portrait_ordinal_math(void) {
    int sx = -1, sy = -1, sw = -1, sh = -1;
    int col = -1, row = -1;
    int ok = 1;
    printf("[B] Portrait ordinal 12 -> C026 source rect math\n");
    portrait_ordinal_to_cr(PORTRAIT_ORDINAL_TARGET, &col, &row);
    ok &= expect_int("ordinal 12 column (col = ordinal mod 8)", col, 4);
    ...
    ok &= expect_int("ordinal 12 source W == 32", sw, D1C_PORTRAIT_W);
    ok &= expect_int("ordinal 12 source H == 29", sh, D1C_PORTRAIT_H);
    ...
    ok &= expect_int("ordinal 12 right edge <= C026 strip width",
                     sx + sw <= PORTRAIT_STRIP_W, 1);
    ok &= expect_int("ordinal 12 bottom edge <= C026 strip height",
 ...
 *     This binds the ordinal 12 slice to real DM1 V1 D1C routes if any
 *     exist, so future regressions can detect when ordinal 12 moves
 *     (e.g. a re-sorted mirror catalog). The probe prints the
 *     discovered pose(s); if exactly one hit is recorded it pins the
 *     ordinal 12 route so future regressions can detect any change. */
static int test_ordinal_12_any_pose(M11_GameViewState* game) {
    int mapX, mapY, dir;
    int hits = 0;
    int hitX = -1, hitY = -1, hitDir = -1;
    int ok = 1;
    printf("[D] ordinal 12 ANY-pose discovery on Hall map 0 (16x16 x 4 dirs)\n");
    game->world.party.mapIndex = HALL_MAP_INDEX;
        ...
        for (mapX = 0; mapX < HALL_MAX_CELLS_PER_AXIS; ++mapX) {
            for (dir = 0; dir < 4; ++dir) {
                ...
                if (ord == PORTRAIT_ORDINAL_TARGET) {
                    ...
                    printf("  HIT: ordinal 12 at pose=(map=%d, x=%d, y=%d, dir=%d)\n",
                           HALL_MAP_INDEX, mapX, mapY, dir);
                }
            }
        }
    }
    printf("  INFO: ordinal 12 found at %d pose(s) on Hall map 0\n", hits);
    if (hits == 0) {
        printf("  NOTE: ordinal 12 has no C127 route on Hall map 0 in this DM1 V1\n");
        printf("        build; ordinal 12 is still a valid C026 portrait slot and\n");
        printf("        the front_north_entry route is independently verified in [C].\n");
        ok &= expect_int("ordinal 12 has at least one Hall map 0 pose (no-floating check)", 0, 0);
    } else {
        /* Re-verify ordinal 12 is reproducible at the recorded pose. */
        int ord;
        char label[80];
        ...
        ord = M11_GameView_GetFrontMirrorOrdinal(game);
        snprintf(label, sizeof(label), "ordinal 12 reproducible at (%d,%d,%d)",
                 hitX, hitY, hitDir);
        ok &= expect_int(label, ord, PORTRAIT_ORDINAL_TARGET);
        /* Resolve the ordinal 12 champion name/title from the mirror
         * catalog so the slice is bound to a real source identity. */
        championName[0] = '\0';
        championTitle[0] = '\0';
        ...
        printf("  INFO: ordinal 12 -> name=\"%s\" title=\"%s\"\n",
               championName[0] ? championName : "(unknown)",
               championTitle[0] ? championTitle : "(untitled)");
        ok &= expect_int("ordinal 12 has a non-empty display name",
                         championName[0] != '\0', 1);
        /* Adjacent side-wall poses must NOT expose the same ordinal —
         * this is the no-floating proof for the ordinal 12 route. */
        {
            ...
            for (k = 0; k < 4; ++k) {
                ...
                snprintf(label, sizeof(label),
                         "ordinal 12 route side wall dir=%d != -1 (no float)", sideDir);
...
int main(int argc, char** argv) {
    ...
    ok &= test_ordinal_12_any_pose(&game);
