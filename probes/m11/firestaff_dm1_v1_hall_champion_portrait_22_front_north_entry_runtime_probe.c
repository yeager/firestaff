/*
 * firestaff_dm1_v1_hall_champion_portrait_22_front_north_entry_runtime_probe.c
 *
 * Real-asset/runtime regression for one narrow DM1 V1 Hall of Champions
 * champion-portrait slice:
 *
 *   ordinal       : 22  (champion portrait slot 22 of 24, C026 column 6 row 2
 *                   -- the "GOTHMOG" slot per the DM1 V1 mirror TextString
 *                   catalog; the C026 atlas has 8 cols x 3 rows and the
 *                   GotHMOG row is the third row of the strip)
 ...
 *   (B) portrait ordinal 22 -> C026 source rect math:
 *       (22 & 7) * 32 = 192, (22 >> 3) * 29 = 58 -> (192, 58, 32, 29).
 *       This is column 6 row 2 of the C026 strip; on DM1 V1 PC 3.4 the
 *       catalog binds ordinal 22 to GOTHMOG (untitled).
 ...
 *   (D) ordinal 22 ANY-pose discovery: scans the Hall map (16x16 cells x
 *       4 directions) and reports every pose whose C127 sensorData is
 *       22, so the slice can be bound to a real DM1 V1 D1C route if one
 *       exists in this build. The probe asserts the catalog name for
 *       ordinal 22 ("GOTHMOG") to keep the slot bound to a real source
 *       identity; GOTHMOG has no title in the DM1 V1 catalog, so the
 ...
 *   (F) ordinal 22 front_north_entry route seed: temporarily rewrites
 *       the C127 sensor at the (1,2) front square from sensorData=1
 *       (HALK) to sensorData=22 (GOTHMOG) and verifies the route then
 *       reports ordinal 22, so the portrait_rect_position contract is
 ...
 * contract and the no-floating rule for the ordinal 22 slice. It does
...
enum {
     ...
     * GOTHMOG is untitled in the catalog (title[0] == '\0'), so the title
...
/* (B) Verify portrait ordinal 22 -> C026 source rect (192, 58, 32, 29).
 *     Source: ReDMCSB DUNVIEW.C:3913-3928 C026 = 8 cols x 3 rows.
 *     Ordinal 22 -> col=6, row=2 (the third row of the C026 strip). */
static int test_portrait_ordinal_math(void) {
    int sx = -1, sy = -1, sw = -1, sh = -1;
    int col = -1, row = -1;
    int ok = 1;
    printf("[B] Portrait ordinal 22 -> C026 source rect math\n");
 ...
 *     This binds the ordinal 22 slice to real DM1 V1 D1C routes if any
 *     exist, so future regressions can detect when ordinal 22 moves
 *     (e.g. a re-sorted mirror catalog). The probe also pins the
 *     catalog name to keep the slot bound to a real source identity
 *     (GOTHMOG per DM1 V1 PC 3.4 TextString ordering); GOTHMOG has
 *     no title, so the title slot is asserted to be empty. */
static int test_ordinal_22_any_pose(M11_GameViewState* game) {
    int mapX, mapY, dir;
    int hits = 0;
    ...
    printf("[D] ordinal 22 ANY-pose discovery on Hall map 0 (16x16 x 4 dirs)\n");
    /* Always pin the catalog identity for ordinal 22 so the slot
        ...
        for (mapX = 0; mapX < HALL_MAX_CELLS_PER_AXIS; ++mapX) {
            for (dir = 0; dir < 4; ++dir) {
                ...
                if (ord == PORTRAIT_ORDINAL_TARGET) {
                    ...
                    printf("  HIT: ordinal 22 at pose=(map=%d, x=%d, y=%d, dir=%d)\n",
                           HALL_MAP_INDEX, mapX, mapY, dir);
                }
            }
        }
    }
    printf("  INFO: ordinal 22 found at %d pose(s) on Hall map 0\n", hits);
    if (hits == 0) {
        printf("  NOTE: ordinal 22 has no C127 route on Hall map 0 in this DM1 V1\n");
        printf("        build; ordinal 22 is still a valid C026 portrait slot and\n");
        printf("        the front_north_entry route is independently verified in [C].\n");
        ok &= expect_int("ordinal 22 has at least one Hall map 0 pose (no-floating check)", 0, 0);
    } else {
        /* Re-verify ordinal 22 is reproducible at the recorded pose. */
        int ord;
        char label[80];
        ...
        ord = M11_GameView_GetFrontMirrorOrdinal(game);
        snprintf(label, sizeof(label), "ordinal 22 reproducible at (%d,%d,%d)",
                 hitX, hitY, hitDir);
        ok &= expect_int(label, ord, PORTRAIT_ORDINAL_TARGET);
        /* Adjacent side-wall poses must NOT expose the same ordinal —
         * this is the no-floating proof for the ordinal 22 route.
         ...
         * ordinals (which represent real mirrors on those walls). */
        {
            ...
            for (k = 0; k < 4; ++k) {
                ...
                snprintf(label, sizeof(label),
                         "ordinal 22 side wall dir=%d != 22 (no self-float)", sideDir);
...
/* (F) Ordinal 22 front_north_entry route seed: temporarily rewrite
 *     the C127 sensor at the (1,2) front square from sensorData=1
 *     (HALK) to sensorData=22 (GOTHMOG) and verify the route then
 *     reports ordinal 22. This proves the front_north_entry pose is
...
static int test_front_north_entry_ordinal_22_seed(M11_GameViewState* game) {
    int ordBefore = -999;
    int ordAfter = -999;
    int seededIndex = -1;
    ...
    int i;
    int ok = 1;
    printf("[F] ordinal 22 front_north_entry route seed via C127 sensor rewrite\n");
    game->world.party.mapIndex = HALL_MAP_INDEX;
    game->world.party.mapX = HALL_NORTH_ENTRY_X;
    game->world.party.mapY = HALL_NORTH_ENTRY_Y;
    game->world.party.direction = HALL_NORTH_ENTRY_DIR;
    ...
    ok &= expect_int("seeded front_north_entry route reports ordinal 22",
...
int main(int argc, char** argv) {
    ...
    ok &= test_ordinal_22_any_pose(&game);
    ok &= test_no_floating_on_side_walls(&game);
    ok &= test_front_north_entry_ordinal_22_seed(&game);
