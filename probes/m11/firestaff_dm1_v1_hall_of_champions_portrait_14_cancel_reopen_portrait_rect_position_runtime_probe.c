 ...
 *
 *   ordinal 14          (mirror catalog row-1, col-6 cell of the
 *                        C026 atlas; canonical DM1 V1 ships only
 *                        14 mirrors (ordinals 0..13) so ordinal 14
 *                        is the first atlas cell beyond the
 *                        shipped catalog.  The probe treats
 *                        ordinal 14 as a fully defined atlas cell
 *                        and seeds the front-cell C127 sensor to
 *                        that ordinal for the runtime drive.)
 ...
 * (256x87 pixels total, ordinals 0..23).  Ordinal 14 sits at row 1,
 ...
 * so we seed that sensor to sensorData=14 to lock the ordinal-14
 * edge case.  This keeps the probe runtime-real: same sensor, same
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 14: verify the C026 atlas contains a
 *       defined portrait at (192, 29, 32, 29) and that the
 *       (14 & 7) << 5 / (14 >> 3) * 29 math matches COORD.C
 *       M027/M028 macro encoding (DEFS.H:821-826).  Verifies
 *       ordinal 14 is a valid C026 atlas entry (not a degenerate
 *       cell).  The row-1 / col-6 cell of the 8x3 grid has its
 *       bottom edge at srcY=58 - i.e. strictly less than the
 *       atlas height of 87 - so it is not on a row boundary and
 *       the source rectangle is fully inside the atlas.
 *
 *   (2) portrait_rect_position: drive a real D1C front-mirror pose
 *       from the actual DM1 V1 DUNGEON.DAT C127 sensor lattice
 *       (with sensorData seeded to 14), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-14 champion portrait sprite, and that
 ...
 *       still carries ordinal-14 pixels after the full
 ...
 *       portrait_14_redraw_after_candidate_runtime_probe (different
 *       route - no cancel between two selects; this probe drives
 *       the full select->cancel->reopen cycle and the panel
 *       guard is verified across all four sample points).
 *
 * Note: ordinal 14 is the first atlas cell beyond the canonical
 * DM1 V1 mirror catalog (14 mirrors per the shipped fixture).  We
 * deliberately do NOT assert a fixed catalog name for ordinal 14
 * because the DM1 V1 mirror catalog has only 14 entries, so a
 * name lookup for ordinal 14 returns empty.  The atlas cell itself
 * is still defined (the C026 atlas is a fixed 24-sprite strip),
 * and the draw path that uses (ordinal >> 3) * 29 as the source Y
 * is what we are locking: a regression in the row math (e.g. 14
 * >> 3 == 0 instead of 1) would shift the source rect to
 * (192, 0, 32, 29) and the runtime drive would fail the match
 * threshold.
 *
 * Source evidence:
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 14 in the C026 atlas: (14 & 7) << 5 = 192,
     *                                 (14 >> 3) * 29 =  29. */
     ...
     * to ordinal 14 for this gate so we can lock the ordinal-14
     * edge case (row 1 / col 6 of the C026 atlas) without
 ...
 * Used to verify ordinal 14 is a defined portrait in the atlas
 ...
 * lock the ordinal-14 edge case on the real DM1 V1 DUNGEON.DAT
 ...
 * seed_first_c127_data the same square reports ordinal 14. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-14 / cancel_reopen / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 14 source rect bottom (srcY+%d = %d) does not "
                 "touch atlas bottom (atlas height %d) - row 1 is "
                 "strictly inside row 0..2 of the 8x3 atlas",
                 D1C_PORTRAIT_H, ORDINAL_14_SRC_Y + D1C_PORTRAIT_H,
                 ATLAS_H);
        CHECK(ORDINAL_14_SRC_Y + D1C_PORTRAIT_H < ATLAS_H, msg);
    }
     ...
     * the row-1 / col-6 cell of C026 is treated as "no portrait"
     * (some debug builds omit the bottom-right 2x2 of the 8x3 grid). */
    {
        int ordinal14Opaque = atlas_cell_opaque_count(portraits, 14);
        char msg[200];
        snprintf(msg, sizeof(msg),
     ...
     * 14.  Same sensor, same map cell, same draw path - only G0289
    ...
    /* The same front route now reports ordinal 14.  After seeding
     ...
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-14
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=14\n");
    ...
    /* The D1C portrait rect must contain ordinal-14 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-14
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 14\n");
     ...
     * 121 panel guard).  Match against ordinal 14 should be low. */
     ...
     * still active (sensorType=127, sensorData=14). */
     ...
     * leader hand empty, sensorData=14 still on the front square, so
     ...
     * strip non-zero pixel count in the wall-ornament band (DUNVIEW.C
     * draws the wall ornament + portrait independently of the panel
     * state, so the top strip should be populated by the wall
     * ornament chrome plus the C040 panel border).  The check is
     * expressed as a range, not a delta, because the panel-off top
     * strip depends on the ordinal's portrait content (an ordinal
     * with a sparse top row has fewer non-zero pixels at panel-off
     * time, while the C040 panel border is constant across
     * ordinals).  Range [200, 700] covers all 24 ordinals of the
     * 8x3 atlas on the (1,2) NORTH C127 sensor pose. */
    {
     ...
     * we verify the same rect lines up with ordinal-14 pixels when
