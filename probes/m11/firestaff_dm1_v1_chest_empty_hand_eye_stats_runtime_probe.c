/*
 * DM1 V1 empty-hand eye stats runtime probe.
 *
 * Firestaff-side runtime evidence: opens an action-hand chest with a hidden
 * ninth tail item, then clicks the empty-hand eye icon through the real M11
 * pointer route.  ReDMCSB routes an empty leader-hand eye click to the
 * champion skills/statistics panel, and Firestaff must first close G0426 so
 * the source visible-slot rewrite compacts the chest to C537..C544 and drops
 * any hidden ninth-and-later tail.
 *
 * Source evidence:
 *   ReDMCSB PANEL.C F0352 lines 2123-2159 handles C546 eye clicks and routes
 *   an empty leader hand to F0351 champion skills/statistics.
 *   ReDMCSB PANEL.C F0351 lines 1965-2108 draws the champion
 *   skills/statistics panel.
 ...
 *   ReDMCSB COMMAND.C G0449 routes C546 eye clicks at viewport-relative
 *   x=12..27, y=13..28 to command 71.
 */
...
int main(int argc, char** argv)
{
    ...
    ok &= expect_int("action-hand chest icon is open before eye click",
                     M11_GameView_GetV1InventorySlotIconIndex(
                         &game, CHAMPION_SLOT_ACTION_HAND),
                     145);
    ok &= expect_int("leader hand is empty before eye click",
                     (int)M11_GameView_GetV1LeaderHandThing(&game),
                     (int)THING_NONE);

    ok &= expect_true("empty-hand eye click routes to champion stats panel",
                      M11_GameView_HandlePointer(
                          &game, PROBE_EYE_X, PROBE_EYE_Y, 1) ==
                      M11_GAME_INPUT_REDRAW);
    ok &= expect_int("G0426 bridge is closed after eye click",
                     (int)M11_GameView_GetV1OpenChestThing(&game),
                     (int)THING_NONE);
    ok &= expect_int("champion stats panel active after eye click",
                     game.v1ChampionStatsPanelActive, 1);
    ok &= expect_int("food/water panel inactive after eye click",
                     game.v1FoodWaterPanelActive, 0);
    ok &= expect_int("object description panel inactive after eye click",
                     game.v1ObjectDescriptionPanelActive, 0);
    ok &= expect_true("stats overlay names active champion",
                      strstr(game.inspectTitle, "TIGGY") != NULL);
    ok &= expect_true("stats overlay contains mana row",
                      strstr(game.inspectDetail, "MANA") != NULL);
    ok &= expect_int("action-hand chest icon is closed after eye click",
                     M11_GameView_GetV1InventorySlotIconIndex(
                         &game, CHAMPION_SLOT_ACTION_HAND),
                     PROBE_CHEST_CLOSED_ICON);
    ok &= expect_int("eye stats close compacted chest to visible slots",
                     chain_count(&game, game.world.things->containers[0].slot,
                                 PROBE_CHAIN_COUNT + 2),
                     PROBE_VISIBLE_CHEST_SLOTS);
    ok &= expect_true("ninth hidden tail dropped by eye stats close",
                      !chain_contains(&game, game.world.things->containers[0].slot,
                                      actionItems[PROBE_VISIBLE_CHEST_SLOTS],
                                      PROBE_CHAIN_COUNT + 2));

    printf("sourceEvidence=PANEL.C:F0352:2123-2159;PANEL.C:F0351:1965-2108;CHEST.C:F0333:53-76;CHEST.C:F0334:112-132;COMMAND.C:G0449:C546\n");
    printf("%s dm1 v1 empty-hand eye stats closes open chest runtime probe\n",
           ok ? "PASS" : "FAIL");
    M11_GameView_Shutdown(&game);
    return ok ? 0 : 1;
}
