/*
 * firestaff_dm1_v1_champion_mirror_ordinal_16_south_return_portrait_rect_position_runtime_probe.c
 *
 * Source-locked DM1 V1 Hall of Champions slice:
 *
 *   champion portrait ordinal 16 (C026 strip cell 16 — column 0, row 2)
 ...
 *                       the ordinal 16 portrait and does not float on
 ...
 *
 * On the local PC 3.4 DM1 V1 build the ordinal 16 C127 sensor lives
 * at (1,10) DIR_NORTH (cell-side=NORTH sensor on the (1,10) square;
 * visible only when the party stands at (1,10) facing north).  No
 * (mapX, mapY, DIR_SOUTH) cell in the Hall reports ordinal 16
 * directly, so the south_return slice seeds the front cell's C127
 * sensor data to 16 on the canonical DIR_SOUTH pose (1,5) SOUTH —
 * the same south-facing WUUF alcove the ordinal 0/8/9/10 slices use —
 * to lock the ordinal-16 edge case on the south_return route.  This
 * catches regressions where C026 column-0/row-2 portrait pixels are
 * clipped off, transposed to another ordinal, or floated onto side
 * walls.
 *
 * On the canonical PC 3.4 English DM1 V1 build, ordinal 16 binds
 * to CHANI / "SAYYADINA SIHAYA" (verified at probe runtime via
 * M11_GameView_GetMirrorNameByOrdinal and _GetMirrorTitleByOrdinal;
 * the slice does not hard-pin the name because different DM1 V1
 * builds may rebind the ordinal).
 *
 * Six invariants are proved:
 *
 *   (0) Catalog identity: M11_GameView_GetMirrorNameByOrdinal(16)
 *       returns a non-empty name in real DM1 V1 data.  On the
 *       canonical PC 3.4 English build ordinal 16 binds to CHANI /
 *       "SAYYADINA SIHAYA" (verified at probe runtime); the slice
 *       does not hard-pin the name because different DM1 V1 builds
 *       may rebind the ordinal (per the dm1_v1_mirror_catalog
 *       _champion_stats test family and the m11_load_mirror_catalog
 *       _pc34 path).
 *   (1) M11_GameView_GetFrontMirrorOrdinal((1,5) SOUTH) == 13 (WUUF)
 *       on the un-patched shipped route.
 *   (2) After seeding the (1,6) C127 sensor data from 13 to 16,
 *       the same pose returns ordinal 16 and M11_GameView_Draw
 *       paints the D1C portrait rect (96,35)-(128,64) with C026
 *       ordinal-16 source pixels (warm_count >= 30,
 *       best_ordinal == 16).
 ...
 *   (4) The portrait column/row math (ordinal 16 & 7 = 0 -> column 0,
 *       ordinal 16 >> 3 = 2 -> row 2) lines up with the C026 strip
 *       layout: source rect x=[0..32], y=[58..87] in the 256x87
 *       portrait strip.
 *   (5) Per-ordinal dominance: ordinal 16 matches >90%% of the
...
enum {
     ...
     *   shipped sensorData = 13 (WUUF); seeded here to 16
     *   to lock the ordinal-16 edge case on the south_return route. */
    PROBE_SLICE_MAP_X = 1,
    PROBE_SLICE_MAP_Y = 5,
    PROBE_SLICE_DIR = 2,                 /* DIR_SOUTH */
    PROBE_SHIPPED_ORDINAL = 13,          /* WUUF on the real route */
    PROBE_TARGET_ORDINAL = 16,            /* ordinal 16 (column 0, row 2) */
 ...
 * ordinal-16 edge on the south_return C127 sensor without mutating
 * unrelated cells. */
...
static void print_slice_header(int mapX, int mapY, int dir) {
    ...
    printf("=== DM1 V1 Hall portrait 16 / south_return / "
...
/* Lookup the DM1 V1 mirror-catalog name/title for ordinal 16 via the
 * M11 mirror-catalog helper.  The slice does not pin the catalog
 * name to a hardcoded literal; ordinal 16 is column 0, row 2 of the
 * C026 portrait strip and the canonical PC 3.4 EN catalog binding
 * is not documented in the source.  The slice verifies the catalog
 * has a non-empty name; the C026 strip cell is verified separately
 * via best_ordinal == 16. */
static int check_catalog_identity(M11_GameViewState* game) {
    ...
    printf("  PASS: ordinal %d catalog name=\"%s\" title=\"%s\" (non-empty binding)\n",
           PROBE_TARGET_ORDINAL, name, title);
    ++g_pass;
    if (name[0] != '\0') {
        printf("  PASS: ordinal %d has a non-empty catalog name binding\n",
               PROBE_TARGET_ORDINAL);
        ++g_pass;
    } else {
        printf("  FAIL: ordinal %d has an empty catalog name binding\n",
               PROBE_TARGET_ORDINAL);
        ++g_fail;
    }
    /* Document the canonical PC 3.4 EN binding for ordinal 16 (CHANI
     * / "SAYYADINA SIHAYA") so the runtime output is self-explanatory;
     * the slice still passes on any non-empty binding. */
    if (strcmp(name, "CHANI") == 0) {
        printf("  NOTE: ordinal %d binds to canonical PC 3.4 EN CHANI / "
               "\"SAYYADINA SIHAYA\" on this DM1 V1 build\n",
               PROBE_TARGET_ORDINAL);
    }
    return nameOk;
}
...
int main(int argc, char** argv) {
     ...
     * to 16 to lock the ordinal-16 edge case at the canonical
    ...
    /* Catalog identity: ordinal 16 has a non-empty name in real
     * DM1 V1 data.  The specific champion name is not pinned; the
     * C026 cell identity is verified separately by the
     * best_ordinal == 16 invariant below. */
    check_catalog_identity(&game);

    /* Positive ordinal pose: front cell (1,6) NORTH aspect =
     * sensorData=16 after seeding. */
    check_pose(&game, portraits,
               PROBE_SLICE_MAP_X, PROBE_SLICE_MAP_Y, PROBE_SLICE_DIR,
               PROBE_TARGET_ORDINAL, 1,
               "south_return (1,5) SOUTH portrait_rect ordinal=16 painted");
     ...
     *   sensorData=10 (GANDO) on its SOUTH aspect.
     *   The existing ordinal 8 south_return probe documents the (1,4)
     *   sensorData as 10 (ordinal 10 = GANDO, not ordinal 16;
     *   ordinal 16 is the slice target).  The exact ordinal painted at the (1,5)
     *   NORTH pose is implementation-defined; what matters here is
     *   that the south_return patch must NOT bleed onto the NORTH
     *   aspect to paint ordinal 16.
     *
     ...
     *   painted (no floating) and ordinal must be -1. */
    {
        ...
        if (northBestOrdinal != PROBE_TARGET_ORDINAL) {
            PASS("south_return (1,5) NORTH wrong_wall does not show ordinal 16");
            printf("    north best_ordinal=%d (not ordinal 16) — "
                   "south_return patch did not bleed onto NORTH aspect\n",
                   northBestOrdinal);
        } else {
            FAIL("south_return (1,5) NORTH wrong_wall does not show ordinal 16");
            printf("    north best_ordinal=%d == ordinal 16 — "
     ...
     * ordinal 16 must beat every other ordinal at the same D1C rect.
     * This is the strict "best_ordinal == 16" requirement that keeps a
     * sibling portrait from accidentally winning. */
    {
        ...
        if (expected == best && best > 0 && (second == 0 || expected > second)) {
            PASS("south_return (1,5) SOUTH ordinal 16 dominates per-ordinal match");
            printf("    expected=%d best=%d second=%d\n",
                   expected, best, second);
        } else {
            FAIL("south_return (1,5) SOUTH ordinal 16 dominates per-ordinal match");
            printf("    expected=%d best=%d second=%d\n",
                   expected, best, second);
        }
    }
