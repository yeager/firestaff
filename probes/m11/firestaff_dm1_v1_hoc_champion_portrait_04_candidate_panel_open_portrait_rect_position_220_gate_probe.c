/*
 * firestaff_dm1_v1_hoc_champion_portrait_04_candidate_panel_open_portrait_rect_position_220_gate_probe.c
 *
 * Source-locked verification gate for one narrow DM1 V1 Hall of
 * Champions slice:
 *
 *   ordinal  4     (LEIF / "SON OF ERIDNU", C026 col 4 row 0 ->
 *                  source rect (128, 0, 32, 29) inside the 256x87
 *                  atlas.  The real DM1 V1 PC 3.4 DUNGEON.DAT
 *                  places a C127 sensor with sensorData=4 on cell
 *                  (2, 2) at the NORTH wall, so the front_mirror
 *                  route is reached by parking at (2, 1) facing
 *                  SOUTH (dir=2).  visibleWallCell = (dir+2)&3 = 0
 *                  = NORTH, and the front cell (2, 2) carries the
 *                  C127 sensor on its NORTH aspect, so
 *                  M011_CELL(sensor) = 0 and the front_mirror
 *                  ordinal lookup returns 4.)
 ...
 *                   side walls never leak ordinal-4 pixels
 *                   through the inventory backdrop wipe)
 *
 * This is the ordinal-4 companion to
 * firestaff_dm1_v1_hoc_champion_portrait_01_candidate_panel_open_
 * portrait_rect_position_097_gate_probe (HALK / ordinal 1).
 * Both probes stay inside the candidate_panel_open state and pin
 * down invariants that the broader ordinal-4 sibling
 * firestaff_dm1_v1_hoc_champion_portrait_04_after_party_shuffle_
 * portrait_rect_position_runtime_probe (which interleaves F0284
 * rotations and a C160 confirm close) does not check.
 *
 * Coverage gap relative to existing ordinal-4 probes:
 *   - firestaff_dm1_v1_hoc_champion_portrait_04_after_party_shuffle_
 *     portrait_rect_position_runtime_probe covers the BUG-120/121
 *     panel-lock rotation guard and the close_after_party_shuffle
 *     re-render path, but opens the C040 panel and immediately
 *     consumes the close in a single flow.  It does not stay
 *     inside the panel-open state to verify the cutout warm
 *     pixels, the C017 backdrop opacity, the C040 panel drawn
 *     ratio, the status / inspect readout text, or the byte-
 *     stable draw stability at panel-open.
 *   - firestaff_dm1_v1_hoc_champion_portrait_04_side_wall_
 *     negative_runtime_probe covers the three source-visible
 *     wrong-wall approaches around the LEIF chamber and asserts
 *     that the C127 sensor does not surface a portrait at those
 *     poses.  It never opens the C040 panel.
 *   - firestaff_dm1_v1_hall_of_champions_champion_portrait_04_
 *     south_return_portrait_rect_position_probe covers the
 *     panel-off (2, 1) DIR_SOUTH LEIF pose with a contract-
 *     portable ordinal sweep.  It does not open the C040 panel.
 *
 * What this probe asserts at the panel-open frame for ordinal 4:
 *   (1) Front-mirror route lookup still returns ordinal 4 at
 *       (2, 1) DIR_SOUTH (the panel-open path must keep the
 ...
 *       bound that the after_party_shuffle probe does not
 *       enforce, so this catches a partial-blit regression at
 *       ordinal 4.
 ...
 *       used by the ordinal-1 097 sibling probe) and the C026
 *       strip cell match for ordinal 4 must be <= 5% — i.e.
 *       the cutout does not contain LEIF pixels.  The C040
 *       panel can have warm palette indices for the buttons,
 *       but those will not match the C026 strip cell, so the
 *       strip-cell-match counter is the real "is LEIF here"
 *       discriminator.
 ...
 *   DUNVIEW.C:4547-4581       - nibble 2 -> ordinal 4 / nibble 1 -> ordinal 0
 ...
 *
 * Run: firestaff_dm1_v1_hoc_champion_portrait_04_candidate_panel_open_
 *      portrait_rect_position_220_gate_probe DATA_DIR
 *
 * SKIP path: probe exits 0 when M12_AssetStatus_GameAvailable("dm1")
 * returns 0 or when (2, 1) DIR_SOUTH does not return ordinal 4 from
 ...
 * the existing portrait 01 candidate_panel_open 097 sibling
 * probe and the ordinal-04 south_return / after_party_shuffle /
 * side_wall_negative probes declare. */
...
enum {
     ...
     * C026_GRAPHIC_CHAMPION_PORTRAITS).  Use the source-locked
     * helper instead of the literal so future atlas migrations
     * keep the probe aligned. */
    PORTRAIT_GRAPHIC_ID_HELPER_NOTE = 0,
    /* Hall of Champions ordinal 4 = LEIF / SON OF ERIDNU.  C127
     * sensorData is 0-indexed per DUNGEON.C:4547-4581, so the
     * M11 ordinal = 4 is the fifth cell in the portrait strip:
     * column 4, row 0. */
    ORDINAL_LEIF = 4,
     ...
     * (2, 1) DIR_SOUTH) and well under the side_wall_negative
     * probe's 35% wrong-ordinal drift floor.
     *
     * The ordinal-1 097 sibling uses 5% and observes ~2% on the
     * shipped DM1 V1 DUNGEON.DAT.  The LEIF atlas cell at (128, 0)
     * retains more opaque pixels in the cutout row band than the
     * HALK atlas cell at (32, 0): the panel-open state at
     * ordinal 4 reports ~9% C026 match on the local DM1 V1
     * PC 3.4 DUNGEON.DAT (50/546 compared pixels).  We use 20%
     * as the regression threshold: this is 2x the observed
     * panel-on baseline, well under the panel-off 90%+ baseline
     * (a missing C017 backdrop wipe would push the match rate
     * back toward the panel-off value), and well under the 35%
     * side_wall_negative wrong-ordinal drift floor (a missing
     * C040 panel blit would also push the match rate higher).
     * The warm-pixel count alone is not a useful test here
     * because the C040 panel's button artwork uses warm palette
     * indices (red / yellow / green). */
 ...
 * for the live (2, 1) DIR_SOUTH frame.  This is the heart of the
...
static void collect_panel_open_evidence(const M11_AssetSlot* rrPanel,
     ...
     * cell for ordinal 4 (LEIF) must NOT match the cutout pixels
    ...
    if (portraits && portraits->loaded && portraits->pixels &&
        ...
        int srcBaseX = (ORDINAL_LEIF & 7) * PORTRAIT_W;
        int srcBaseY = (ORDINAL_LEIF >> 3) * PORTRAIT_H;
        for (y = 0; y < PORTRAIT_H; ++y) {
            int srcY = srcBaseY + y;
            int dstY = fbRectY + y;
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s DATA_DIR\n"
                "  verifies ordinal 4 (LEIF) candidate_panel_open portrait_rect_position\n",
                argv[0]);
        return 2;
    }
    dataDir = argv[1];
    ...
    printf("=== DM1 V1 HoC portrait ordinal 4 (LEIF) candidate_panel_open ===\n");
    printf("dataDir=%s pose=(map 0, x=2, y=1) facing SOUTH\n", dataDir);

    /* Stage 0: front-mirror route lookup must return ordinal 4 for
     * (2, 1) DIR_SOUTH per DUNGEON.C:2573 / MOVESENS.C:1501-1503.
     * The (2, 2) front cell carries the C127 sensor with
     * sensorData=4 on its NORTH aspect, so the visible-wall cell
     * is 0 (NORTH) when the party is at (2, 1) DIR_SOUTH. */
    ...
    if (frontOrdinal != ORDINAL_LEIF) {
        printf("SKIP this DM1 V1 build does not place the C127 sensor "
               "with sensorData=4 at (2,2) front cell visible from "
               "(2,1) DIR_SOUTH (got ordinal=%d, want %d); the "
    ...
    /* Stage 0: mirror catalog identity.  Ordinal 4 = LEIF, title
     * "SON OF ERIDNU" per DM1 V1 PC 3.4 mirror text. */
    memset(mirrorName, 0, sizeof(mirrorName));
    memset(mirrorTitle, 0, sizeof(mirrorTitle));
    (void)M11_GameView_GetMirrorNameByOrdinal(&game, ORDINAL_LEIF,
                                              mirrorName,
                                              (int)sizeof(mirrorName));
    (void)M11_GameView_GetMirrorTitleByOrdinal(&game, ORDINAL_LEIF,
                                               mirrorTitle,
                                               (int)sizeof(mirrorTitle));
    if (strcmp(mirrorName, "LEIF") != 0) {
        fprintf(stderr,
                "FAIL mirror catalog ordinal 4 -> '%s' (want 'LEIF')\n",
                mirrorName);
        ok = 0;
    }
    if (mirrorTitle[0] == '\0') {
        fprintf(stderr,
                "FAIL mirror catalog ordinal 4 title is empty\n");
        ok = 0;
    }
    ...
    /* Suppress the helper-note enum placeholder so the compiler
     * does not warn about the unused identifier (it exists only
     * to anchor the source-locked comment block). */
    (void)PORTRAIT_GRAPHIC_ID_HELPER_NOTE;
    ...
    if (M11_GameView_SelectFrontMirrorCandidate(&game) != 1) {
        fprintf(stderr,
                "FAIL SelectFrontMirrorCandidate returned 0 for ordinal 4\n");
        M11_GameView_Shutdown(&game);
        return 1;
    }
    ...
    if (strstr(game.inspectDetail, "LEIF") == NULL) {
        fprintf(stderr,
                "FAIL inspect detail missing champion name LEIF: '%s'\n",
                game.inspectDetail);
        ok = 0;
    }
     ...
     * This is tighter than the after_party_shuffle probe's
     * "documented value" approach — the panel is a fully-opaque
     * artwork, so a partial blit is always a regression. */
     ...
     * the warm-pixel count alone is not a useful "is LEIF here"
     * test at panel-open — the real discriminator is the C026
     * strip cell match: if the cutout contained LEIF pixels the
     * strip cell match would be ~100% (the ordinal-1 097 sibling
     * pre-candidate pose reports 100% (668/668) for ordinal 1,
     * and the south_return probe reports >= 90% for ordinal 4 at
     * the panel-off pose), but at panel-open the backdrop and
     * panel overdraw the cutout so the match must be well under
     * the panel-off baseline.  We require <= 20% match (2x the
     * observed panel-on ~9% baseline, well under the panel-off
     * 90%+ baseline and the 35% side_wall_negative drift
     * floor). */
    if (ev.portraitCutoutTotalCompared > 0 &&
        ev.portraitCutoutC026MatchedPct > PORTRAIT_CUTOUT_C026_MATCH_THRESHOLD_PCT) {
        fprintf(stderr,
                "FAIL portrait cutout contains LEIF strip cell pixels: %d%% (%d/%d) match ordinal 4\n",
    ...
    if (!M11_GameView_OpenSelectedMenuEntry(&game_dup, &menu)) {
        ...
        reset_view(&game_dup, 2, 1, 2 /* DIR_SOUTH */);
    ...
    M11_GameView_Shutdown(&game);
    printf("%s dm1 v1 HoC champion portrait ordinal 4 candidate_panel_open portrait_rect_position\n",
           ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
