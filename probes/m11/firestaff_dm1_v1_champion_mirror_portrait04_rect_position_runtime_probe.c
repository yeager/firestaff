 ...
 *   - ordinal 4 resolves through the mirror catalog to LEIF;
 *   - C026 portrait ordinal 4 is drawn at the fixed D1C source rectangle
...
static void check_leif_front_north_entry(M11_GameViewState* game,
    ...
    expect_true("ordinal 4 mirror catalog name is LEIF",
    ...
    expect_int("best portrait ordinal at D1C rect", match.bestOrdinal,
               PROBE_EXPECTED_ORDINAL);
    expect_true("ordinal 4 pixels match at D1C rect >= 90%",
                match.compared > 0 &&
...
static void check_no_floating_from_wrong_wall(M11_GameViewState* game,
    ...
    expect_true("wrong-wall/ordinary-side pose does not float ordinal-4 portrait in D1C rect",
                compared > 0 && stale * 100 < compared * 35);
    printf("  INFO: %s stale ordinal-4 pixels=%d/%d\n", label, stale, compared);
}
