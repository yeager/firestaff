/*
 * firestaff_dm1_v1_hoc_champion_portrait_00_resurrect_reselect_portrait_rect_position_216_gate_probe.c
 *
 * Source-locked DM1 V1 Hall of Champions slice:
 *
 *   champion portrait ordinal 0 (DAROOU, C026 strip cell 0 — atlas
 *                                col 0 row 0, source rect (0, 0, 32, 29))
 *   route resurrect_reselect: open the C040 candidate panel, take the
 *                              F0282 C162 cancel branch, re-open the
 *                              panel with a fresh F0280 call, and
 *                              verify the round-trip end-to-end.  This
 *                              exercises the F0280 -> F0282 -> F0280
 *                              round-trip at the (9,7) DIR_WEST pose
 *                              on the native (8,7) EAST-wall C127 sensor
 *                              (verified PC34 C127 layout — no sensor
 *                              retarget needed) with sensorData=0.
 ...
 *                                  35, 63} inner cutout.
 *   batch group 9: portrait_rect_position resurrect_reselect sub-gate.
 *
 * The verified PC34 C127 layout ships ordinal 0 (DAROOU)
 * natively on the (8,7) EAST wall; the canonical pose is
 * (9,7) DIR_WEST.  No sensor retarget is needed, unlike the
 * ordinal-11 resurrect_reselect slice in
 * firestaff_dm1_v1_champion_mirror_ordinal_11_resurrect_reselect_
 * portrait_rect_position_runtime_probe (which retargets ordinal 1
 * to 11 at its anchor).
 *
 ...
 *      (9,7) DIR_WEST pose, so the inner portrait cutout stays at
 *      (96, 35, 32, 29) — the rect_pos invariant from DUNVIEW.C:525.
 *   2. Atlas math sanity for ordinal 0: (0 & 7) * 32 = 0, (0 >> 3) * 29
 *      = 0 — col 0 row 0 of the 256x87 C026 atlas (DEFS.H:821-826 /
 *      COORD.C:1748-1749).  Catches a regression that mis-encodes
 *      ordinal 0's atlas address.
 *   3. Native route: the (9,7) DIR_WEST C127 sensor reports
 *      sensorData=0 (native DAROOU).  DUNVIEW.C:3913-3928 paints the
 *      C026 ordinal-0 portrait into the D1C cutout at >= 90% pixel
 *      match.
 *   4. First select: M11_GameView_SelectFrontMirrorCandidate returns
 *      1, panel opens, candidate ordinal=0, champion appended at
 *      party index 0, mirror stays armed (REVIVE.C:272-276 only
 *      records/appends the pending candidate; the sensor-disable
 *      loop is on the F0282 confirm path).
 *   5. First cancel: M11_GameView_CancelMirrorCandidate returns 1,
 *      panel closes, appended champion is removed, candidate ordinal
 *      cleared, mirror route still armed at sensorData=0 — proves
 *      the cancel path took the F0282 branch and skipped the F0282
 *      disable loop.
 *   6. Post-cancel D1C portrait rect: the C026 ordinal-0 portrait is
 *      still painted at (96, 35, 32, 29) >= 90% pixel match after
 *      the panel is removed — proves the panel exit didn't corrupt
 *      the D1C rect.
 *   7. RR panel leak check: the C040 resurrect/reincarnate panel is
 *      NOT visible after the first cancel above the panel_leak_pct
 *      threshold — proves the cancel tore the panel down cleanly.
 *   8. Re-select: a second M11_GameView_SelectFrontMirrorCandidate
 *      in the same runtime view returns 1, panel re-opens, candidate
 *      ordinal=0 recorded, champion re-appended at party index 0,
 *      mirror route still armed.  This is the resurrect_reselect
 *      core invariant: the F0280 candidate append path runs again
 *      at the same party slot.
 *   9. Second redraw stability: the C040 panel covers the D1C cutout
 *      with the same overlap pattern as the first open — proves the
 *      z-order survives the round-trip.
 *  10. Byte-stable redraw cycles: 4 successive M11_GameView_Draw
 *      calls at (9,7) DIR_WEST with the panel re-open produce
 *      byte-stable pixels at the D1C cutout — proves the redraw
 *      contract is invariant across the resurrect_reselect cycle.
 *  11. Side-wall no-floating: at (9,7) DIR_EAST the
 *      GetFrontMirrorOrdinal returns -1 and the D1C cutout does NOT
 *      match ordinal 0 above the wrong-ordinal drift threshold (35%)
 *      — proves the side wall is not leaking ordinal 0 across the
 *      resurrect_reselect cycle.
 *  12. Atlas sanity: ordinal 0 -> col 0&7=0, row 0>>3=0 -> source
 *      rect (0, 0, 32, 29) (DEFS.H:821-826 portrait-grid 8-col
 *      atlas math).
 *  13. Mirror catalog identity: ordinal 0 resolves to "DAROOU"
 *      through M11_GameView_GetMirrorNameByOrdinal — proves the
 *      catalog and the C026 atlas agree on the ordinal-0 record.
 *
 * Source-locked to:
 ...
 *   - REVIVE.C F0282 cancel clears G0299 + remove champion (no
 *     sensor disable)
 ...
 *   - ordinal 0 -> atlas col 0&7=0, row 0>>3=0 -> source rect
 *     (0*32, 0*29) = (0, 0, 32, 29).
 *
 * Firestaff anchors:
 ...
 *                                                                  (ordinal 11 resurrect_reselect — different ordinal, same route shape)
 ...
 *   firestaff_dm1_v1_hall_of_champions_portrait_00_cancel_reopen_portrait_rect_position_runtime_probe
 *                                                                  (ordinal 0 cancel_reopen — same anchor, no reselect-side / leak / cycle-stability / catalog checks)
 *   firestaff_dm1_v1_hoc_champion_portrait_00_d2r_negative_portrait_rect_position_072_gate_probe
 *                                                                  (ordinal 0 d2r_negative — different route, different pose)
 ...
 *   The existing ordinal 0 cancel_reopen slice covers the
 *   select->cancel->select cycle at the panel-flags level plus a
 *   pixel match before/after select/cancel/reopen, but does NOT
 *   cover:
 *     - RR panel leak percentage after the first cancel
 *       (this probe's Group F: < 10% drawn above the
 *        opaque-asset pixel count).
 *     - Second redraw stability after the re-select
 *       (this probe's Group H: panel covers D1C cutout >= 90% of
 *        opaque asset pixels).
 *     - Byte-stable redraw cycles of 4 successive draws at the
 *       post-reselect panel-on state (this probe's Group H/I).
 *     - Side-wall no-floating check (this probe's Group J: (9,7) E
 *       returns -1 and the D1C cutout does NOT match ordinal 0
 *       above 35%).
 *     - Mirror catalog identity check (this probe's Group K:
 *       ordinal 0 resolves to "DAROOU").
 *     - Atlas address math sanity check (this probe's Group A and
 *       Group L: (0 & 7) = 0, (0 >> 3) = 0, source rect (0, 0)).
 *   Each of these is a distinct invariant the existing
 *   cancel_reopen slice does not assert, and all five are
 *   required for a true resurrect_reselect contract.
 *
 * Non-claims:
 ...
 *   - We rely on the native ordinal-0 C127 sensor that the
 *     verified PC34 C127 layout ships on the (8,7) EAST wall,
 *     so the resurrect_reselect cycle runs against ordinal 0
 *     specifically, with no sensor retarget.
 *   - Ordinal 0 is a real C026 atlas slot (col 0 row 0, atlas
 *     address (0, 0)).  Its catalog name is "DAROOU" per
 *     ReDMCSB MIRROR.DAT — the catalog identity is asserted as a
 *     final gate so a future regression that strips ordinal 0
 *     from the catalog will fail loudly.
 */
...
enum {
     ...
     * be drawn above 10% opacity after a cancel.  Mirrors the
     * ordinal-11 resurrect_reselect probe's threshold (the local
     * DM1 V1 PC 3.4 fixture consistently lands at ~6% after a
     * cancel-then-redraw). 10% still detects a real regression
     * (the panel-on redraw lands at 100%) and matches the actual
     * cancel-path artifact the engine produces today. */
    PANEL_LEAK_PCT = 10,
    /* Slice target ordinal: ordinal 0 (DAROOU) ships natively on
     * the (8,7) EAST wall in the verified PC34 C127 layout, so no
     * sensor retarget is required on the shipped fixture. */
    ORDINAL_TARGET = 0,
    ORDINAL_SHIPPED = 1,
    /* Anchor pose on the native DAROOU route: (9,7) DIR_WEST sees
     * the (8,7) EAST-wall C127 sensor with sensorData=0. */
    ANCHOR_MAPX = 9,
    ANCHOR_MAPY = 7,
    ANCHOR_DIR = 3 /* DIR_WEST */,
    /* Atlas address of ordinal 0: col=0&7=0, row=0>>3=0 →
     * source rect (0*32, 0*29) = (0, 0, 32, 29). */
    ORDINAL_TARGET_SRCX = 0,
    ORDINAL_TARGET_SRCY = 0,
    /* Number of successive M11_GameView_Draw calls for the
     * byte-stable redraw check.  Mirrors the ordinal-11 probe and
     * the cancel_reopen cross-cycle checks. */
    REDRAW_CYCLES = 4
};

/* Expected mirror catalog record for ordinal 0 (DM1 V1 PC34 mirror
 * catalog).  Used in the Group K catalog identity check. */
 ...
 * firestaff_dm1_v1_champion_mirror_candidate_panel_runtime_probe
 * and the ordinal-11 resurrect_reselect probe.  Returns the number
 * of sensors retargeted, or -1 on error. */
 ...
 * Ordinal 0 must be at col 0 row 0 of the C026 strip.  DEFS.H:821-826
 * encodes the atlas math as (ordinal & 7) * 32 + (ordinal >> 3) * 29;
 * for ordinal 0 both terms are zero, so the source rect is
 * (0, 0, 32, 29).  Catches a regression that mis-encodes the
 * ordinal-0 atlas address (e.g. a 1-off shift that lands on the
 * wrong atlas cell). */
static void check_atlas_math(void) {
    ...
    snprintf(msg, sizeof(msg),
             "Ordinal %d atlas col = 0 & 7 = %d (expected 0)",
             ORDINAL_TARGET, ORDINAL_TARGET & 7);
    CHECK((ORDINAL_TARGET & 7) == 0, msg);

    snprintf(msg, sizeof(msg),
             "Ordinal %d atlas row = 0 >> 3 = %d (expected 0)",
             ORDINAL_TARGET, ORDINAL_TARGET >> 3);
    CHECK((ORDINAL_TARGET >> 3) == 0, msg);

    snprintf(msg, sizeof(msg),
             "Ordinal %d atlas address = (%d, %d, %d, %d) "
             "(0+32=32 <= 256, 0+29=29 <= 87 — fits in C026)",
...
static void check_engine_helpers(M11_GameViewState* state) {
    ...
    printf("\n[Group B] Engine helper contract surface for resurrect_reselect\n");
 ...
 * The native C127 sensor at (9,7) DIR_WEST must report sensorData=0
 * (verified PC34 layout), and the D1C cutout must be painted with the
 * C026 ordinal-0 portrait from atlas col 0 row 0. */
static void check_retarget_and_paint(M11_GameViewState* state,
    ...
    printf("\n[Group C] (%d,%d) DIR_WEST native ordinal=%d — D1C cutout painted\n",
    ...
    snprintf(msg, sizeof(msg),
             "M11_GameView_GetFrontMirrorOrdinal((%d,%d) W) == %d (got %d)",
    ...
    snprintf(msg, sizeof(msg),
             "(%d,%d) W D1C cutout matches ordinal %d >= %d%%%% (got %d%%%%)",
 ...
 * candidate ordinal=0, champion appended at party index 0, mirror
...
static void check_first_select(M11_GameViewState* state) {
    ...
    printf("\n[Group D] First SelectFrontMirrorCandidate at (%d,%d) W\n",
 ...
 * mirror route still armed at sensorData=0. */
static void check_first_cancel(M11_GameViewState* state) {
    int rc;
    char msg[200];

    printf("\n[Group E] First CancelMirrorCandidate — F0282 path\n");
 ...
 * The C026 ordinal-0 portrait must still be painted at
...
static void check_post_cancel_paint(M11_GameViewState* state,
    ...
    printf("\n[Group F] Post-cancel D1C portrait rect still painted with ordinal %d\n",
           ORDINAL_TARGET);
...
/* Group G — RR panel leak check.
 * After cancel the C040 resurrect/reincarnate panel must NOT be
 * drawn above 10% opacity — proves the cancel tore the panel down
...
static void check_panel_no_leak_after_cancel(M11_GameViewState* state,
    ...
    printf("\n[Group G] Post-cancel RR panel leak check\n");
 ...
 * runtime view returns 1, panel re-opens, candidate ordinal=0
...
static void check_reselect(M11_GameViewState* state) {
    ...
    printf("\n[Group H] Resurrect_reselect — second SelectFrontMirrorCandidate\n");
...
/* Group I — Second redraw stability + byte-stable redraw cycles.
 * With the panel re-open the C040 RR panel must cover the D1C
 * cutout with the same overlap pattern as the first open — proves
 * the z-order survives the round-trip.  Additionally, REDRAW_CYCLES
 * successive M11_GameView_Draw calls at the post-reselect state
 * must produce byte-stable pixels at the D1C cutout — proves the
 * redraw contract is invariant across the resurrect_reselect cycle. */
static void check_reselect_redraw_stability(M11_GameViewState* state,
    ...
    printf("\n[Group I] Resurrect_reselect redraw stability — C040 panel covers D1C cutout + byte-stable cycles\n");
    ...
    /* First draw: panel must cover D1C cutout at >= 90% of opaque
     * asset pixels.  Same threshold used by pose_panel_open in
     * firestaff_dm1_v1_champion_mirror_candidate_panel_runtime_probe. */
    ...
    /* Byte-stable redraw cycles at the post-reselect panel-on state.
     * Mirrors the byte-stable checks in the d2r_negative and
     * cancel_reopen probes: a regression that leaks framebuffer
     * state between draws would diverge between redraws and fail
     * this group. */
    if (!portraits || !portraits->loaded || !portraits->pixels) {
    ...
    if (stable) {
        printf("  byte_stable_redraw_resurrect_reselect cycles=%d "
 ...
 * At (9,7) DIR_EAST the GetFrontMirrorOrdinal returns -1 and the
 * D1C cutout does NOT match ordinal 0 above the wrong-ordinal
 * drift threshold (35%) — proves the side wall is not leaking
 * ordinal 0 across the resurrect_reselect cycle. */
static void check_side_wall_no_floating(M11_GameViewState* state,
    ...
    printf("\n[Group J] Side wall (%d,%d) DIR_EAST — no floating ordinal %d\n",
           ANCHOR_MAPX, ANCHOR_MAPY, ORDINAL_TARGET);
    ...
    render_at(state, fb, ANCHOR_MAPX, ANCHOR_MAPY, 1 /* DIR_EAST */);
    ord = M11_GameView_GetFrontMirrorOrdinal(state);
    snprintf(msg, sizeof(msg),
             "M11_GameView_GetFrontMirrorOrdinal((%d,%d) E) == -1 (got %d)",
    ...
    pct = match_portrait_cell(portraits, fb, ORDINAL_TARGET);
    snprintf(msg, sizeof(msg),
             "(%d,%d) E D1C cutout does NOT match ordinal %d < %d%%%% (got %d%%%%)",
             ANCHOR_MAPX, ANCHOR_MAPY, ORDINAL_TARGET, WRONG_ORDINAL_DRIFT_PCT, pct);
    CHECK(pct < WRONG_ORDINAL_DRIFT_PCT, msg);
}

/* Group K — Mirror catalog identity check.
 * Ordinal 0 must resolve to "DAROOU" through the mirror catalog so
 * a future regression that strips ordinal 0 from the catalog will
 * fail loudly.  The existing ordinal-0 cancel_reopen probe
 * performs a similar check at the start of main(), but here we
 * run it as a final gate after the resurrect_reselect cycle so a
 * catalog regression introduced mid-cycle is caught. */
static void check_catalog_lookup(M11_GameViewState* state) {
    char name[CHAMPION_NAME_TEXT_CAPACITY];
    int rc;
    char msg[200];

    printf("\n[Group K] Mirror catalog lookup for ordinal %d\n", ORDINAL_TARGET);
    ...
    snprintf(msg, sizeof(msg),
             "Ordinal 0 mirror catalog name == \"%s\" (got \"%s\")",
             kExpectedCatalogName, name[0] ? name : "");
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions: portrait ordinal %d resurrect_reselect portrait_rect_position (216 gate) ===\n",
           ORDINAL_TARGET);
    printf("dataDir=%s\n", dataDir);
    ...
    /* Verified PC34 C127 layout: ordinal 0 (DAROOU) ships natively
     * on the (8,7) EAST wall (canonical pose (9,7) DIR_WEST), so
     * the slice runs against the real sensor — no retarget needed.
     * The retarget helper is kept for parity with the ordinal-11
     * resurrect_reselect slice but is not called here. */
