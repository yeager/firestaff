 ...
 *
 *   ordinal 15           (mirror catalog record MOPHUS, "THE HEALER")
 ...
 * column 7 of the atlas (the last column, second row):
 *
 *     srcX = (15 & 7) << 5 = 224
 *     srcY = (15 >> 3) * 29 = 29
 *
 ...
 * seed that sensor to sensorData=15 to lock the ordinal-15 edge case
 * (row 1 / col 7, MOPHUS).  This keeps the probe runtime-real: same
 * sensor, same DUNGEON.DAT, same draw path - only the ordinal is
 * shifted for the test.  This is the same approach used by the
 * portrait-00 / portrait-01 / portrait-02 / portrait-03 / portrait-05
 * cancel_reopen gates.
 *
 ...
 *       the 320x200 framebuffer contains the ordinal-15 champion
 ...
 *       still carries ordinal-15 pixels after the full
...
enum {
     ...
     * MOPHUS / "THE HEALER" champion cell. */
     ...
     * to ordinal 15 for this gate so we can lock the row-1 col-7
     * edge case (MOPHUS) without changing the map layout. */
 ...
 * pixels that differ.  Used to verify ordinal 15 is a distinct portrait
 * from its row-1 neighbours (14 LEYLA, 16 CHANI).  The DM1 champion-
...
int main(int argc, char** argv) {
    ...
    int ordinal15Vs14;
    int ordinal15Vs16;
    int ordinal15Vs23; /* above in same column (row 0 col 7 = ordinal 7) is
                        * also checked in the portrait_07_redraw_after_
                        * candidate gate, so we use ordinal 23 (row 2 col 7)
                        * as a row-2-neighbour check here. */
    ...
    printf("=== DM1 V1 Hall of Champions portrait-15 / cancel_reopen / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 15
     * ----------------------------------------------------------------
    ...
    printf("\n[Group A] C026 atlas math for ordinal 15\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 col = 15 & 7 = %d (expected 7)",
                 ORDINAL_15_COL);
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 row = 15 >> 3 = %d (expected 1)",
                 ORDINAL_15_ROW);
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 atlas bottom row boundary: srcY(%d) + "
                 "portraitH(%d) = %d <= atlasH(%d) (row 1 stays inside)",
                 ORDINAL_15_SRC_Y, D1C_PORTRAIT_H,
                 ORDINAL_15_SRC_Y + D1C_PORTRAIT_H, ATLAS_H);
        CHECK(ORDINAL_15_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 atlas last-column boundary: srcX(%d) + "
                 "portraitW(%d) = %d <= atlasW(%d) (col 7 stays inside)",
                 ORDINAL_15_SRC_X, D1C_PORTRAIT_W,
                 ORDINAL_15_SRC_X + D1C_PORTRAIT_W, ATLAS_W);
        CHECK(ORDINAL_15_SRC_X + D1C_PORTRAIT_W <= ATLAS_W, msg);
    }

    /* Ordinal 15 must be a defined portrait: opaque count > 50% of the
     ...
     * ordinal 15 is treated as "no portrait" or the wrong atlas cell
     * is blitted at (224, 29, 32, 29). */
    ...
    /* Ordinal 15 must be visually distinct from its row-1 neighbours
     * (14 LEYLA, 16 CHANI) and from the row-2 col-7 cell (23 NABI,
     * which is the row-2 portrait directly below ordinal 15 in the
     * same column).  The DM1 champion-portrait atlas carries 24
     * distinct champions (one per ordinal), so a duplicate would be a
     * real regression. */
    ordinal15Vs14 = atlas_cell_distinct_percent(portraits, 15, 14);
    ordinal15Vs16 = atlas_cell_distinct_percent(portraits, 15, 16);
    ordinal15Vs23 = atlas_cell_distinct_percent(portraits, 15, 23);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 vs ordinal 14 (left neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal15Vs14);
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 vs ordinal 16 (right neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal15Vs16);
        CHECK(ordinal15Vs16 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 vs ordinal 23 (col-7 row-2 below) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal15Vs23);
        CHECK(ordinal15Vs23 >= 30, msg);
    }

    /* Ordinal 15 must resolve to MOPHUS through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-15 record. */
    nameBuf[0] = '\0';
     ...
     * 15 (MOPHUS).  Same sensor, same map cell, same draw path - only
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-15
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 15\n");
     ...
     * but the wall-ornament border stays in place.  Ordinal-15 (MOPHUS)
     * portrait pixels in the top strip are denser for some champions
     * than others (the row-1 col-7 cell is roughly 20% opaque in its
    ...
    }
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "vs 16 = %d%%, vs 23 = %d%% (all >= 30%%)",
                 ordinal15Vs14, ordinal15Vs16, ordinal15Vs23);
        CHECK(ordinal15Vs14 >= 30 && ordinal15Vs16 >= 30 &&
              ordinal15Vs23 >= 30, msg);
    }
