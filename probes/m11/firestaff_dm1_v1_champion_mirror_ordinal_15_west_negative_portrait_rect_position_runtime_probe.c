/*
 * firestaff_dm1_v1_champion_mirror_ordinal_15_west_negative_portrait_rect_position_runtime_probe.c
 *
 * Source-locked DM1 V1 Hall of Champions slice:
 *
 *   champion portrait ordinal 15 (C026 strip cell 15 — atlas col 7 row 1,
 *                                source rect (224, 29, 32, 29))
 *   route west_negative: face west from every corridor cell on (x=1)
 *                        where ordinal 15 could conceivably leak — the
 *                        engine must consistently return -1 because no
 *                        C127 sensor with sensorData=15 lives on the
 ...
 * which lists the real DM1 V1 PC 3.4 ordinal-by-cell mapping; ordinal 15
 * (MOPHUS) is the source-visible C127 sensorData=15 on map 0 cell
 * (2, 5) with M011_CELL=0 (north wall, since (2,5) is itself a WALL
 * square the only reachable pose is the forced (2,4) facing SOUTH).
 * The MOPHUS east_walkpath slice is locked in
 * firestaff_dm1_v1_hoc_mophus_ordinal15_unreachable_probe.c
 * (commit 0bbc2a2b).  This probe enforces the negative-route invariant
 * for ordinal 15 across the corridor (x=1, y=2..6) DIR_WEST band:
 *
 ...
 *      returns -1, the D1C cutout does NOT match C026 ordinal-15
 ...
 *   - ordinal 15 → atlas col 15 & 7 = 7, row 15 >> 3 = 1 →
 *     source rect (7*32, 1*29) = (224, 29, 32, 29).
 *
 * Firestaff anchors:
 ...
 *   firestaff_dm1_v1_hoc_mophus_ordinal15_unreachable_probe      (ordinal 15 east_walkpath slice)
 ...
 *     reference parity.
 *   - We do not assume a C127 sensor with sensorData=15 exists on
 ...
 *     ordinal 15 must not be painted into the D1C portrait cutout.
 *   - Ordinal 15 is a real C026 atlas slot (col 7 row 1, atlas
 *     address (224, 29)).  In the local PC 3.4 fixture the C127
 *     sensor carrying sensorData=15 is anchored to map 0 cell
 *     (2, 5) (a WALL square) with M011_CELL=0 (the cell's own
 *     north wall); the canonical forced pose is (2, 4) facing
 *     SOUTH.  The east_walkpath slice (sibling probe) already
 *     proves the MOPHUS portrait paints correctly at the forced
 *     pose; this probe proves the *west* corridor wall never
 *     leaks ordinal 15.
 */
...
enum {
     ...
     * must not contain a C026 ordinal-15 portrait.  We allow up to
     * 35% pixel match against ordinal 15 (the wrong-ordinal drift
     * threshold used by the actual-pose probe's
     * check_no_stale_ordinal_in_rect).  Above 35% means a stale
     * ordinal-15 sprite is floating over the corridor west wall. */
     ...
     * 15 (the slice target) must drift below the same wrong-ordinal
     * drift threshold (35%) used by the actual-pose probe.  Ordinals
     * 10 and 15 sit two atlas cells apart on row 0/row 1 — their
     * shared C026 backdrop and palette indices produce a non-zero
     * pixel overlap even on a correct cross-check. */
    ...
    /* Atlas address of ordinal 15: col=15&7=7, row=15>>3=1 →
     * source rect (7*32, 1*29) = (224, 29, 32, 29). */
    ORDINAL_TARGET_SRCX = 224,
...
static void check_engine_helpers(M11_GameViewState* state) {
    ...
    /* Atlas address sanity for ordinal 15: col=15&7=7, row=15>>3=1,
     * source rect (7*32, 1*29) = (224, 29, 32, 29).  If a regression
     * introduced a wrong-atlas lookup (e.g. forgetting the &7 mask)
     * ordinal 15 would resolve to (15*32, 0)=(480, 0) instead, well
     * past the 256-wide C026 strip. */
    snprintf(msg, sizeof(msg),
             "Ordinal %d atlas address is col 7 row 1 -> (224, 29) (sanity)",
             ORDINAL_TARGET);
    CHECK((ORDINAL_TARGET & 7) == 7, msg);
    snprintf(msg, sizeof(msg),
 ...
 * sensor with sensorData=15 is on the front square.  The D1C portrait
 * cutout (96, 35, 32, 29) must therefore contain zero pixels
 * matching C026 ordinal 15. */
static void check_west_negative_pixel_contract(M11_GameViewState* state,
    ...
    /* Pixel-match against C026 ordinal 15.  The cutout must NOT
     * match ordinal 15 above the wrong-ordinal drift threshold
     * (35%).  A regression that paints a stale ordinal-15 sprite
 ...
 * DIR_WEST and confirm no C127 sensor resolves to ordinal 15 on
...
static void check_positive_cross_check(M11_GameViewState* state,
    ...
    /* And it must NOT match ordinal 15 (the slice target) above
     * the tight cross-check drift threshold — proves the
     * cross-check painted the right portrait, not ordinal 15 by
     * accident.  Ordinals 10 and 15 sit in different atlas rows
     * (row 0 vs row 1) so palette/skin overlap is non-trivial;
     * 35% is the corridor-wall drift threshold and the right
     * floor here. */
