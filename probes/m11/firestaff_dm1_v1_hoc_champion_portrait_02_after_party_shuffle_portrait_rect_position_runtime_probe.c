/*
 * firestaff_dm1_v1_hoc_champion_portrait_02_after_party_shuffle_portrait_rect_position_runtime_probe.c
 *
 * Source-locked verification gate for one narrow Hall of Champions slice:
 *
 *   ordinal 2                  (mirror catalog record WU TSE / SON OF
 *                              HEAVEN; C026 atlas row 0 / col 2;
 *                              atlas address (2*32, 0*29) = (64, 0);
 ...
 *                                rect must again paint ordinal-2 at
 ...
 * M028_PORTRAIT_Y macro encoding; ordinals 0..23).  Ordinal 2 sits
 * at row 0, column 2:
 *
 *     srcX = (2 & 7) << 5 =  64
 *     srcY = (2 >> 3) * 29 =  0
 ...
 *
 * Honest slice note:
 *   The shipped DM1 V1 PC 3.4 DUNGEON.DAT does not natively map any
 *   C127 sensorData byte to ordinal 2 on the (1, 2) NORTH route --
 *   the existing ordinal-02 probes confirm this on the same build
 *   (see firestaff_dm1_v1_champion_mirror_ordinal_2_south_return_
 *   portrait_rect_position_runtime_probe and firestaff_dm1_v1_
 *   champion_mirror_ordinal_2_west_negative_portrait_rect_position
 *   _runtime_probe).  The (1, 1) front-cell south wall sensor on the
 *   canonical (1, 2) NORTH-route pose carries sensorData=1 (HALK).
 *   We seed that sensor data 1 -> 2 to lock the ordinal-2 edge case
 *   on the same (1, 2) NORTH-route cell used by the ordinal-03 and
 *   ordinal-11 sibling probes.  The seed does NOT change the map
 *   layout, the wall geometry, the F0128 redraw order, or the
 *   F0284 rotation mechanics -- it only rewrites one C127
 *   sensorData byte so the D1C blit resolves to the C026 atlas
 *   cell 2 instead of cell 1.
 *
 *   Ordinal 2 (WU TSE) is a real C026 atlas slot (DEFS.H:821-826
 *   macro math) and a real mirror-catalog record
 *   (firestaff_dm1_v1_hoc_champion_portrait_02_palette_match_rect
 *   _runtime_probe confirms 590 opaque pixels at the atlas cell
 *   and the WU TSE / SON OF HEAVEN catalog identity), but the
 *   local DM1 V1 PC 3.4 DUNGEON.DAT does not natively map any
 *   C127 sensorData byte to ordinal 2 -- the existing ordinal-2
 *   south_return probe explicitly documents the no-fixture
 *   invariant and the existing ordinal-2 west_negative probe locks
 *   the corridor-band negative.
 *
 ...
 *     - covers ordinal 2 at corridor cells using the south_return
 *       route framing (no candidate panel, no F0284 between draws).
 *       The probe SKIPs on the live DM1 V1 PC 3.4 DUNGEON.DAT
 *       because ordinal 2 has no corridor C127 sensor on that
 *       build; this probe covers the orthogonal positive
 *       portrait_rect_position invariant via sensor seeding.
 *
 *   firestaff_dm1_v1_champion_mirror_ordinal_2_west_negative_portrait_rect_position_runtime_probe
 *     - covers ordinal 2 on the corridor west wall (x=1, y=2..6)
 *       with DIR_WEST and asserts the empty D1C cutout / no
 *       floating-portrait invariant.  Negative invariant only --
 *       this probe covers the positive portrait_rect_position
 *       invariant.
 *
 *   firestaff_dm1_v1_hall_champion_portrait_02_east_walkpath_rect_position_runtime_probe
 *     - covers ordinal 2 via a synthetic atlas-slot blit at the
 *       (64, 0) source cell on a corridor east-walk sequence.  No
 *       live C127 sensor drive; this probe covers the seeded
 *       (1, 2) NORTH route.
 *
 *   firestaff_dm1_v1_hoc_champion_portrait_02_palette_match_rect_runtime_probe
 *     - covers ordinal 2 via the palette_match_rect route on the
 *       same (1, 2) NORTH seeded pose, but with a different
 *       aspect: 99% aggregate / 90% per-row / 80% per-column
 *       palette match, side_wall_no_float negative control, and
 *       redraw_after_candidate panel-on check.  This probe uses
 *       the 90% aggregate / no per-line dominance check used by
 *       the ordinal-3 / ordinal-7 / ordinal-11 after_party_shuffle
 *       sibling probes, and adds the C040 select -> F0282 cancel ->
 *       F0284 x4 -> reopen cycle that the palette_match_rect
 *       probe does NOT exercise.
 *
 *   firestaff_dm1_v1_hoc_champion_portrait_02_wake_repaint_portrait_rect_position_050_gate_probe
 *     - covers ordinal 2 (and the ordinal-1 baseline) via the
 *       wake_repaint / C146 sleep-wake route on the (1, 2) NORTH
 *       HALK pose.  No F0284 rotations; this probe adds the
 *       after_party_shuffle rotation sequence.
 *
 ...
 *     - covers ordinal 3 (AZIZI / JOHARI) on the after_party_shuffle
 *       route at (1, 2) NORTH -- same cell, same seeding pattern,
 *       but the seeded ordinal is 3 not 2.  This probe locks the
 *       orthogonal atlas row 0 / col 2 ordinal 2.
 *
 *   firestaff_dm1_v1_hoc_champion_portrait_07_after_party_shuffle_portrait_rect_position_runtime_probe
 *     - covers ordinal 7 (TIGGY / TAMAL) on the after_party_shuffle
 *       route at (2, 17) SOUTH -- natural sensor, no seeding.  This
 *       probe locks the seeded ordinal-2 edge on (1, 2) NORTH.
 *
 *   firestaff_dm1_v1_hoc_champion_portrait_11_after_party_shuffle_portrait_rect_position_runtime_probe
 *     - covers ordinal 11 (STAMM / BLADECASTER) on the
 *       after_party_shuffle route at (1, 2) NORTH with the same
 *       seeding pattern but the seeded ordinal is 11 (atlas row 1
 *       / col 3).  This probe locks the seeded ordinal-2 edge on
 *       (1, 2) NORTH at the orthogonal atlas row 0 / col 2.
 *
 * This probe proves these invariants for the after_party_shuffle
 * slice only:
 *
 *   (1) Mirror catalog identity: ordinal 2 resolves to "WU TSE" /
 *       "SON OF HEAVEN" via M11_GameView_GetMirrorNameByOrdinal /
 *       GetMirrorTitleByOrdinal.  The catalog count is at least 3
 *       so ordinal 2 is real.
 ...
 *   (3) Baseline portrait_rect_position at (1, 2) NORTH with the
 *       sensor data seeded to 2: the D1C rect paints ordinal-2
 *       pixels at >= 90% match, side walls (left x<96 row band
 *       33..64 and right x>=128 row band 33..64) stay below the
 *       warm-color threshold so the portrait does NOT float.
 ...
 *       the party NORTH -> EAST -> SOUTH at the same cell (1, 2).
 *       After the two rotations, the D1C rect must NOT paint
 *       ordinal-2 pixels (front cell changed from (1, 1) to
 *       (1, 3), the seeded C127 sensor on (1, 1) is no longer
 *       in view), and the side walls must stay below the warm
 *       threshold so the portrait does NOT float onto them.
 *   (6) F0284 second half: two more consecutive TURN_RIGHT inputs
 *       rotate the party SOUTH -> WEST -> NORTH back to (1, 2)
 *       NORTH.  The D1C rect MUST paint ordinal-2 pixels at
 *       >= 90% match again; the front cell is back at (1, 1)
 *       with the seeded sensorData=2 so the F0128 redraw must
 ...
 *       at (1, 2) NORTH opens the C040 panel with ordinal 2
 *       again; the panel-on redraw must suppress the portrait
 *       rect (BUG-120/121 panel guard).
 *   (8) Cancel + F0284 + side-wall invariant at (1, 2) EAST:
 *       after a cancel and one F0284 rotation to EAST, the
 *       D1C rect must NOT paint ordinal-2 pixels and the side
 ...
 * claimed.  The seeded C127 sensorData byte is a Firestaff
 * runtime seed for this probe only -- the bytes on disk remain
 * PC 3.4 sensorData=1 (HALK).  The panel-on redraw portrait
 * suppression is a Firestaff runtime behavior (BUG-120/121 guard),
 * not an original-DM1 behavior; that suppression is locked here as
 * a Firestaff invariant.
 */
...
enum {
     ...
     * front square (1,1) carries sensorData=1 (HALK).  We seed it to
     * TARGET_ORDINAL to lock the ordinal-2 edge case (atlas row 0 /
     * col 2 -- distinct atlas column from HALK which sits at row 0 /
 ...
 * portrait03 / cancel_reopen / ordinal-07 / ordinal-11 sibling
 * probes. */
 ...
 * ordinal-3 cancel_reopen probe, and ordinal-11
 * after_party_shuffle probe.  Resets all candidate-panel state so
 * the probe starts from a clean slate. */
...
/* Drive a single F0284 party-direction rotation via
 * M11_GameView_HandleInput(TURN_RIGHT).  Returns the input result
 * code.  Same M11_GAME_INPUT_REDRAW contract used by the ordinal-11
 * after_party_shuffle probe. */
static M11_GameInputResult rotate_party_once_right(M11_GameViewState* state) {
    return M11_GameView_HandleInput(state, M12_MENU_INPUT_TURN_RIGHT);
}
...
int main(int argc, char** argv) {
    ...
    int matchAfterReopen, matchAfterEast;
    ...
    int leftSideAfterEast, rightSideAfterEast;
    ...
    unsigned char fbAfterEast[FB_W * FB_H];
    ...
    printf("=== DM1 V1 HoC champion portrait 02 after_party_shuffle "
           "portrait_rect_position probe ===\n");
    printf("dataDir=%s target_ordinal=%d (atlas row=%d col=%d "
           "src=(%d,%d) dst=(%d,%d))\n",
           dataDir, TARGET_ORDINAL,
           TARGET_ORDINAL >> 3, TARGET_ORDINAL & 7,
           (TARGET_ORDINAL & 7) * D1C_PORTRAIT_W,
           (TARGET_ORDINAL >> 3) * D1C_PORTRAIT_H,
           96, 35);
    ...
    CHECK(catalogCount >= TARGET_ORDINAL + 1,
          "Hall mirror catalog has at least 3 entries (ordinal 2 must exist)");
     ...
     * sensor data seeded to 2 (C026 row 0 / col 2, atlas address
     * (64, 0)).
     * ---------------------------------------------------------------- */
     ...
     * still carries sensorData=2 on (1, 1), so the D1C rect must
     * paint ordinal-2 pixels again (the central no-floating
     * check). */
     ...
     * ordinal-2 pixels and the side walls must stay below the warm
     * threshold. */
     ...
     * seeded sensorData=2, so the F0128 redraw must paint
     * ordinal-2 pixels at the source-locked (96, 35) destination
    ...
    printf("\n[Group D] F0284 x2 again: SOUTH -> WEST -> NORTH at (1, 2) "
           "(portrait_rect_position must re-paint ordinal 2)\n");
     ...
     * C040 panel with ordinal 2 again; the panel-on redraw must
    ...
    /* ----------------------------------------------------------------
     * Group F - cancel + single F0284 to EAST: portrait_rect_position
     * must NOT show ordinal 2 on the side walls (no-floating
     * invariant for the after_party_shuffle route).
     * ---------------------------------------------------------------- */
    printf("\n[Group F] cancel + single F0284 to EAST: portrait must not "
    ...
    {
        M11_GameInputResult r1east;
        r1east = rotate_party_once_right(&state);
        {
            char msg[160];
            snprintf(msg, sizeof(msg),
                     ...
                     (int)r1east);
            CHECK(r1east == M11_GAME_INPUT_REDRAW, msg);
        }
        {
            char msg[160];
            snprintf(msg, sizeof(msg),
    ...
    memset(fbAfterEast, 0, sizeof(fbAfterEast));
    M11_GameView_Draw(&state, fbAfterEast, FB_W, FB_H);
    matchAfterEast = match_portrait_at_rect(portraits, fbAfterEast,
                                            TARGET_ORDINAL);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 ...
                 TARGET_ORDINAL, matchAfterEast, MATCH_DOMINANCE_PCT);
        CHECK(matchAfterEast < MATCH_DOMINANCE_PCT, msg);
    }
    leftSideAfterEast = rect_warm_count(fbAfterEast,
                                        SIDE_WALL_LEFT_X0, PORTRAIT_BAND_Y0,
                                        SIDE_WALL_LEFT_X1 - SIDE_WALL_LEFT_X0,
                                        PORTRAIT_BAND_Y1 - PORTRAIT_BAND_Y0);
    rightSideAfterEast = rect_warm_count(fbAfterEast,
                                         SIDE_WALL_RIGHT_X0, PORTRAIT_BAND_Y0,
                                         SIDE_WALL_RIGHT_X1 - SIDE_WALL_RIGHT_X0,
                                         PORTRAIT_BAND_Y1 - PORTRAIT_BAND_Y0);
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 PORTRAIT_WARM_THRESHOLD, leftSideAfterEast);
        CHECK(leftSideAfterEast < PORTRAIT_WARM_THRESHOLD, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 ...
                 "threshold %d (got %d) -- portrait must not float",
                 PORTRAIT_WARM_THRESHOLD, rightSideAfterEast);
        CHECK(rightSideAfterEast < PORTRAIT_WARM_THRESHOLD, msg);
    }
