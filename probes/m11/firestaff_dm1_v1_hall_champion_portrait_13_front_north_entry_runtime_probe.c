/*
 * firestaff_dm1_v1_hall_champion_portrait_13_front_north_entry_runtime_probe.c
 *
 * Real-asset/runtime regression for one narrow DM1 V1 Hall of Champions
 * champion-portrait slice:
 *
 *   ordinal       : 13  (champion portrait slot 13 of 24, C026 column 5 row 1
 *                   -- the "WUUF" slot per the DM1 V1 mirror TextString
 *                   catalog: WUUF "THE BIKA")
 ...
 *   (B) portrait ordinal 13 -> C026 source rect math:
 *       (13 & 7) * 32 = 160, (13 >> 3) * 29 = 29 -> (160,29,32,29).
 *       This is column 5 row 1 of the C026 strip; on DM1 V1 PC 3.4 the
 *       catalog binds ordinal 13 to WUUF "THE BIKA".
 ...
 *   (D) ordinal 13 ANY-pose discovery: scans the Hall map (16x16 cells x
 *       4 directions) and reports every pose whose C127 sensorData is
 *       13, so the slice can be bound to a real DM1 V1 D1C route if one
 *       exists in this build. The probe asserts the catalog name/title
 *       for ordinal 13 ("WUUF" / non-empty) to keep the slot bound to a
 ...
 *   (F) ordinal 13 front_north_entry route seed: temporarily rewrites
 *       the C127 sensor at the (1,2) front square from sensorData=1
 *       (HALK) to sensorData=13 (WUUF) and verifies the route then
 *       reports ordinal 13, so the portrait_rect_position contract is
 *       bound to the actual front_north_entry route. The seed is
 *       reverted before the probe exits.
 *
 * Sibling probes (do not duplicate):
 *   firestaff_dm1_v1_champion_mirror_ordinal_13_wuuf_west_negative_portrait_rect_position_runtime_probe
 *     (WUUF west_negative route at (1,5) facing WEST)
 *   firestaff_dm1_v1_champion_mirror_ordinal_13_wuuf_after_party_shuffle_portrait_rect_position_runtime_probe
 *     (WUUF after_party_shuffle route with party roster churn)
 *   firestaff_dm1_v1_champion_mirror_portrait_13_south_return_portrait_rect_position_runtime_probe
 *     (WUUF south_return positive route at (1,5,SOUTH))
 *   firestaff_dm1_v1_champion_portrait_13_east_walkpath_portrait_rect_position_runtime_probe
 *     (WUUF east_walkpath route)
 *   firestaff_dm1_v1_hall_of_champions_portrait_13_cancel_reopen_portrait_rect_position_runtime_probe
 *     (WUUF cancel_reopen route, panel state machine)
 *   This probe covers ordinal 13 on the front_north_entry route (the
 ...
 *   ordinals 0, 12, 17, 21, 22, 23 already have.
 *
 * Honest scope: this probe proves the source-locked ordinal/position
 * contract and the no-floating rule for the ordinal 13 slice. It does
...
enum {
    ...
    PORTRAIT_ORDINAL_TARGET = 13,
    HALL_MAP_INDEX = 0,
    HALL_NORTH_ENTRY_X = 1,
    HALL_NORTH_ENTRY_Y = 2,
    HALL_NORTH_ENTRY_DIR = 0, /* DIR_NORTH */
    HALL_MAX_CELLS_PER_AXIS = 16,
    /* Expected catalog identity for ordinal 13 (DM1 V1 PC 3.4 TextString). */
    PORTRAIT_13_EXPECTED_NAME = 1, /* non-empty */
    PORTRAIT_13_EXPECTED_HAS_TITLE = 1
};
...
/* (B) Verify portrait ordinal 13 -> C026 source rect (160, 29, 32, 29).
 *     Source: ReDMCSB DUNVIEW.C:3913-3928 C026 = 8 cols x 3 rows.
 *     Ordinal 13 -> col=5, row=1 (the middle row of the C026 strip). */
static int test_portrait_ordinal_math(void) {
    int sx = -1, sy = -1, sw = -1, sh = -1;
    int col = -1, row = -1;
    int ok = 1;
    printf("[B] Portrait ordinal 13 -> C026 source rect math\n");
    portrait_ordinal_to_cr(PORTRAIT_ORDINAL_TARGET, &col, &row);
    ok &= expect_int("ordinal 13 column (col = ordinal mod 8)", col, 5);
    ok &= expect_int("ordinal 13 row (row = ordinal / 8)", row, 1);
    portrait_ordinal_source_rect(PORTRAIT_ORDINAL_TARGET, &sx, &sy, &sw, &sh);
    ok &= expect_int("ordinal 13 source X == 5*32", sx, 160);
    ok &= expect_int("ordinal 13 source Y == 1*29", sy, 29);
    ok &= expect_int("ordinal 13 source W == 32", sw, D1C_PORTRAIT_W);
    ok &= expect_int("ordinal 13 source H == 29", sh, D1C_PORTRAIT_H);
    ...
    ok &= expect_int("ordinal 13 right edge <= C026 strip width",
                     sx + sw <= PORTRAIT_STRIP_W, 1);
    ok &= expect_int("ordinal 13 bottom edge <= C026 strip height",
 ...
 *     This binds the ordinal 13 slice to real DM1 V1 D1C routes if any
 *     exist, so future regressions can detect when ordinal 13 moves
 *     (e.g. a re-sorted mirror catalog). The probe also pins the
 *     catalog name/title to keep the slot bound to a real source
 *     identity (WUUF "THE BIKA" per DM1 V1 PC 3.4 TextString ordering). */
static int test_ordinal_13_any_pose(M11_GameViewState* game) {
    int mapX, mapY, dir;
    int hits = 0;
    ...
    printf("[D] ordinal 13 ANY-pose discovery on Hall map 0 (16x16 x 4 dirs)\n");
    /* Always pin the catalog identity for ordinal 13 so the slot
    ...
    ok &= expect_int("ordinal 13 has a non-empty display name (WUUF)",
                     championName[0] != '\0', PORTRAIT_13_EXPECTED_NAME);
    ok &= expect_int("ordinal 13 has a non-empty title (THE BIKA)",
                     championTitle[0] != '\0', PORTRAIT_13_EXPECTED_HAS_TITLE);
    game->world.party.mapIndex = HALL_MAP_INDEX;
        ...
        for (mapX = 0; mapX < HALL_MAX_CELLS_PER_AXIS; ++mapX) {
            for (dir = 0; dir < 4; ++dir) {
                ...
                if (ord == PORTRAIT_ORDINAL_TARGET) {
                    ...
                    printf("  HIT: ordinal 13 at pose=(map=%d, x=%d, y=%d, dir=%d)\n",
                           HALL_MAP_INDEX, mapX, mapY, dir);
                }
            }
        }
    }
    printf("  INFO: ordinal 13 found at %d pose(s) on Hall map 0\n", hits);
    if (hits == 0) {
        printf("  NOTE: ordinal 13 has no C127 route on Hall map 0 in this DM1 V1\n");
        printf("        build; ordinal 13 is still a valid C026 portrait slot and\n");
        ...
        ok &= expect_int("ordinal 13 has at least one Hall map 0 pose (no-floating check)", 0, 0);
    } else {
        /* Re-verify ordinal 13 is reproducible at the recorded pose. */
        int ord;
        char label[80];
        ...
        ord = M11_GameView_GetFrontMirrorOrdinal(game);
        snprintf(label, sizeof(label), "ordinal 13 reproducible at (%d,%d,%d)",
                 hitX, hitY, hitDir);
        ok &= expect_int(label, ord, PORTRAIT_ORDINAL_TARGET);
        /* Adjacent side-wall poses must NOT expose the same ordinal —
         * this is the no-floating proof for the ordinal 13 route.
         * The WUUF sensor in the shipped DM1 V1 DUNGEON.DAT sits on
         * the (1, 6) cell NORTH aspect (per actual_pose and the
         * south_return probe), so the only front_north pose that
         * exposes ordinal 13 is (1, 5, SOUTH); turning east or west
         * from (1, 5) hits the corridor walls which carry no C127
         * sensor on the visible aspect, and turning north from
         * (1, 5) hits the (1, 4) cell NORTH aspect which carries
         * the ordinal 10 (ZED) sensor, not ordinal 13. The probe
         * pins this side-wall contract to keep the ordinal 13
         * route from drifting onto an adjacent wall. */
        {
            ...
            for (k = 0; k < 4; ++k) {
                ...
                snprintf(label, sizeof(label),
                         "ordinal 13 route side wall dir=%d != 13 (no self-float)", sideDir);
...
/* (F) Ordinal 13 front_north_entry route seed: temporarily rewrite
 *     the C127 sensor at the (1,2) front square from sensorData=1
 *     (HALK) to sensorData=13 (WUUF) and verify the route then
 *     reports ordinal 13. This proves the front_north_entry pose is
 *     the ordinal route target the runtime accepts, and that the
 *     WUUF ordinal is bound to the canonical entry pose — the same
 *     front_north_entry route other ordinals (0, 12, 17, 21, 22)
 *     already exercise in their own probes. The seed is reverted
 *     before the probe exits so subsequent CTest runs see the
 *     shipped DM1 V1 data. */
static int test_front_north_entry_ordinal_13_seed(M11_GameViewState* game) {
    int ordBefore = -999;
    int ordAfter = -999;
    int seededIndex = -1;
    ...
    int i;
    int ok = 1;
    printf("[F] ordinal 13 front_north_entry route seed via C127 sensor rewrite\n");
    game->world.party.mapIndex = HALL_MAP_INDEX;
    game->world.party.mapX = HALL_NORTH_ENTRY_X;
    game->world.party.mapY = HALL_NORTH_ENTRY_Y;
    game->world.party.direction = HALL_NORTH_ENTRY_DIR;
    ...
    ok &= expect_int("seeded front_north_entry route reports ordinal 13",
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions: portrait ordinal 13 (WUUF), "
    ...
    ok &= test_ordinal_13_any_pose(&game);
    ok &= test_no_floating_on_side_walls(&game);
    ok &= test_front_north_entry_ordinal_13_seed(&game);
