 ...
 * portrait_13 south_return slice.
 *
 * The probe covers the three coupled concerns of cancel_reopen in
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 13: verify the C026 atlas contains
 *       a defined portrait at (160, 29, 32, 29) and that the
 *       (13 & 7) << 5 / (13 >> 3) * 29 math matches COORD.C
 *       M027/M028 macro encoding (DEFS.H:821-826).  Verifies
 *       ordinal 13 is a valid C026 atlas entry (not a degenerate
 *       cell).
 *
 *   (2) portrait_rect_position: drive a real D1C front-mirror
 *       pose from the actual DM1 V1 DUNGEON.DAT C127 sensor
 *       lattice, pixel-prove the destination rectangle
 *       (96, 35, 32, 29) on the 320x200 framebuffer contains the
 *       ordinal-13 champion portrait, and that the side walls
 *       (left of x=96 and right of x=127 in the portrait row band)
 *       do NOT carry the portrait's palette.
 *
 ...
 *       still carries ordinal-13 pixels after the full
 *       select->cancel->select cycle.  This is the cancel_reopen
 *       slice for ordinal 13 (WUUF); it is disjoint from the
 *       existing portrait_13 south_return probe (different aspect:
 *       no select/cancel cycle, no panel-off/panel-on rect
 *       transitions) and from the wuuf_after_party_shuffle probe
 *       (different route: cancel_reopen does not invoke F0284
 *       rotations or C160 close, only F0282 C162 cancel).
 *
 * Source evidence:
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 13 in the C026 atlas: (13 & 7) << 5 = 160,
     * (13 >> 3) * 29 = 29. */
    ORDINAL_13_COL = 13 & 7,         /* = 5 */
    ORDINAL_13_ROW = 13 >> 3,        /* = 1 */
    ORDINAL_13_SRC_X = ORDINAL_13_COL << 5,    /* = 160 */
    ORDINAL_13_SRC_Y = ORDINAL_13_ROW * 29,    /* =  29 */
...
static const char kExpectedCatalogName[] = "WUUF";
 ...
 * Used to verify ordinal 13 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 13 is a distinct
 * portrait from its row-1 neighbours (12, 14).  The DM1 champion-
 ...
 * the D1C portrait rect at viewport (96, 35). */
static void park_wuuf_south_route(M11_GameViewState* state) {
    state->world.party.mapIndex = 0;
    state->world.party.mapX = WUUF_ROUTE_X;
    state->world.party.mapY = WUUF_ROUTE_Y;
    state->world.party.direction = WUUF_ROUTE_DIR;
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-13 / cancel_reopen / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 13
     * ----------------------------------------------------------------
    ...
    printf("\n[Group A] C026 atlas math for ordinal 13\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 col = 13 & 7 = %d (expected 5)",
                 ORDINAL_13_COL);
        CHECK(ORDINAL_13_COL == 5, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 row = 13 >> 3 = %d (expected 1)",
                 ORDINAL_13_ROW);
        CHECK(ORDINAL_13_ROW == 1, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_13_SRC_X, ORDINAL_13_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_13_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_13_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    /* Ordinal 13 must be a defined portrait: opaque count > 50% of the
     ...
     * ordinal 13 is treated as "no portrait" or the wrong atlas cell
     * is blitted at (160, 29, 32, 29). */
    ordinal13Opaque = atlas_cell_opaque_count(portraits, 13);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal13Opaque);
        CHECK(ordinal13Opaque >= 100, msg);
    }

    /* Ordinal 13 must be visually distinct from its row-1 neighbours
     * (12 ALORN, 14 VENZAR).  The DM1 champion-portrait atlas carries
     * 24 distinct champions (one per ordinal), so a duplicate would
     * be a real regression. */
    ordinal13Vs12 = atlas_cell_distinct_percent(portraits, 13, 12);
    ordinal13Vs14 = atlas_cell_distinct_percent(portraits, 13, 14);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 vs ordinal 12 (left neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal13Vs12);
        CHECK(ordinal13Vs12 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 vs ordinal 14 (right neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal13Vs14);
        CHECK(ordinal13Vs14 >= 30, msg);
    }

    /* Ordinal 13 must resolve to WUUF through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-13 record. */
    nameBuf[0] = '\0';
                                                        ...
                                                        (int)sizeof(nameBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 13 to \"%s\" (expected \"%s\")",
     ...
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-13
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,5,SOUTH)=13\n");
    ...
    /* The D1C portrait rect must contain ordinal-13 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-13
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 13\n");
    ...
    countAfterSelect = state.world.party.championCount;
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "SelectFrontMirrorCandidate on (1,5,SOUTH) returns 1 (got %d)",
                 selectRc);
     ...
     * 121 panel guard).  Match against ordinal 13 should be low. */
     ...
     * leader hand empty, sensorData=13 still on the front square
     * (the cancel branch returns BEFORE the mirror sensor disable
     * loop at REVIVE.C:785-799, so the route remains armed for the
     * next sensor trigger), so the reopen must succeed and the panel
     * must come back live. */
    ...
    countAfterReopen = state.world.party.championCount;
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "SelectFrontMirrorCandidate reopen on (1,5,SOUTH) returns 1 "
                 "(got %d)",
                 reopenRc);
     ...
     * but the wall-ornament border stays in place.  Ordinal-13 (WUUF)
     * portrait pixels in the top strip are denser for some champions
     * than others (the row-1 col-5 cell is roughly 60% opaque in its
     ...
     * we verify the same rect lines up with ordinal-13 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 13 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 13 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (160, 29, 32, 29) in the atlas.  This is the
     * "ordinal 13 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 13 atlas round-trip: source (160, 29) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 13 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((13 & 7) << 5, (13 >> 3) * 29, 32, 29)",
                 ORDINAL_13_SRC_X, ORDINAL_13_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_13_SRC_X == 160 && ORDINAL_13_SRC_Y == 29, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 13 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal13Opaque);
        CHECK(ordinal13Opaque >= 200 && ordinal13Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 13 round-trip distinctness: vs 12 = %d%%, "
                 "vs 14 = %d%% (both >= 30%%)",
                 ordinal13Vs12, ordinal13Vs14);
        CHECK(ordinal13Vs12 >= 30 && ordinal13Vs14 >= 30, msg);
    }
