/*
 * firestaff_dm1_v1_hoc_champion_portrait_02_door_nearby_no_float_runtime_probe.c
 *
 * Source-locked verification gate for one narrow Hall of Champions slice
 * that the existing portrait_02 cancel_reopen, leave_and_reenter,
 * palette_match_rect, wake_repaint, after_party_shuffle, east_walkpath,
 * west_negative, d2l_negative, and south_return probes do not cover:
 *
 *   ordinal 2                  (mirror catalog record GOTHMOG in the
 *                              shipped PC 3.4 EN DUNGEON.DAT;
 *                              "WU TSE" / "SON OF HEAVEN" alias slot
 *                              in some multilingual ship-outs.
 *                              The probe asserts the catalog lookup
 *                              succeeds and the name resolves to a
 *                              non-empty string but does NOT hard-code
 *                              the name itself, mirroring the
 *                              ordinal-02 leave_and_reenter /
 *                              palette_match_rect probes' honest-data
 *                              treatment of catalog variance.)
 ...
 *
 *   - palette_match_rect exercises strict per-pixel palette match
 *     on the (1,2) NORTH pose with the C127 sensor retargeted
 *     to ordinal 2.  This probe drives the door-block leg AFTER
 *     the same sensor retarget, so palette_match_rect covers the
 *     panel-off pixel math and this probe covers the door-block
 *     redraw stability.  Different route, different assertion
 *     surface, no duplicated invariants.
 *
 *   - wake_repaint covers the C146 sleep/wake-up repaint cycle
 *     on the (1,2) HALK ordinal-1 baseline (no ordinal-2 path).
 *
 *   - after_party_shuffle covers the F0284 rotation + C160 confirm
 *     close path, leaving the panel-open state immediately.
 *     This probe never opens the C040 panel.
 *
 *   - east_walkpath covers the corridor walk east at y=2..5 with
 *     a synthetic ordinal-2 atlas blit; this probe drives a
 *     forward-north step into the closed mirror door.
 *
 *   - south_return / west_negative / d2l_negative cover
 *     corridor scan / view-square no-float / corridor west wall
 *     for ordinal 2.  None of them seed the (1,2) NORTH front
 *     mirror or drive the door-block leg.
 *
 *   - the existing wall_ornament_no_float slice targets ordinal
 *     19 (HAWK) and verifies the C346 frame backing + C026/C346
 *     isolation.  This probe targets ordinal 2 and verifies the
 *     no-float invariant under the specific movement context of
 *     a door-block attempt.
 *
 *   - the existing ordinal-06 door_nearby_no_float slice targets
 *     ordinal 6 (SYRA).  This probe targets ordinal 2 and uses
 *     the same seed pattern (HALK ordinal-1 -> ordinal-2 retarget
 *     via seed_first_c127_data).  Different ordinal, different
 *     atlas row (still row 0, but col 2 vs col 6), different
 *     catalog record.  No invariant overlap.
 *
 * The "door_nearby_no_float" route verifies three coupled concerns
 * on the (1,2) NORTH closed-mirror-door pose:
 *
 *   (1) Portrait rect still holds ordinal 2 after a door-block
 ...
 *       ordinal-2 pixels at >= 90% match after the door-block
 *       attempt; this proves the door-block redraw does not
 *       suppress or distort the GOTHMOG portrait sprite.
 *
 ...
 *       A subsequent SelectFrontMirrorCandidate must still return 1
 *       (sensor still alive), and a CancelMirrorCandidate must
 *       close the panel cleanly so the engine exits in a clean
 *       state.
 *
 * Source evidence:
 ...
 *   - DEFS.H:2186 (C026_GRAPHIC_CHAMPION_PORTRAITS = 26,
 *                   256x87 strip of 32x29 portraits)
 ...
 *   - firestaff_dm1_v1_hall_of_champions_portrait_02_leave_and_reenter
 *     (companion probe for the (1,2) NORTH pose; this probe
 *     adds the door-block INPUT_UP axis)
 *   - firestaff_dm1_v1_hall_of_champions_portrait_02_cancel_reopen
 *     (companion probe for the select->cancel->select axis)
 *   - firestaff_dm1_v1_hoc_champion_portrait_02_palette_match_rect
 *     (companion probe for the strict per-pixel palette match
 *     on the same sensor-retargeted pose)
 *   - firestaff_dm1_v1_hoc_champion_portrait_06_door_nearby_no_float
 *     (template: same route variant on ordinal 6 SYRA)
 *   - firestaff_dm1_v1_hoc_champion_portrait_19_wall_ornament_no_float
 *     (template: sibling no-float pattern; ordinal 19 HAWK)
 */
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 2 in the C026 atlas: (2 & 7) << 5 = 64, (2 >> 3) * 29 = 0.
     * This is the THIRD column of the TOP row of the 8x3 atlas
     * (row 0, col 2) - the ordinal-2 slot. */
    ORDINAL_2_COL = 2 & 7,           /* = 2 */
    ORDINAL_2_ROW = 2 >> 3,          /* = 0 */
    ORDINAL_2_SRC_X = ORDINAL_2_COL << 5,   /* = 64 */
    ORDINAL_2_SRC_Y = ORDINAL_2_ROW * 29,   /* =  0 */
 ...
 * Used to verify ordinal 2 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 2 is a distinct portrait
 * from its row-0 neighbours (1, 3). */
 ...
 * the ordinal-06 door_nearby_no_float / leave_and_reenter /
 * palette_match_rect probes, kept local so this probe is self
 * contained. */
 ...
 * seed_first_c127_data the same square reports ordinal 2. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-02 / door_nearby_no_float / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 2
     * ----------------------------------------------------------------
     * Verify the C026 atlas contains a defined portrait at row 0 /
     * column 2 and that the math matches COORD.C / DEFS.H:821-826.
     * Identical Group A pattern to the ordinal-06 door_nearby_no_float
     * probe and the ordinal-02 leave_and_reenter /
     * palette_match_rect probes so the ordinal-02 probes share a
     * common atlas baseline. */
    printf("\n[Group A] C026 atlas math for ordinal 2\n");
    ...
    }
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 ORDINAL_2_ROW);
        CHECK(ORDINAL_2_ROW == 0, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 2 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_2_SRC_X, ORDINAL_2_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_2_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_2_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    ordinal2Opaque = atlas_cell_opaque_count(portraits, 2);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 2 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal2Opaque);
        CHECK(ordinal2Opaque >= 100, msg);
    }

    ordinal2Vs1 = atlas_cell_distinct_percent(portraits, 2, 1);
    ordinal2Vs3 = atlas_cell_distinct_percent(portraits, 2, 3);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 2 vs ordinal 1 (left neighbour, HALK) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal2Vs1);
        CHECK(ordinal2Vs1 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 2 vs ordinal 3 (right neighbour, WU TSE/GOTHMOG "
                 "row-0 neighbour) differ by >= 30%% (got %d%%)",
                 ordinal2Vs3);
        CHECK(ordinal2Vs3 >= 30, msg);
    }

    /* Mirror catalog identity: ordinal 2 resolves to a non-empty
     * catalog record in the shipped DM1 V1 DUNGEON.DAT (GOTHMOG in
     * PC 3.4 EN, WU TSE in some multilingual ship-outs).  The probe
     * only asserts the catalog returns a non-empty name + title so
     * a future catalog edit doesn't break the probe. */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
                                                         ...
                                                         (int)sizeof(titleBuf));
    {
        char msg[240];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 2 to a non-empty name "
                 "(\"%s\", lookup rc=%d) and title (\"%s\", lookup rc=%d)",
                 nameBuf[0] ? nameBuf : "", nameLookupRc,
                 titleBuf[0] ? titleBuf : "", titleLookupRc);
        CHECK(nameLookupRc > 0 && nameBuf[0] != '\0' &&
              titleLookupRc > 0 && titleBuf[0] != '\0', msg);
    }

    /* Park the party on the (1,2,0) NORTH-route front mirror, then
     * seed the C127 sensor from HALK (1) to ordinal 2.  Same
     ...
     * ordinal-2 pixels.  Same pose as the cancel_reopen /
     * leave_and_reenter / palette_match_rect probes' Group B so
     * all ordinal-02 probes share a common baseline. */
    printf("\n[Group B] portrait_rect_position baseline on real C127 sensor pose (1,2,0)=2\n");
     ...
     *           D1C portrait rect must still carry ordinal-2
     *           pixels at >= 90% match.
     * ----------------------------------------------------------------
     ...
     * still carry ordinal-2 pixels after the door-block attempt;
     * this proves the door-block redraw does not suppress the
     * GOTHMOG portrait sprite. */
     ...
     * movement), so the front mirror must still report ordinal 2. */
    frontOrdinal = M11_GameView_GetFrontMirrorOrdinal(&state);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                                                  ...
                                                  TARGET_ORDINAL);
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "redraw does not suppress the GOTHMOG portrait sprite",
     ...
     * portrait rect is still correctly indexed to ordinal 2. */
     ...
     *           and the same rect lines up with ordinal-2 pixels
