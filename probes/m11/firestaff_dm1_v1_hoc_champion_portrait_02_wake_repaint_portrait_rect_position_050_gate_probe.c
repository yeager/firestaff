 ...
 * wake_repaint / portrait_rect_position runtime probe.
 *
 * Targeted slice:
 *   ordinal    = 2 (the C127 sensorData=2 / GOTHMOG slot in the
 *                 reference DUNGEON.DAT; retargeted on (1,2) NORTH
 *                 for real-asset build coverage — see the
 *                 portrait_02 cancel_reopen companion probe for the
 *                 retarget rationale)
 *   route      = wake_repaint
 ...
 *                  through F0128.  The resting flag additionally
 *                  draws a small RESTING... overlay on top of the
 *                  viewport at framebuffer (100, 70, 120, 30) via
 *                  m11_game_view.c, so the wake_repaint route
 *                  asserts: (1) the resting overlay is on while
 *                  resting, (2) the wake flag clear plus a redraw
 *                  removes the overlay, and (3) the D1C portrait
 *                  cutout (96, 35, 32, 29) holds the ordinal 2
 *                  portrait at every stage of the cycle, not just
 *                  the post-cancel state that the existing
 *                  cancel_reopen / redraw_after_candidate probes
 *                  cover.)
 ...
 *   - firestaff_dm1_v1_hall_of_champions_portrait_02_cancel_reopen_*
 ...
 *     and the equivalent 21 probe cover panel state transitions
 *     (open/close/reopen) but never set G0300_B_PartyIsResting,
 *     so the wake repaint slice is uncovered.
 ...
 *     wake_repaint cycle through the viewport redraw path.
 *   - firestaff_dm1_v1_champion_mirror_walkpath_runtime_probe
 *     and the east_walkpath ordinal-2 probe cover cardinal
 *     navigation but not the rest/wake flag cycle.
 *
 * What the probe asserts at each stage:
 *   Stage 1 (pre-rest):    D1C cutout (96,35,32,29) shows ordinal 2
 ...
 *                          (96,35,32,29) with the ordinal 2
 *                          portrait visible behind the overlay
 *                          (the cutout is a different rectangle
 *                          from the resting overlay, so the
 *                          portrait_rect position invariant must
 *                          still hold even when the overlay is
 *                          drawn on top of the rest of the
 *                          viewport).
 ...
 *                          must still show the ordinal 2 portrait
 ...
 *                          NOT show ordinal 2 pixels (no floating).
 ...
 * to ordinal 2 — same retarget the cancel_reopen companion probe
 * uses).  The RESTING overlay path is sourced from
 ...
 *
 * Usage:
 *   firestaff_dm1_v1_hoc_champion_portrait_02_wake_repaint_portrait_rect_position_050_gate_probe DATA_DIR
 */
...
enum {
     ...
     * framebuffer-local.  The viewport starts at (0, 33) so the
     * viewport-local rectangle is (100, 37, 120, 30).  The D1C
     * portrait cutout (96, 35, 32, 29) viewport-local is
     * disjoint from the RESTING overlay (the cutout occupies
     * x=96..127 viewport-local and y=35..63 viewport-local;
     * the overlay occupies x=100..219 viewport-local and
     * y=37..66 viewport-local — they overlap in x and y but the
     * D1C cutout is the top-left 32x29 corner of the overlay
     * region; the wake_repaint slice must show that the
     * portrait_rect position invariant still holds while the
     * RESTING overlay is drawn on top of the lower-right
     * portion of the cutout.  Since the overlay is drawn on
     * the final overlay pass (after F0128), the wake_repaint
     * slice asserts the portrait_rect is preserved in the
     * D1C cutout area: the cutout (96, 35, 32, 29) viewport
     * is the same anchored rectangle regardless of the resting
     * flag. */
    RESTING_X_FB = 100,
    RESTING_Y_FB = 70,
    RESTING_W = 120,
    RESTING_H = 30,
    /* Hall of Champions ordinal 2 = GOTHMOG (per ReDMCSB DUNVIEW.C
     * G0289 nibble decode table).  C127 sensorData is 0-indexed so
     * the M11 ordinal = 2 is the third cell in the portrait strip:
     * column 2, row 0. */
    ORDINAL_TARGET = 2,
     ...
     * tolerance is the same as the cancel_reopen probe
 ...
 * real DM1 V1 fixture, HALK) to ordinal 2.  Same retarget pattern the
 * cancel_reopen probe uses for the (1,2) NORTH slice — the
...
static int check_stage_pre_rest(M11_GameViewState* game,
    ...
    if (frontOrdinal != ORDINAL_TARGET) {
         ...
         * the sensor's sensorData is not the target ordinal 2.  Apply
...
static int check_stage_resting(M11_GameViewState* game,
     ...
     *       RECTANGLE, not "ordinal 2 visible at all times".  The
     ...
     * The wake_repaint slice does NOT assert ordinal-2 dominance
...
/* ── Stage 5: side/no-front pose does not float ordinal 2 ───────── */
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s DATA_DIR\n"
                "  verifies ordinal 2 wake_repaint portrait_rect_position\n",
                argv[0]);
        return 2;
    }
    dataDir = argv[1];
    ...
    printf("=== DM1 V1 HoC champion portrait ordinal 2 wake_repaint "
    ...
    if (rc < 0) {
        M11_GameView_Shutdown(&game);
        printf("SKIP dm1 v1 HoC champion portrait ordinal 2 wake_repaint portrait_rect_position\n");
        return 0;
    }
    if (rc) PASS();
