/*
 * firestaff_dm1_v1_hall_of_champions_portrait_09_popup_focus_return_portrait_rect_position_runtime_probe.c
 *
 * Source-locked verification gate for one narrow Hall of Champions slice:
 *
 *   ordinal 9           (mirror catalog record ZED, title DUKE OF BANVILLE)
 ...
 * so we seed that sensor to sensorData=9 to lock the ordinal-9 edge
 ...
 *
 * This probe covers the four coupled concerns of popup_focus_return in
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 9: verify the C026 atlas contains a
 *       defined portrait at (32, 29, 32, 29) and that the
 *       (9 & 7) << 5 / (9 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 9 is a
 *       valid C026 atlas entry (not a degenerate cell), and that
 *       the row-1 column-1 atlas math is identical to row-1 col-0
 *       (ordinal 8 IAIDO, already covered) plus 32 pixels of
 *       horizontal stride.
 *
 *   (2) portrait_rect_position: drive a real D1C front-mirror pose
 *       from the actual DM1 V1 DUNGEON.DAT C127 sensor lattice
 *       (with sensorData seeded to 9), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-9 champion portrait (ZED), and that
 ...
 *       (candidateMirrorPanelActive=1, candidateMirrorOrdinal=9,
 ...
 *       ordinal-9 pixels after the full
 ...
 *
 * This slice is disjoint from the existing row-0/row-1 cancel_reopen
 * probes for ordinals 0, 1, 2, 3, 5, 6, 8 (different ordinals +
 * different route: no focus-absorption step between select and
 * cancel), from the row-2 cancel_reopen probes for ordinals 16, 17,
 * 18, 19, 20 (different ordinals), from the existing
 * portrait_09 south_return portrait_rect_position probe (different
 * route: south-facing pose at (1,5) SOUTH, sensor seeded at the
 * (1,6) NORTH aspect, no panel interaction), from the
 * portrait_09 west_negative portrait_rect_position probe (different
 * route: facing-WEST negative prove), and from
 * portrait_14_redraw_after_candidate / portrait_22_redraw_after_candidate /
 * portrait_23_redraw_after_candidate (different ordinals +
 * different route: no focus-absorption step).  Ordinal 9 has no
...
enum {
    ...
    ORDINAL_9_COL = 9 & 7,        /* = 1 */
    ORDINAL_9_ROW = 9 >> 3,       /* = 1 (the second row of the 8x3 grid) */
     ...
     * to ordinal 9 (ZED) for this gate so we can lock the
     * ordinal-9 edge case without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1,
    /* Row-1 atlas neighbours of ordinal 9 (IAIDO = 8 left,
     * STAMM = 10 right).  The DM1 champion-portrait atlas carries
     * 24 distinct champions (one per ordinal), so a duplicate
     * would be a real regression. */
    ROW1_LEFT_ORDINAL = 8,
    ROW1_RIGHT_ORDINAL = 10
};
/* Mirror catalog record name for ordinal 9 (DM1 V1 PC34 mirror
 * catalog).  Used to assert the catalog resolves correctly.  ZED
 * (title DUKE OF BANVILLE) is the 10th valid mirror text string
 * in the shipped DM1 V1 DUNGEON.DAT (the 10th ordinal = 9, after
 ...
 * pixels that differ.  Used to verify ordinal 9 is a distinct
 * portrait from its row-1 neighbours (8 IAIDO, 10 STAMM). */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-09 / popup_focus_return / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 9 (row 1 / column 1)
     * ----------------------------------------------------------------
     ...
     * row-1 ordinal after ordinal 8 (IAIDO) covered by the
     * cancel_reopen route: ordinals 0, 1, 2, 3, 5, 6, 8 are
     * already covered for row-0/row-1.  Ordinal 9 sits at row 1 /
     * col 1, advancing the row-1 atlas layout one column farther
     * right. */
    printf("\n[Group A] C026 atlas math for ordinal 9 (row 1 / col 1)\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 9 row = 9 >> 3 = %d (expected 1, second row)",
                 ORDINAL_9_ROW);
    ...
    /* Ordinal 9 must be a defined portrait: opaque count > 100 of the
     ...
     * ordinal 9 (ZED) is treated as "no portrait" - in particular
     * a regression where (ordinal >> 3) * 29 yields the wrong row on
     * the C026 strip (e.g. row 1 -> row 0 off-by-one). */
    ...
    /* Ordinal 9 must be visually distinct from its row-1 neighbours
     * 8 (IAIDO) and 10 (STAMM).  The DM1 champion-portrait atlas
     * carries 24 distinct champions (one per ordinal), so a
     * duplicate would be a real regression.  Picking the row-1
     * neighbours here pins the row-1 atlas layout specifically and
     * complements the row-1 ordinal 8 (IAIDO, row 1 col 0) row-1
     * anchor check. */
    ordinal9Vs8 = atlas_cell_distinct_percent(portraits, 9, 8);
    ordinal9Vs10 = atlas_cell_distinct_percent(portraits, 9, 10);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 9 vs ordinal 8 (row-1 left neighbour, IAIDO) "
                 "differ by >= 30%% (got %d%%)",
                 ordinal9Vs8);
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 9 vs ordinal 10 (row-1 right neighbour, STAMM) "
                 "differ by >= 30%% (got %d%%)",
                 ordinal9Vs10);
        CHECK(ordinal9Vs10 >= 30, msg);
    }

    /* Ordinal 9 must resolve to ZED through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-9 record - in particular a
     * regression where the catalog is shorter than 10 entries (a
     * catalog count < 10 would have rejected sensorData=9 on the
     * real DUNGEON.DAT). */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
     ...
     * seed the C127 sensor from HALK (1) to ordinal 9 (ZED).  Same
     ...
     * 9.  Same sensor, same map cell, same draw path - only G0289
     * changes. */
     ...
     * has at least 10 entries (which is the source-locked DM1 V1
     ...
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=9\n");
    ...
    /* The D1C portrait rect must contain ordinal-9 source pixels at
     ...
     * 121 panel guard).  Match against ordinal 9 should be low. */
     ...
     * still carry ordinal-9 pixels after the full
     * select->(focus-stays-during-absorbed-inputs)->cancel->select
    ...
    printf("\n[Group D] popup_focus_return: cancel then reopen, portrait rect still carries ordinal 9\n");
     ...
     * still active (sensorType=127, sensorData=9 after seed). */
     ...
     * leader hand empty, sensorData=9 still on the front square, so
     ...
     * we verify the same rect lines up with ordinal-9 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group E - ordinal 9 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 9 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (32, 29, 32, 29) in the atlas.  This is the
     * "ordinal 9 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group E] ordinal 9 atlas round-trip: source (32, 29) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 9 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((9 & 7) << 5, (9 >> 3) * 29, 32, 29)",
                 ORDINAL_9_SRC_X, ORDINAL_9_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_9_SRC_X == 32 && ORDINAL_9_SRC_Y == 29, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 9 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal9Opaque);
        CHECK(ordinal9Opaque >= 200 && ordinal9Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 9 round-trip distinctness: vs 8 = %d%%, "
                 "vs 10 = %d%% (both >= 30%%)",
                 ordinal9Vs8, ordinal9Vs10);
        CHECK(ordinal9Vs8 >= 30 && ordinal9Vs10 >= 30, msg);
    }
