/*
 * DM1 V1 Hall of Champions portrait ordinal 18 (SONJA / SHE DEVIL)
 * redraw_after_candidate / portrait_rect_position runtime probe.
 *
 * Targeted slice:
 *   ordinal = 18 (SONJA, C026 atlas col 2 row 2, source rect
 *                  (64, 58, 32, 29))
 *   pose    = (map 0, x=1, y=3) facing EAST
 *             (real PC 3.4 Hall route from the actual-pose fixture:
 *              the front cell (2,3) has the SONJA C127 sensor on the
 *              WEST wall, sensorData=18)
 *   route   = redraw_after_candidate
 *             pre-candidate -> panel-open -> confirm-disable, plus a
 *             fresh select/cancel redraw and repeated portrait-rect
 *             byte-stability checks
 *   aspect  = portrait_rect_position
 *             C026 portrait pixels stay anchored at the source-locked
 *             D1C cutout (96, 35, 32, 29) viewport-local.
 *
 * Source-locked to ReDMCSB WIP 20210206:
 *   DUNGEON.C:2558       resets G0289 before wall-square sensor scan.
 *   DUNGEON.C:2570-2573  maps M011_CELL(sensor) against view direction.
 *   DUNGEON.C:2608-2612  stores C127 sensorData in G0289 for portrait use.
 *   DUNVIEW.C:525        G0109 portrait box = {96, 127, 35, 63}.
 *   DUNVIEW.C:3913-3919  D1C champion portrait blit from C026; ordinal
 *                         source = ((ord & 7) << 5, (ord >> 3) * 29).
 *   DUNVIEW.C:8318-8618  F0128 viewport redraw from party pose.
 *   MOVESENS.C:1501-1503 dispatches C127 sensorData to REVIVE.C F0280.
 *   REVIVE.C:124-132     F0280 candidate gate requires empty hand/party slot.
 *   REVIVE.C:744-783     F0282 cancel branch preserves the mirror route.
 *   REVIVE.C:785-799     F0282 confirm path disables the mirror sensor.
 *   COMMAND.C:1956-1990  resurrect/reincarnate panel click dispatch.
 *   DEFS.H:821-826       M027_PORTRAIT_X / M028_PORTRAIT_Y atlas math.
 *   DEFS.H:2186          C026_GRAPHIC_CHAMPION_PORTRAITS.
 *   DEFS.H:2552          M552_FRONT_WALL_ORNAMENT_ORDINAL.
 *
 * This is Firestaff runtime evidence against local DM1 PC 3.4 data. It
 * does not claim original DOS pixel parity.
 *
 * Usage:
 *   firestaff_dm1_v1_hoc_champion_portrait_18_redraw_after_candidate_portrait_rect_position_runtime_probe DATA_DIR
 */
...
static int portrait_match_count(const M11_AssetSlot* portraits,
    ...
    for (y = 0; y < PORTRAIT_H; ++y) {
        ...
        for (x = 0; x < PORTRAIT_W; ++x) {
            ...
            /* ReDMCSB DUNVIEW.C:3913-3919 blits C026 with color 1
             * transparent. SONJA's row-2 cell has dark backing pixels, so
             * skip source color 12 as the ordinal-10/21 redraw probes do. */
...
static int check_catalog_identity(M11_GameViewState* game) {
    ...
    name[0] = '\0';
    title[0] = '\0';
    (void)M11_GameView_GetMirrorNameByOrdinal(game, ORDINAL_SONJA,
                                              name, sizeof(name));
    (void)M11_GameView_GetMirrorTitleByOrdinal(game, ORDINAL_SONJA,
                                               title, sizeof(title));
    ok = strcmp(name, "SONJA") == 0 && strcmp(title, "SHE DEVIL") == 0;
    if (ok) {
        pass("ordinal 18 catalog identity is SONJA / SHE DEVIL");
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg),
                 "ordinal 18 catalog identity got name=\"%s\" title=\"%s\"",
                 name, title);
        fail(msg);
    }
    return ok;
...
static int draw_and_expect_portrait(M11_GameViewState* game,
    ...
    snprintf(msg, sizeof(msg),
             "%s ordinal 18 is best C026 match best=%d matched=%d second=%d",
             label, ev.bestOrdinal, ev.bestMatched, ev.secondMatched);
    if (ev.bestOrdinal == ORDINAL_SONJA &&
...
static int draw_and_expect_no_sonja(M11_GameViewState* game,
                                    const M11_AssetSlot* portraits,
                                    const char* label) {
    unsigned char fb[FB_W * FB_H];
    PortraitEvidence ev;
    ...
    collect_portrait_evidence(portraits, fb, ORDINAL_SONJA, &ev);
    snprintf(msg, sizeof(msg),
             "%s ordinal 18 absent from D1C cutout (<%d%%) got=%d%%",
...
static int check_panel_open(M11_GameViewState* game,
    ...
    memset(fb, 0, sizeof(fb));
    M11_GameView_Draw(game, fb, FB_W, FB_H);
    collect_portrait_evidence(portraits, fb, ORDINAL_SONJA, &portrait);
    ...
    ok &= expect_int("panel_open candidateMirrorOrdinal",
                     game->candidateMirrorOrdinal, ORDINAL_SONJA);
    snprintf(msg, sizeof(msg),
    ...
    snprintf(msg, sizeof(msg),
             "panel_open ordinal 18 no longer has a full D1C portrait match (<%d%%) got=%d%%",
...
static void copy_portrait_rect(unsigned char* dst, const unsigned char* fb) {
    int y;
    for (y = 0; y < PORTRAIT_H; ++y) {
        const unsigned char* row = fb + fb_y(PORTRAIT_Y_VP + y) * FB_W +
                                   fb_x(PORTRAIT_X_VP);
        memcpy(dst + y * PORTRAIT_W, row, PORTRAIT_W);
    }
}

static int check_rect_byte_stability(M11_GameViewState* game,
                                     const M11_AssetSlot* portraits,
                                     const char* label) {
    unsigned char fb[FB_W * FB_H];
    unsigned char baseline[PORTRAIT_W * PORTRAIT_H];
    unsigned char current[PORTRAIT_W * PORTRAIT_H];
    int i;
    int ok = 1;
    char msg[256];
    ...
    memset(fb, 0, sizeof(fb));
    M11_GameView_Draw(game, fb, FB_W, FB_H);
    copy_portrait_rect(baseline, fb);
    ok &= draw_and_expect_portrait(game, portraits, label);

    for (i = 0; i < 4; ++i) {
        memset(fb, 0, sizeof(fb));
        M11_GameView_Draw(game, fb, FB_W, FB_H);
        copy_portrait_rect(current, fb);
        snprintf(msg, sizeof(msg),
                 "%s portrait_rect bytes stable redraw cycle %d", label, i + 1);
        if (memcmp(baseline, current, sizeof(baseline)) == 0) {
            pass(msg);
        } else {
            fail(msg);
            ok = 0;
...
static int check_no_sonja_pose(M11_GameViewState* game,
                               const M11_AssetSlot* portraits,
                               int x, int y, int dir,
                               const char* label) {
    int ordinal;
    ...
    return draw_and_expect_no_sonja(game, portraits, label);
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s DATA_DIR\n"
                "  verifies DM1 V1 HoC portrait ordinal 18 "
    ...
    printf("=== DM1 V1 HoC portrait ordinal 18 (SONJA) redraw_after_candidate ===\n");
    printf("dataDir=%s pose=(map 0, x=%d, y=%d) facing EAST\n",
    ...
    printf("D1C cutout viewport=(%d,%d,%d,%d), C026 source=(%d,%d,%d,%d)\n",
           ...
           (ORDINAL_SONJA & 7) * PORTRAIT_W,
           (ORDINAL_SONJA >> 3) * PORTRAIT_H,
    ...
    set_hall_pose(&game, POSE_X, POSE_Y, PROBE_DIR_EAST);
    frontOrdinal = M11_GameView_GetFrontMirrorOrdinal(&game);
    if (frontOrdinal != ORDINAL_SONJA) {
        printf("SKIP hoc_portrait18_fixture_mismatch "
               "(%d,%d) EAST front ordinal=%d expected=%d; this DM1 V1 "
               "build does not expose ordinal 18 on this reference Hall route.\n",
               POSE_X, POSE_Y, frontOrdinal, ORDINAL_SONJA);
        M11_GameView_Shutdown(&game);
        return 0;
    }
    pass("reference route reports ordinal 18 at (1,3) EAST");
    ...
    if (M11_GameView_SelectFrontMirrorCandidate(&game) != 1) {
        fail("SelectFrontMirrorCandidate ordinal 18");
        ok = 0;
    } else {
    ...
    if (M11_GameView_ConfirmMirrorCandidate(&game, 0) != 1) {
        fail("ConfirmMirrorCandidate ordinal 18");
        ok = 0;
    } else {
        ok &= expect_int("post_confirm front mirror disabled",
                         M11_GameView_GetFrontMirrorOrdinal(&game), -1);
        ok &= draw_and_expect_no_sonja(&game, portraits, "post_confirm");
    }
    ...
    if (!open_dm1(dataDir, &menuCancel, &gameCancel)) {
        ...
        const M11_AssetSlot* portraitsCancel = M11_AssetLoader_Load(
            ...
            (unsigned int)M11_GameView_GetV1ChampionPortraitGraphicId());
        set_hall_pose(&gameCancel, POSE_X, POSE_Y, PROBE_DIR_EAST);
        if (M11_GameView_GetFrontMirrorOrdinal(&gameCancel) != ORDINAL_SONJA) {
            fail("cancel view reference route reports ordinal 18");
            ok = 0;
        } else if (M11_GameView_SelectFrontMirrorCandidate(&gameCancel) != 1) {
            fail("cancel view select ordinal 18");
            ok = 0;
        } else if (M11_GameView_CancelMirrorCandidate(&gameCancel) != 1) {
            fail("CancelMirrorCandidate ordinal 18");
            ok = 0;
        } else {
            ...
            ok &= draw_and_expect_portrait(&gameCancel, portraitsCancel,
                                           "post_cancel");
            ok &= check_rect_byte_stability(&gameCancel, portraitsCancel,
                                            "post_cancel_cycle");
        }
        M11_GameView_Shutdown(&gameCancel);
    }

    printf("\n[Stage 5] no-floating checks for ordinal 18\n");
    ok &= check_no_sonja_pose(&game, portraits,
                              POSE_X, POSE_Y, PROBE_DIR_NORTH,
                              "same_cell_north_wrong_wall");
    ok &= check_no_sonja_pose(&game, portraits,
                              POSE_X, POSE_Y, PROBE_DIR_WEST,
                              "same_cell_west_wrong_wall");
    ok &= check_no_sonja_pose(&game, portraits,
                              3, 3, PROBE_DIR_WEST,
                              "approach_from_right_wrong_wall");

    M11_GameView_Shutdown(&game);
    printf("\n=== Summary: %d passed, %d failed ===\n", g_pass, g_fail);
    printf("%s dm1 v1 HoC champion portrait ordinal 18 redraw_after_candidate portrait_rect_position\n",
