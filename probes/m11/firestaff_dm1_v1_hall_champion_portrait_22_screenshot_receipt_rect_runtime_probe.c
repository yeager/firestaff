/*
 * firestaff_dm1_v1_hall_champion_portrait_22_screenshot_receipt_rect_runtime_probe.c
 *
 ...
 *
 *   ordinal       : 22 (C026 col 6 row 2; mirror-catalog record GOTHMOG,
 *                   untitled in the DM1 V1 PC 3.4 TextString catalog
 *                   per the ordinal-22 ANY-pose discovery result in the
 *                   front_north_entry probe)
 *   route variant : screenshot_receipt_rect — the GOTHMOG pose
 ...
 *                   probes use, then captured to a real 24-bit BMP
 *                   on disk via M11_Screenshot_Capture, then re-read
 *                   from disk and the on-disk source-locked D1C
 *                   portrait rectangle (96,35,32,29) of the captured
 *                   pixels is decoded and asserted to match the C026
 *                   ordinal-22 opaque pixels.  The "receipt" half of
 *                   the name documents this as the on-disk BMP being
 *                   the actual receipt of the GOTHMOG portrait — the
 *                   BMP writer is in the contract, not just the
 ...
 *                   the existing ordinal-22 probes leave uncovered
 *                   for the screenshot round-trip at the GOTHMOG pose:
 *
 *                   (A) RECEIPT BYTE STABILITY: the GOTHMOG pose is
 ...
 *
 *                   (B) RECT RECEIVED ORDINAL 22: the captured BMP is
 ...
 *       HALK, the ordinal 22 any-pose discovery on Hall map 0, and the
 ...
 *     - covers the D2C-far view (party at (1,3) facing NORTH) where
 *       the C127 sensor at (1,1) is observable at the D2C F0128
 ...
 * front_south_entry probes lock.  Original DM1 PC 3.4 captures
 * live under parity-evidence/ and are referenced by separate
 * parity gates.  The probe is host-agnostic (no SDL window
 * required) and uses a probe-controlled temp directory under HOME
 * so it does not pollute the user-facing
 * ~/.firestaff/screenshots/ directory.
 *
 * Usage:
 *   firestaff_dm1_v1_hall_champion_portrait_22_screenshot_receipt_rect_runtime_probe DATA_DIR
 *
 * Exit codes:
 *   0  probe PASSED
 *   1  probe FAILED
...
enum {
     ...
     * present in the D1C rect for the GOTHMOG pose to be
...
static int decode_d1c_rect_from_bmp(const char* path,
     ...
     * compare decoded-vs-fb pixel-by-pixel. */
    for (y = 0; y < PROBE_PORTRAIT_H; ++y) {
 ...
 * correctly drawn at the canonical GOTHMOG pose.
 *
 ...
 * pixels at 569/569 match for their canonical poses. */
...
/* (A) RECEIPT BYTE STABILITY: render the GOTHMOG pose to the
 ...
 *     the GOTHMOG pose — the BMP writer is in the contract, not
...
static int test_receipt_byte_stability(M11_GameViewState* game,
    ...
    printf("[A] Receipt byte stability: GOTHMOG pose -> BMP on disk\n");
     ...
     * (PROBE_PORTRAIT_X, PROBE_PORTRAIT_Y) = (96, 68). */
     ...
     * M11_Screenshot_Capture uses the raw framebuffer byte as the
     * palette index.  Per DUNVIEW.C the framebuffer stores a 4-bit
     * palette index in the low nibble and a 4-bit brightness level
     * in the high nibble (M11_FB_INDEX_MASK=0x0F,
     * M11_FB_LEVEL_MASK=0xF0).  We pre-mask with 0x0F to keep the
     * screenshot_receipt_rect round-trip self-consistent with the
     * M11_Screenshot_CaptureCurrent contract. */
    {
...
/* (B) RECT RECEIVED ORDINAL 22: decode the source-locked D1C
 ...
 *          source-locked portrait_rect_position contract: the
 *          engine drew the right portrait into the D1C rect, so
 *          the BMP that captures it also contains the right
 *          portrait pixels.
 *
 ...
 *     portrait pixels for the GOTHMOG pose. */
static int test_rect_received_ordinal_22(const M11_AssetSlot* portraits,
    ...
    printf("[B] Rect received ordinal 22: decode D1C rect from BMP\n");
    if (bmpPath[0] == '\0') {
     ...
     * rect is dominated by the C026 ordinal-22 opaque pixels. */
    ...
    printf("  in-memory fb D1C rect ordinal-22 matched=%d compared=%d pct=%d\n",
           inMemoryOrdinalMatched, inMemoryOrdinalCompared, inMemoryOrdinalPct);
    if (inMemoryOrdinalPct < PROBE_POSITIVE_MATCH_PCT) {
        ++g_fail;
        printf("  FAIL: in-memory fb D1C rect ordinal-22 match %d%% (< %d%%)\n",
               inMemoryOrdinalPct, PROBE_POSITIVE_MATCH_PCT);
        ok = 0;
    } else {
        ++g_pass;
        printf("  PASS: in-memory fb D1C rect ordinal-22 match %d%% (>= %d%%)\n",
               inMemoryOrdinalPct, PROBE_POSITIVE_MATCH_PCT);
    }
    return ok;
}
 ...
 *     ordinal-22 opaque pixel match count in the in-memory fb
 *     (not the decoded BMP rect) so the side pose wall-frame
 *     pixel remap is not conflated with the ordinal-22 source
 *     strip.  Same pattern the existing east_walkpath /
 *     walkpath_from_entrance / front_south_entry probes use for
 *     the no-floating side poses. */
static int test_no_floating_receipt(const M11_AssetSlot* portraits,
     ...
     * in-memory fb's ordinal-22 match count must be far below the
     * canonical pose's match count (matching the 35% leak
     * threshold the existing east_walkpath /
     * walkpath_from_entrance / front_south_entry probes lock for
     * side poses).  This proves the GOTHMOG portrait pixels do
     * not "float" onto the side wall via the screenshot receipt
     * path. */
    inMemoryOrdinalMatched =
...
int main(int argc, char** argv) {
     ...
     * GOTHMOG-pose capture here; section (C) writes the side-pose
     * capture to a sibling "-south" sub-directory so the two
     ...
     * filename match). */
    {
        ...
        snprintf(bmpDirSouth, sizeof(bmpDirSouth),
                 "%s/.firestaff/firestaff-probe-screenshots/south", home);
        ...
        (void)FSP_CreateDirectoryRecursive(bmpDirSouth);
    }
    bmpPath[0] = '\0';
    ...
    if (ord != PROBE_ORDINAL_TARGET) {
        fprintf(stderr,
                "FAIL canonical GOTHMOG pose front-mirror ordinal=%d want=%d\n",
     ...
     * front_south_entry / d2c_far_positive / walkpath_from_entrance
     * probes' lock.  The row-2 source cell bottom (58 + 29 = 87)
     * must exactly equal the C026 strip height (87) — this is the
     * 3rd-row boundary check that catches row-count and stride
     * regressions. */
    {
        ...
        ok &= expect_int("ordinal 22 source right edge inside C026 strip",
                         sx + PROBE_PORTRAIT_W <= PROBE_PORTRAIT_STRIP_W, 1);
        ok &= expect_int("ordinal 22 source bottom edge inside C026 strip",
    ...
    ok &= test_receipt_byte_stability(&game, currFb, bmpDir, bmpPath,
                                      (int)sizeof(bmpPath));
    ok &= test_rect_received_ordinal_22(portraits, currFb, bmpPath);
    ok &= test_no_floating_receipt(portraits, &game, currFb, bmpDirSouth);
