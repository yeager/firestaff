/*
 * firestaff_dm1_v1_hall_of_champions_portrait_06_inventory_exit_restore_portrait_rect_position_runtime_probe.c
 *
 ...
 *
 * Note that ordinal 6 is in row 0 (DAROOU..TIGGY) of the shipped
 * DM1 V1 PC34 catalog, but it is NOT on the (1,2) NORTH-route front
 * square's C127 sensor (which ships as sensorData=1, HALK).  The
 * existing row-0 ordinals 0, 1, 2, 3, 5 are already covered by the
 * portrait_NN_cancel_reopen probes (different route variant) and by
 * the south_return / west_negative / d2l_negative / east_walkpath
 * probes anchored at the (2,16) / corridor poses.  Ordinal 6 is the
 * only row-0 ordinal that does NOT have a corridor-shipped sensor
 * pose for the inventory_exit_restore route, so this gate follows
 * the same synthetic-sensor-seed pattern as the ordinal-17
 * inventory_exit_restore probe: the (1,2,0) C127 sensor is seeded
 * from HALK (1) to ordinal 6 (SYRA) for this test.  Same sensor,
 * same DUNGEON.DAT, same draw path - only the ordinal that
 ...
 * (which already covers ordinal 6 portrait_rect_position)?  The
 ...
 *       rect still carries ordinal-6 pixels after the close, and
...
enum {
    ...
    ORDINAL_6_COL = 6 & 7,        /* = 6 */
    ORDINAL_6_ROW = 6 >> 3,       /* = 0 (first row of the 8x3 grid) */
 ...
 * (title CHILD OF NATURE) is the 7th valid mirror text string in
 * the shipped DM1 V1 DUNGEON.DAT (the 7th ordinal = 6, after
 * DAROOU / HALK / WU TSE / AZIZI / LEIF / ELIJA). */
static const char kExpectedCatalogName[]  = "SYRA";
static const char kExpectedCatalogTitle[] = "CHILD OF NATURE";
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-06 / inventory_exit_restore / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 6 (row 0 / column 6)
     * ----------------------------------------------------------------
     ...
     * portraits: 8 columns by 3 rows").  This is the row-0 atlas
     * math gate for ordinal 6 - sibling row-0 ordinals 0, 1, 2, 3,
     * and 5 are already covered by their cancel_reopen probes
     * (different route variant). */
    printf("\n[Group A] C026 atlas math for ordinal 6 (row 0 / col 6)\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 6 row = 6 >> 3 = %d (expected 0, first row)",
                 ORDINAL_6_ROW);
    ...
    /* Ordinal 6 must be a defined portrait: opaque count > 100 of the
     ...
     * ordinal 6 (SYRA) is treated as "no portrait" - in particular
     * a regression where (ordinal & 7) << 5 yields the wrong column
     * on the C026 strip (e.g. col 6 -> col 0 off-by-six). */
    ...
    /* Ordinal 6 must resolve to SYRA / CHILD OF NATURE through the
     * mirror catalog.  This catches a regression where the catalog
     * and the C026 atlas disagree on the ordinal-6 record - in
     * particular a regression where the catalog is shorter than 7
     * entries (a catalog count < 7 would have rejected sensorData=6
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 6 title to \"%s\" (expected \"%s\")",
     ...
     * 6.  Same sensor, same map cell, same draw path - only G0289
     * changes. */
     ...
     * has at least 7 entries (which is the source-locked DM1 V1
     ...
     * must still carry ordinal-6 pixels after the inventory close
     ...
     * the D1C portrait rect must hold ordinal-6 pixels because the
     ...
     * so we verify the same rect lines up with ordinal-6 pixels when
