 ...
 * (256x87 pixels total, ordinals 0..23). Ordinal 6 sits at row 0,
 * column 6 of the atlas:
 *
 *     srcX = (6 & 7) << 5 = 192
 *     srcY = (6 >> 3) * 29 =  0
 *
 ...
 * so we seed that sensor to sensorData=6 to lock the ordinal-6 edge
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 6: verify the C026 atlas contains a
 *       defined portrait at (192, 0, 32, 29) and that the
 *       (6 & 7) << 5 / (6 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 6 is a
 ...
 *       (with sensorData seeded to 6), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-6 champion portrait (SYRA), and that
 ...
 *       still carries ordinal-6 pixels after the full
 ...
 * lock the ordinal-6 edge case on the real DM1 V1 DUNGEON.DAT
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-06 / cancel_reopen / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* Ordinal 3 must be a defined portrait: opaque count > 100 of the
     ...
     * ordinal 6 (SYRA) is treated as "no portrait". */
    ...
    /* Ordinal 3 must be visually distinct from its row-0 neighbours
     * 5 (ELIJA) and 7 (SONJA).  The DM1 champion-portrait atlas
    ...
    /* Ordinal 3 must resolve to SYRA through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-6 record. */
    nameBuf[0] = '\0';
     ...
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=6\n");
    ...
    /* The D1C portrait rect must contain ordinal-6 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-6
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 6\n");
     ...
     * still active (sensorType=127, sensorData=3 after seed). */
     ...
     * leader hand empty, sensorData=3 still on the front square, so
     ...
     * we verify the same rect lines up with ordinal-6 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 6 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 6 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (192, 0, 32, 29) in the atlas.  This is the "ordinal
     * 6 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 6 atlas round-trip: source (192, 0) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 6 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((6 & 7) << 5, (6 >> 3) * 29, 32, 29)",
                 ORDINAL_6_SRC_X, ORDINAL_6_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_6_SRC_X == 192 && ORDINAL_6_SRC_Y == 0, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 6 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal6Opaque);
        CHECK(ordinal6Opaque >= 200 && ordinal6Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 6 round-trip distinctness: vs 5 (ELIJA) = %d%%, "
                 "vs 7 (SONJA) = %d%% (both >= 30%%)",
                 ordinal6Vs5, ordinal6Vs7);
        CHECK(ordinal6Vs5 >= 30 && ordinal6Vs7 >= 30, msg);
    }
