/*
 * firestaff_dm1_v1_champion_mirror_ordinal_11_west_negative_portrait_rect_position_runtime_probe.c
 *
 ...
 *                        where ordinal 11 could conceivably leak — the
 *                        engine must consistently return -1 because no
 *                        C127 sensor with sensorData=11 lives on the
 ...
 * which lists the real DM1 V1 PC 3.4 ordinal-by-cell mapping; ordinal 11
 * is NOT among the source-valid C127 sensors in the local fixture
 * (Hall (1,2)N=1 HALK, (1,3)E=18 SONJA, (1,5)N=10 ZED, (1,5)S=13 WUUF,
 * (2,1)S=4 LEIF, (2,4)S=15 MOPHUS, (3,3)N=19).  This probe enforces
 * the negative-route invariant for ordinal 11 across the corridor
 * (x=1, y=2..6) DIR_WEST band:
 *
 ...
 *      returns -1, the D1C cutout does NOT match C026 ordinal-11
 ...
 *      ordinal -1, so the engine never paints ordinal 11 over the
 *      corridor west wall.
 *   4. Cross-check that the D1C rectangle is alive: at (1,5)N the
 *      C127 sensorData=10 (ZED) path paints ordinal-10 (ZED) into
 *      the SAME rectangle (>= 90% pixel match), and ordinal-11
 *      drift stays below 2% — proves the empty west_negative
 *      rectangle is not silently dead.
 *   5. Engine-helper invariant at the corridor west_negative band:
 *      every cell's D1C wall box is the source-locked (80, 29, 64,
 *      43), regardless of whether the front is a wall, door, or
 *      empty floor.
 *
 * If a future DM1 V1 fixture were to place a C127 sensorData=11
 * sensor on the (0, y) west wall (custom dungeon / ROM hack), Group
 * C logs it as a discovery hit and the pixel-match contract would
 * correctly mark ordinal-11 as a present portrait — but the
 ...
 *   firestaff_dm1_v1_champion_mirror_ordinal_1_halk_pose_probe    (ordinal 1 (1,2)N + (1,2)W)
 ...
 *   firestaff_dm1_v1_champion_mirror_walkpath_runtime_probe       (input-routed cell walks)
 *
 * Non-claims:
 ...
 *     the local DM1 V1 build.  The west_negative slice is the
 *     negative route: every (1, y) DIR_WEST corridor cell must
 *     produce ordinal -1 from the front-mirror sensor lookup, and
 *     ordinal 11 must not be painted into the D1C portrait cutout.
 *   - Ordinal 11 is a real C026 atlas slot (col 3 row 1, atlas
 *     address (96, 29)).  Its name/title in the local fixture is
 *     not asserted by this probe (Group A is helper-only); the
 *     existing actual_pose probe covers positive ordinal identity
 *     for the source-visible Hall sensors.
 */
...
enum {
     ...
     * must not contain a C026 ordinal-11 portrait.  We allow up to
     * 35% pixel match against ordinal 11 (the wrong-ordinal drift
     * threshold used by the actual-pose probe's
     * check_no_stale_ordinal_in_rect).  Above 35% means a stale
     * ordinal-11 sprite is floating over the corridor west wall. */
     ...
     * 11 (the slice target) must drift below the same wrong-ordinal
     * drift threshold (35%) used by the actual-pose probe.  Adjacent
     * C026 atlas rows share skin/hair/clothing tones so we expect
     * 5-15% pixel overlap even on a correct cross-check; 35% is the
     * stale-sprite floor.  Above 35% means ordinal 11 is actually
     * painted in place of ordinal 10. */
...
static void check_engine_helpers(M11_GameViewState* state) {
    ...
    /* Atlas address sanity for ordinal 11: col=11&7=3, row=11>>3=1,
     * source rect (3*32, 1*29) = (96, 29, 32, 29).  If a regression
     * introduced a wrong-atlas lookup (e.g. forgetting the &7 mask)
     * ordinal 11 would resolve to (11*32, 0) instead. */
    snprintf(msg, sizeof(msg),
 ...
 * sensor with sensorData=11 is on the front square.  The D1C portrait
 * cutout (96, 35, 32, 29) must therefore contain zero pixels
 * matching C026 ordinal 11. */
static void check_west_negative_pixel_contract(M11_GameViewState* state,
    ...
    /* Pixel-match against C026 ordinal 11.  The cutout must NOT
     * match ordinal 11 above the wrong-ordinal drift threshold
     * (35%).  A regression that paints a stale ordinal-11 sprite
 ...
 * DIR_WEST and confirm no C127 sensor resolves to ordinal 11 on
...
static void check_positive_cross_check(M11_GameViewState* state,
    ...
    /* And it must NOT match ordinal 11 (the slice target) above
     * the tight cross-check drift threshold — proves the
     * cross-check painted the right portrait, not ordinal 11 by
     * accident.  Ordinal 11 sits in the second atlas row
     * (col 3 row 1 = (96, 29)); ordinal 10 sits in row 0 (col 2).
     * Pixel similarity between adjacent atlas rows is non-zero
     * but bounded; 2% is well below the 35% corridor-wall drift
     * threshold and gives a clear PASS. */
