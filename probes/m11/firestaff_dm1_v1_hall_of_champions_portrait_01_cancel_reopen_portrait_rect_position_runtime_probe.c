 ...
 *
 *   ordinal 1            (mirror catalog record HALK)
 ...
 * (256x87 pixels total, ordinals 0..23). Ordinal 1 sits at row 0,
 ...
 * NORTH-route front square (1,1) with sensorData=1, which is exactly
 * the ordinal-1 / HALK edge case this gate locks.  Unlike the ordinal-0
 * gate (which has to seed the sensor from 1 to 0), this gate reads
 * the sensor's shipped value, so the draw path is the unmodified
 * DM1 V1 (1,2,0) NORTH front-mirror pose.
 *
 * This probe covers the three coupled concerns of cancel_reopen in
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 1: verify the C026 atlas contains a
 *       defined portrait at (32, 0, 32, 29) and that the
 *       (1 & 7) << 5 / (1 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 1 is a
 *       valid C026 atlas entry (not a degenerate cell).
 *
 *   (2) portrait_rect_position: drive the real D1C front-mirror pose
 *       (1,2,0)=1 from the unmodified DM1 V1 DUNGEON.DAT C127 sensor
 *       lattice, pixel-prove the destination rectangle (96, 35, 32, 29)
 *       on the 320x200 framebuffer contains the ordinal-1 champion
 ...
 *       still carries ordinal-1 pixels after the full
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 1 in the C026 atlas: (1 & 7) << 5 = 32, (1 >> 3) * 29 = 0. */
    ORDINAL_1_COL = 1 & 7,         /* = 1 */
    ORDINAL_1_ROW = 1 >> 3,        /* = 0 */
    ORDINAL_1_SRC_X = ORDINAL_1_COL << 5,   /* = 32 */
    ORDINAL_1_SRC_Y = ORDINAL_1_ROW * 29,   /* =  0 */
...
/* Mirror catalog record name for ordinal 1 (DM1 V1 PC34 mirror
 * catalog).  Used to assert the catalog resolves correctly.  HALK
 * is the champion shipped on the (1,2) NORTH-route front square
 * (1,1) C127 sensor (sensorData=1, ordinal=1). */
static const char kExpectedCatalogName[] = "HALK";
 ...
 * Used to verify ordinal 1 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 1 is a distinct portrait
 * from its row-0 neighbours (0, 2). */
 ...
 * wall) with sensorData=1 (HALK, mirror ordinal 1).  This is exactly
 * the ordinal-1 edge case this gate locks - no sensor seeding is
 * required because the shipped value already matches the target. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-01 / cancel_reopen / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 1
     * ----------------------------------------------------------------
    ...
    printf("\n[Group A] C026 atlas math for ordinal 1\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 col = 1 & 7 = %d (expected 1)",
                 ORDINAL_1_COL);
        CHECK(ORDINAL_1_COL == 1, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 row = 1 >> 3 = %d (expected 0)",
                 ORDINAL_1_ROW);
        CHECK(ORDINAL_1_ROW == 0, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_1_SRC_X, ORDINAL_1_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_1_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_1_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    /* Ordinal 1 must be a defined portrait: opaque count > 50% of the
     ...
     * ordinal 1 (HALK) is treated as "no portrait". */
    ordinal1Opaque = atlas_cell_opaque_count(portraits, 1);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal1Opaque);
        CHECK(ordinal1Opaque >= 100, msg);
    }

    /* Ordinal 1 must be visually distinct from its row-0 neighbours
     * 0 and 2.  The DM1 champion-portrait atlas carries 24 distinct
     * champions (one per ordinal), so a duplicate would be a real
     * regression. */
    ordinal1Vs0 = atlas_cell_distinct_percent(portraits, 1, 0);
    ordinal1Vs2 = atlas_cell_distinct_percent(portraits, 1, 2);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 vs ordinal 0 (left neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal1Vs0);
        CHECK(ordinal1Vs0 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 vs ordinal 2 (right neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal1Vs2);
        CHECK(ordinal1Vs2 >= 30, msg);
    }

    /* Ordinal 1 must resolve to HALK through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-1 record. */
    nameBuf[0] = '\0';
                                                       ...
                                                       (int)sizeof(nameBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 1 to \"%s\" (expected \"%s\")",
                 nameBuf[0] ? nameBuf : "", kExpectedCatalogName);
        CHECK(nameLookupRc > 0 &&
              strcmp(nameBuf, kExpectedCatalogName) == 0, msg);
    }

    /* The (1,2,0) D1C front-mirror route must report ordinal 1 from
     * the shipped DM1 V1 DUNGEON.DAT.  No sensor seeding is required
     * because sensorData=1 is the canonical HALK value at this cell. */
    park_d1c_front_route(&state);
    frontOrdinal = M11_GameView_GetFrontMirrorOrdinal(&state);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "front-mirror ordinal at (1,2,0) = %d (expected %d, HALK)",
                 frontOrdinal, TARGET_ORDINAL);
        CHECK(frontOrdinal == TARGET_ORDINAL, msg);
    }
    if (frontOrdinal != TARGET_ORDINAL) {
        fprintf(stderr,
                "FATAL: front ordinal did not lock to %d at (1,2,0); "
    ...
    /* ----------------------------------------------------------------
     * Group B - portrait_rect_position on the unmodified shipped pose
     * ----------------------------------------------------------------
     * Render the framebuffer before any selection, and verify the
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-1
     * pixels. */
    ...
    /* The D1C portrait rect must contain ordinal-1 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-1
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 1\n");
     ...
     * 121 panel guard).  Match against ordinal 1 should be low. */
     ...
     * still active (sensorType=127, sensorData=1). */
     ...
     * leader hand empty, sensorData=1 still on the front square, so
     ...
     * we verify the same rect lines up with ordinal-1 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 1 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 1 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (32, 0, 32, 29) in the atlas.  This is the "ordinal
     * 1 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 1 atlas round-trip: source (32, 0) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 1 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((1 & 7) << 5, (1 >> 3) * 29, 32, 29)",
                 ORDINAL_1_SRC_X, ORDINAL_1_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_1_SRC_X == 32 && ORDINAL_1_SRC_Y == 0, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 1 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal1Opaque);
        CHECK(ordinal1Opaque >= 200 && ordinal1Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 1 round-trip distinctness: vs 0 = %d%%, "
                 "vs 2 = %d%% (both >= 30%%)",
                 ordinal1Vs0, ordinal1Vs2);
        CHECK(ordinal1Vs0 >= 30 && ordinal1Vs2 >= 30, msg);
    }
