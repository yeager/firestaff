/*
 * DM1 V1 Hall of Champions portrait ordinal 21 (HISSSSA)
 * redraw_after_candidate / portrait_rect_position runtime probe.
 *
 * Targeted slice:
 *   ordinal = 21 (HISSSSA / LIZAR OF MAKAN, C026 col 5 row 2)
 *   pose    = (map 0, x=3, y=10) facing NORTH
 *             (real PC 3.4 Hall C127 route discovered by the existing
 *              front_north_entry ordinal-21 probe)
...
static int portrait_match_count(const M11_AssetSlot* portraits,
    ...
    for (y = 0; y < PORTRAIT_H; ++y) {
        ...
        for (x = 0; x < PORTRAIT_W; ++x) {
             ...
             * color 1 transparent. The ordinal-21 sprite also carries dark
             * gray niche pixels; skip those on source side so the test keys
             * on champion pixels instead of the C346 wall background. */
...
static int check_catalog_identity(M11_GameViewState* game) {
    ...
    name[0] = '\0';
    title[0] = '\0';
    (void)M11_GameView_GetMirrorNameByOrdinal(game, ORDINAL_HISSSSA,
                                              name, sizeof(name));
    (void)M11_GameView_GetMirrorTitleByOrdinal(game, ORDINAL_HISSSSA,
                                               title, sizeof(title));
    ok = strcmp(name, "HISSSSA") == 0 && strcmp(title, "LIZAR OF MAKAN") == 0;
    if (ok) {
        pass("ordinal 21 catalog identity is HISSSSA / LIZAR OF MAKAN");
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg),
                 "ordinal 21 catalog identity got name=\"%s\" title=\"%s\"",
                 name, title);
        fail(msg);
    }
    return ok;
...
static int draw_and_expect_portrait(M11_GameViewState* game,
    ...
    snprintf(msg, sizeof(msg),
             "%s ordinal 21 is best C026 match best=%d matched=%d second=%d",
             label, ev.bestOrdinal, ev.bestMatched, ev.secondMatched);
    if (ev.bestOrdinal == ORDINAL_HISSSSA &&
...
static int draw_and_expect_no_portrait(M11_GameViewState* game,
    ...
    collect_portrait_evidence(portraits, fb, ORDINAL_HISSSSA, &ev);
    snprintf(msg, sizeof(msg),
             "%s ordinal 21 absent from D1C cutout (<%d%%) got=%d%%",
...
static int check_panel_open(M11_GameViewState* game,
    ...
    memset(fb, 0, sizeof(fb));
    M11_GameView_Draw(game, fb, FB_W, FB_H);
    collect_portrait_evidence(portraits, fb, ORDINAL_HISSSSA, &portrait);
    ...
    ok &= expect_int("panel_open candidateMirrorOrdinal",
                     game->candidateMirrorOrdinal, ORDINAL_HISSSSA);
    snprintf(msg, sizeof(msg),
    ...
    snprintf(msg, sizeof(msg),
             "panel_open ordinal 21 no longer has a full D1C portrait match (<%d%%) got=%d%%",
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 HoC portrait ordinal 21 redraw_after_candidate ===\n");
    ...
    printf("D1C cutout viewport=(%d,%d,%d,%d), C026 source=(%d,%d,%d,%d)\n",
           ...
           (ORDINAL_HISSSSA & 7) * PORTRAIT_W,
           (ORDINAL_HISSSSA >> 3) * PORTRAIT_H,
    ...
    if (frontOrdinal != ORDINAL_HISSSSA) {
        printf("SKIP hoc_portrait21_fixture_mismatch "
               "(%d,%d) NORTH front ordinal=%d expected=%d; this DM1 V1 "
               "build does not expose ordinal 21 on this reference Hall route.\n",
               POSE_X, POSE_Y, frontOrdinal, ORDINAL_HISSSSA);
        M11_GameView_Shutdown(&game);
        return 0;
    }
    pass("reference route reports ordinal 21 at (3,10) NORTH");
    ...
    if (M11_GameView_SelectFrontMirrorCandidate(&game) != 1) {
        fail("SelectFrontMirrorCandidate ordinal 21");
        ok = 0;
    } else {
    ...
    if (M11_GameView_ConfirmMirrorCandidate(&game, 0) != 1) {
        fail("ConfirmMirrorCandidate ordinal 21");
        ok = 0;
    } else {
    ...
    if (!open_dm1(dataDir, &menuCancel, &gameCancel)) {
        ...
        if (M11_GameView_GetFrontMirrorOrdinal(&gameCancel) != ORDINAL_HISSSSA) {
            fail("cancel view reference route reports ordinal 21");
            ok = 0;
        } else if (M11_GameView_SelectFrontMirrorCandidate(&gameCancel) != 1) {
            fail("cancel view select ordinal 21");
            ok = 0;
        } else if (M11_GameView_CancelMirrorCandidate(&gameCancel) != 1) {
            fail("CancelMirrorCandidate ordinal 21");
            ok = 0;
        } else {
    ...
    ok &= check_wrong_wall(&game, portraits, PROBE_DIR_SOUTH,
                           "same_cell_south_wrong_wall");
    ok &= check_wrong_wall(&game, portraits, PROBE_DIR_WEST,
                           "same_cell_west_wrong_wall");

    M11_GameView_Shutdown(&game);
    printf("\n=== Summary: %d passed, %d failed ===\n", g_pass, g_fail);
    printf("%s dm1 v1 HoC champion portrait ordinal 21 redraw_after_candidate portrait_rect_position\n",
