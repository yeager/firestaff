 ...
 *                               C127 sensor (seeded to ordinal 19 in
 *                               the same way as the portrait_19
 *                               cancel_reopen probe), opens the C040
 *                               candidate panel, cancels it, then the
 *                               party physically leaves the cell by
 *                               stepping south to (7,13) via
 *                               M11_GameView_HandleInput, then reenters
 *                               the (7,9) cell via the same handler,
 *                               and finally verifies the portrait
 *                               rect still carries ordinal-19 pixels
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 19 (Group A).  Verifies the C026
 *       atlas contains a defined portrait at (96, 58, 32, 29) and
 *       that the (19 & 7) << 5 / (19 >> 3) * 29 math matches
 *       COORD.C M027/M028 macro encoding (DEFS.H:821-826).
 *       Verifies ordinal 19 is a valid C026 atlas entry (not a
 *       degenerate cell), and that the mirror-catalog name
 *       resolves to "HAWK" with the title "THE FEARLESS".
 *
 *   (2) portrait_rect_position baseline (Group B).  Re-seeds the
 *       (7,8) C127 sensor to sensorData=19 (HAWK) on the (7,9)
 *       NORTH-route front square, then pixel-proves the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-19 champion portrait (>= 90% match)
 ...
 *       carries ordinal-19 pixels after the full leave-and-reenter
 *       cycle.  This is the leave_and_reenter slice from the gate
 *       table; it is disjoint from the existing cancel_reopen
 *       (no movement), wake_repaint (rest flag cycle, no
 *       movement), and wall_ornament_no_float (sprite containment
 *       only, no movement) probes for the same ordinal 19.
 *
 * Source evidence:
...
enum {
     ...
     * stale HAWK state bleeds into the leave framebuffer. */
    LEAVE_TARGET_MAPX = 7,
 ...
 * shipped DM1 V1 DUNGEON.DAT (the candidates that come up after
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-19 / leave_and_reenter / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
     ...
     * ordinal-19 pixels.  Same pose as the cancel_reopen probe's
     * Group B so the two probes can be cross-checked. */
    printf("\n[Group B] portrait_rect_position baseline on real C127 sensor pose (1,2,0)=19\n");
     ...
     *           walk back -> portrait rect still carries ordinal 19.
     * ----------------------------------------------------------------
     ...
     *        portrait rect still carries ordinal-19 pixels, the
     ...
     * rather than HAWK).  We assert the rect is drawn (warm
     * pixels >= 30) but NOT a HAWK/ordinal 19 match (which would
                                          ...
                                          TARGET_ORDINAL);
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "NOT match ordinal %d (got %d%%) - no stale HAWK from "
     ...
     * so the front mirror must report ordinal 19 and the D1C
     * portrait rect must carry ordinal-19 pixels at the same
     ...
     * sensorData=19 must still be the live sensor for the (7,8)
     * front square. */
     ...
     * same rect lines up with ordinal-19 pixels when the panel is
