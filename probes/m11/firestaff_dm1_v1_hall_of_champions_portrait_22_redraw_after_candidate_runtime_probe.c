 ...
 * Ordinal 22 sits at row 2, column 6 of the atlas (the BOTTOM row
 * of the 8x3 grid, last 8 portraits of the catalog):
 *
 *     srcX = (22 & 7) * 32 = 192
 *     srcY = (22 >> 3) * 29 =  58
 *
 ...
 * probe specifically targets ordinal 22, which is the last row
 * (row 2) of the atlas.  The last-row source cell is the only
 * one that ends exactly at the bottom edge of the C026 strip
 * (srcY=58 + 29 = 87, matching the 256x87 atlas height from
 * DUNVIEW.C:3916-3919), so this slice also implicitly proves
 * that the (ordinal >> 3) * 29 source math is correct for the
 * 3rd row boundary.
 *
 * This probe covers the three coupled concerns in one runtime
 * drive:
 *
 *   (1) Atlas math for ordinal 22: verify the C026 atlas contains
 *       a defined portrait at (192, 58, 32, 29) and that the
 *       (22 & 7) * 32 / (22 >> 3) * 29 math matches COORD.C
 *       M027/M028 macro encoding (DEFS.H:821-826).  The row-2
 *       source rect bottom must equal the atlas height (87), so a
 *       regression that uses the wrong row count (e.g. only 2
 *       rows) or wrong stride (e.g. 30 instead of 29) will fail
 *       this gate even before any framebuffer evidence.
 *
 ...
 * handles all 24 ordinals (0..23), and ordinal 22 is a specific
 * row 2 / column 6 portrait whose (192, 58) source coordinates
 ...
 * requested ordinal.  Used to verify ordinal 22 is a defined portrait
 ...
 * pixels that differ.  Used to verify ordinal 22 is a distinct
 * portrait from its row-2 neighbours (21, 23) - if the atlas stored
 * a duplicate, the cell match would be 100% and we would fail the
 * distinctness assertion.  The two row-2 ordinals flanking col 6
 * are 21 (col 5) and 23 (col 7); ordinal 15 is in row 1 and is the
 * wrong row to compare against for distinctness in the same row. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-22 / redraw_after_candidate / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
     ...
     * equals the atlas height (87), so a regression that uses 2
     * rows (height=58) or the wrong stride (30) will fail this
     * gate before any framebuffer evidence. */
    ...
    /* Ordinal 22 must be visually distinct from row-2 neighbours 21
     * and 23 (the two row-2 ordinals flanking col 6).  The DM1
     * champion-portrait atlas carries 24 distinct champions
     * (one per ordinal), so a duplicate would be a real regression.
     * Note: ordinal 22 = 2*8+6, so its row-2 neighbours are
     * ordinal 21 (col 5) and ordinal 23 (col 7), NOT ordinal 15
     * (which is row 1, col 7).  Using 15 here would conflate
     * row-stride with col-stride and produce a noisier signal. */
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 22 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 22 must be self-consistent:
     * the destination (96, 35, 32, 29) on the framebuffer lines up
     * with the source (192, 58, 32, 29) in the atlas, and the
     * destination would receive ordinal-22 pixels if the runtime
     * ever routed to that ordinal (e.g. in a CSB or extended-catalog
     * port of the Hall).  This is the "ordinal 22 maps to the
     * expected champion" check from the slice description.
     * Row 2 is the LAST row of the 8x3 atlas, so this assertion
     * also implicitly proves that the (ordinal >> 3) * 29 source
     * math doesn't run off the bottom of the C026 strip. */
    printf("\n[Group D] ordinal 22 atlas round-trip: source (192, 58) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 22 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((22 & 7) * 32, (22 >> 3) * 29, 32, 29)",
                 ORDINAL_22_SRC_X, ORDINAL_22_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_22_SRC_X == 192 && ORDINAL_22_SRC_Y == 58, msg);
    }
    {
        int opaqueCol5Row2 = atlas_cell_opaque_count(portraits, 5 + 16); /* col 5, row 2 = ordinal 21 */
        int opaqueCol7Row2 = atlas_cell_opaque_count(portraits, 7 + 16); /* col 7, row 2 = ordinal 23 */
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "row-2 atlas cells are all defined: ordinal 21 opaque=%d, "
                 "ordinal 22 opaque=%d, ordinal 23 opaque=%d "
                 "(row 2 = 8 portraits: ordinals 16..23)",
                 opaqueCol5Row2, ordinal22Opaque, opaqueCol7Row2);
        CHECK(opaqueCol5Row2 >= 100 && opaqueCol7Row2 >= 100, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 22 opaque count = %d "
