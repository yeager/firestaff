 ...
 *
 *   ordinal 18          (mirror catalog record SONJA, title SHE DEVIL)
 ...
 * (256x87 pixels total, ordinals 0..23).  Ordinal 18 sits at row 2,
 * column 2 of the atlas:
 *
 *     srcX = (18 & 7) << 5 = 64
 *     srcY = (18 >> 3) * 29 = 58
 *
 ...
 *
 * Ordinal 18 is another "row 2" ordinal in the shipped DM1 V1
 * PC34 catalog after ordinals 16, 17, and 19: the row-2 atlas path
 * (ordinal >> 3) * 29 yields 58 for ordinals 16..23.  This gate
 * extends the row-2 atlas math coverage to the third cell of the
 * row-2 strip and pins the catalog identity for SONJA / SHE DEVIL.
 *
 ...
 * so we seed that sensor to sensorData=18 to lock the ordinal-18 edge
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 18: verify the C026 atlas contains a
 *       defined portrait at (64, 58, 32, 29) and that the
 *       (18 & 7) << 5 / (18 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 18 is a
 *       valid C026 atlas entry (not a degenerate cell), and that the
 *       row-2 atlas path through (18 >> 3) * 29 is not a regression
 ...
 *       (with sensorData seeded to 18), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-18 champion portrait (SONJA), and that
 ...
 *       still carries ordinal-18 pixels after the full
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 18 in the C026 atlas: (18 & 7) << 5 = 64, (18 >> 3) * 29 = 58. */
     ...
     * to ordinal 18 (SONJA) for this gate so we can lock the
     * ordinal-18 edge case without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1,
    /* Row-2 atlas neighbours of ordinal 18 (BORIS = 17 left,
     * HAWK = 19 right).  The DM1 champion-portrait atlas carries
     * 24 distinct champions (one per ordinal), so a duplicate
     * would be a real regression. */
    ROW2_LEFT_ORDINAL = 17,
    ROW2_RIGHT_ORDINAL = 19
};
/* Mirror catalog record name for ordinal 18 (DM1 V1 PC34 mirror
 * catalog).  Used to assert the catalog resolves correctly.  SONJA
 * (title SHE DEVIL) is the 19th valid mirror text string in the
 * shipped DM1 V1 DUNGEON.DAT (the 19th ordinal = 18, after
 ...
 * Used to verify ordinal 18 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 18 is a distinct
 * portrait from its row-2 neighbours (17 BORIS, 19 HAWK). */
 ...
 * lock the ordinal-18 edge case on the real DM1 V1 DUNGEON.DAT
 ...
 * seed_first_c127_data the same square reports ordinal 18. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-18 / cancel_reopen / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 18 (row 2 / column 2)
     * ----------------------------------------------------------------
     ...
     * already covered.  Ordinal 18 sits at row 2 / col 2, pushing
     * the row-2 atlas layout one column farther right. */
    printf("\n[Group A] C026 atlas math for ordinal 18 (row 2 / col 2)\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 18 col = 18 & 7 = %d (expected 2)",
                 ORDINAL_18_COL);
        CHECK(ORDINAL_18_COL == 2, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 18 row = 18 >> 3 = %d (expected 2, last row)",
                 ORDINAL_18_ROW);
        CHECK(ORDINAL_18_ROW == 2, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 18 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_18_SRC_X, ORDINAL_18_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_18_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_18_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    /* Ordinal 18 must be a defined portrait: opaque count > 100 of the
     ...
     * ordinal 18 (SONJA) is treated as "no portrait" - in particular
     * a regression where (ordinal >> 3) * 29 yields the wrong row on
     * the C026 strip (e.g. row 2 -> row 0 off-by-three). */
    ordinal18Opaque = atlas_cell_opaque_count(portraits, 18);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 18 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal18Opaque);
        CHECK(ordinal18Opaque >= 100, msg);
    }

    /* Ordinal 18 must be visually distinct from its row-2 neighbours
     * 17 (BORIS) and 19 (HAWK).  The DM1 champion-portrait atlas
    ...
    ordinal18Vs17 = atlas_cell_distinct_percent(portraits, 18, 17);
    ordinal18Vs19 = atlas_cell_distinct_percent(portraits, 18, 19);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
    ...
    }
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "differ by >= 30%% (got %d%%)",
                 ordinal18Vs19);
        CHECK(ordinal18Vs19 >= 30, msg);
    }

    /* Ordinal 18 must resolve to SONJA through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-18 record - in particular a
     * regression where the catalog is shorter than 19 entries (a
     * catalog count < 19 would have rejected sensorData=18 on the
     * real DUNGEON.DAT). */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
                                                        ...
                                                        (int)sizeof(titleBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 18 to \"%s\" (expected \"%s\")",
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 18 title to \"%s\" (expected \"%s\")",
     ...
     * seed the C127 sensor from HALK (1) to ordinal 18 (SONJA).  Same
    ...
    /* The same front route now reports ordinal 18.  After seeding
     ...
     * has at least 19 entries (which is the source-locked DM1 V1
     ...
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-18
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=18\n");
    ...
    /* The D1C portrait rect must contain ordinal-18 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-18
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 18\n");
     ...
     * 121 panel guard).  Match against ordinal 18 should be low. */
     ...
     * still active (sensorType=127, sensorData=18 after seed). */
     ...
     * leader hand empty, sensorData=18 still on the front square, so
     ...
     * we verify the same rect lines up with ordinal-18 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 18 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 18 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (64, 58, 32, 29) in the atlas.  This is the
     * "ordinal 18 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 18 atlas round-trip: source (64, 58) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 18 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((18 & 7) << 5, (18 >> 3) * 29, 32, 29)",
                 ORDINAL_18_SRC_X, ORDINAL_18_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_18_SRC_X == 64 && ORDINAL_18_SRC_Y == 58, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 18 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal18Opaque);
        CHECK(ordinal18Opaque >= 200 && ordinal18Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 18 round-trip distinctness: vs 17 = %d%%, "
                 "vs 19 = %d%% (both >= 30%%)",
                 ordinal18Vs17, ordinal18Vs19);
        CHECK(ordinal18Vs17 >= 30 && ordinal18Vs19 >= 30, msg);
    }
