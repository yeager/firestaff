/*
 * firestaff_dm1_v1_hoc_champion_portrait_11_after_party_shuffle_portrait_rect_position_runtime_probe.c
 *
 * Source-locked verification gate for one narrow Hall of Champions slice:
 *
 *   ordinal 11                 (mirror catalog record row 1 / col 3 of the
 *                              C026 atlas; atlas address (3*32, 1*29)
 *                              = (96, 29); source rect (96, 29, 32, 29))
 ...
 *                                rect must again paint ordinal-11 at
 ...
 * M028_PORTRAIT_Y macro encoding; ordinals 0..23).  Ordinal 11 sits
 * at row 1, column 3:
 *
 *     srcX = (11 & 7) << 5 =  96
 *     srcY = (11 >> 3) * 29 =  29
 ...
 *   with sensorData == 11 on the (1, 2) NORTH-route front square
 *   (1, 1) only in ROM-hack / custom-dungeon fixtures; the
 *   canonical PC 3.4 Hall of Champions build uses
 *   sensorData = 1 (HALK) on that front square.  We seed the
 *   sensor data 1 -> 11 to lock the ordinal-11 edge case on the
 *   same (1, 2) NORTH-route cell used by the ordinal-03 sibling
 *   probes.  The seed does NOT change the map layout, the wall
 *   geometry, the F0128 redraw order, or the F0284 rotation
 *   mechanics -- it only rewrites one C127 sensorData byte so the
 *   D1C blit resolves to the C026 atlas cell 11 instead of cell 1.
 *
 *   Ordinal 11 is a real C026 atlas slot (DEFS.H:821-826 macro math)
 *   but the local DM1 V1 PC 3.4 DUNGEON.DAT does not natively map
 *   any C127 sensorData byte to ordinal 11 -- the firestaff_dm1_v1_
 *   champion_mirror_ordinal_11_west_negative_portrait_rect_position
 *   _runtime_probe explicitly documents the negative invariant for
 *   ordinal 11 on the corridor west wall.  This probe covers the
 *   orthogonal positive invariant: with the seeded sensor in place,
 *   the D1C portrait_rect_position must paint ordinal-11 pixels
 *   and survive a full 360-degree spin via four F0284 rotations.
 *
 ...
 *     - covers ordinal 11 on the corridor west wall (x=1, y=2..6)
 ...
 *     - covers ordinal 7 at (2, 17, SOUTH) without any F0284
 *       rotation; the (2, 18) sensor naturally carries
 *       sensorData=7 so no seeding is needed.
 *
 ...
 *       but the seeded ordinal is 3 not 11.  This probe locks the
 *       orthogonal atlas row 1 / col 3 ordinal 11.
 *
 ...
 *       probe locks the seeded ordinal-11 edge on (1, 2) NORTH.
 *
 * This probe proves these invariants for the after_party_shuffle
 * slice only:
 *
 *   (1) Mirror catalog identity: ordinal 11 resolves to a non-empty
 *       name and title via M11_GameView_GetMirrorNameByOrdinal /
 *       GetMirrorTitleByOrdinal.  The catalog count is at least 12
 *       so ordinal 11 is real.
 ...
 *       sensor data seeded to 11: the D1C rect paints ordinal-11
 ...
 *       ordinal-11 pixels (front cell changed from (1, 1) to
 ...
 *       NORTH.  The D1C rect MUST paint ordinal-11 pixels at
 *       >= 90% match again; the front cell is back at (1, 1)
 *       with the seeded sensorData=11 so the F0128 redraw must
 ...
 *       at (1, 2) NORTH opens the C040 panel with ordinal 11
 ...
 *       D1C rect must NOT paint ordinal-11 pixels and the side
 ...
 * not an original-DM1 behavior; that suppression is locked here
 * as a Firestaff invariant.
 */
...
enum {
     ...
     * TARGET_ORDINAL to lock the ordinal-11 edge case (atlas row 1 /
     * col 3 -- distinct atlas row from HALK which sits at row 0 /
 ...
 * portrait03 / cancel_reopen / ordinal-07 sibling probes. */
 ...
 * Same coordinates as the existing ordinal-3 portrait03 probe and
 * ordinal-3 cancel_reopen probe.  Resets all candidate-panel state
 * so the probe starts from a clean slate. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 HoC champion portrait 11 after_party_shuffle "
    ...
    CHECK(catalogCount >= TARGET_ORDINAL + 1,
          "Hall mirror catalog has at least 12 entries (ordinal 11 must exist)");
    printf("[catalog] ordinal count=%d\n", catalogCount);
    dump_first_catalog(&state, catalogCount < 12 ? catalogCount : 12);
     ...
     * sensor data seeded to 11 (C026 row 1 / col 3, atlas address
     * (96, 29)).
     * ---------------------------------------------------------------- */
     ...
     * still carries sensorData=11 on (1, 1), so the D1C rect must
     * paint ordinal-11 pixels again (the central no-floating
     * check). */
     ...
     * ordinal-11 pixels and the side walls must stay below the warm
     * threshold. */
     ...
     * seeded sensorData=11, so the F0128 redraw must paint
     * ordinal-11 pixels at the source-locked (96, 35) destination
    ...
    printf("\n[Group D] F0284 x2 again: SOUTH -> WEST -> NORTH at (1, 2) "
           "(portrait_rect_position must re-paint ordinal 11)\n");
     ...
     * C040 panel with ordinal 11 again; the panel-on redraw must
     ...
     * must NOT show ordinal 11 on the side walls (no-floating
