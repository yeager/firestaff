 ...
 *
 *   ordinal 0           (mirror catalog record DAROOU)
 ...
 * (256x87 pixels total, ordinals 0..23). Ordinal 0 sits at row 0,
 * column 0 of the atlas:
 *
 *     srcX = (0 & 7) * 32 =   0
 *     srcY = (0 >> 3) * 29 =  0
 *
 ...
 * seed that sensor to sensorData=0 to lock the ordinal-0 edge case.
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 0: verify the C026 atlas contains a
 *       defined portrait at (0, 0, 32, 29) and that the
 *       (0 & 7) * 32 / (0 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 0 is a
 ...
 *       the 320x200 framebuffer contains the ordinal-0 champion
 ...
 *       still carries ordinal-0 pixels after the full
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 0 in the C026 atlas: (0 & 7) * 32 = 0, (0 >> 3) * 29 = 0. */
    ORDINAL_0_COL = 0,
    ORDINAL_0_ROW = 0,
    ORDINAL_0_SRC_X = 0,
    ORDINAL_0_SRC_Y = 0,
     ...
     * to ordinal 0 for this gate so we can lock the ordinal-0
     * edge case without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1
};
/* Mirror catalog record name for ordinal 0 (DM1 V1 PC34 mirror
 * catalog).  Used to assert the catalog resolves correctly. */
static const char kExpectedCatalogName[] = "DAROOU";
 ...
 * Used to verify ordinal 0 is a defined portrait in the atlas
 ...
 * seed_first_c127_data the same square reports ordinal 0. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-00 / cancel_reopen / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 0
     * ----------------------------------------------------------------
    ...
    printf("\n[Group A] C026 atlas math for ordinal 0\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 0 col = 0 & 7 = %d (expected 0)",
                 ORDINAL_0_COL);
        CHECK(ORDINAL_0_COL == 0, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 0 row = 0 >> 3 = %d (expected 0)",
                 ORDINAL_0_ROW);
        CHECK(ORDINAL_0_ROW == 0, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 0 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_0_SRC_X, ORDINAL_0_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_0_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_0_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    /* Ordinal 0 must be a defined portrait: opaque count > 50% of the
     ...
     * ordinal 0 is treated as "no portrait" (some debug builds omit
     * the leftmost column of C026). */
    {
        int ordinal0Opaque = atlas_cell_opaque_count(portraits, 0);
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 0 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal0Opaque);
        CHECK(ordinal0Opaque >= 100, msg);
    }

    /* Ordinal 0 must resolve to DAROOU through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-0 record. */
    nameBuf[0] = '\0';
                                                       ...
                                                       (int)sizeof(nameBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 0 to \"%s\" (expected \"%s\")",
     ...
     * 0.  Same sensor, same map cell, same draw path - only G0289
     * shifts.  This keeps the probe runtime-real. */
    ...
    /* The same front route now reports ordinal 0.  After seeding
     ...
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-0
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=0\n");
    ...
    /* The D1C portrait rect must contain ordinal-0 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-0
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 0\n");
     ...
     * 121 panel guard).  Match against ordinal 0 should be low. */
     ...
     * we verify the same rect lines up with ordinal-0 pixels when
