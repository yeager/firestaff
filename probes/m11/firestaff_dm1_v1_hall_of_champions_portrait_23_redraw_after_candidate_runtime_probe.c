/*
 * firestaff_dm1_v1_hall_of_champions_portrait_23_redraw_after_candidate_runtime_probe.c
 *
 ...
 * Ordinal 23 sits at row 2, column 7 of the atlas - the BOTTOM-RIGHT
 * corner of the 8x3 grid, the LAST portrait in the catalog:
 *
 *     srcX = (23 & 7) * 32 = 224
 *     srcY = (23 >> 3) * 29 =  58
 *
 ...
 *
 * The destination rectangle is constant across all 24 ordinals;
 * only the source cell sampled from the C026 atlas changes.  This
 * probe specifically targets ordinal 23, which is the last column
 * (col 7) of the last row (row 2) of the atlas.  That corner cell is
 * the only one whose source cell BOTH:
 *   - bottom edge exactly equals the atlas height (srcY=58 + 29 = 87,
 *     matching the 256x87 atlas height from DUNVIEW.C:3916-3919), AND
 *   - right edge exactly equals the atlas width (srcX=224 + 32 = 256,
 *     matching the 8 cols * 32 = 256 width from DUNVIEW.C:3916-3919).
 * So this slice implicitly proves that the (ordinal & 7) * 32 col
 * math is correct for the 8th-column boundary, complementing the
 * ordinal-22 slice which only exercises the 3rd-row boundary.
 *
 * This probe covers the three coupled concerns in one runtime
 * drive:
 *
 *   (1) Atlas math for ordinal 23: verify the C026 atlas contains
 *       a defined portrait at (224, 58, 32, 29) and that the
 *       (23 & 7) * 32 / (23 >> 3) * 29 math matches COORD.C
 *       M027/M028 macro encoding (DEFS.H:821-826).  The corner-
 *       cell source rect right edge must equal the atlas width
 *       (256) AND the bottom edge must equal the atlas height
 *       (87), so a regression that uses 7 columns (width=224), the
 *       wrong row count (e.g. only 2 rows, height=58), or the wrong
 *       stride (30 instead of 29) will fail this gate even before
 *       any framebuffer evidence.
 *
 ...
 * handles all 24 ordinals (0..23), and ordinal 23 is a specific
 * row 2 / column 7 portrait whose (224, 58) source coordinates
...
enum {
    ...
    ORDINAL_23_COL = 23 & 7,        /* = 7 */
    ORDINAL_23_ROW = 23 >> 3,       /* = 2 */
    ORDINAL_23_SRC_X = ORDINAL_23_COL * 32,   /* = 224 */
    ORDINAL_23_SRC_Y = ORDINAL_23_ROW * 29,   /* = 58 */
 ...
 * requested ordinal.  Used to verify ordinal 23 is a defined portrait
 ...
 * pixels that differ.  Used to verify ordinal 23 is a distinct
 * portrait from its neighbours - if the atlas stored a duplicate,
 * the cell match would be 100% and we would fail the distinctness
 * assertion.  For the LAST atlas cell (row 2, col 7) there is no
 * col-8 neighbour in row 2, so we compare against:
 *   - ordinal 22 (row 2, col 6) - the cell immediately to the left
 *     in the same row (tests col-stride correctness)
 *   - ordinal 15 (row 1, col 7) - the cell directly above in the
 *     same column (tests row-stride correctness)
 * Using row-1 col-7 (ordinal 15) instead of an out-of-bounds
 * "col 8" cell keeps the source cell fully inside the 256x87
 * atlas, so the comparison is well-defined. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-23 / redraw_after_candidate / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 23
     * ----------------------------------------------------------------
     * Verify the C026 atlas contains a defined portrait at row 2 /
     * column 7 (the BOTTOM-RIGHT corner) and that the math matches
     * COORD.C / DEFS.H:821-826.  The atlas dimensions and the 8x3
     * cell layout come from DUNVIEW.C:3916-3919 (C026 is "256x87
     * strip of 32x29 portraits: 8 columns by 3 rows").  This
     * corner-cell case is the only one where the source cell BOTH
     * (a) bottom edge exactly equals the atlas height (srcY=58 +
     * 29 = 87) AND (b) right edge exactly equals the atlas width
     * (srcX=224 + 32 = 256), so a regression that uses 7 columns
     * (width=224), only 2 rows (height=58), or the wrong stride
     * (30 instead of 29) will fail this gate before any
    ...
    }
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 ORDINAL_23_ROW);
        CHECK(ORDINAL_23_ROW == 2, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_23_SRC_X, ORDINAL_23_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_23_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_23_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 source cell bottom exactly reaches "
                 "atlas height: srcY(%d) + portraitH(%d) == atlasH(%u)",
                 ORDINAL_23_SRC_Y, D1C_PORTRAIT_H, portraits->height);
        CHECK(ORDINAL_23_SRC_Y + D1C_PORTRAIT_H == (int)portraits->height, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 source cell right exactly reaches "
                 "atlas width: srcX(%d) + portraitW(%d) == atlasW(%u) "
                 "- last column boundary is correct",
                 ORDINAL_23_SRC_X, D1C_PORTRAIT_W, portraits->width);
        CHECK(ORDINAL_23_SRC_X + D1C_PORTRAIT_W == (int)portraits->width, msg);
    }

    /* Ordinal 23 must be a defined portrait: opaque count > 50% of the
    ...
    ordinal23Opaque = atlas_cell_opaque_count(portraits, 23);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal23Opaque);
        CHECK(ordinal23Opaque >= 100, msg);
    }

    /* Ordinal 23 must be visually distinct from its row-2 col-6
     * neighbour (ordinal 22) and from its row-1 col-7 cell
     * (ordinal 15).  The DM1 champion-portrait atlas carries 24
     * distinct champions (one per ordinal), so a duplicate would
     * be a real regression.  The two distinctness checks are:
     *   (a) ordinal 23 vs 22 - same row, col-stride = 1.  This
     *       catches a col-stride regression (e.g. using 33-pixel
     *       columns would scramble neighbour pixels).
     *   (b) ordinal 23 vs 15 - same column, row-stride = 1.  This
     *       catches a row-stride regression (e.g. using 30-pixel
     *       rows).  Ordinal 15 (row 1, col 7) is the cell directly
     *       above ordinal 23 in the atlas column. */
    ordinal23Vs22 = atlas_cell_distinct_percent(portraits, 23, 22);
    ordinal23Vs15 = atlas_cell_distinct_percent(portraits, 23, 15);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 vs ordinal 22 (left row-2 neighbour) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal23Vs22);
        CHECK(ordinal23Vs22 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 vs ordinal 15 (above in same column) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal23Vs15);
        CHECK(ordinal23Vs15 >= 30, msg);
    }
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 23 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 23 must be self-consistent:
     * the destination (96, 35, 32, 29) on the framebuffer lines up
     * with the source (224, 58, 32, 29) in the atlas, and the
     * destination would receive ordinal-23 pixels if the runtime
     * ever routed to that ordinal (e.g. in a CSB or extended-catalog
     * port of the Hall).  This is the "ordinal 23 maps to the
     * expected champion" check from the slice description.
     * Ordinal 23 is the LAST cell of the 8x3 atlas (row 2, col 7),
     * so this assertion also implicitly proves that BOTH the
     * (ordinal >> 3) * 29 source math doesn't run off the bottom
     * of the C026 strip AND the (ordinal & 7) * 32 source math
     * doesn't run off the right edge of the strip. */
    printf("\n[Group D] ordinal 23 atlas round-trip: source (224, 58) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 23 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((23 & 7) * 32, (23 >> 3) * 29, 32, 29)",
                 ORDINAL_23_SRC_X, ORDINAL_23_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_23_SRC_X == 224 && ORDINAL_23_SRC_Y == 58, msg);
    }
    {
        int opaqueCol6Row2 = atlas_cell_opaque_count(portraits, 6 + 16); /* col 6, row 2 = ordinal 22 (left neighbour) */
        int opaqueCol7Row1 = atlas_cell_opaque_count(portraits, 7 + 8);  /* col 7, row 1 = ordinal 15 (above in same column) */
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 23 corner-cell neighbours are defined: "
                 "ordinal 22 (left in same row) opaque=%d, "
                 "ordinal 23 opaque=%d, "
                 "ordinal 15 (above in same column) opaque=%d",
                 opaqueCol6Row2, ordinal23Opaque, opaqueCol7Row1);
        CHECK(opaqueCol6Row2 >= 100 && opaqueCol7Row1 >= 100, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 23 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal23Opaque);
        CHECK(ordinal23Opaque >= 200 && ordinal23Opaque <= 900, msg);
    }
