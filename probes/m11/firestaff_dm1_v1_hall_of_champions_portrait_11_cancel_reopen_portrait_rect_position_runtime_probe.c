/*
 * firestaff_dm1_v1_hall_of_champions_portrait_11_cancel_reopen_portrait_rect_position_runtime_probe.c
 *
 * Source-locked verification gate for one narrow Hall of Champions slice:
 *
 *   ordinal 11          (mirror catalog record STAMM)
 ...
 * (256x87 pixels total, ordinals 0..23). Ordinal 11 sits at row 1
 * (top half is row 0 / ordinals 0..7, middle row is ordinals 8..15,
 * bottom row is ordinals 16..23), column 3 of the atlas:
 *
 *     srcX = (11 & 7) << 5 =  96
 *     srcY = (11 >> 3) * 29 =  29
 *
 ...
 * seed that sensor to sensorData=11 to lock the ordinal-11 edge
 * case.  This keeps the probe runtime-real: same sensor, same
 * DUNGEON.DAT, same draw path - only the ordinal is shifted for
 * the test.  Ordinal 11 is on the *middle* atlas row (row 1) rather
 * than the top row (row 0), so this gate is the first cancel_reopen
 * portrait_rect_position gate that exercises the non-zero srcY math
 * (ordinals 0..7 are all row 0).  The destination rectangle, draw
 * path, and panel guard are identical to the ordinal-0/1/3/5 gates
 * - the only delta is the atlas row.
 *
 * This probe covers the three coupled concerns of cancel_reopen in
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 11: verify the C026 atlas contains a
 *       defined portrait at (96, 29, 32, 29) and that the
 *       (11 & 7) << 5 / (11 >> 3) * 29 math matches COORD.C M027/
 *       M028 macro encoding (DEFS.H:821-826).  Verifies ordinal 11
 *       is a valid C026 atlas entry (not a degenerate cell) and
 *       that the row-1 srcY math is right (a regression that
 *       hardcoded row 0 would still pass for ordinal 11 only if
 *       row 1 happened to align, so we explicitly assert srcY == 29).
 *
 ...
 *       the 320x200 framebuffer contains the ordinal-11 champion
 *       portrait, and that the side walls (left of x=96 and right
 *       of x=127 in the portrait row band) do NOT carry the
 *       portrait's palette.
 *
 ...
 *       still carries ordinal-11 pixels after the full
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 11 in the C026 atlas: (11 & 7) << 5 = 96, (11 >> 3) * 29 = 29.
     * Ordinal 11 sits on atlas row 1 (middle row), col 3 - this is the
     * first cancel_reopen portrait_rect_position gate that exercises
     * a non-zero srcY in the row multiplier (every prior ordinal-0/1/3/5
     * gate was on row 0). */
    ORDINAL_11_COL = 11 & 7,         /* = 3 */
     ...
     * to ordinal 11 for this gate so we can lock the ordinal-11
     * edge case (atlas row 1) without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1
};
/* Mirror catalog record name for ordinal 11 (DM1 V1 PC34 mirror
 * catalog - STAMM is the 12th champion on the wall).  Used to assert
 * the catalog resolves correctly. */
static const char kExpectedCatalogName[] = "STAMM";
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-05 / cancel_reopen / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* Ordinal 5 must resolve to ELIJA through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-5 record. */
    nameBuf[0] = '\0';
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-5
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 5\n");
     ...
     * 121 panel guard).  Match against ordinal 5 should be low. */
     ...
     * still active (sensorType=127, sensorData=0). */
     ...
     * leader hand empty, sensorData=0 still on the front square, so
     ...
     * but the wall-ornament border stays in place.  Ordinal-5 (ELIJA)
     * portrait pixels in the top strip are denser for some champions
     * than others (the row-0 col-5 cell is roughly 60% opaque in its
     * top third, vs 99% for HALK), so the absolute |diff| between
     * panel-off and panel-on can vary widely per champion.  We
     * therefore assert the panel-on top strip lands in the expected
     * wall-ornament density band (>=200 non-zero pixels - enough to
     * prove the wall-ornament frame is still being drawn) instead of
     * forcing panel-off == panel-on. */
    {
     ...
     * we verify the same rect lines up with ordinal-5 pixels when
     ...
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 5 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (160, 0, 32, 29) in the atlas.  This is the "ordinal
     * 5 maps to the expected champion" check from the slice
