 ...
 *                               (7,13) via M11_GameView_HandleInput,
 *                               then reenters the (7,9) cell via
 ...
 *     locks per-pixel palette alignment on the same (7,9) NORTH
 ...
 * open Hall of Champions column x=7 on the verified PC34 layout:
 * the (7,10)..(7,13) floor cells sit between the (7,8) S-face
 * HALK mirror and the (7,14) N-face ZED mirror.  The (7,9)
 * NORTH -> (7,13) SOUTH walk exercises the COMMAND.C F0359/F0361
 ...
 *       (7,8) C127 sensor to sensorData=2 on the (7,9)
 ...
 *       then 4x UP for the return), and pixel-prove the D1C portrait rect
...
enum {
    ...
    /* The (7,9) NORTH seed pose is the existing ordinal-02
...
int main(int argc, char** argv) {
     ...
     * a stale state leak from the cancelled (7,9) NORTH
     * candidate). */
     ...
     * sensorData=2 must still be the live sensor for the (7,8)
     * front square. */
