 ...
 *
 *   ordinal 2              (C026 atlas row 0 / col 2 cell — the
 *                          GOTHMOG slot in the shipped PC 3.4 catalog;
 *                          the "WU TSE" / "SON OF HEAVEN" alias slot
 *                          in multilingual ship-outs.  The probe
 *                          asserts the catalog lookup succeeds and
 *                          the name resolves to a non-empty string
 *                          but does NOT hard-code the name itself.)
 *   route   leave_and_reenter  (party parks at the (1,2) NORTH-route
 *                               C127 sensor (seeded to ordinal 2 in
 *                               the same way as the existing
 *                               portrait_02 cancel_reopen probe),
 *                               opens the C040 candidate panel,
 *                               cancels it, then the party physically
 *                               leaves the cell by stepping south to
 *                               (1,3) via M11_GameView_HandleInput,
 *                               then reenters the (1,2) cell via
 *                               the same handler, and finally
 *                               verifies the portrait rect still
 *                               carries ordinal-2 pixels and the
 *                               candidate state survived the
 *                               leave-and-reenter cleanly).
 *   aspect  portrait_rect_position
 *
 * This probe is a *companion* to the existing ordinal-02 slices:
 *   - firestaff_dm1_v1_hall_of_champions_portrait_02_cancel_reopen_*
 *     locks the panel state machine (select -> cancel -> reopen)
 *     on the same seeded (1,2) NORTH pose without movement.
 *   - firestaff_dm1_v1_hoc_champion_portrait_02_palette_match_rect_*
 *     locks per-pixel palette alignment on the same (1,2) NORTH
 *     pose with the same sensor retarget.
 *   - firestaff_dm1_v1_hoc_champion_portrait_02_wake_repaint_*
 *     locks the resting/wake redraw cycle for ordinal 2.
 *   - firestaff_dm1_v1_hall_champion_portrait_02_east_walkpath_*
 *     covers the cardinal east walkpath on ordinal 2.
 *   - firestaff_dm1_v1_champion_mirror_ordinal_2_south_return_*
 *     covers south-return rotation on the (1,4) cell.
 *
 * None of those probes drive the party OUT of the mirror cell and
 * back IN.  This probe adds the *physical* leave-and-reenter axis
 * to the ordinal-02 slice: after the cancel, the party must
 * actually walk away from the mirror cell (using
 * M11_GameView_HandleInput) and walk back, and the portrait rect
 * must redraw correctly on reentry with no stale state from the
 * cancelled candidate.
 *
 * The (1,2) NORTH -> (1,3) SOUTH -> (1,2) NORTH path is the
 * canonical walkaround path of the DM1 V1 hall corridor: per
 * firestaff_m11_hall_walkaround_runtime_probe the (1,3) cell is
 * the savegame start and the (1,3) SOUTH -> (1,4) SOUTH step
 * proves the corridor cells are walkable.  The reverse (1,2)
 * NORTH -> (1,3) SOUTH walk exercises the COMMAND.C F0359/F0361
 * input interleave and proves the engine's leave-and-reenter
 * behaviour for the candidate panel state.
 *
 * The probe covers three coupled concerns of leave_and_reenter in
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 2 (Group A).  Verifies the C026
 *       atlas contains a defined portrait at (64, 0, 32, 29) and
 *       that the (2 & 7) << 5 / (2 >> 3) * 29 math matches
 *       COORD.C M027/M028 macro encoding (DEFS.H:821-826).
 *       Verifies ordinal 2 is a valid C026 atlas entry (not a
 *       degenerate cell), and that the mirror-catalog name +
 *       title resolves to non-empty strings (the catalog name
 *       varies across DM1 builds — GOTHMOG in PC 3.4 EN,
 *       WU TSE in some multilingual ship-outs — so we assert
 *       a non-empty resolvable name rather than a hard-coded
 *       literal).
 *
 *   (2) portrait_rect_position baseline (Group B).  Re-seeds the
 *       (1,1) C127 sensor to sensorData=2 on the (1,2)
 *       NORTH-route front square, then pixel-proves the
 *       destination rectangle (96, 35, 32, 29) on the 320x200
 *       framebuffer contains the ordinal-2 champion portrait
 *       (>= 70% match — generous tolerance to absorb the dark-
 *       grey transparency convention the existing ordinal-02
 *       cancel_reopen probe already locks) and that the side
 *       walls do NOT carry the portrait's palette.
 *
 ...
 *       UP for the south-bound step, TURN_RIGHT twice then UP
 *       for the return), and pixel-prove the D1C portrait rect
 *       still carries ordinal-2 pixels after the full
 *       leave-and-reenter cycle.  This is the leave_and_reenter
 *       slice from the gate table; it is disjoint from the
 *       existing cancel_reopen (no movement), wake_repaint
 *       (rest cycle), and the south_return rotation probes.
 *
 * Source evidence:
...
enum {
     ...
     * to ordinal 2 (GOTHMOG / WU TSE) for this gate so we can
     * lock the ordinal-2 edge case without changing the map
     * layout. */
    SHIPPED_HALK_ORDINAL = 1,
    /* The (1,2) NORTH seed pose is the existing ordinal-02
     * cancel_reopen probe's pose, so the probes share a common
     * baseline. */
    SEED_POSE_MAPX = 1,
    SEED_POSE_MAPY = 2,
     ...
     * stale GOTHMOG/WU_TSE state bleeds into the leave
     * framebuffer. */
    LEAVE_TARGET_MAPX = 1,
    LEAVE_TARGET_MAPY = 3,
    ...
    ZED_ORDINAL = 10,
    /* Match tolerance for ordinal dominance in the D1C rect.
     * Generous (>=70%) to absorb the dark-grey transparency
     * convention the existing ordinal-02 cancel_reopen probe
     * already locks (PROBE_MATCH_PERCENT_MIN = 85 there; we use
     * 70 here because the leave_and_reenter path redraws the
     * viewport through F0128 after movement, and any per-frame
     * palette drift in the M11 path adds ~10% noise to the
     * matched-pixel count). */
    MATCH_PERCENT_MIN = 70,
    /* Transparency color for the C026 champion portrait sprites
...
static int atlas_cell_opaque_count(const M11_AssetSlot* portraits,
    ...
    for (y = 0; y < D1C_PORTRAIT_H; ++y) {
        for (x = 0; x < D1C_PORTRAIT_W; ++x) {
            ...
            if (src != 0 && src != PROBE_TRANSPARENT_COLOR) ++cnt;
        }
    }
    return cnt;
}
    ...
    for (y = 0; y < D1C_PORTRAIT_H; ++y) {
        for (x = 0; x < D1C_PORTRAIT_W; ++x) {
            ...
            if (sx >= (int)portraits->width ||
                sy >= (int)portraits->height) continue;
            src = (unsigned char)(portraits->pixels[sy * (int)portraits->width + sx] & 0x0F);
            if (src == PROBE_TRANSPARENT_COLOR) continue; /* transparent */
 ...
 * the ordinal-06 leave_and_reenter probe, kept local so this
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-02 / leave_and_reenter / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
     ...
     * Identical pattern to Group A in the ordinal-06 leave_and_reenter
     * probe so the two probes can be cross-checked on the same
     * fixture. */
    ...
    ordinal2Opaque = atlas_cell_opaque_count(portraits, 2);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 2 atlas cell has >= 200 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal2Opaque);
        CHECK(ordinal2Opaque >= 200, msg);
    }
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 2 vs ordinal 3 (right neighbour, WU TSE "
                 "alias in some ship-outs) differ by >= 30%% (got %d%%)",
                 ordinal2Vs3);
        CHECK(ordinal2Vs3 >= 30, msg);
    }

    /* Mirror catalog identity.  In real DM1 V1 PC 3.4 the
     * ordinal-2 slot is GOTHMOG (title varies by version); in
     * multilingual ship-outs the same slot is WU TSE / SON OF
     * HEAVEN.  The probe asserts only that the catalog returns
     * non-empty name + title so a future catalog edit does not
     * break the probe. */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
                                                         ...
                                                         (int)sizeof(titleBuf));
    {
        char msg[240];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 2 to name=\"%s\" "
                 "title=\"%s\" (nameRc=%d titleRc=%d; non-empty "
                 "name + title required, literal name varies across "
                 "ship-outs: GOTHMOG in PC 3.4 EN, WU TSE in some "
                 "multilingual)",
                 nameBuf[0] ? nameBuf : "", titleBuf[0] ? titleBuf : "",
                 nameLookupRc, titleLookupRc);
     ...
     * sensor, same map cell, same draw path - only the G0289
     * ordinal that DUNVIEW.C:3913-3928 reads through
     * M000_INDEX_TO_ORDINAL (DUNGEON.C:2610-2612) is shifted for
     * the test. */
     ...
     * ordinal-2 pixels.  Same pose as the ordinal-02 cancel_reopen
     * probe's Group B so the two probes share a common baseline. */
    ...
    matchBefore = match_portrait_at_rect(portraits, fbBefore, TARGET_ORDINAL);
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "at >= %d%% match (got %d%%)",
                 TARGET_ORDINAL, MATCH_PERCENT_MIN, matchBefore);
        CHECK(matchBefore >= MATCH_PERCENT_MIN, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
     ...
     *           walk back -> portrait rect still carries ordinal 2.
     * ----------------------------------------------------------------
     ...
     *        portrait rect still carries ordinal-2 pixels, the
     ...
     * 121 panel guard).  Match against ordinal 2 should be low. */
     ...
     * rather than ordinal 2).  We assert the rect is drawn (warm
     * pixels >= 30) but NOT an ordinal-2 match (which would be
     * a stale state leak from the cancelled (1,2) NORTH
     * candidate). */
                                           ...
                                           TARGET_ORDINAL);
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "NOT match ordinal %d (got %d%%) - no stale ordinal-2 "
     ...
     * so the front mirror must report ordinal 2 and the D1C
     * portrait rect must carry ordinal-2 pixels at the same
     * >= %d%% match as the baseline. */
                                              ...
                                              TARGET_ORDINAL);
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 TARGET_ORDINAL, MATCH_PERCENT_MIN, matchAfterReturn);
        CHECK(matchAfterReturn >= MATCH_PERCENT_MIN, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
     ...
     * sensorData=2 must still be the live sensor for the (1,1)
     * front square. */
     ...
     * same rect lines up with ordinal-2 pixels when the panel is
     ...
     * (after select, after reselect). */
    {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 "after-reselect=%d%% (panel-off >=%d, panel-on <=20)",
                 matchBefore, matchAfterSelect,
                 matchAfterReturn, matchAfterReselect,
                 MATCH_PERCENT_MIN);
        CHECK(matchBefore >= MATCH_PERCENT_MIN &&
              matchAfterSelect <= 20 &&
              matchAfterReturn >= MATCH_PERCENT_MIN &&
