 ...
 * covers champion portrait ordinal 6 in the route east_walkpath and
 * the aspect portrait_rect_position.
 *
 * Ordinal 6 is the SYRA ("CHILD OF NATURE") slot in the C026 atlas.
 * In the canonical DM1 PC 3.4 English DUNGEON.DAT (SHA256
 * d90b6b1c38fd17e41d63682f8afe5ca3341565b5f5ddae5545f0ce78754bdd85)
 * ordinal 6 is exposed only via a single EAST-facing pose:
 *   map=0 pose=(2,4) dir=EAST front=(3,4) C127 sensor data=6
 * That pose is the eastern side-room entry from the (2,4) corridor
 * cell looking east — the natural east_walkpath entry for SYRA.
 * The matching front_north_entry slice advanced to this pose because
 * ordinal 6 has no north-facing front route in the reference DUNGEON.DAT;
 * this east_walkpath probe locks the same SYRA pose from the east-bound
 * walk direction.
 *
 * Three invariants are verified against real DM1 V1 DUNGEON.DAT and
 * the live C026 GRAPHICS.DAT portrait strip:
 *
 *   1. Portrait ordinal 6 maps to the expected C026 atlas slot:
 *      ordinal 6 -> atlas column (6 & 7) = 6, atlas row (6 >> 3) = 0,
 *      source pixel rect (192, 0)-(223, 28).  The probe clears a
 *      framebuffer, blits ordinal 6 portrait at the D1C destination
 ...
 *      mirror poses at (1,2)/(1,4)/(1,5) EAST, then locks SYRA's
 *      east_walkpath entry at (2,4) EAST (ordinal 6).  The probe also
 *      walks east at NORTH-facing corridor cells where the existing
 *      walkpath probe locks ordinal 1 HALK / ordinal -1 / ordinal 10
 *      ZED patterns, so the D1C rect position is verified across both
 ...
 *   firestaff_dm1_v1_champion_mirror_ordinal_6_syra_pose_probe
 *     - companion from the front_north_entry slice: documents that
 *       ordinal 6 has no north-facing route and advances to the
 *       (2,4) EAST pose; this east_walkpath probe re-uses that pose
 *       as the canonical SYRA entry and locks the portrait_rect
 *       invariant at the same destination box.
 *
 * This probe does NOT duplicate any of the above.  It focuses on:
 *   (a) ordinal 6 (the SYRA slot in the C026 atlas, seventh column
 ...
 *       walkpath pose, including the SYRA (2,4) EAST pose and the
 *       no-portrait corridor poses adjacent to it.
 *
 * Source evidence:
 ...
 *   Firestaff mirror catalog decoders F0660_CHAMPION_MirrorCatalogGetName_Compat
 *     and F0661_CHAMPION_MirrorCatalogGetTitle_Compat return "SYRA"
 *     and "CHILD OF NATURE" respectively for ordinal 6 in DM1 PC 3.4
 *     English (verified separately by the helper probe
 *     firestaff_dm1_v1_hall_mirror_ordinal_champion_name_probe).
 *
...
enum {
     ...
     * Ordinal 6 -> atlas column 6, atlas row 0 -> source rect
     * (192, 0, 32, 29). */
    PROBE_PORTRAIT_W = 32,
    PROBE_PORTRAIT_H = 29,
...
/* Synthetic ordinal 6 contract: clear a framebuffer, blit ordinal 6
 ...
 * matches the C026 source rect for ordinal 6 exactly (matched/compared
 ...
 * in [0, 23]; the probe locks it for ordinal 6 (SYRA slot in C026:
 * column 6 of the top row).
 *
 * The probe deliberately does not assume ordinal 6 is exposed on a
 * Hall cell the player would walk through; in real DM1 V1 DUNGEON.DAT
 * the C127 sensors on the canonical Hall cells carry sensorData values
 * 1, 4, 10, 13, 15, 18 (HALK/LEIF/ZED/WUUF/MOPHUS/SONJA).  Ordinal 6
 * (SYRA) is exposed on the (3,4) EAST wall cell reached from the
 * (2,4) EAST side-room entry, which is the east_walkpath target pose
 * exercised above.  This synthetic test pins the ordinal-6 atlas slot
 * independently of any user DUNGEON.DAT. */
static int check_ordinal_6_atlas_slot(M11_GameViewState* game,
                                       const M11_AssetSlot* portraits,
                                       unsigned char* outFb) {
    int srcX0, srcY0;
    ...
    srcX0 = (PROBE_TARGET_ORDINAL & (PROBE_PORTRAIT_COLS - 1)) * PROBE_PORTRAIT_W;
    srcY0 = (PROBE_TARGET_ORDINAL >> 3) * PROBE_PORTRAIT_H;
    ...
    printf("  ordinal_6_atlas_slot rect=(%d,%d,%d,%d) -> dst=(%d,%d) matched=%d/%d (%d%%) d1c_warm=%d side_warm=%d\n",
           srcX0, srcY0, PROBE_PORTRAIT_W, PROBE_PORTRAIT_H,
           PROBE_D1C_DST_X, PROBE_D1C_DST_Y,
           matched, compared, pct, d1cWarm, sideWarm);

    if (compared <= 0 || pct < 90) {
        FAIL("ordinal 6 atlas slot blit: matched=%d/%d (%d%%) want>=90%%",
             matched, compared, pct);
        ok = 0;
    }
    if (d1cWarm < 30) {
        FAIL("ordinal 6 atlas slot blit: D1C warm_count=%d want>=30",
             d1cWarm);
        ok = 0;
    }
    if (sideWarm > 5) {
        FAIL("ordinal 6 atlas slot blit: side_wall warm=%d want<=5 (no floating)",
             sideWarm);
        ok = 0;
    }

    /* Verify ordinal 6 maps to atlas slot column 6 row 0 by checking
     * the source pixels are different from ordinal 0 and ordinal 7
     * (the adjacent atlas slots in the same row), confirming ordinal
     * 6 is a distinct C026 atlas slot, not SYRA aliased to HALK or
     * TIGGY. */
    {
        int refOrdinals[2] = { 0, 7 };
        int r;
        for (r = 0; r < 2; ++r) {
            int refOrdinal = refOrdinals[r];
            int refSrcX0 = (refOrdinal & (PROBE_PORTRAIT_COLS - 1)) * PROBE_PORTRAIT_W;
            int refSrcY0 = (refOrdinal >> 3) * PROBE_PORTRAIT_H;
            int distinct = 1;
            int diffPixels = 0;
            int x, y;
            for (y = 0; y < PROBE_PORTRAIT_H && distinct; ++y) {
                for (x = 0; x < PROBE_PORTRAIT_W; ++x) {
                    unsigned char ord6 = (unsigned char)(portraits->pixels[
                        (srcY0 + y) * (int)portraits->width + (srcX0 + x)] & 0x0F);
                    unsigned char ordRef = (unsigned char)(portraits->pixels[
                        (refSrcY0 + y) * (int)portraits->width + (refSrcX0 + x)] & 0x0F);
                    if (ord6 == PROBE_CHAMPION_TRANSPARENT &&
                        ordRef == PROBE_CHAMPION_TRANSPARENT) continue;
                    if (ord6 != ordRef) ++diffPixels;
                }
            }
            if (diffPixels < 50) {
                FAIL("ordinal 6 atlas slot not distinct from ordinal %d (diffPixels=%d)",
                     refOrdinal, diffPixels);
                ok = 0;
            } else {
                printf("  ordinal 6 vs ordinal %d distinct (diffPixels=%d)\n",
                       refOrdinal, diffPixels);
            }
        }
    }
     ...
     * pixel of the blitted ordinal 6 portrait (framebuffer position
     * (96+16, 33+35+14) = (112, 82)) and asserting it is the same
     * palette index as the C026 atlas row 0 column 6 center pixel
     * (192+16, 14) = (208, 14). */
    {
        ...
        if (centerDst != centerSrc) {
            FAIL("ordinal 6 center pixel mismatch dst(112,82)=0x%02x src(208,14)=0x%02x",
                 centerDst, centerSrc);
            ok = 0;
        } else {
            printf("  ordinal 6 center pixel dst(112,82)=0x%02x == src(208,14)=0x%02x\n",
                 centerDst, centerSrc);
        }
    }
...
int main(int argc, char** argv) {
    ...
    printf("target ordinal=%d (SYRA / CHILD OF NATURE per DM1 PC 3.4 EN mirror catalog)\n",
    ...
    /* Ordinal-6 synthetic contract: clear a framebuffer, blit the
     * ordinal-6 portrait from the C026 atlas to the D1C destination
     * (96, 35), and verify the destination matches the C026 source
     * slot for ordinal 6 exactly.  This pins the ordinal-6 atlas
     * slot (column 6 row 0) independent of any user DUNGEON.DAT. */
    if (!check_ordinal_6_atlas_slot(&game, portraits, currFb)) {
        ok = 0;
    }
