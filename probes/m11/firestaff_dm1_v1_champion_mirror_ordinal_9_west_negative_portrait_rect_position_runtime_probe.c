/*
 * firestaff_dm1_v1_champion_mirror_ordinal_9_west_negative_portrait_rect_position_runtime_probe.c
 *
 * Source-locked DM1 V1 Hall of Champions slice:
 *
 *   champion portrait ordinal 9 (C026 strip cell 9 — atlas col 1 row 1,
 *                               source rect (32, 29, 32, 29))
 *                               catalog: ZED / "DUKE OF BANVILLE"
 *                               in PC 3.4 EN (per the
 *                               dm1_v1_mirror_catalog_champion_stats
 *                               test family and the
 *                               m11_load_mirror_catalog_pc34 path).
 *   route west_negative: face west from a corridor cell where no
 *                        C127 sensor with sensorData=9 lives on the
 *                        front (west) wall — the engine returns -1
 *                        because the front-cell filter (ReDMCSB
 *                        DUNGEON.C:2573 + MOVESENS.C:1501-1503 +
 *                        REVIVE.C F0280) rejects the wrong-wall
 *                        direction.  The D1C portrait_rect_position
 *                        (96, 35, 32, 29) in viewport coords must
 *                        therefore be empty: no portrait floats
 ...
 *                                 35, 63} blit destination (inner
 *                                 portrait cutout is (96, 35, 32, 29)).
 *
 ...
 * and the ordinal-9 south_return sibling.  On that fixture the
 * ordinal 9 C127 sensor lives at (1,10) DIR_NORTH (cell-side NORTH
 * on the (1,10) square) — it is NEVER on a corridor west wall.
 * Therefore the (x=1, y=2..6) corridor band facing west must
 * consistently return -1 for ordinal 9.  This probe:
 *
 ...
 *      C026 ordinal-9 pixels.  A regression that mistakenly
 *      paints ordinal 9 over the corridor west wall would push
 *      the C026 ordinal-9 pixel-match above the 35% threshold.
 *   3. Cross-checks that at the positive-ordinal (1,5) DIR_NORTH
 *      pose (C127 sensorData=10 GANDO at front cell (1,4)) the
 *      SAME rectangle IS painted with the ordinal-10 portrait,
 *      NOT ordinal 9 — so an empty rectangle at (1,4) W does
 *      not silently mean the rectangle is dead.  Ordinal 10
 *      shares the C026 row=1 row-band with ordinal 9 (different
 *      columns), so this cross-check directly proves the row 1
 *      band is alive at the source-locked rect.
 *   4. Walks the corridor west-negative band (x=1, y=2..6) and
 *      confirms no C127 sensor resolves to ordinal 9 when the
 ...
 *      returns -1 at every corridor west_negative pose, and the
 *      C040 panel-guard must agree (panel guard is the
 *      2026-06-20 BUG-120/121 fix).
 *   6. Re-entry: re-rendering (1,4) DIR_WEST still returns
 *      ordinal -1 and 0% pixel match against ordinal 9.
 *
 * If the local DM1 V1 build happens to have a C127 sensor at the
 * (1,2)..(1,6) west wall with sensorData=9 (some custom-port /
 * ROM-hack distributions may), Group D logs that as a discovery
 * hit and Groups E+F still prove the portrait_rect_position is
 ...
 *     reference parity.
 *   - We do not assume a C127 sensor with sensorData=9 exists in
...
enum {
     ...
     * must not contain a C026 ordinal-9 portrait.  We allow up to
     * 35% pixel match against ordinal 9 (the wrong-ordinal drift
     * threshold used by the actual-pose probe's
     * check_no_stale_ordinal_in_rect).  Above 35% means a stale
     * ordinal-9 sprite is floating over the corridor west wall. */
    ...
    ORDINAL_TARGET = 9,
    /* The cross-check ordinal from (1,5) DIR_NORTH.  Ordinal 10
     * (GANDO) shares the C026 row=1 row-band with ordinal 9 (ZED)
     * but lives at column 2; this cross-check proves the row=1
     * portrait band is alive at the source-locked rect without
     * painting the same ordinal as the slice target. */
    ORDINAL_CROSSCHECK = 10,
    /* Corridor west_negative band bounds (x=1, y=2..6).  The
     * ordinal 9 C127 sensor lives at (1,10) DIR_NORTH on the
     * local PC 3.4 DUNGEON.DAT, so no cell in this band should
     * resolve to ordinal 9 when the party faces west. */
    CORRIDOR_X = 1,
    CORRIDOR_Y_MIN = 2,
    CORRIDOR_Y_MAX = 6
};
...
static void check_engine_helpers(M11_GameViewState* state) {
    ...
    /* Pose the party at (1,4) W — the canonical ordinal-9
     * west_negative route.  Ordinal 9 is the slice target because
     * its C127 sensor lives at (1,10) DIR_NORTH on the local PC 3.4
     * DUNGEON.DAT — never on a corridor west wall. */
    state->world.party.mapIndex = 0;
 ...
 * sensor with sensorData=9 is on the front square.  The D1C portrait
 * cutout (96, 35, 32, 29) must therefore contain zero pixels
 * matching C026 ordinal 9. */
static void check_west_negative_pixel_contract(M11_GameViewState* state,
    ...
    /* Pixel-match against C026 ordinal 9.  The cutout must NOT
     * match ordinal 9 above the wrong-ordinal drift threshold
     * (35%).  A regression that paints a stale ordinal-9 sprite
 ...
 * sensors in the source-visible DM1 V1 PC 3.4 fixture, and the
 * ordinal-9 sensor lives at (1,10) DIR_NORTH, never on the west
 * wall. */
 ...
 * At (1,5) DIR_NORTH the C127 sensor on the front cell (1,4)
 * carries sensorData=10 (GANDO) and the engine paints the
 * ordinal-10 portrait into the D1C cutout.  This proves the D1C
 * rectangle is alive at the source position: an empty (1,4) W
 * cutout cannot silently mean the rectangle is dead.  Ordinal 10
 * shares the C026 row=1 row-band with ordinal 9 (different
 * columns), so the cross-check proves the row=1 portrait band is
 * painted correctly without colliding with the slice target. */
static void check_positive_cross_check(M11_GameViewState* state,
    ...
    snprintf(msg, sizeof(msg),
             "(1,5) N D1C cutout matches ordinal %d (GANDO) >= %d%%%% (got %d%%%%)",
...
/* Group F — Wrong-wall poses at the same (1,5) cell.
 * Sanity check that the ordinal-9 west_negative invariant is
 * shared across all "facing west from a Hall cell" poses, not
 * only at (1,4).  (1,5) W has the same corridor-west-wall
 * structure as (1,4) W and must also return -1. */
static void check_west_negative_secondary(M11_GameViewState* state,
                                          const M11_AssetSlot* portraits) {
    unsigned char fb[FB_W * FB_H];
    int ord;
    int pct;
    ...
    printf("\n[Group F] (1,5) DIR_WEST secondary west_negative — same corridor west wall, ordinal %d must NOT be in D1C cutout\n",
           ORDINAL_TARGET);
    ...
    pct = match_portrait_cell(portraits, fb, ORDINAL_TARGET);
    snprintf(msg, sizeof(msg),
             "(1,5) W D1C cutout does NOT match ordinal %d < %d%%%% (got %d%%%%)",
             ORDINAL_TARGET, WRONG_ORDINAL_MATCH_PCT, pct);
    CHECK(pct < WRONG_ORDINAL_MATCH_PCT, msg);
}
...
int main(int argc, char** argv) {
    ...
    check_west_negative_secondary(&state, portraits);
