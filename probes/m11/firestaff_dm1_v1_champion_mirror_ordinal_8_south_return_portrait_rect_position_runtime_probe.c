/*
 * firestaff_dm1_v1_champion_mirror_ordinal_8_south_return_portrait_rect_position_runtime_probe.c
 *
 * Source-locked DM1 V1 Hall of Champions slice:
 *
 *   champion portrait ordinal 8 (C026 strip cell 8 — column 0, row 1)
 *   route south_return: walk south of (2,1), observe at the side-cell
 *                       while the party still faces south, then turn
 *                       back to face east and confirm the D1C portrait
 *                       rectangle (96, 35, 32, 29) draws the ordinal 8
 *                       portrait and does not float on the back wall,
 *                       the south wall, or the surrounding side walls.
 ...
 *
 * On the local PC 3.4 DM1 V1 build the ordinal 8 C127 sensor lives
 * at (2,1) DIR_EAST (cell-side=EAST sensor on the (2,1) square;
 * visible when the party stands at (2,1) facing east).  No (mapX,
 * mapY, DIR_SOUTH) cell in the Hall reports ordinal 8 directly, so
 * the south_return slice seeds the front cell's C127 sensor data to
 * 8 on a canonical DIR_SOUTH pose (1,5) SOUTH — the same
 * south-facing WUUF alcove the ordinal 0 slice uses — to lock the
 * ordinal-8 edge case on the south_return route.  This catches
 * regressions where C026 column-0/row-1 portrait pixels are clipped
 * off, transposed to another ordinal, or floated onto side walls.
 *
 * Six invariants are proved:
 *
 *   (0) Catalog identity: M11_GameView_GetMirrorNameByOrdinal(8) ==
 *       "IAIDO" — the canonical PC 3.4 English mirror for ordinal 8
 *       (per the dm1_v1_mirror_catalog_champion_stats test family and
 *       the m11_load_mirror_catalog_pc34 path).  Title is "RUYITO
 *       CHIBURI" (Ninja champion per PC 3.4 EN mirror text).
 *   (1) M11_GameView_GetFrontMirrorOrdinal((1,5) SOUTH) == 13 (WUUF)
 *       on the un-patched shipped route.
 *   (2) After seeding the (1,6) C127 sensor data from 13 to 8, the
 *       same pose returns ordinal 8 and M11_GameView_Draw paints the
 *       D1C portrait rect (96,35)-(128,64) with C026 ordinal-8
 *       source pixels (warm_count >= 30, best_ordinal == 8).
 ...
 *   (4) The portrait column/row math (ordinal 8 & 7 = 0 -> column 0,
 *       ordinal 8 >> 3 = 1 -> row 1) lines up with the C026 strip
 *       layout: source rect x=[0..32], y=[29..58] in the 256x87
 *       portrait strip.
 *   (5) Per-ordinal dominance: ordinal 8 matches >90%% of the
 ...
 *
 * Companion slice to:
 *   firestaff_dm1_v1_champion_mirror_ordinal_8_front_north_entry_portrait_rect_position_runtime_probe
 *   (front_north_entry / portrait_rect_position — disjoint cell,
 *    same ordinal).
 *
 * Disjoint from the existing actual-pose / capture / visibility /
 * walkpath / zorder / candidate-panel probes and from the other
 * ordinal-XX portrait_rect_position probes (0, 1, 2, 3, 4, 6, 7,
 * 10, 12, 13, 15, 16, 20, 21, 22, 23).
 */
...
enum {
     ...
     *   shipped sensorData = 13 (WUUF); seeded here to 8 (GWYRTH)
     *   to lock the ordinal-8 edge case on the south_return route. */
    PROBE_SLICE_MAP_X = 1,
    PROBE_SLICE_MAP_Y = 5,
    PROBE_SLICE_DIR = 2,                 /* DIR_SOUTH */
    PROBE_SHIPPED_ORDINAL = 13,          /* WUUF on the real route */
    PROBE_TARGET_ORDINAL = 8,            /* ordinal 8 (column 0, row 1) */
 ...
 * ordinal-8 edge on the south_return C127 sensor without mutating
 * unrelated cells. */
...
static void print_slice_header(int mapX, int mapY, int dir) {
    ...
    printf("=== DM1 V1 Hall portrait 08 / south_return / "
...
/* Lookup the DM1 V1 mirror-catalog name/title for ordinal 8 via the
 * M11 mirror-catalog helper so the slice is bound to the real
 * catalog identity (not a probe-side hardcoded literal).  In the
 * PC 3.4 English catalog, ordinal 8 maps to IAIDO with title
 * "RUYITO CHIBURI" (the catalog contains 24 ordinals; ordinal 8 is
 * column 0, row 1). */
...
int main(int argc, char** argv) {
     ...
     * to 8 to lock the ordinal-8 edge case at the canonical
    ...
    /* Catalog identity (must be real DM1 V1 data; ordinal 8 maps to
     * IAIDO in the canonical PC 3.4 English build). */
    check_catalog_identity(&game);

    /* Positive ordinal pose: front cell (1,6) NORTH aspect =
     * sensorData=8 after seeding. */
    check_pose(&game, portraits,
               PROBE_SLICE_MAP_X, PROBE_SLICE_MAP_Y, PROBE_SLICE_DIR,
               PROBE_TARGET_ORDINAL, 1,
               "south_return (1,5) SOUTH portrait_rect ordinal=8 painted");
     ...
     *   sensorData=10 (ZED) on its SOUTH aspect, so on real data the
     *   rect is painted with ordinal 10 — NOT floating.  We use that
     *   to confirm the rect does NOT show ordinal 8 (GWYRTH) at this
     *   pose: if the rect were painted with ordinal 8 when the party
     *   looks NORTH, that would mean the south_return sensor patch
     *   bled onto the wrong wall aspect.  The probe's contract is
     *   therefore "best_ordinal != 8" at the NORTH pose, not "no
     *   portrait at all".
     *
     ...
     *   painted (no floating) and ordinal must be -1. */
    {
        ...
        if (northBestOrdinal != PROBE_TARGET_ORDINAL) {
            PASS("south_return (1,5) NORTH wrong_wall does not show ordinal 8");
            printf("    north best_ordinal=%d (not ordinal 8) — "
                   "south_return patch did not bleed onto NORTH aspect\n",
                   northBestOrdinal);
        } else {
            FAIL("south_return (1,5) NORTH wrong_wall does not show ordinal 8");
            printf("    north best_ordinal=%d == ordinal 8 — "
     ...
     * ordinal 8 must beat every other ordinal at the same D1C rect.
     * This is the strict "best_ordinal == 8" requirement that keeps a
     * sibling portrait from accidentally winning. */
    {
        ...
        if (expected == best && best > 0 && (second == 0 || expected > second)) {
            PASS("south_return (1,5) SOUTH ordinal 8 dominates per-ordinal match");
            printf("    expected=%d best=%d second=%d\n",
                   expected, best, second);
        } else {
            FAIL("south_return (1,5) SOUTH ordinal 8 dominates per-ordinal match");
            printf("    expected=%d best=%d second=%d\n",
                   expected, best, second);
        }
    }
