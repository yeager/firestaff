 ...
 *
 *   ordinal 3           (mirror catalog record AZIZI, title JOHARI)
 ...
 * (256x87 pixels total, ordinals 0..23). Ordinal 3 sits at row 0,
 ...
 * so we seed that sensor to sensorData=3 to lock the ordinal-3 edge
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 3: verify the C026 atlas contains a
 *       defined portrait at (96, 0, 32, 29) and that the
 *       (3 & 7) << 5 / (3 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 3 is a
 ...
 *       (with sensorData seeded to 3), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-3 champion portrait (AZIZI), and that
 ...
 *       still carries ordinal-3 pixels after the full
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 3 in the C026 atlas: (3 & 7) << 5 = 96, (3 >> 3) * 29 = 0. */
    ORDINAL_3_COL = 3 & 7,         /* = 3 */
    ORDINAL_3_ROW = 3 >> 3,        /* = 0 */
    ORDINAL_3_SRC_X = ORDINAL_3_COL << 5,   /* = 96 */
    ORDINAL_3_SRC_Y = ORDINAL_3_ROW * 29,   /* =  0 */
     ...
     * to ordinal 3 (AZIZI) for this gate so we can lock the
     * ordinal-3 edge case without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1
};
/* Mirror catalog record name for ordinal 3 (DM1 V1 PC34 mirror
 * catalog).  Used to assert the catalog resolves correctly.  AZIZI
 * (title JOHARI) is the 4th valid mirror text string in the shipped
 * DM1 V1 DUNGEON.DAT (the 4 candidates that come up after DAROOU /
 * HALK / WU TSE). */
static const char kExpectedCatalogName[] = "AZIZI";
 ...
 * Used to verify ordinal 3 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 3 is a distinct portrait
 ...
 * lock the ordinal-3 edge case on the real DM1 V1 DUNGEON.DAT
 ...
 * seed_first_c127_data the same square reports ordinal 3. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-03 / cancel_reopen / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    printf("\n[Group A] C026 atlas math for ordinal 3\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 col = 3 & 7 = %d (expected 3)",
                 ORDINAL_3_COL);
        CHECK(ORDINAL_3_COL == 3, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 row = 3 >> 3 = %d (expected 0)",
                 ORDINAL_3_ROW);
        CHECK(ORDINAL_3_ROW == 0, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_3_SRC_X, ORDINAL_3_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_3_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_3_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }
     ...
     * ordinal 3 (AZIZI) is treated as "no portrait". */
    ordinal3Opaque = atlas_cell_opaque_count(portraits, 3);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal3Opaque);
        CHECK(ordinal3Opaque >= 100, msg);
    }

    /* Ordinal 3 must be visually distinct from its row-0 neighbours
     * 2 (WU TSE) and 4 (LEIF).  The DM1 champion-portrait atlas
     * carries 24 distinct champions (one per ordinal), so a
     * duplicate would be a real regression. */
    ordinal3Vs2 = atlas_cell_distinct_percent(portraits, 3, 2);
    ordinal3Vs4 = atlas_cell_distinct_percent(portraits, 3, 4);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 vs ordinal 2 (left neighbour, WU TSE) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal3Vs2);
        CHECK(ordinal3Vs2 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 vs ordinal 4 (right neighbour, LEIF) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal3Vs4);
        CHECK(ordinal3Vs4 >= 30, msg);
    }

    /* Ordinal 3 must resolve to AZIZI through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-3 record. */
    nameBuf[0] = '\0';
                                                       ...
                                                       (int)sizeof(nameBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 3 to \"%s\" (expected \"%s\")",
    ...
    /* The same front route now reports ordinal 3.  After seeding
     ...
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=3\n");
    ...
    /* The D1C portrait rect must contain ordinal-3 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-3
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 3\n");
     ...
     * 121 panel guard).  Match against ordinal 3 should be low. */
     ...
     * we verify the same rect lines up with ordinal-3 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 3 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 3 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (96, 0, 32, 29) in the atlas.  This is the "ordinal
     * 3 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 3 atlas round-trip: source (96, 0) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 3 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((3 & 7) << 5, (3 >> 3) * 29, 32, 29)",
                 ORDINAL_3_SRC_X, ORDINAL_3_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_3_SRC_X == 96 && ORDINAL_3_SRC_Y == 0, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 3 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal3Opaque);
        CHECK(ordinal3Opaque >= 200 && ordinal3Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 3 round-trip distinctness: vs 2 = %d%%, "
                 "vs 4 = %d%% (both >= 30%%)",
                 ordinal3Vs2, ordinal3Vs4);
        CHECK(ordinal3Vs2 >= 30 && ordinal3Vs4 >= 30, msg);
    }
