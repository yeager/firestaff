 ...
 * "LION OF YAITOPYA") wake_repaint / portrait_rect_position
 * runtime probe.
 *
 * Targeted slice:
 *   ordinal    = 5  (the C127 sensorData=5 retargeted on the (1,2)
 *                 NORTH front cell of the DM1 V1 DUNGEON.DAT; the
 *                 shipped DM1 V1 fixture places a single C127
 *                 sensor with sensorData=1 (HALK) on the (1,2)
 *                 front cell, so this probe uses the same
 *                 retarget pattern the existing
 *                 firestaff_dm1_v1_hall_of_champions_portrait_05_
 *                 cancel_reopen and portrait_05_after_party_shuffle
 *                 companion probes use — seed the shipped HALK
 *                 ordinal 1 sensor to ordinal 5 (ELIJA) — to lock
 *                 the row-0 / col-5 atlas cell.  Ordinal 5 is
 *                 distinct from ordinal 2 and ordinal 19 because
 *                 it lives in row 0 of the C026 strip (the "ground
 *                 floor" champions), not row 2.  This gate is the
 *                 missing wake_repaint entry for the row-0 / col-5
 *                 atlas cell so the wake redraw path is locked for
 *                 that slice and not just for the row-0 / col-2
 *                 (ordinal 2) and row-2 / col-3 (ordinal 19)
 *                 slices the existing wake_repaint gates cover.)
 *   route      = wake_repaint
 ...
 *                  the C146_COMMAND_WAKE_UP route while
 *                  G0300_B_PartyIsResting is set; the wake path clears
 *                  the resting flag and re-paints the viewport from
 *                  the party map/x/y/direction through F0128.  The
 *                  resting flag additionally draws a small RESTING...
 *                  overlay on top of the viewport at framebuffer
 *                  (100, 70, 120, 30) via m11_game_view.c:27730-27737,
 *                  so the wake_repaint route asserts: (1) the resting
 *                  overlay is on while resting, (2) the wake flag
 *                  clear plus a redraw removes the overlay, and (3)
 *                  the D1C portrait cutout (96, 35, 32, 29) holds
 *                  the ordinal 5 portrait at every stage of the
 *                  cycle, not just the post-cancel state that the
 *                  existing ordinal-05 cancel_reopen and the
 *                  ordinal-05 after_party_shuffle companion probes
 *                  cover.)
 ...
 *
 * Coverage gap relative to existing ordinal-05 champion-mirror
 * probes:
 *   - firestaff_dm1_v1_hall_of_champions_portrait_05_cancel_reopen_*
 ...
 *     shuffle_* covers the post-shuffle redraw, but again never
 *     sets G0300_B_PartyIsResting, so the wake redraw slice is
 *     uncovered.
 *   - firestaff_dm1_v1_champion_mirror_ordinal5_rect_runtime_probe
 *     covers the (2, 16) DIR_NORTH pose with the real DM1 V1
 *     ordinal-5 sensor, but never drives the rest/wake flag
 *     cycle.
 *   - firestaff_dm1_v1_champion_mirror_ordinal_5_front_south_entry
 *     _portrait_rect_position_runtime_probe uses the real (2, 15)
 *     cell 2 sensor and locks the south-entry approach invariants,
 *     but again never sets the resting flag and never drives the
 *     wake redraw cycle.
 *   - firestaff_dm1_v1_hoc_champion_portrait_02_wake_repaint_*
 *     covers ordinal 2 (GOTHMOG) through the rest/wake flag cycle
 *     at row 0 / col 2.
 *   - firestaff_dm1_v1_hoc_champion_portrait_19_wake_repaint_*
 *     covers ordinal 19 (HAWK) through the rest/wake flag cycle
 *     at row 2 / col 3.
 *   This probe extends the wake_repaint gate table to row 0 /
 *   col 5 (ELIJA), the third champion-portrait row-0 entry
 *   locked in the existing matrix (and the first ELIJA wake
 *   gate).  Atlas math: srcX = (5 & 7) * 32 = 160, srcY = (5
 *   >> 3) * 29 = 0.
 *
 * What the probe asserts at each stage:
 *   Stage 1 (pre-rest):    D1C cutout (96, 35, 32, 29) shows
 *                          ordinal 5 (ELIJA) with C026 strip cell
 *                          match >= 70% and warm pixel count
 *                          >= 30; the RESTING overlay region
 *                          (100, 70, 120, 30) fb-local is NOT
 *                          painted with the RESTING... text fill
 *                          (black-on-blue backdrop); resting=0.
 *   Stage 2 (resting):     After setting resting=1, the next
 *                          redraw paints the RESTING overlay
 *                          region with the source-locked fill
 *                          (>= 30 black pixels in the backdrop),
 *                          and the D1C portrait cutout is still
 *                          anchored at (96, 35, 32, 29).  The
 *                          D1C zone check still passes; the cutout
 *                          rectangle is still inside the public
 *                          D1C zone even when the overlay is
 *                          drawn on top of the lower-right portion
 *                          of the cutout.
 *   Stage 3 (wake):        After clearing resting=0, the next
 *                          redraw must NOT contain the RESTING
 *                          overlay fill (the black backdrop is
 *                          gone), and the D1C cutout (96, 35, 32,
 *                          29) must still show the ordinal 5
 *                          portrait (warm count >= 30, C026 strip
 *                          cell match >= 70%).
 ...
 *                          NOT show ordinal 5 pixels (no
 *                          floating).  The portrait_rect position
 *                          invariant is trivially satisfied because
 *                          there is no front-mirror ordinal to
 *                          draw, but the cutout rectangle is
 *                          still inside the public D1C zone.
 *
 * Source-locked to:
 ...
 *   ReDMCSB DUNVIEW.C:4547-4581 nibble 5 -> ordinal 5 (G0289 decode)
 *   ReDMCSB MOVESENS.C:1501-1503 passes C127 sensorData to F0280
 *   ReDMCSB COMMAND.C:2336-2359 rest path sets
 *                       G0300_B_PartyIsResting and redraws the
 *                       viewport; rejects while G0299 is live
 ...
 * sensor to ordinal 5 — the same retarget pattern the existing
 * ordinal_05 cancel_reopen and ordinal_05 after_party_shuffle
...
enum {
     ...
     * Ordinal 5 -> atlas column 5, atlas row 0 -> source rect
     * (160, 0, 32, 29).  This is the row-0 / col-5 atlas cell
     * that the existing ordinal_05 cancel_reopen and ordinal_05
     * after_party_shuffle companion probes cover; this gate is
     * the wake_repaint entry for that slice. */
    ...
    RESTING_W = 120,
    RESTING_H = 30,
    /* Hall of Champions ordinal 5 = ELIJA ("LION OF YAITOPYA")
     * per the mirror catalog.  C127 sensorData is 0-indexed so
     * the M11 ordinal = 5 is the sixth cell in the portrait
     * strip: column 5, row 0.  This row-0 / col-5 atlas cell is
     * exactly the slice the ordinal_05 cancel_reopen and
     * ordinal_05 after_party_shuffle companion probes cover;
     ...
     * tolerance is the same as the ordinal_02 and ordinal_19
     * wake_repaint gates (<= 35% of compared pixels match). */
 ...
 * real DM1 V1 fixture, HALK) to ordinal 5.  Same retarget pattern
 * the ordinal_05 cancel_reopen and ordinal_05 after_party_shuffle
 * companion probes use — they seed the shipped HALK ordinal 1
 * sensor to ordinal 5 (ELIJA) to lock the ordinal-5 edge case
 * without changing the map layout.  This probe uses the same seed
 * so the wake_repaint slice is exercised against the same row-0 /
 * col-5 atlas cell (160, 0, 32, 29) that those companion probes
 * cover.  Returns the number of sensors retargeted (1 on the real
 * DM1 V1 fixture, 0 if the fixture does not have an ordinal-1 C127
 * sensor on map 0). */
...
static int check_stage_pre_rest(M11_GameViewState* game,
    ...
    if (frontOrdinal != ORDINAL_TARGET) {
         ...
         * the sensor's sensorData is not the target ordinal 5.  Apply
         * the same retarget pattern the ordinal_05 cancel_reopen and
         * ordinal_05 after_party_shuffle companion probes use:
         * seed the shipped HALK ordinal 1 sensor to ordinal 5. */
...
static int check_stage_resting(M11_GameViewState* game,
     ...
     *       RECTANGLE, not "ordinal 5 visible at all times".  The
     ...
     * The wake_repaint slice does NOT assert ordinal-5 dominance
...
/* ── Stage 5: side/no-front pose does not float ordinal 5 ─────── */
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s DATA_DIR\n"
                "  verifies ordinal 5 wake_repaint portrait_rect_position\n",
                argv[0]);
        return 2;
    }
    dataDir = argv[1];
    ...
    printf("=== DM1 V1 HoC champion portrait ordinal 5 (ELIJA) wake_repaint "
    ...
    if (rc < 0) {
        M11_GameView_Shutdown(&game);
        printf("SKIP dm1 v1 HoC champion portrait ordinal 5 wake_repaint portrait_rect_position\n");
        return 0;
    }
    if (rc) PASS();
