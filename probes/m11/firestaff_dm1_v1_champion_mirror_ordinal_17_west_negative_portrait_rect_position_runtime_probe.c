/*
 * firestaff_dm1_v1_champion_mirror_ordinal_17_west_negative_portrait_rect_position_runtime_probe.c
 *
 * Source-locked DM1 V1 Hall of Champions slice:
 *
 *   champion portrait ordinal 17 (C026 strip cell 17 — atlas col 1 row 2,
 *                                source rect (32, 58, 32, 29))
 *   route  west_negative: face west from the (1, 5) corridor cell — the
 *                         canonical Hall end-corridor pose whose south
 *                         wall carries the C127 sensor with sensorData=13
 *                         (WUUF).  When the party turns west from (1, 5)
 *                         the front cell becomes (0, 5), which is the
 *                         western hall wall.  No C127 sensor with
 *                         sensorData=17 is on that visible aspect in the
 *                         source-visible DM1 V1 PC 3.4 fixture.  The
 *                         D1C portrait_rect_position (96, 35, 32, 29)
 *                         must therefore be empty: no ordinal-17 portrait
 *                         sprite floats over the corridor west wall.
 ...
 * for ordinal 17.  Ordinal 17 is NOT a Hall of Champions C127
 * sensorData value in the source-visible DM1 V1 PC 3.4 DUNGEON.DAT:
 *
 ...
 *   (3,7,S)=3
 *
 * Ordinal 17 is missing from that table — but ordinal 17 IS a valid
 * C026 atlas cell (graphic 26 is 256x87 = 8 cols x 3 rows of 32x29
 * portraits; ordinal 17 lives at row 2, col 1 = atlas pixel (32,
 * 58)-(64, 87)).  This probe is the regression gate that says:
 *
 *   "No Hall of Champions pose may route ordinal 17 to the D1C
 *    portrait rectangle (96, 35, 32, 29) unless that pose's front
 *    cell has a C127 sensor with sensorData==17. In particular,
 *    no west-facing corridor pose at (1, 5) may float ordinal 17
 *    over the visible side wall."
 *
 ...
 *      returns -1 because no C127 sensor with sensorData=17 is on the
 *      (0, 5) front cell aspect.  The D1C cutout does NOT match C026
 *      ordinal 17 above the 35% drift threshold (got 0% on the local
 *      PC 3.4 fixture).  The corridor west half still has rendered
 *      content so the empty D1C cutout cannot be explained away by
 *      a missing draw cycle.
 *
 *   C. Corridor west_negative band scan — for every (x=1, y=2..6)
 *      cell with DIR_WEST, the engine returns ordinal -1.  No C127
 *      sensor with sensorData=17 lives on the corridor west wall for
 *      any Hall y-coordinate, so the engine never paints ordinal 17
 ...
 *      ordinal 17 pixels above the 35% drift threshold.  Re-entry
 ...
 *   firestaff_dm1_v1_champion_portrait_12_west_negative_portrait_rect_position_runtime_probe
 *                                                                  (ordinal-12 corridor west_negative)
 *   firestaff_dm1_v1_champion_mirror_ordinal_2_west_negative_portrait_rect_position_runtime_probe
 *                                                                  (ordinal-2 corridor west_negative)
 *   firestaff_dm1_v1_champion_mirror_ordinal_13_wuuf_west_negative_portrait_rect_position_runtime_probe
 *                                                                  (WUUF negative route + (1,5)E/W/N no-floating)
...
enum {
     ...
     * must not contain a C026 ordinal-17 portrait.  We allow up to
     * 35% pixel match against ordinal 17 (the wrong-ordinal drift
     * threshold used by the actual-pose probe and the ordinal-12 /
     * ordinal-13 / ordinal-2 west_negative sibling probes).  Above
     * 35% means a stale ordinal-17 sprite is floating over the
     * corridor west wall. */
     ...
     * shipped C127 sensor with sensorData=13. */
...
static void check_engine_helpers(M11_GameViewState* state) {
    ...
    /* Pose the party at (1, 5) W — the canonical ordinal-17
     * west_negative route. */
    state->world.party.mapIndex = 0;
 ...
 * for ordinal 17.  The engine must return -1 from
 * M11_GameView_GetFrontMirrorOrdinal and the D1C portrait cutout
 * must NOT contain pixels matching C026 ordinal 17 above the 35%
 * drift threshold (the same tolerance used by the ordinal-2 /
 * ordinal-12 / ordinal-13 west_negative sibling probes and the
 * actual-pose probe). */
static void check_west_negative_pixel_contract(M11_GameViewState* state,
    ...
    printf("\n[Group B] (1, 5) DIR_WEST pixel contract — ordinal 17 must NOT be in the D1C cutout\n");
    ...
    /* Pixel-match against C026 ordinal 17.  The cutout must NOT
     * match ordinal 17 above the wrong-ordinal drift threshold
     * (35%).  A regression that paints a stale ordinal-17 sprite
 ...
 * DIR_WEST and confirm no C127 sensor resolves to ordinal 17 on
 * the corridor west wall.  The corridor west wall has no C127
 * sensors with sensorData=17 in the source-visible DM1 V1 PC 3.4
 * fixture — the only Hall mirror sensors facing west live at
 * (2, 4, W) = sensorData=10 and (2, 6, W) = sensorData=13, both of
 * which are off the (x=1) corridor band. */
static void check_corridor_west_negative_scan(M11_GameViewState* state,
    ...
    snprintf(msg, sizeof(msg),
             ...
             CORRIDOR_WEST_Y_MIN, CORRIDOR_WEST_Y_MAX, ORDINAL_TARGET, foundOrdinal17);
     ...
     * carry ordinal-17 pixels above the 35% drift threshold.  We
...
static void check_positive_cross_check(M11_GameViewState* state,
     ...
     * above the 35% drift threshold — proves the cross-check
