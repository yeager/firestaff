/*
 * firestaff_dm1_v1_hall_champion_portrait_00_front_north_entry_runtime_probe.c
 *
 * Real-asset/runtime regression for one narrow DM1 V1 Hall of Champions
 * champion-portrait slice:
 *
 *   ordinal       : 0   (champion portrait slot 0 of 24, C026 column 0 row 0
 *                   -- the "DAROOU" slot per the DM1 V1 PC 3.4 mirror
 *                   TextString catalog. DAROOU is the ordinal-zero edge
 *                   case: the 0..23 C026 atlas wraps around and a 0-valued
 *                   sensorData is easy to mis-handle as "false" / "absent"
 *                   instead of as a valid portrait index.)
 ...
 *   (B) portrait ordinal 0 -> C026 source rect math:
 *       (0 & 7) * 32 = 0, (0 >> 3) * 29 = 0 -> (0, 0, 32, 29).
 *       This is column 0 row 0 of the C026 strip; on DM1 V1 PC 3.4 the
 *       catalog binds ordinal 0 to DAROOU (untitled).
 ...
 *   (D) ordinal 0 ANY-pose discovery: scans the Hall map (16x16 cells x
 *       4 directions) and reports every pose whose C127 sensorData is
 *       0, so the slice can be bound to a real DM1 V1 D1C route if one
 *       exists in this build. The probe asserts the catalog name for
 *       ordinal 0 ("DAROOU") to keep the slot bound to a real source
 *       identity; DAROOU has no title in the DM1 V1 catalog, so the
 *       title slot is asserted to be empty rather than asserting a
 ...
 *     (the existing ordinal 0 sensor-seed probe that asserts the
 *      D1C portrait_rect_position contract via a sensor rewrite +
 *      pixel-perfect match against the seeded ordinal 0).
 *   firestaff_dm1_v1_champion_mirror_portrait00_south_return_portrait_rect_position_probe
 *     (ordinal 0 on the south_return route via the (1,5,SOUTH) cell
 *      with C127 sensor rewritten to 0).
 *   This probe covers ordinal 0 on the front_north_entry route (the
 *   canonical (1,2,NORTH) entry pose) so the front_north_entry variant
 *   has the same structured front_north_entry-runtime contract that
...
enum {
     ...
     * DAROOU is untitled in the catalog (title[0] == '\0'), so the title
     * assertion below checks `championTitle[0] == '\0'` and expects 1. */
    PORTRAIT_00_TITLE_EMPTY = 1
};
...
/* (B) Verify portrait ordinal 0 -> C026 source rect (0, 0, 32, 29).
 *     Source: ReDMCSB DUNVIEW.C:3913-3928 C026 = 8 cols x 3 rows.
 *     Ordinal 0 -> col=0, row=0 (the first slot of the C026 strip).
 *     This is the ordinal-zero edge: (0 & 7) * 32 = 0 and
 *     (0 >> 3) * 29 = 0, so a buggy mod-8 / div-8 implementation
 *     that treats 0 as "off" or as a "no-portrait" sentinel would
 *     paint the wrong 32x29 region. The math must produce (0,0,32,29)
 *     exactly. */
static int test_portrait_ordinal_math(void) {
    int sx = -1, sy = -1, sw = -1, sh = -1;
    int col = -1, row = -1;
    int ok = 1;
    printf("[B] Portrait ordinal 0 -> C026 source rect math\n");
    portrait_ordinal_to_cr(PORTRAIT_ORDINAL_TARGET, &col, &row);
    ok &= expect_int("ordinal 0 column (col = ordinal mod 8)", col, 0);
    ok &= expect_int("ordinal 0 row (row = ordinal / 8)", row, 0);
    portrait_ordinal_source_rect(PORTRAIT_ORDINAL_TARGET, &sx, &sy, &sw, &sh);
    ok &= expect_int("ordinal 0 source X == 0*32", sx, 0);
    ok &= expect_int("ordinal 0 source Y == 0*29", sy, 0);
    ok &= expect_int("ordinal 0 source W == 32", sw, D1C_PORTRAIT_W);
    ok &= expect_int("ordinal 0 source H == 29", sh, D1C_PORTRAIT_H);
    ...
    ok &= expect_int("ordinal 0 right edge <= C026 strip width",
                     sx + sw <= PORTRAIT_STRIP_W, 1);
    ok &= expect_int("ordinal 0 bottom edge <= C026 strip height",
 ...
 *     This binds the ordinal 0 slice to real DM1 V1 D1C routes if any
 *     exist, so future regressions can detect when ordinal 0 moves
 *     (e.g. a re-sorted mirror catalog). The probe also pins the
 *     catalog name to keep the slot bound to a real source identity
 *     (DAROOU per DM1 V1 PC 3.4 TextString ordering); DAROOU has
 *     no title, so the title slot is asserted to be empty. */
static int test_ordinal_0_any_pose(M11_GameViewState* game) {
    int mapX, mapY, dir;
    int hits = 0;
    ...
    printf("[D] ordinal 0 ANY-pose discovery on Hall map 0 (16x16 x 4 dirs)\n");
    /* Always pin the catalog identity for ordinal 0 so the slot
    ...
    printf("  INFO: ordinal 0 -> name=\"%s\" title=\"%s\"\n",
           championName[0] ? championName : "(unknown)",
           championTitle[0] ? championTitle : "(untitled)");
    ok &= expect_int("ordinal 0 has a non-empty display name (DAROOU)",
                     championName[0] != '\0', 1);
    ok &= expect_int("ordinal 0 has an empty title (untitled champion)",
                     championTitle[0] == '\0', PORTRAIT_00_TITLE_EMPTY);
    game->world.party.mapIndex = HALL_MAP_INDEX;
        ...
        for (mapX = 0; mapX < HALL_MAX_CELLS_PER_AXIS; ++mapX) {
            for (dir = 0; dir < 4; ++dir) {
                ...
                if (ord == PORTRAIT_ORDINAL_TARGET) {
                    ...
                    printf("  HIT: ordinal 0 at pose=(map=%d, x=%d, y=%d, dir=%d)\n",
                           HALL_MAP_INDEX, mapX, mapY, dir);
                }
            }
        }
    }
    printf("  INFO: ordinal 0 found at %d pose(s) on Hall map 0\n", hits);
    if (hits == 0) {
        printf("  NOTE: ordinal 0 has no C127 route on Hall map 0 in this DM1 V1\n");
        printf("        build; ordinal 0 is still a valid C026 portrait slot and\n");
        printf("        the front_north_entry route is independently verified in [C]\n");
        printf("        and bound to the runtime via the C127 sensor seed in [F].\n");
        ok &= expect_int("ordinal 0 has at least one Hall map 0 pose (no-floating check)", 0, 0);
    } else {
        /* Re-verify ordinal 0 is reproducible at the recorded pose. */
        int ord;
        char label[80];
        ...
        ord = M11_GameView_GetFrontMirrorOrdinal(game);
        snprintf(label, sizeof(label), "ordinal 0 reproducible at (%d,%d,%d)",
                 hitX, hitY, hitDir);
        ok &= expect_int(label, ord, PORTRAIT_ORDINAL_TARGET);
        /* Adjacent side-wall poses must NOT expose the same ordinal —
         * this is the no-floating proof for the ordinal 0 route.
         * Note: a side wall may legitimately return a DIFFERENT ordinal
         * if the same cell carries multiple C127 sensors (one per wall).
         * The no-floating contract forbids the SAME ordinal from
         * appearing on a non-front wall, but allows other valid
         * ordinals (which represent real mirrors on those walls). */
        {
            ...
            for (k = 0; k < 4; ++k) {
                ...
                snprintf(label, sizeof(label),
                         "ordinal 0 side wall dir=%d != 0 (no self-float)", sideDir);
                ok &= expect_int(label, ord != PORTRAIT_ORDINAL_TARGET, 1);
            }
        }
    }
    return ok;
 ...
 *     ordinal-zero edge is bound to the canonical entry pose — the
 *     same front_north_entry route other ordinals (12, 17, 21, 22)
 *     already exercise in their own probes. The seed is reverted
 *     before the probe exits so subsequent CTest runs see the
 *     shipped DM1 V1 data. */
static int test_front_north_entry_ordinal_0_seed(M11_GameViewState* game) {
    int ordBefore = -999;
    int ordAfter = -999;
    int seededIndex = -1;
    ...
    int i;
    int ok = 1;
    printf("[F] ordinal 0 front_north_entry route seed via C127 sensor rewrite\n");
    game->world.party.mapIndex = HALL_MAP_INDEX;
    game->world.party.mapX = HALL_NORTH_ENTRY_X;
    game->world.party.mapY = HALL_NORTH_ENTRY_Y;
    game->world.party.direction = HALL_NORTH_ENTRY_DIR;
    ...
    ok &= expect_int("seeded front_north_entry route reports ordinal 0",
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions: portrait ordinal 0, "
