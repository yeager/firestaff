 ...
 *
 *   ordinal 16          (mirror catalog record CHANI,
 ...
 * (256x87 pixels total, ordinals 0..23).  Ordinal 16 sits at row 2,
 * column 0 of the atlas - the FIRST cell of the BOTTOM row, directly
 * below ordinal 0 (DAROOU):
 *
 *     srcX = (16 & 7) << 5 =   0
 *     srcY = (16 >> 3) * 29 = 58
 *
 ...
 * seed that sensor to sensorData=16 to lock the ordinal-16 edge case.
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 16: verify the C026 atlas contains a
 *       defined portrait at (0, 58, 32, 29) and that the
 *       (16 & 7) << 5 / (16 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 16 is a
 *       valid C026 atlas entry (not a degenerate cell), distinct
 *       from its vertical neighbours (8 IAIDO above, 24 - wrap -
 *       row 0 ordinal 0 below).
 *
 ...
 *       the 320x200 framebuffer contains the ordinal-16 champion
 ...
 *       still carries ordinal-16 pixels after the full
 *       select->cancel->select cycle.  This is the cancel_reopen
 *       slice from the gate table; it is disjoint from the existing
 *       portrait_05_cancel_reopen_portrait_rect_position_runtime_probe
 *       (which targets row 0 / col 5 / ordinal 5 ELIJA) and from
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 16 in the C026 atlas: (16 & 7) << 5 = 0, (16 >> 3) * 29 = 58. */
    ORDINAL_16_COL = 16 & 7,        /* = 0 */
    ORDINAL_16_ROW = 16 >> 3,       /* = 2 */
    ORDINAL_16_SRC_X = ORDINAL_16_COL << 5,   /* =   0 */
    ORDINAL_16_SRC_Y = ORDINAL_16_ROW * 29,   /* = 58 */
     ...
     * to ordinal 16 for this gate so we can lock the ordinal-16
     * edge case without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1
};
/* Mirror catalog record name and title for ordinal 16 (DM1 V1 PC34
 * mirror catalog).  Used to assert the catalog resolves correctly. */
static const char kExpectedCatalogName[] = "CHANI";
static const char kExpectedCatalogTitle[] = "SAYYADINA SIHAYA";
 ...
 * Used to verify ordinal 16 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 16 is a distinct portrait
 * from its row-2 neighbours (15 MOPHUS to the right? no - col 7 right
 * is ordinal 23, so we compare vs 17 BORIS at col 1) and the row-0
 * ordinal directly above (col 0, ordinal 8 IAIDO).  The DM1 champion-
 ...
 * lock the ordinal-16 edge case on the real DM1 V1 DUNGEON.DAT
 ...
 * seed_first_c127_data the same square reports ordinal 16. */
...
int main(int argc, char** argv) {
    ...
    int titleLookupRc;
    int ordinal16Opaque;
    int ordinal16Vs8;   /* above (row 1, col 0 = ordinal 8 IAIDO) */
    int ordinal16Vs17;  /* right (row 2, col 1 = ordinal 17 BORIS) */
    int ordinal16Vs15;  /* left wraps - col 7 row 2 = ordinal 23 GOTHMOG;
                          * but here we use the row-1 same-col cell
                          * which is ordinal 8; left neighbour across
                          * the column wrap is ordinal 15 MOPHUS (row 1
                          * col 7) for cross-cell sanity */
    ...
    printf("=== DM1 V1 Hall of Champions portrait-16 / cancel_reopen / portrait_rect_position (v2.7.27) ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 16
     * ----------------------------------------------------------------
    ...
    printf("\n[Group A] C026 atlas math for ordinal 16\n");
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 col = 16 & 7 = %d (expected 0)",
                 ORDINAL_16_COL);
        CHECK(ORDINAL_16_COL == 0, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 row = 16 >> 3 = %d (expected 2)",
                 ORDINAL_16_ROW);
        CHECK(ORDINAL_16_ROW == 2, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 srcX = %d, srcY = %d "
                 "(within 256x87 atlas: must be < %d and < %d)",
                 ORDINAL_16_SRC_X, ORDINAL_16_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_16_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_16_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    /* Ordinal 16 must be a defined portrait: opaque count > 50% of the
     ...
     * ordinal 16 is treated as "no portrait" or the wrong atlas cell
     * is blitted at (0, 58, 32, 29). */
    ordinal16Opaque = atlas_cell_opaque_count(portraits, 16);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal16Opaque);
        CHECK(ordinal16Opaque >= 100, msg);
    }

    /* Ordinal 16 must be visually distinct from its atlas neighbours:
     *   - ordinal 8  (row 1, col 0 - IAIDO, directly above)
     *   - ordinal 17 (row 2, col 1 - BORIS, immediately to the right)
     *   - ordinal 15 (row 1, col 7 - MOPHUS, last cell of the row above;
     *     cross-check that the bottom-row portraits are not duplicates
     *     of unrelated middle-row champions)
     * The DM1 champion-portrait atlas carries 24 distinct champions
     * (one per ordinal), so a duplicate would be a real regression. */
    ordinal16Vs8 = atlas_cell_distinct_percent(portraits, 16, 8);
    ordinal16Vs17 = atlas_cell_distinct_percent(portraits, 16, 17);
    ordinal16Vs15 = atlas_cell_distinct_percent(portraits, 16, 15);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 vs ordinal 8 (above, IAIDO) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal16Vs8);
        CHECK(ordinal16Vs8 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 vs ordinal 17 (right, BORIS) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal16Vs17);
        CHECK(ordinal16Vs17 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 vs ordinal 15 (cross-row, MOPHUS) differ "
                 "by >= 30%% (got %d%%)",
                 ordinal16Vs15);
        CHECK(ordinal16Vs15 >= 30, msg);
    }

    /* Ordinal 16 must resolve to CHANI / SAYYADINA SIHAYA through the
     * mirror catalog.  This catches a regression where the catalog and
     * the C026 atlas disagree on the ordinal-16 record. */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
                                                         ...
                                                         (int)sizeof(titleBuf));
    {
        char msg[240];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 16 to \"%s\" (expected \"%s\")",
    ...
    }
    {
        char msg[240];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 16 title to \"%s\" (expected \"%s\")",
     ...
     * 16 (CHANI).  Same sensor, same map cell, same draw path - only
    ...
    /* The same front route now reports ordinal 16.  After seeding
     ...
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-16
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=16\n");
    ...
    /* The D1C portrait rect must contain ordinal-16 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-16
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 16\n");
     ...
     * 121 panel guard).  Match against ordinal 16 should be low. */
     ...
     * but the wall-ornament border stays in place.  We assert the
     ...
     * panel-on, because absolute |diff| can vary widely per champion
     * (different champions have different portrait opacity in the
     * top strip - row-2 champions like CHANI carry a different
     * density than row-0 / row-1 champions). */
    {
     ...
     * we verify the same rect lines up with ordinal-16 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 16 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 16 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (0, 58, 32, 29) in the atlas.  This is the "ordinal
     * 16 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 16 atlas round-trip: source (0, 58) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 16 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((16 & 7) << 5, (16 >> 3) * 29, 32, 29)",
                 ORDINAL_16_SRC_X, ORDINAL_16_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_16_SRC_X == 0 && ORDINAL_16_SRC_Y == 58, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 16 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal16Opaque);
        CHECK(ordinal16Opaque >= 200 && ordinal16Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 16 round-trip distinctness: vs 8 = %d%%, "
                 "vs 17 = %d%%, vs 15 = %d%% (all >= 30%%)",
                 ordinal16Vs8, ordinal16Vs17, ordinal16Vs15);
        CHECK(ordinal16Vs8 >= 30 && ordinal16Vs17 >= 30 && ordinal16Vs15 >= 30, msg);
    }
