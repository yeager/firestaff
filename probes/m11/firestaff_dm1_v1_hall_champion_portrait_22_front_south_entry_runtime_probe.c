 ...
 *                   of (12, 13).  The "front_south_entry" route
 ...
 *                   is on the WEST wall (cell 3) of (11, 13) (visible
 *                   from (12, 13) facing WEST per the front_north_entry
 *                   probe's [D] scan) — i.e. the south wall of (12, 13)
 ...
 *                   (B) ordinal 22 lives at (12, 13, WEST) on Hall map
 ...
 *                       (12, 13) cell returns front-mirror ordinal -1
 ...
 *                       ordinal-22 cell).  The (12, 13) SOUTH pose is
 ...
 *                       the south corridor).
 *                   (D) South-entry approach pose (party at (12, 14)
 *                       facing NORTH, one square south of the
 *                       ordinal-22 cell) returns front-mirror
 *                       ordinal -1 (the south wall of (12, 13) has no
 ...
 *                       panel at (12, 13, WEST) (the canonical
 ...
 *       on the (1,2) HALK sensor.  Does NOT cover the (12, 13) cell
 *       south-facing pose, the south-entry approach from (12, 14, N),
 ...
 *       (12, 13) or (12, 14) (the ordinal-22 cell and its south-entry
 ...
 *       south-facing pose at (12, 13), or the south-entry approach
 *       from (12, 14, N).
 *
 * Source evidence:
 ...
 * 22 placement at the canonical ordinal-22 pose (12, 13, WEST),
 * the no-route contract for the south-facing C127 sensor at the
 * (12, 13) cell, the south-entry approach pose (12, 14, NORTH)
 * no-portrait contract, the south-facing side-pose no-floating
 * contract at (12, 13), and the candidate-panel return behavior
...
enum {
    ...
    /* The south-entry approach pose: party at (12, 14) facing NORTH,
     * one square south of the ordinal-22 cell.  The front cell is
     * (12, 13) (the ordinal-22 cell), the visible wall is the south
     * wall (cell 2) of (12, 13) - which has NO C127 sensor for
     * ordinal 22 (the ordinal-22 sensor is on the WEST wall). */
    PROBE_SOUTH_ENTRY_X = 12,
    PROBE_SOUTH_ENTRY_Y = 14,
...
/* (B) ordinal 22 lives at (12, 13, WEST) on Hall map 0 in the real
...
static int test_ordinal_22_hall_location(M11_GameViewState* game) {
    ...
    int ok = 1;
    printf("[B] ordinal 22 Hall map 0 location = (12, 13, WEST) in real DM1 V1 DUNGEON.DAT\n");
    game->world.party.mapIndex = HALL_MAP_INDEX;
    ...
    ok &= expect_int("ordinal 22 found at (12, 13, WEST) (the front_south_entry cell)",
                     (hits == 1 &&
             ...
             PROBE_GOTHMOG_FRONT_DIR);
    {
        ...
        ok &= expect_int("front_mirror ordinal at (12, 13, WEST) == 22 (GOTHMOG)",
                         ord, PROBE_ORDINAL_TARGET);
    }
    return ok;
}

/* (C) South-facing side pose at the ordinal-22 cell: (12, 13)
 ...
 *     player turning to face SOUTH at (12, 13) must NOT see the
 *     GOTHMOG portrait on the south wall of (12, 13).  Mirrors the
...
static int test_south_side_pose_no_floating(M11_GameViewState* game,
    ...
    int compared;
    int leakPct;
    printf("[C] South-facing side pose at (12, 13) - no-floating of ordinal 22\n");
    set_pose(game, PROBE_GOTHMOG_CELL_X, PROBE_GOTHMOG_CELL_Y,
             PROBE_GOTHMOG_SOUTH_DIR);
    ord = M11_GameView_GetFrontMirrorOrdinal(game);
    ok &= expect_int("(12, 13, SOUTH) front_mirror ordinal == -1 (no float)",
                     ord, -1);
    ...
    printf("  (12, 13, SOUTH) ordinal-22 matched=%d compared=%d leakPct=%d\n",
           matched, compared, leakPct);
    if (leakPct >= PROBE_FLOOR_LEAK_PCT) {
        ++g_fail;
        printf("  FAIL: (12, 13, SOUTH) ordinal-22 leaked matched=%d/%d (>= %d%%)\n",
               matched, compared, PROBE_FLOOR_LEAK_PCT);
        ok = 0;
    } else {
        ++g_pass;
        printf("  PASS: (12, 13, SOUTH) ordinal-22 leak %d%% (< %d%%)\n",
               leakPct, PROBE_FLOOR_LEAK_PCT);
    }
    /* Also exercise the east and north side poses at (12, 13) so a
     * future regression that breaks one side wall but not another
     * is caught by the same probe.  West is excluded because (12, 13,
     * WEST) is the canonical ordinal-22 pose, not a no-portrait
     * side pose.
     *
     * Note: in the verified PC34 C127 layout the (12, 13, NORTH)
     * side pose exposes NO mirror (-1): the only C127 sensor bound
     * to ordinal 22 sits on the EAST wall of the sensor cell
     * (11, 13), visible only from (12, 13) facing WEST.  The
     * contract the probe locks is the no-floating of ordinal 22
     * onto the side wall: front_mirror ordinal != 22 AND D1C rect
     * ordinal-22 leak < 35% (the 35% threshold matches the existing
     * reblt / walkpath_from_entrance / front_south_entry probes).
     * This is the source-locked proof that ordinal 22 (GOTHMOG)
     * does not float onto any D1C wall cell on the (12, 13) cell
     * regardless of the player's facing direction. */
    {
        static const struct { int dir; const char* label; } kSidePoses[] = {
            {PROBE_GOTHMOG_EAST_DIR, "(12, 13, EAST) side pose"},
            {PROBE_GOTHMOG_NORTH_DIR, "(12, 13, NORTH) side pose"}
        };
        size_t i;
        for (i = 0; i < sizeof(kSidePoses) / sizeof(kSidePoses[0]); ++i) {
             ...
             * ordinal, e.g. (12, 13, NORTH) -> 11 in DM1 V1 PC 3.4.) */
     ...
     * (12, 13, SOUTH) pose must produce byte-stable framebuffers (no
     ...
     * front_south_entry probe locks for the (2, 16) cell. */
    {
        ...
        ok &= expect_int("(12, 13, SOUTH) redraw byte stability across cycles",
                         diffBytes, 0);
    }
    return ok;
}

/* (D) South-entry approach pose: party at (12, 14) facing NORTH, one
 ...
 *     (12, 13) when the player approaches from the south). */
static int test_south_entry_approach(M11_GameViewState* game,
    ...
    int compared;
    int leakPct;
    printf("[D] South-entry approach pose at (12, 14, NORTH) - no-portrait "
           "south wall of (12, 13)\n");
    set_pose(game, PROBE_SOUTH_ENTRY_X, PROBE_SOUTH_ENTRY_Y,
             PROBE_SOUTH_ENTRY_DIR);
    ord = M11_GameView_GetFrontMirrorOrdinal(game);
    ok &= expect_int("(12, 14, NORTH) front_mirror ordinal == -1 "
                     "(south wall of (12, 13) has no C127 sensor)",
                     ord, -1);
    memset(fb, 0, sizeof(*fb) * (size_t)(PROBE_FB_W * PROBE_FB_H));
    M11_GameView_Draw(game, fb, PROBE_FB_W, PROBE_FB_H);
    matched = count_ordinal_matched_pixels(portraits, fb, PROBE_ORDINAL_TARGET);
    compared = count_ordinal_compared_pixels(portraits, PROBE_ORDINAL_TARGET);
    leakPct = compared > 0 ? (matched * 100) / compared : 0;
    printf("  (12, 14, NORTH) ordinal-22 matched=%d compared=%d leakPct=%d\n",
           matched, compared, leakPct);
    if (leakPct >= PROBE_FLOOR_LEAK_PCT) {
        ++g_fail;
        printf("  FAIL: (12, 14, NORTH) ordinal-22 leaked matched=%d/%d (>= %d%%)\n",
               matched, compared, PROBE_FLOOR_LEAK_PCT);
        ok = 0;
    } else {
        ++g_pass;
        printf("  PASS: (12, 14, NORTH) ordinal-22 leak %d%% (< %d%%)\n",
               leakPct, PROBE_FLOOR_LEAK_PCT);
    }
    /* Sanity: confirm the canonical ordinal-22 pose at (12, 13, WEST)
    ...
    set_pose(game, PROBE_GOTHMOG_CELL_X, PROBE_GOTHMOG_CELL_Y,
             PROBE_GOTHMOG_FRONT_DIR);
    ord = M11_GameView_GetFrontMirrorOrdinal(game);
    ok &= expect_int("(12, 13, WEST) returns ordinal 22 after south-entry waypoint",
                     ord, PROBE_ORDINAL_TARGET);
    return ok;
}

/* (E) Candidate-panel return at the ordinal-22 cell: open the C040
 *     resurrect/reincarnate panel at (12, 13, WEST), cancel the
 ...
 *     this slice widens the same coverage to the (12, 13, WEST) /
...
static int test_candidate_panel_return_at_south(M11_GameViewState* game,
    ...
    int compared;
    int leakPct;
    printf("[E] Candidate-panel return at (12, 13, WEST) -> cancel -> turn SOUTH\n");
    /* Seat the party at the canonical ordinal-22 pose (12, 13, WEST)
    ...
    ok &= expect_int("SelectFrontMirrorCandidate at (12, 13, WEST) returns 1",
                     selected, 1);
    ...
    compared = count_ordinal_compared_pixels(portraits, PROBE_ORDINAL_TARGET);
    leakPct = compared > 0 ? (matched * 100) / compared : 0;
    printf("  (12, 13, SOUTH) post-cancel ordinal-22 matched=%d compared=%d leakPct=%d\n",
           matched, compared, leakPct);
    if (leakPct >= PROBE_FLOOR_LEAK_PCT) {
        ++g_fail;
        printf("  FAIL: (12, 13, SOUTH) post-cancel ordinal-22 leaked matched=%d/%d (>= %d%%)\n",
               matched, compared, PROBE_FLOOR_LEAK_PCT);
        ok = 0;
    } else {
        ++g_pass;
        printf("  PASS: (12, 13, SOUTH) post-cancel ordinal-22 leak %d%% (< %d%%)\n",
               leakPct, PROBE_FLOOR_LEAK_PCT);
    }
    return ok;
}
