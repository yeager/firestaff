 ...
 * (256x87 pixels total, ordinals 0..23).  Ordinal 19 sits at row 2,
 * column 3 of the atlas:
 *
 *     srcX = (19 & 7) << 5 =  96
 *     srcY = (19 >> 3) * 29 = 58
 *
 ...
 *
 * Ordinal 19 is the second "row 2" ordinal in the shipped DM1 V1
 * PC34 catalog after ordinal 17 (BORIS): the row-2 atlas path
 * (ordinal >> 3) * 29 yields 58 for both ordinals 16..23.  This
 * gate extends the row-2 atlas math coverage (portraits 16 and 17
 * are already covered) to the third column of the row-2 strip and
 * pins the catalog identity for HAWK / THE FEARLESS.
 *
 * The shipped DM1 V1 DUNGEON.DAT places a C127 sensor on the (7,9)
 * NORTH-route front square (7,8) with sensorData=1 (HALK, ordinal 1),
 * so we seed that sensor to sensorData=19 to lock the ordinal-19 edge
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 19: verify the C026 atlas contains a
 *       defined portrait at (96, 58, 32, 29) and that the
 *       (19 & 7) << 5 / (19 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 19 is a
 *       valid C026 atlas entry (not a degenerate cell), and that the
 *       row-2 atlas path through (19 >> 3) * 29 is not a regression
 *       vs the row-2 paths already covered by ordinals 16 and 17.
 *
 *   (2) portrait_rect_position: drive a real D1C front-mirror pose
 *       from the actual DM1 V1 DUNGEON.DAT C127 sensor lattice
 *       (with sensorData seeded to 19), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-19 champion portrait (HAWK), and that
 ...
 *       still carries ordinal-19 pixels after the full
 ...
 *       cancel_reopen probes for ordinals 16 and 17 (different
...
enum {
    ...
    ATLAS_COLS = 8,
    ATLAS_ROWS = 3,
    /* Ordinal 19 in the C026 atlas: (19 & 7) << 5 = 96, (19 >> 3) * 29 = 58. */
    ORDINAL_19_COL = 19 & 7,       /* = 3 */
    ORDINAL_19_ROW = 19 >> 3,      /* = 2 (last row of the 8x3 grid) */
     ...
     * to ordinal 19 (HAWK) for this gate so we can lock the
     * ordinal-19 edge case without changing the map layout. */
    SHIPPED_HALK_ORDINAL = 1,
    /* Row-2 atlas neighbours of ordinal 19 (SONJA = 18 left,
     * ALEX = 20 right).  The DM1 champion-portrait atlas carries
     * 24 distinct champions (one per ordinal), so a duplicate
     * would be a real regression. */
    ROW2_LEFT_ORDINAL = 18,
    ROW2_RIGHT_ORDINAL = 20
};
/* Mirror catalog record name for ordinal 19 (DM1 V1 PC34 mirror
 * catalog).  Used to assert the catalog resolves correctly.  HAWK
 * (title THE FEARLESS) is the 20th valid mirror text string in the
 * shipped DM1 V1 DUNGEON.DAT (the 20th ordinal = 19, after
 * DAROOU / HALK / WU TSE / AZIZI / LEIF / ELIJA / SYRA / TIGGY /
 * IAIDO / ZED / GANDO / STAMM / LINFLAS / WUUF / LEYLA / MOPHUS /
 * CHANI / BORIS / SONJA). */
static const char kExpectedCatalogName[] = "HAWK";
static const char kExpectedCatalogTitle[] = "THE FEARLESS";
 ...
 * Used to verify ordinal 19 is a defined portrait in the atlas
 ...
 * pixels that differ.  Used to verify ordinal 19 is a distinct
 ...
 * lock the ordinal-19 edge case on the real DM1 V1 DUNGEON.DAT
 ...
 * seed_first_c127_data the same square reports ordinal 19. */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-19 / cancel_reopen / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* ----------------------------------------------------------------
     * Group A - Atlas math for ordinal 19 (row 2 / column 3)
     * ----------------------------------------------------------------
     ...
     * covered for row-0/row-1 and ordinal 17 (BORIS, row 2 col 1)
     * is already covered.  Ordinal 19 sits at row 2 / col 3,
     * pushing the row-2 atlas layout one column to the right. */
    ...
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 19 col = 19 & 7 = %d (expected 3)",
                 ORDINAL_19_COL);
        CHECK(ORDINAL_19_COL == 3, msg);
    }
    {
        char msg[160];
        snprintf(msg, sizeof(msg),
                 "ordinal 19 row = 19 >> 3 = %d (expected 2, last row)",
                 ORDINAL_19_ROW);
        CHECK(ORDINAL_19_ROW == 2, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 ...
                 ORDINAL_19_SRC_X, ORDINAL_19_SRC_Y,
                 ATLAS_W, ATLAS_H);
        CHECK(ORDINAL_19_SRC_X + D1C_PORTRAIT_W <= ATLAS_W &&
              ORDINAL_19_SRC_Y + D1C_PORTRAIT_H <= ATLAS_H, msg);
    }

    /* Ordinal 19 must be a defined portrait: opaque count > 100 of the
     ...
     * ordinal 19 (HAWK) is treated as "no portrait" - in particular
     * a regression where (ordinal >> 3) * 29 yields the wrong row on
     * the C026 strip (e.g. row 2 -> row 0 off-by-three). */
    ordinal19Opaque = atlas_cell_opaque_count(portraits, 19);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 19 atlas cell has >= 100 opaque pixels (got %d) "
                 "- defined portrait, not blank/unused",
                 ordinal19Opaque);
        CHECK(ordinal19Opaque >= 100, msg);
    }

    /* Ordinal 19 must be visually distinct from its row-2 neighbours
     * 18 (SONJA) and 20 (ALEX).  The DM1 champion-portrait atlas
    ...
    ordinal19Vs18 = atlas_cell_distinct_percent(portraits, 19, 18);
    ordinal19Vs20 = atlas_cell_distinct_percent(portraits, 19, 20);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 19 vs ordinal 18 (row-2 left neighbour, SONJA) "
                 "differ by >= 30%% (got %d%%)",
                 ordinal19Vs18);
        CHECK(ordinal19Vs18 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 19 vs ordinal 20 (row-2 right neighbour, ALEX) "
                 "differ by >= 30%% (got %d%%)",
                 ordinal19Vs20);
        CHECK(ordinal19Vs20 >= 30, msg);
    }

    /* Ordinal 19 must resolve to HAWK through the mirror catalog.
     * This catches a regression where the catalog and the C026 atlas
     * disagree on the ordinal-19 record - in particular a
     * regression where the catalog is shorter than 20 entries (a
     * catalog count < 20 would have rejected sensorData=19 on the
     * real DUNGEON.DAT). */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
                                                        ...
                                                        (int)sizeof(titleBuf));
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 19 to \"%s\" (expected \"%s\")",
    ...
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "mirror catalog resolves ordinal 19 title to \"%s\" (expected \"%s\")",
     ...
     * seed the C127 sensor from HALK (1) to ordinal 19 (HAWK).  Same
    ...
    /* The same front route now reports ordinal 19.  After seeding
     ...
     * D1C destination rectangle (96, 35, 32, 29) holds ordinal-19
     * pixels. */
    printf("\n[Group B] portrait_rect_position on real C127 sensor pose (1,2,0)=19\n");
    ...
    /* The D1C portrait rect must contain ordinal-19 source pixels at
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-19
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 19\n");
     ...
     * 121 panel guard).  Match against ordinal 19 should be low. */
     ...
     * still active (sensorType=127, sensorData=19 after seed). */
     ...
     * leader hand empty, sensorData=19 still on the front square, so
     ...
     * we verify the same rect lines up with ordinal-19 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 19 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 19 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (96, 58, 32, 29) in the atlas.  This is the
     * "ordinal 19 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 19 atlas round-trip: source (96, 58) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 19 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((19 & 7) << 5, (19 >> 3) * 29, 32, 29)",
                 ORDINAL_19_SRC_X, ORDINAL_19_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_19_SRC_X == 96 && ORDINAL_19_SRC_Y == 58, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 19 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal19Opaque);
        CHECK(ordinal19Opaque >= 200 && ordinal19Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 19 round-trip distinctness: vs 18 = %d%%, "
                 "vs 20 = %d%% (both >= 30%%)",
                 ordinal19Vs18, ordinal19Vs20);
        CHECK(ordinal19Vs18 >= 30 && ordinal19Vs20 >= 30, msg);
    }
