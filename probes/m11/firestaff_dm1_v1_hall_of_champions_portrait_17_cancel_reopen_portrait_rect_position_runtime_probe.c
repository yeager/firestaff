 ...
 * gate is the first one to exercise the second-row atlas math on the
 * real C026 strip; row-0/row-1 ordinals 0, 1, 2, 3, 5 are already
 * covered by the portrait_NN_cancel_reopen probes.
 *
 ...
 * one runtime drive:
 *
 *   (1) Atlas math for ordinal 17: verify the C026 atlas contains a
 *       defined portrait at (32, 58, 32, 29) and that the
 *       (17 & 7) << 5 / (17 >> 3) * 29 math matches COORD.C M027/M028
 *       macro encoding (DEFS.H:821-826).  Verifies ordinal 17 is a
 *       valid C026 atlas entry (not a degenerate cell), and that the
 *       row-2 atlas path through (17 >> 3) * 29 is not a regression
 *       vs the row-0/row-1 paths already covered by the ordinals 0,
 ...
 *       (with sensorData seeded to 17), pixel-prove the destination
 *       rectangle (96, 35, 32, 29) on the 320x200 framebuffer
 *       contains the ordinal-17 champion portrait (BORIS), and that
 ...
 *       still carries ordinal-17 pixels after the full
 ...
 *       that does not link the engine), from the row-0 cancel_reopen
 *       probes for ordinals 0, 1, 2, 3, 5 (different atlas row +
 *       different catalog record), and from
 ...
 * pixels that differ.  Used to verify ordinal 17 is a distinct
 * portrait from its row-2 neighbours (16 CHANI, 18 SONJA). */
...
int main(int argc, char** argv) {
    ...
    printf("=== DM1 V1 Hall of Champions portrait-17 / cancel_reopen / portrait_rect_position ===\n");
    printf("dataDir=%s\n", dataDir);
    ...
    /* Ordinal 17 must be visually distinct from its row-2 neighbours
     * 16 (CHANI) and 18 (SONJA).  The DM1 champion-portrait atlas
     ...
     * pins the row-2 atlas layout specifically. */
    ordinal17Vs16 = atlas_cell_distinct_percent(portraits, 17, 16);
    ordinal17Vs18 = atlas_cell_distinct_percent(portraits, 17, 18);
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 17 vs ordinal 16 (row-2 left neighbour, CHANI) "
                 "differ by >= 30%% (got %d%%)",
                 ordinal17Vs16);
        CHECK(ordinal17Vs16 >= 30, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 17 vs ordinal 18 (row-2 right neighbour, SONJA) "
                 "differ by >= 30%% (got %d%%)",
                 ordinal17Vs18);
        CHECK(ordinal17Vs18 >= 30, msg);
    }
     ...
     * disagree on the ordinal-17 record - in particular a
     * regression where the catalog is shorter than 18 entries (a
     * catalog count < 18 would have rejected sensorData=17 on the
     * real DUNGEON.DAT). */
    nameBuf[0] = '\0';
    titleBuf[0] = '\0';
     ...
     * The framebuffer's D1C portrait rect must still carry ordinal-17
     * pixels after the full select->cancel->select cycle. */
    printf("\n[Group C] cancel_reopen: select, cancel, reopen, portrait rect still carries ordinal 17\n");
     ...
     * 121 panel guard).  Match against ordinal 17 should be low. */
     ...
     * still active (sensorType=127, sensorData=17 after seed). */
     ...
     * leader hand empty, sensorData=17 still on the front square, so
     ...
     * we verify the same rect lines up with ordinal-17 pixels when
    ...
    /* ----------------------------------------------------------------
     * Group D - ordinal 17 atlas round-trip
     * ----------------------------------------------------------------
     * The C026 atlas math for ordinal 17 must be self-consistent: the
     * destination (96, 35, 32, 29) on the framebuffer lines up with
     * the source (32, 58, 32, 29) in the atlas.  This is the
     * "ordinal 17 maps to the expected champion" check from the slice
     * description - the round-trip is independent of the runtime
     * drive and pins the macro math against the atlas itself. */
    printf("\n[Group D] ordinal 17 atlas round-trip: source (32, 58) maps to dst (96, 35)\n");
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas cell for ordinal 17 is at source "
                 "(%d, %d, %d, %d) - matches "
                 "((17 & 7) << 5, (17 >> 3) * 29, 32, 29)",
                 ORDINAL_17_SRC_X, ORDINAL_17_SRC_Y,
                 D1C_PORTRAIT_W, D1C_PORTRAIT_H);
        CHECK(ORDINAL_17_SRC_X == 32 && ORDINAL_17_SRC_Y == 58, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "C026 atlas for ordinal 17 opaque count = %d "
                 "(in expected 200..900 range for a defined champion)",
                 ordinal17Opaque);
        CHECK(ordinal17Opaque >= 200 && ordinal17Opaque <= 900, msg);
    }
    {
        char msg[200];
        snprintf(msg, sizeof(msg),
                 "ordinal 17 round-trip distinctness: vs 16 = %d%%, "
                 "vs 18 = %d%% (both >= 30%%)",
                 ordinal17Vs16, ordinal17Vs18);
        CHECK(ordinal17Vs16 >= 30 && ordinal17Vs18 >= 30, msg);
    }
