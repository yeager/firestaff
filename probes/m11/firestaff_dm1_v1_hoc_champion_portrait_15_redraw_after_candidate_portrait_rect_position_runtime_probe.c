/*
 * DM1 V1 Hall of Champions portrait ordinal 15 (MOPHUS)
 ...
 *   REVIVE.C:704-799     F0282 consumes cancel/confirm candidate actions;
...
static int portrait_match_count(const M11_AssetSlot* portraits,
    ...
    for (y = 0; y < PORTRAIT_H; ++y) {
        ...
        for (x = 0; x < PORTRAIT_W; ++x) {
             ...
             * color 1 transparent. Skip dark gray source pixels too so
             * the check keys on MOPHUS pixels instead of the C346 niche. */
...
static int check_catalog_identity(M11_GameViewState* game) {
    ...
    name[0] = '\0';
    title[0] = '\0';
    (void)M11_GameView_GetMirrorNameByOrdinal(game, ORDINAL_MOPHUS,
                                              name, sizeof(name));
    (void)M11_GameView_GetMirrorTitleByOrdinal(game, ORDINAL_MOPHUS,
                                               title, sizeof(title));
    ok = strcmp(name, "MOPHUS") == 0 && strcmp(title, "THE HEALER") == 0;
    if (ok) {
        pass("ordinal 15 catalog identity is MOPHUS / THE HEALER");
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg),
                 "ordinal 15 catalog identity got name=\"%s\" title=\"%s\"",
                 name, title);
        fail(msg);
    }
    return ok;
...
static int draw_and_expect_portrait(M11_GameViewState* game,
    ...
    snprintf(msg, sizeof(msg),
             "%s ordinal 15 is best C026 match best=%d matched=%d second=%d",
             label, ev.bestOrdinal, ev.bestMatched, ev.secondMatched);
    if (ev.bestOrdinal == ORDINAL_MOPHUS &&
...
static int draw_and_expect_no_portrait(M11_GameViewState* game,
    ...
    collect_portrait_evidence(portraits, fb, ORDINAL_MOPHUS, &ev);
    snprintf(msg, sizeof(msg),
             "%s ordinal 15 absent from D1C cutout (<%d%%) got=%d%%",
...
static int check_panel_open(M11_GameViewState* game,
    ...
    memset(fb, 0, sizeof(fb));
    M11_GameView_Draw(game, fb, FB_W, FB_H);
    collect_portrait_evidence(portraits, fb, ORDINAL_MOPHUS, &portrait);
    ...
    ok &= expect_int("panel_open candidateMirrorOrdinal",
                     game->candidateMirrorOrdinal, ORDINAL_MOPHUS);
    snprintf(msg, sizeof(msg),
    ...
    snprintf(msg, sizeof(msg),
             "panel_open ordinal 15 no longer has a full D1C portrait match (<%d%%) got=%d%%",
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s DATA_DIR\n"
                "  verifies DM1 V1 HoC portrait ordinal 15 "
    ...
    printf("=== DM1 V1 HoC portrait ordinal 15 (MOPHUS) redraw_after_candidate ===\n");
    ...
    printf("D1C cutout viewport=(%d,%d,%d,%d), C026 source=(%d,%d,%d,%d)\n",
           ...
           (ORDINAL_MOPHUS & 7) * PORTRAIT_W,
           (ORDINAL_MOPHUS >> 3) * PORTRAIT_H,
    ...
    if (frontOrdinal != ORDINAL_MOPHUS) {
        printf("SKIP hoc_portrait15_fixture_mismatch "
               "(%d,%d) SOUTH front ordinal=%d expected=%d; this DM1 V1 "
               "build does not expose ordinal 15 on this reference Hall route.\n",
               POSE_X, POSE_Y, frontOrdinal, ORDINAL_MOPHUS);
        M11_GameView_Shutdown(&game);
        return 0;
    }
    ...
    if (M11_GameView_SelectFrontMirrorCandidate(&game) != 1) {
        fail("SelectFrontMirrorCandidate ordinal 15");
        ok = 0;
    } else {
    ...
    if (M11_GameView_ConfirmMirrorCandidate(&game, 0) != 1) {
        fail("ConfirmMirrorCandidate ordinal 15");
        ok = 0;
    } else {
    ...
    if (!open_dm1(dataDir, &menuCancel, &gameCancel)) {
        ...
        if (M11_GameView_GetFrontMirrorOrdinal(&gameCancel) != ORDINAL_MOPHUS) {
            fail("cancel view reference route reports ordinal 15");
            ok = 0;
        } else if (M11_GameView_SelectFrontMirrorCandidate(&gameCancel) != 1) {
            fail("cancel view select ordinal 15");
            ok = 0;
        } else if (M11_GameView_CancelMirrorCandidate(&gameCancel) != 1) {
            fail("CancelMirrorCandidate ordinal 15");
            ok = 0;
        } else {
    ...
    printf("%s dm1 v1 HoC champion portrait ordinal 15 redraw_after_candidate portrait_rect_position\n",
