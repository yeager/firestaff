...
int main(int argc, char** argv) {
    ...
    struct BootProgramStep_Compat steps[4] = {
        {11},
        {12},
        {694},
        {13}
    ...
    if (!F9009_RUNTIME_RunBootPlan_Compat(
            graphicsDatPath,
            outputPrefix,
            dialogGraphicIndex,
            firstFrameNumber,
            ...
            4,
            &result)) {
        fprintf(stderr, "failed: boot plan did not complete\n");
        return 1;
    }
    ...
    printf("requestedStepCount=%u\n", result.program.requestedStepCount);
    ...
    printf("phase=%d\n", (int)result.phase);
    printf("completedPhaseCount=%u\n", result.completedPhaseCount);
