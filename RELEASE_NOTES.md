...
- DM1 alternative food/water use now shares the original consumption rules;
  eating also correctly releases the consumed food record.
...
- CSB FM Towns: chest slots now respond to clicks using their original
  ...
  in Original and V2.1, preserving the original oversized-item restrictions.
  Native save-resume and the other CSB platforms remain under verification.
...
- `DM2 Macintosh gameplay MIDI clock`: Advance authentic `Midi` map cues on
  ...
  backend is available; each event is consumed once per source loop and no
  DOS HMP or synthetic music is substituted.
...
- `dm2_v1_dos_startup_read_verified`: Use a single file pass that fills the
  prefix buffer and drives SHA-256 as bytes are read, replacing the earlier
  open-twice pattern that rehashed intro and end from a freshly allocated
  full-size buffer.
- `dm2_v1_dos_startup_media`: Use `CC_SHA256` on macOS through a
  CommonCrypto shim; the software FIPS 180-4 path stays as fallback.
- `dm2_scan_known_hash_assets`: Bind the on-disk scan cache once around the
  probes of `<base>/dm2`, `<base>/data`, and `<base>` instead of opening and
  saving it per subroot attempt.

### Fixed

- `dm2_v1_mve_video`: Fix opcode 3 to cast the copy delta to `int` before
  negating so unsigned wraparound cannot produce an out-of-range offset.
- `dm2_v1_mve_stream`: Correct the opcode 0x08 minimum size check to require
  at least six bytes so the stream parser matches the PCM and timeline
  validators.
- `dm2_v1_game_load_world_owner`: Fix the party target coordinate write to
  use `uint8_t` so signed truncation cannot corrupt the pose.
...
- `theron_v1_world_hash`: Reject a NULL world argument instead of
  dereferencing it.
...
- `asset_find_by_hash`: Add `asset_scan_cache_batch_begin` and
  `asset_scan_cache_batch_end` so callers can bind one on-disk scan cache
  load and save across a run of related lookups.
...
- DM1 FM Towns end-to-end menu text pipeline: `M11_GameView_LoadDm1FmtownsMenuFontIfAvailable` loads asset 557 and `M11_GameView_RenderDm1FmtownsMenu` composes region lookup, panel fill, label walk and glyph rasterisation into the action-menu path. English DYNAMENU labels paint byte-exact against EDM.EXP; Japanese sessions keep the blank-text policy pending a JDM-owned Shift-JIS font.
- DM1 FM Towns menu font rasteriser `dm1_v1_fmtowns_font_rasterise_string_pc34` round-trip verified against real EDM.EXP asset 557 (6 rows x 128 ASCII glyphs, MSB-first, CHAR_X_SIZE=5 body right-aligned).
- DM1 FM Towns complete 23-block region registry (994 records) exposed as `dm1_v1_fmtowns_region_blocks` + `dm1_v1_fmtowns_region_records`; `dm1_v1_fmtowns_region_lookup_pc34` covers every source-verified region.
- DM1 FM Towns icon geometry constants `DM1_V1_FMTOWNS_ICON_SIZE_BYTES` (=256), `ICON_X_SIZE` (=16), `ICON_Y_SIZE` (=16), `SCR_X_SIZE` (=320) recovered from EDM.EXP with real-data round-trip test.
- DM1 FM Towns icon category threshold table `dm1_v1_fmtowns_icon_category_thresholds` (7 buckets x 32 slots = 224 icons) plus `dm1_v1_fmtowns_icon_classify_pc34` mirroring LOAD_ICON's category-select loop.
- DM1 FM Towns complete 224 x 6 OICON descriptor table `dm1_v1_fmtowns_oicon_descriptor` (1344 bytes) driving `DRAW_ICN_BUTTON` dispatch decisions.

## Fixed

- CI Windows `-Werror=unused-function` on `external_archive_entry_may_match_md5` (and `_list`) - the helpers are only called from the POSIX-only external-archive scan block; guard the definitions with `#ifndef _WIN32` too.
- CI macOS undefined `_DM1_V1_MirrorCandidateIconRefresh_DrawChangedObjectIconsPc34Compat` symbol in `test_dm1_v1_hoc_champion_portrait_13_double_click_stability_pc34_compat` - target now lists the `icon_refresh` and `close_button` sibling sources it calls.
...
- Artpack Studio is listed as a desktop application: a separate macOS app,
...
- Windows bundling now accepts both native and MSYS Python virtual-environment
  layouts.
- MSYS releases use the distribution's matching Pillow and PyInstaller builds.
- macOS packaging remains compatible with the runner's Bash 3 shell.
...
presentation ownership pass landed after `v3.0.66` was tagged.

## Highlights since v3.0.66

- **Theron's Quest startup ownership**: M11 now consumes the Theron-owned
  startup presentation receipt for boot phase, animation and title readiness.
- **Release metadata**: In-app version strings, CMake metadata and release
  notes are synchronized to `v3.0.67`.

## Verification

- Local release verification covers version synchronization, the Theron startup
  flow probe, the Phase A probe and `git diff --check` before GitHub Actions
  packaging.
- The GitHub release workflow builds and packages macOS arm64, macOS x86_64,
  Windows x86_64, Linux x86_64, Linux arm64 and Steam Deck x86_64 artifacts
  from the `v3.0.67` release run.
...
explosion render-helper cleanup landed after `v3.0.62` was tagged.

## Highlights since v3.0.62

- **DM1 projectile/explosion helper ownership**: DM1 V1 projectile subtype,
  aspect metadata, scale, flip, and explosion queries now live in the
  source-locked DM1 compat helper used by M11 instead of being duplicated in
  the game-view renderer.
- **Subtype graphic parity guard**: The DM1 projectile test now covers subtype
  to native projectile graphic mapping, including the kinetic fallback.
- **Release metadata**: In-app version strings, CMake metadata and release
  notes are synchronized to `v3.0.63`.

## Verification

- Local release verification covers the release-CI CSB test targets, the
  focused DM1 projectile/explosion render gate, the Phase A probe and
  `git diff --check` before GitHub Actions packaging.
- The GitHub release workflow builds and packages macOS arm64, macOS x86_64,
  Windows x86_64, Linux x86_64, Linux arm64 and Steam Deck x86_64 artifacts
  from the `v3.0.63` release run.
...
draw binding ownership pass.

## Highlights since v3.0.58

- **CSB runtime object draw binding**: CSB runtime object overlay placements now
  carry sprite thing type, subtype, relative cell, pile index and icon index
  from the CSB overlay collector, so M11 consumes the CSB-owned draw binding
  instead of re-reading runtime object info during rendering.
- **Release metadata**: In-app version strings, CMake metadata and release
  notes are synchronized to `v3.0.59`.

## Verification

- Local release verification covers version synchronization, the focused CSB V1
  viewport/rendering gate, the Phase A probe and `git diff --check` before
  GitHub Actions packaging.
- The GitHub release workflow builds and packages macOS arm64, macOS x86_64,
  Windows x86_64, Linux x86_64, Linux arm64 and Steam Deck x86_64 artifacts
  from the `v3.0.59` release run.
...
runtime overlay rendering ownership work.

## Highlights since v3.0.52

- **CSB V1 runtime overlay marker drawing**: CSB viewport code now owns the
  fallback marker drawing for runtime floor objects and creature groups instead
  of keeping those pixel routines in the shared M11 game view.
- **CSB V1 rendering ownership**: M11 now consumes CSB-owned object/group
  overlay placement and marker helpers, reducing remaining shared-game-view
  assumptions before exact sprite/material parity work.
- **Release metadata**: In-app version strings, CMake metadata and release
  notes are synchronized to `v3.0.53`.

## Verification

- Local release verification covers version synchronization, the focused CSB V1
  viewport/render gates, the Phase A probe and `git diff --check` before GitHub
  Actions packaging.
- The GitHub release workflow builds and packages macOS arm64, macOS x86_64,
  Windows x86_64, Linux x86_64, Linux arm64 and Steam Deck x86_64 artifacts
  from the `v3.0.53` release run.
...
presentation ownership work.

## Highlights since v3.0.48

- **CSB startup title palettes**: CSB owns the special-palette decisions for
  PRESENTS, CHAOS zoom and STRIKES BACK title stages used by M11.
- **CSB entrance rendering metadata**: CSB startup plans expose entrance door
  source assets, closed-door boxes, opening-door boxes and credits layout data
  so M11 consumes structured startup render metadata instead of local constants.
- **CSB utility preview rendering**: CSB utility flow owns the DM1 import status
  row and imported champion preview rows before M11 draws them.
- **Release metadata**: In-app version strings, CMake metadata and release
  notes are synchronized to `v3.0.49`.
...
inscription, side-wall and arrow-feedback gates.

## Highlights since v3.0.45

- **DM1 V1 wall inscriptions**: Escape-glyph handling is preserved through the
  inscription font path and covered by the focused inscription-font test.
- **DM1 V1 side-wall rendering**: Side-wall draw ordering stays source-shaped
  and covered by the focused action/stamina runtime gate.
- **DM1 V1 arrow feedback**: The overlay command queue gate covers the aligned
  on-screen arrow feedback path.
- **Release metadata**: In-app version strings, CMake metadata and release notes
  are synchronized to `v3.0.46`.

## Verification

- Local release verification covers version synchronization, focused DM1 V1
  inscription/action/overlay gates and the Phase A probe before GitHub Actions
  packaging.
- The GitHub release workflow builds and packages macOS arm64, macOS x86_64,
  Windows x86_64, Linux x86_64, Linux arm64 and Steam Deck x86_64 artifacts
  from the `v3.0.46` release run.
...
pointer-action and title-phase ownership work on `main` after v3.0.41.

## Highlights since v3.0.41

- **CSB utility input**: CSB utility startup now owns keyboard action routing
  for import, load, view, preview close and start-new decisions before M11
  executes entrance commands.
- **Nexus startup pointer actions**: Nexus startup now owns pointer actions for
  title, save-select and champion-select phases before M11 mutates startup
  state.
- **Nexus title phases**: Nexus title sequence now exposes named boot-reveal,
  hold and start-ready phases with frame counters for tests and UI consumers.

## Verification

- Local release-prep verification covers version synchronization, focused Nexus
  and CSB startup gates, and the Phase A probe before GitHub Actions packaging.
- The GitHub release workflow builds and packages macOS arm64, macOS x86_64,
  Windows x86_64, Linux x86_64, Linux arm64 and Steam Deck x86_64 artifacts
  from the `v3.0.42` release run.
...
- **CSB champion import parity guard**: `csb_v1_character_import` now asserts that DM1-save imported champions retain the CSB reincarnation-control defaults (`reincarnateAttributePenalty`, `reincarnateStatPenalty`, and `randomPoints`) before later reincarnation logic consumes them. The public gap list was also corrected so the remaining CSB champion work points at the real-asset F0282 panel integration gate, not already-landed data-free fixtures.
