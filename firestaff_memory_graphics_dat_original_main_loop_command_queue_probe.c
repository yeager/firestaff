...
static int try_candidate(
    ...
    struct MemoryGraphicsDatMainLoopCommandQueueResult_Compat* queueResult,
    ...
    ok = F0479_MEMORY_RunMainLoopCommandQueueMini_Compat(
        graphicsDatPath,
        &fileState,
        dialogGraphicIndex,
...
int main(int argc, char** argv) {
    ...
    for (dialogIndex = dialogStart; dialogIndex < dialogLimit; ++dialogIndex) {
        for (viewportIndex = 0; viewportIndex < viewportLimit; ++viewportIndex) {
            memset(&queueResult, 0, sizeof(queueResult));
            attempts++;
            if (!try_candidate(graphicsDatPath, dialogIndex, viewportIndex, &queueResult, &header)) {
                continue;
            }
            printf("ok\n");
            ...
            printf("requestedQueueLength=%u\n", queueResult.requestedQueueLength);
            printf("completedQueueLength=%u\n", queueResult.completedQueueLength);
            printf("handledCommandCount=%u\n", queueResult.handledCommandCount);
            printf("queueReady=%d\n", queueResult.queueReady);
            printf("queueCompleted=%d\n", queueResult.queueCompleted);
            printf("lastCommandHandled=%d\n", queueResult.lastLoop.lastCommand.commandHandled);
            F0479_MEMORY_FreeMainLoopCommandQueueMini_Compat(&queueResult);
            F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
            return 0;
        }
    }

    fprintf(stderr, "failed: no main-loop command-queue candidate succeeded in %u attempts\n", attempts);
    F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
    return 1;
}
