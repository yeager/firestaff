...
static int try_candidate(
    ...
    struct MemoryGraphicsDatMainLoopTypedCommandQueueResult_Compat* typedResult,
    struct MemoryGraphicsDatHeader_Compat* header) {
    struct MemoryGraphicsDatState_Compat fileState = {0};
    enum MemoryGraphicsDatMainLoopCommand_Compat commands[3] = {
        MEMORY_GRAPHICS_DAT_MAIN_LOOP_COMMAND_NONE,
        MEMORY_GRAPHICS_DAT_MAIN_LOOP_COMMAND_TICK,
        MEMORY_GRAPHICS_DAT_MAIN_LOOP_COMMAND_TICK
    };
    ...
    ok = F0479_MEMORY_RunMainLoopTypedCommandQueueMini_Compat(
        graphicsDatPath,
        &fileState,
        dialogGraphicIndex,
...
int main(int argc, char** argv) {
    ...
    for (dialogIndex = dialogStart; dialogIndex < dialogLimit; ++dialogIndex) {
        for (viewportIndex = 0; viewportIndex < viewportLimit; ++viewportIndex) {
            memset(&typedResult, 0, sizeof(typedResult));
            attempts++;
            if (!try_candidate(graphicsDatPath, dialogIndex, viewportIndex, &typedResult, &header)) {
                continue;
            }
            printf("ok\n");
            ...
            printf("requestedQueueLength=%u\n", typedResult.queue.requestedQueueLength);
            printf("completedQueueLength=%u\n", typedResult.queue.completedQueueLength);
            printf("noopCommandCount=%u\n", typedResult.noopCommandCount);
            printf("tickCommandCount=%u\n", typedResult.tickCommandCount);
            printf("handledTickCommandCount=%u\n", typedResult.handledTickCommandCount);
            printf("queueCompleted=%d\n", typedResult.queue.queueCompleted);
            F0479_MEMORY_FreeMainLoopTypedCommandQueueMini_Compat(&typedResult);
            F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
            return 0;
        }
    }

    fprintf(stderr, "failed: no typed-command queue candidate succeeded in %u attempts\n", attempts);
    F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
    return 1;
}
