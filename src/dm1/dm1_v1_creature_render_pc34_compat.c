...
unsigned int dm1_creature_sprite_for_view(int creatureType,
    ...
    int directionDelta;
    int pose;
    uint8_t aspectBits = attacking ? DM1_CREATURE_ASPECT_IS_ATTACKING : 0;
    (void)depthIndex;
    if (creatureType < 0 || creatureType >= DM1_CREATURE_TYPE_COUNT ||
        creatureDir < 0 || partyDir < 0) {
        if (outMirror) *outMirror = 0;
        return 0;
    }
    directionDelta = dm1_creature_direction_delta(partyDir, creatureDir);
    pose = dm1_creature_pose_from_delta(directionDelta, attacking,
                                        s_aspects[creatureType].graphicInfo);
    if (outMirror) {
        *outMirror = dm1_creature_should_flip(directionDelta, pose, attacking,
                                               s_aspects[creatureType].graphicInfo,
                                               aspectBits);
    }
    /* F0115 derives D2/D3 surfaces from this native C584+ source. M11 owns
     * original GRAPHICS.DAT records, not the transient derived-cache slots. */
...
int dm1_creature_replacement_colors(int creatureType,
                                    int *outReplDst9,
                                    int *outReplDst10) {
    static const unsigned char k_repl_color_d2[13] =
        {9, 0, 10, 9, 9, 10, 9, 10, 10, 5, 10, 12, 10};
    int setIdx9;
    int setIdx10;
    if (creatureType < 0 || creatureType >= DM1_CREATURE_TYPE_COUNT) return 0;
    ...
    if (outReplDst9) {
        *outReplDst9 = (setIdx9 > 0 && setIdx9 < 13)
                     ? (int)k_repl_color_d2[setIdx9 - 1] : 9;
    }
    if (outReplDst10) {
        *outReplDst10 = (setIdx10 > 0 && setIdx10 < 13)
                      ? (int)k_repl_color_d2[setIdx10 - 1] : 10;
    }
    return 1;
}
...
int dm1_creature_palette_for_depth(int creatureType, int depthIndex,
    ...
    /* ReDMCSB DUNVIEW.C G2025, I34E: replacement set index -> D2/D3 color. */
    static const unsigned char k_repl_color_d2[13] =
        {9, 0, 10, 9, 9, 10, 9, 10, 10, 5, 10, 12, 10};
    static const unsigned char k_repl_color_d3[13] =
        {9, 0, 10, 0, 10, 0, 0, 9, 5, 3, 10, 0, 5};
    const unsigned char *base = NULL;
    int setIdx9;
    int setIdx10;
    int i;

    if (!outPalette || creatureType < 0 || creatureType >= DM1_CREATURE_TYPE_COUNT) {
        return 0;
    }
    if (depthIndex == 1) base = s_paletteD2;
    else if (depthIndex >= 2) base = s_paletteD3;
    for (i = 0; i < 16; ++i) outPalette[i] = base ? base[i] : (uint8_t)i;
    if (!base) return 1;
    setIdx9 = dm1_creature_repl_color9_index(&s_aspects[creatureType]);
    setIdx10 = dm1_creature_repl_color10_index(&s_aspects[creatureType]);
    if (setIdx9 > 0 && setIdx9 < 13) {
        outPalette[9] = depthIndex == 1 ? k_repl_color_d2[setIdx9 - 1]
                                        : k_repl_color_d3[setIdx9 - 1];
    }
    if (setIdx10 > 0 && setIdx10 < 13) {
        outPalette[10] = depthIndex == 1 ? k_repl_color_d2[setIdx10 - 1]
                                         : k_repl_color_d3[setIdx10 - 1];
    }
    return 1;
}
