...
int dm1_creature_raw_c3200_draw_plan(const int *creatureTypes,
    ...
    outPlan->count = 0;
    if (!creatureTypes || !creatureCounts || !creatureDirections ||
        groupCount < 1 || rawRow < 0) return 0;
    for (gi = 0; gi < groupCount && outCount < DM1_CREATURE_DRAW_PLAN_MAX; ++gi) {
        int di;
        int visible;
        int coordSet;
        if (creatureTypes[gi] < 0 || creatureTypes[gi] >= DM1_CREATURE_TYPE_COUNT) continue;
        visible = dm1_creature_visible_duplicates(creatureCounts[gi], 3);
        coordSet = dm1_creature_coordinate_set(creatureTypes[gi]);
        for (di = 0; di < visible && outCount < DM1_CREATURE_DRAW_PLAN_MAX; ++di) {
            DM1_CreatureDrawPlanEntry *entry = &outPlan->entries[outCount];
            int x = 0;
            int y = 0;
            if (!dm1_viewport_3d_c3200_creature_raw_zone_point(
                    coordSet, rawRow, visible, di, &x, &y)) continue;
            ...
            entry->duplicate_index = di;
            entry->first_in_group = di == 0;
            entry->placement.source_anchor_x = x;
            entry->placement.source_anchor_y = y;
            entry->placement.source_depth_index = 2; /* F0676/F0677 are D3 */
            entry->placement.source_anchor_valid = 1;
            ++outCount;
        }
    }
    outPlan->count = outCount;
