 ...
 * G0204_auc_Graphic558_UnreadableInscriptionBoxY2[15] (PC 3.4 EN init).
 *
 * G0204 is the 15-int frame coord table
 * for UnreadableInscriptionBoxY2.
 *
...
static const int s_g0204[kTableSize] = {
    ...
    DM1_V1_G0204ResultPc34 *out)
{
    ...
    for (i = 0; i < kTableSize; ++i) {
        out->tableEntries[i] = s_g0204[i];
    }
    out->tableSize = kTableSize;

    for (i = 0; i < kTableSize; ++i) {
        if (s_g0204[i] < 0 || s_g0204[i] > 255) {
            all_in_byte_range = 0;
        }
    }
    ...
    {
        static const int kExpected[kTableSize] = { 5, 8, 13, 7, 13, 20, 5, 12, 19, 10, 17, 27, 11, 22, 33 };
        for (i = 0; i < kTableSize; ++i) {
            if (s_g0204[i] != kExpected[i]) {
    ...
    for (i = 0; i < kTableSize; ++i) {
        if (dm1_v1_g0204_get_pc34(i) != s_g0204[i]) {
            lookup_function_correct = 0;
        }
    }
    out->lookupFunctionCorrect = lookup_function_correct;

    if (dm1_v1_g0204_get_pc34(-1) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0204_get_pc34(kTableSize) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0204_get_pc34(999) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
