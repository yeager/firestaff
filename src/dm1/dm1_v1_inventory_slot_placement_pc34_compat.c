...
int dm1_v1_inventory_champion_slot_for_source_slot_box_pc34(
    int sourceSlotBoxIndex) {
    /* ReDMCSB COMMAND.C C034..C059: layout-696 C507..C536 slot boxes map
     * onto the champion inventory slots via the source drop-order table,
     * not a linear offset (restored from 3cde9a3e7 after the worktree
     * merge drift replaced the switch with `index - 8`). */
...
int dm1_v1_inventory_source_slot_box_for_champion_slot_pc34(
     ...
     * source slot-box zones C507..C536 (restored from 3cde9a3e7). */
    switch (championSlot) {
...
/* Restored after the worktree merge drift (contract kept at 9c1c2c828);
 * the full leader-hand floor-drop gate test consumes the xywh view. */
int dm1_v1_inventory_chest_slot_box_zone_xywh_pc34(
    ...
    DM1_V1_InventorySlotBoxZonePc34 zone;
    if (!outX || !outY || !outW || !outH ||
        !dm1_v1_inventory_chest_slot_box_zone_pc34(
            chestOrdinal, &zone)) {
        return 0;
    }
    *outX = zone.x;
