...
/*
 * ReDMCSB source-lock map for this gate (G8161_MASTER):
 * - DRAWVIEW.C:301 - declaration + PC 3.4 init
 * - ANIMTOWN.C:284 - duplicate declaration
 * - ANIMTOWN.C:411 - G8176_PaletteTable[14] = G8161_MASTER
 * - ANIMTOWN.C:624-629 - palette-walk loop reads until Index == 0xFF
 *
 * Disjoint from pass784+ non-mirror-candidate contract gates.
 * Disjoint from pass784-790.
 * - DRAWVIEW.C:301/284/411/624-629, ANIMTOWN.C:284/411/624-629 — anchor references
 */
...
static const unsigned char s_g8161[kTableSize * 4] = {
    /* 0  */ 0x0F, 0x3F, 0x00, 0x00,
...
dm1_v1_animtown_color_master_get_pc34(int entry_index, int field_index)
{
    ...
    return (int)s_g8161[entry_index * 4 + field_index];
}
    ...
    DM1_V1_AnimtownColorMasterResultPc34 *out)
{
    ...
    for (i = 0; i < kTableSize * 4; ++i) {
        out->tableEntries[i] = (int)s_g8161[i];
    }
    out->tableSize = kTableSize;

    if (s_g8161[kTableSize * 4 - 4] != 0xFF) last_entry_sentinel_index_0xff = 0;

    for (i = 0; i < kTableSize; ++i) {
        if (s_g8161[i * 4 + 1] > 63) all_rgb_in_byte_range = 0;
        if (s_g8161[i * 4 + 2] > 63) all_rgb_in_byte_range = 0;
        if (s_g8161[i * 4 + 3] > 63) all_rgb_in_byte_range = 0;
    }
    out->allRgbInByteRange = all_rgb_in_byte_range;

    for (i = 0; i < kTableSize - 1; ++i) {
        if (s_g8161[i * 4 + 0] > 0x1F) all_indices_nonzero_except_last = 0;
    }
    ...
    for (i = 0; i < kTableSize; ++i) {
        int j;
        for (j = 0; j < 4; ++j) {
            if (dm1_v1_animtown_color_master_get_pc34(i, j) !=
                (int)s_g8161[i * 4 + j]) {
    ...
    if (dm1_v1_animtown_color_master_get_pc34(-1, 0) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_master_get_pc34(0, -1) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_master_get_pc34(kTableSize, 0) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_master_get_pc34(0, 4) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_master_get_pc34(999, 999) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
