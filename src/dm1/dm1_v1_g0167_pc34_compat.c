 ...
 * G0167_auc_Graphic558_Frame_DoorFrameRight_D3C[8] (PC 3.4 EN init).
 *
 * G0167 is the 8-int frame coord table
 * for Frame_DoorFrameRight_D3C.
 *
...
static const int s_g0167[kTableSize] = {
    128, 159, 27, 70, 16, 44, 0, 0
};
    ...
    DM1_V1_G0167ResultPc34 *out)
{
    ...
    for (i = 0; i < kTableSize; ++i) {
        out->tableEntries[i] = s_g0167[i];
    }
    out->tableSize = kTableSize;

    for (i = 0; i < kTableSize; ++i) {
        if (s_g0167[i] < 0 || s_g0167[i] > 255) {
            all_in_byte_range = 0;
        }
    }
    ...
    {
        static const int kExpected[kTableSize] = { 128, 159, 27, 70, 16, 44, 0, 0 };
        for (i = 0; i < kTableSize; ++i) {
            if (s_g0167[i] != kExpected[i]) {
    ...
    for (i = 0; i < kTableSize; ++i) {
        if (dm1_v1_g0167_get_pc34(i) != s_g0167[i]) {
            lookup_function_correct = 0;
        }
    }
    out->lookupFunctionCorrect = lookup_function_correct;

    if (dm1_v1_g0167_get_pc34(-1) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0167_get_pc34(kTableSize) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0167_get_pc34(999) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
