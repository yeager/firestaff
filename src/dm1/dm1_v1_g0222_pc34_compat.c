 ...
 * G0222_auc_Graphic558_PaletteChanges_Creature_D2[16] (PC 3.4 EN init).
 *
 * G0222 is the 16-int frame coord table
 * for PaletteChanges_Creature_D2.
 *
...
static const int s_g0222[kTableSize] = {
    ...
    DM1_V1_G0222ResultPc34 *out)
{
    ...
    for (i = 0; i < kTableSize; ++i) {
        out->tableEntries[i] = s_g0222[i];
    }
    out->tableSize = kTableSize;

    for (i = 0; i < kTableSize; ++i) {
        if (s_g0222[i] < 0 || s_g0222[i] > 255) {
            all_in_byte_range = 0;
        }
    }
    ...
    {
        static const int kExpected[kTableSize] = { 0, 1, 2, 3, 4, 3, 6, 7, 5, 0, 0, 11, 12, 13, 14, 15 };
        for (i = 0; i < kTableSize; ++i) {
            if (s_g0222[i] != kExpected[i]) {
    ...
    for (i = 0; i < kTableSize; ++i) {
        if (dm1_v1_g0222_get_pc34(i) != s_g0222[i]) {
            lookup_function_correct = 0;
        }
    }
    out->lookupFunctionCorrect = lookup_function_correct;

    if (dm1_v1_g0222_get_pc34(-1) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0222_get_pc34(kTableSize) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0222_get_pc34(999) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
