 ...
 * G0214_auc_Graphic558_PaletteChanges_FloorOrnament_D2[16] (PC 3.4 EN init).
 *
 * G0214 is the 16-int frame coord table
 * for PaletteChanges_FloorOrnament_D2.
 *
...
static const int s_g0214[kTableSize] = {
    ...
    DM1_V1_G0214ResultPc34 *out)
{
    ...
    for (i = 0; i < kTableSize; ++i) {
        out->tableEntries[i] = s_g0214[i];
    }
    out->tableSize = kTableSize;

    for (i = 0; i < kTableSize; ++i) {
        if (s_g0214[i] < 0 || s_g0214[i] > 255) {
            all_in_byte_range = 0;
        }
    }
    ...
    for (i = 0; i < kTableSize; ++i) {
        if (dm1_v1_g0214_get_pc34(i) != s_g0214[i]) {
            lookup_function_correct = 0;
        }
    }
    out->lookupFunctionCorrect = lookup_function_correct;

    if (dm1_v1_g0214_get_pc34(-1) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0214_get_pc34(kTableSize) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_g0214_get_pc34(999) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
