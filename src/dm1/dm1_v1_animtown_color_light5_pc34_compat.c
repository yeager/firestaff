#include "firestaff/dm1/v1/animtown_color_light5_pc34_compat.h"

#include <string.h>

/*
 * ReDMCSB source-lock map for this gate (G8156_LIGHT5):
 * - DRAWVIEW.C:187 - declaration + PC 3.4 init (17 COLOR_DEF entries)
 * - ANIMTOWN.C:171 - duplicate declaration
 * - ANIMTOWN.C:402 - G8176_PaletteTable[5] = G8156_LIGHT5
 * - ANIMTOWN.C:624-629 - palette-walk loop reads until Index == 0xFF
 *
 * Disjoint from pass784+ non-mirror-candidate contract gates.
 * Disjoint from pass784-790.
 * - DRAWVIEW.C:187/171/402/624-629 — read sites
 */
...
static const unsigned char s_g8156[kTableSize * 4] = {
    ...
    /* 7  */ 0x17, 0x00, 0x09, 0x00,
    /* 8  */ 0x18, 0x12, 0x00, 0x00,
    /* 9  */ 0x19, 0x12, 0x00, 0x00,
    /* 10 */ 0x1A, 0x09, 0x00, 0x00,
    /* 11 */ 0x1B, 0x1B, 0x12, 0x00,
    /* 12 */ 0x1C, 0x00, 0x00, 0x00,
    /* 13 */ 0x1D, 0x00, 0x00, 0x00,
    /* 14 */ 0x1E, 0x00, 0x00, 0x12,
    /* 15 */ 0x1F, 0x12, 0x12, 0x12,
...
dm1_v1_animtown_color_light5_get_pc34(int entry_index, int field_index)
{
    ...
    return (int)s_g8156[entry_index * 4 + field_index];
}
    ...
    DM1_V1_AnimtownColorLight5ResultPc34 *out)
{
    ...
    for (i = 0; i < kTableSize * 4; ++i) {
        out->tableEntries[i] = (int)s_g8156[i];
    }
    out->tableSize = kTableSize;

    if (s_g8156[0] != 0x10) first_entry_index_zero = 0;
    if (s_g8156[kTableSize * 4 - 4] != 0xFF) last_entry_sentinel_index_0xff = 0;

    for (i = 0; i < kTableSize; ++i) {
        if (s_g8156[i * 4 + 1] > 63) all_rgb_in_byte_range = 0;
        if (s_g8156[i * 4 + 2] > 63) all_rgb_in_byte_range = 0;
        if (s_g8156[i * 4 + 3] > 63) all_rgb_in_byte_range = 0;
    }
    out->allRgbInByteRange = all_rgb_in_byte_range;

    for (i = 0; i < kTableSize - 1; ++i) {
        if (s_g8156[i * 4 + 0] > 0x1F) all_indices_nonzero_except_last = 0;
    }
    ...
    for (i = 0; i < kTableSize; ++i) {
        int j;
        for (j = 0; j < 4; ++j) {
            if (dm1_v1_animtown_color_light5_get_pc34(i, j) !=
                (int)s_g8156[i * 4 + j]) {
    ...
    if (dm1_v1_animtown_color_light5_get_pc34(-1, 0) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_light5_get_pc34(0, -1) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_light5_get_pc34(kTableSize, 0) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_light5_get_pc34(0, 4) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
    if (dm1_v1_animtown_color_light5_get_pc34(999, 999) != kOutOfRange) {
        lookup_out_of_range_returns_minus_one = 0;
    }
