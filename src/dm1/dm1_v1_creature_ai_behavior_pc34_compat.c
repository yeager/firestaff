    ...
    struct DM1CreatureProjectileAttack_Compat* out)
{
    ...
    /* GROUP.C F0207 BUG0_13 has no initialized projectile Thing for these
     * two non-retail caster types. Do not replace that absence with a made-up
     * fireball or consume unrelated RNG state. */
    if (ctx->creatureType == DM1_CREATURE_TYPE_LORD_ORDER ||
        ctx->creatureType == DM1_CREATURE_TYPE_GREY_LORD) {
        out->rngCallCount = 0;
        return 1;
    }
    ...
    struct DM1BehaviorResult_Compat* result)
{
    ...
    if (eventType < DM1_EVENT_UPDATE_BEHAVIOR_GROUP) {
        if (ctx->distanceToVisibleParty > 0) {
            if (behavior != DM1_BEHAVIOR_ATTACK &&
                ...
                if (behavior == DM1_BEHAVIOR_WANDER) {
                    ...
                    /* GROUP.C T0209054 then T0209136: the approach
                     * increment precedes addition of incoming C.Ticks. */
                    result->nextEventDelayTicks = 1 + ctx->eventTicks;
                    return 1;
                }
            }
            activeGroup->targetMapX = ctx->partyMapX;
        ...
        /* GROUP.C F0209 T0209136: behavior time adds C.Ticks, including
         * zero. F0179 owns a separate aspect deadline for F0208. */
        result->nextEventDelayTicks = ctx->eventTicks;
        return 1;
    }
