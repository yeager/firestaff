...
/* Version string comes from the CMake-generated firestaff_version.h
 * (derived from project(Firestaff VERSION ...) in CMakeLists.txt). */
...
static const M12_MuseumCategory g_museumCategories[M12_MUSEUM_CATEGORY_COUNT] = {
    ...
    },
    {
        ...
        _("ABOUT AND CREDITS"),
        {
            ...
            {_("CREDITS"), _("DANIEL NYLANDER 2026 GITHUB.COM/YEAGER/FIRESTAFF"), _("FTL GAMES AND SOFTWARE HEAVEN CREATED THE ORIGINALS"), _("CHRISTOPHE FONTANEL DOCUMENTED VITAL HISTORY"), _("REDMCSB SKPROJECT SDL3 ZLIB DMWEB")},
...
const char* M12_StartupMenu_TranslateForLocale(int languageIndex,
                                                const char* english) {
    return m12_translate_for_locale(languageIndex, english);
}
...
static int m12_data_directory_dialog_token_is_placeholder(const char* path) {
    ...
    /* Accept only a sequence of single-dot path components. This catches
     * SDL/macOS variants such as "./." and ".//" without rejecting the
     * valid parent-directory selection ".." or "../Games/DM". */
    cursor = path;
    while (*cursor) {
...
static const M12_AssetVersionStatus* m12_selected_version_status(const M12_StartupMenuState* state,
    ...
    /* m12_apply_auto_platform_versions establishes the AUTO default after
...
int M12_StartupMenu_SetDM1HoCPresentedCaptureReceipt(
    ...
    memset(&state->dm1HoCPresentedCaptureReceipt,
           0,
           sizeof(state->dm1HoCPresentedCaptureReceipt));
