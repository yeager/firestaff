    ...
    DM2_V1_InventoryPanelSurveyFrameReceipt *out_receipt)
{
    ...
    if (!out_receipt) return 0;
    memset(out_receipt, 0, sizeof(*out_receipt));
    /* skproject SKWIN/SkWinCore.cpp DRAW_ITEM_SURVEY:13962-14019 begins the
     * item-description path with DRAW_STATIC_PIC(INTERFACE_CHARSHEET, 0, 1,
     * 0x1ee, -1). It has no alternate frame when that exact material is
     * absent; the item preview is a later optional operation. */
    if (!loader ||
        !dm2_v1_asset_load_image_metadata(
            loader, DM2_GDAT_CATEGORY_INTERFACE_CHARSHEET, 0u, 1u,
            &out_receipt->image_metadata) ||
        out_receipt->image_metadata.bits_per_pixel != 4u ||
        !dm2_v1_asset_load_image_local_palette(
            loader, DM2_GDAT_CATEGORY_INTERFACE_CHARSHEET, 0u, 1u,
    ...
    DM2_V1_InventoryPanelHudConsumptionReceipt *out_receipt)
{
    ...
    if (!out_receipt) return 0;
    memset(out_receipt, 0, sizeof(*out_receipt));
    if (!loader || !frame || !frame->valid || frame->receipt_hash == 0u ||
        frame->image_field != 1u || !blit || !surface || !surface->pixels ||
        surface->width <= 0 || surface->height <= 0 ||
        surface->stride < surface->width ||
        blit->rect_number != frame->expanded_rect_index ||
        blit->transparent_index != UINT8_MAX || blit->source_x < 0 ||
        blit->source_y < 0 || blit->destination_x < 0 ||
        blit->destination_y < 0 || blit->width == 0u || blit->height == 0u) {
        return 0;
    ...
    if (!dm2_v1_asset_load_image_local_palette(
            loader, DM2_GDAT_CATEGORY_INTERFACE_CHARSHEET, 0u,
            frame->image_field, palette16, &palette_hash) ||
        palette_hash == 0u || palette_hash != frame->local_palette_hash ||
        memcmp(palette16, frame->local_palette16, sizeof(palette16)) != 0) {
        return 0;
    }
    source_pixels = dm2_v1_asset_load_image_field(
        loader, DM2_GDAT_CATEGORY_INTERFACE_CHARSHEET, 0u, frame->image_field,
        &source_width, &source_height, &source_format);
    if (!source_pixels || source_width != (int)frame->decoded_width ||
        source_height != (int)frame->decoded_height ||
        source_format != frame->decoded_format ||
        (source_format != DM2_IMG_FMT_IMG3 && source_format != DM2_IMG_FMT_U4) ||
        blit->source_x + (int)blit->width > source_width ||
        blit->source_y + (int)blit->height > source_height ||
        blit->destination_x + (int)blit->width > surface->width ||
    ...
    if (pixels_hash != frame->decoded_pixels_hash) {
        dm2_v1_asset_free_pixels(source_pixels);
        return 0;
    }
    for (uint16_t y = 0u; y < blit->height; ++y) {
