...
int dm2_v1_gdat_wall_tile_receipt_build(uint8_t cell,const DM2_V1_GdatWallM11CommandPlan*w,const DM2_V1_Dm2ViewportM11CompositionReceipt*c,DM2_V1_GdatWallTileReceipt*out){uint32_t h=2166136261u;if(!out)return 0;memset(out,0,sizeof(*out));/* c_gui_vp.cpp:6703-6741; dm2data.cpp:602-605,266-273. */if(!w||!c||cell>=23||!w->valid||!w->command_count||!w->command_hash||!c->valid||!c->no_draw||c->wall_command_hash!=w->command_hash||!c->identity_hash)return 0;out->valid=out->no_draw=1;out->cell=cell;out->table_branch=branch[cell];out->delegate_count=branch[cell]==3?2:(branch[cell]?1:0);out->wall_orientation=orientation[cell];out->wall_hash=w->command_hash;out->composition_hash=c->identity_hash;h=mix(h,cell);h=mix(h,out->table_branch);h=mix(h,(uint8_t)out->wall_orientation);h=mix(h,out->wall_hash);h=mix(h,out->composition_hash);out->identity_hash=h?h:1;return 1;}
