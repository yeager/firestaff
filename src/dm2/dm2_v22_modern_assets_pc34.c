/*
 * dm2_v22_modern_assets_pc34.c — DM2 V2.2 Modern Graphics Fallback Pipeline
 *
...
static int dm2_v22_file_exists(const char* path) {
...
/* Trimming helpers */
static void dm2_v22_trim(char* dst, const char* src, size_t dstSize) {
...
static int dm2_v22_read_line(FILE* fp, char* out, size_t outSize) {
    if (!fp || !out || outSize == 0U) return 0;
    out[0] = '\0';
    if (!fgets(out, (int)outSize, fp)) return 0;
    dm2_v22_trim(out, out, outSize);
    return 1;
}

/* Extract a string value for a key from a JSON line of the form: "key": "value"
 * Returns 1 on success, 0 if not found. */
static int dm2_v22_extract_string(const char* line, const char* key, char* out, size_t outSize) {
 ...
 * Returns 1 on success, 0 if not found. */
static int dm2_v22_extract_int(const char* line, const char* key, int* out) {
...
static void __attribute__((unused)) dm2_v22_skip_to_next_object (FILE* fp) {
    int depth = 0;
    char line[256];
...
void dm2_v22_set_manifest_path(const char* dataDir) {
    ...
    /* Build: ~/.firestaff/assets/dm2/modern/modern_asset_manifest.json
     * Walk up two levels from dataDir (e.g. data/dm2 -> data -> ~/.firestaff)
     * then append assets/dm2/modern/. */
    ...
    FSP_JoinPath(modernDir, sizeof(modernDir), assetsRoot, "dm2");
...
int dm2_v22_validate_manifest(const char* manifest_path) {
    ...
    while (dm2_v22_read_line(fp, line, sizeof(line))) {
        ...
        if (strchr(line, '[') == NULL && strchr(line, '{') == NULL) {
            ...
            if (!dm2_v22_read_line(fp, line, sizeof(line))) break;
        }
        ...
        for (;;) {
            ...
            if (strchr(line, '{') != NULL || strchr(line, '"') != NULL) {
                ...
                int has_id = dm2_v22_extract_string(line, "id", id_val, sizeof(id_val));
                int has_file = dm2_v22_extract_string(line, "source_file", file_val, sizeof(file_val));
                int has_width = dm2_v22_extract_int(line, "width", &width_val);
                int has_height = dm2_v22_extract_int(line, "height", &height_val);
...
/* dm2_v22_modern_assets_available — checks if the modern asset pack
 * is installed and at least critical shape categories are present.
 *
 * Checks:
 *   - ~/.firestaff/assets/dm2/modern/modern_asset_manifest.json exists
...
int dm2_v22_modern_assets_available(void) {
    ...
    while (dm2_v22_read_line(fp, line, sizeof(line))) {
        ...
        for (int ci = 0; ci < 3; ++ci) {
            ...
            if (dm2_v22_extract_string(line, "id", id_val, sizeof(id_val))) {
                found_critical[ci] = 1;
                current_cat = -1;
            } else {
...
/* dm2_v22_set_epx_cache_warm / dm2_v22_get_epx_cache_warm — EPX cache
...
/* dm2_v22_best_available_shape_source — returns the best available
...
DM2_V22_ShapeSource dm2_v22_best_available_shape_source(int presentation_mode_index) {
    switch (presentation_mode_index) {
        ...
        case 2: /* M12_PRESENTATION_V21_UPSCALED */
            if (g_epx_cache_warm) {
                return DM2_V22_SHAPE_SOURCE_V2_UPSCALED;
            }
...
int dm2_v22_get_shape_path(const char* category, const char* asset_id,
    ...
    while (dm2_v22_read_line(fp, line, sizeof(line))) {
        ...
        char val[256];
        if (dm2_v22_extract_string(line, "id", val, sizeof(val))) {
            dm2_v22_trim(resolved_id, val, sizeof(resolved_id));
        }
        if (dm2_v22_extract_string(line, "source_file", val, sizeof(val))) {
            dm2_v22_trim(resolved_file, val, sizeof(resolved_file));
        }
    }
    ...
    return dm2_v22_file_exists(out_path) ? 1 : 0;
}
...
const char* dm2_v22_source_evidence(void) {
    return "SKULL.ASM T400 (DM2 dungeon viewport); "
           "SKULL.ASM T520 (DM2 champion panel); "
           "SKULL.ASM T600 (DM2 outdoor viewport); "
           "SKULL.ASM torchlight routine; "
