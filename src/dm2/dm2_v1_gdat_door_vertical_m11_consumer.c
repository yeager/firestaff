...
static int matches(const DM2_V1_GdatDoorVerticalM11Receipt *receipt,
                   ...
                   DM2_V1_ViewportSurfaceSnapshot *out_surface)
{
    ...
    /* DRAW_DOOR m_439C4: vertical intermediate states select base + state,
     * retain the full source bitmap, then issue one DRAW_TEMP_PICST. */
    if (command->kind != DM2_V1_GDAT_DOOR_PANEL ||
        command->category != DM2_GDAT_CATEGORY_DOORS || command->door_state < 1u ||
        command->door_state > 3u || command->door_opening_dir != 1u ||
        command->door_state != receipt->state ||
        command->door_open_pct != receipt->open_pct || command->movement_active ||
        command->mirror_flip || command->palette_darkness ||
        command->palette_light_receipt_hash || command->palette_transform_hash ||
        ...
        command->color_key != receipt->color_key || command->raw_hash != receipt->raw_hash ||
    ...
    DM2_V1_GdatDoorVerticalM11Receipt *out_receipt)
{
    ...
    receipt.valid = 1;
    receipt.command_index = command_index;
    receipt.state = command->door_state;
    receipt.open_pct = command->door_open_pct;
    receipt.color_key = (uint8_t)command->color_key;
    receipt.rect_number = command->rect_number;
    receipt.destination_x = command->rect_x;
    receipt.destination_y = command->rect_y;
    receipt.width = command->width;
    receipt.height = command->height;
    receipt.door_hash = plan->command_hash;
    receipt.raw_hash = command->raw_hash;
    receipt.decoded_hash = command->decoded_hash;
    receipt.palette_hash = command->palette_hash;
    receipt.geometry_hash = command->geometry_hash;
