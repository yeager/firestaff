...
static uint32_t dm2_door_material_hash_step(uint32_t hash, uint32_t value)
{
    hash ^= value;
    ...
    DM2_V1_DoorGdatMaterialReceipt *out_receipt)
{
    ...
    /* skproject SKWIN/SkWinCore.cpp DRAW_DOOR 46405-46457: the map's
     * door-GDAT index and source-selected image field are inputs. A zero
     * GDAT_IMG_COLORKEY_1 bypasses the source draw branch, so it cannot be
     * promoted into a host fallback. */
    if (!loader ||
        !dm2_v1_asset_load_word_value(loader, DM2_GDAT_CATEGORY_DOORS,
                                      door_gfx_index,
                                      DM2_V1_DOOR_GDAT_COLORKEY_FIELD,
                                      &out_receipt->color_key) ||
        out_receipt->color_key == 0u ||
    ...
    for (size_t i = 0u; i < pixel_count; ++i) {
        hash = dm2_door_material_hash_step(hash, pixels[i]);
    }
    dm2_v1_asset_free_pixels(pixels);
    if (hash == 0u) return 0;
    out_receipt->door_gfx_index = door_gfx_index;
    ...
    hash = dm2_door_material_hash_step(hash,
                                       out_receipt->image_metadata.metadata_hash);
    hash = dm2_door_material_hash_step(hash, out_receipt->local_palette_hash);
    hash = dm2_door_material_hash_step(hash, out_receipt->color_key);
    if (hash == 0u) return 0;
