...
static const M12_RequiredFileSpec g_requiredFiles[] = {
    ...
    {"dm1", "dungeon", "DUNGEON.DAT", "766450c940651fc021c92fe5d0d0b3a6;050fb2cfded1b502ec2c53956b94c5bd;30028fb6a301ecb20127ef0b3af32b05;3dc0a932d0e0adfe59878f07c51700c5;dfd82fb7d44e4b5cc81add257655c966;fe098f70ce83cfe3f2333565093daf35", 0},
 ...
 * original image in memory, then leave the archive untouched for the DM2
 * boot owner to reopen.  This is discovery, never extraction to disk. */
...
/* The DM1 FM Towns release is a raw MODE1/2048 BIN inside a ZIP.  The
 * language payloads are ISO members, not ZIP members, so filename/hash
 * discovery alone cannot admit them.  Verify both the real GRAPHICS.DAT and
 * DUNGEON.DAT before exposing either language to the launcher. */
static int m12_admit_dm1_fmtowns_archive(
    M12_AssetStatus* status, int gameIndex,
    const char roots[M12_SEARCH_ROOT_COUNT][M12_ASSET_DATA_DIR_CAPACITY],
    size_t rootCount, const char* preferredArchive) {
    static const char archiveName[] = "Dungeon-Master_FM-Towns_JA-EN.zip";
    static const char* const graphicsMd5[] = {
        "c10c512f63461ebe79b5ac365115b61b",
        "edf47d7da5de8184604d6d80477ef01f"
    };
    static const char* const dungeonMd5[] = {
    ...
    for (rootIndex = 0U; rootIndex < rootCount; ++rootIndex) {
        ...
        snprintf(candidate[candidateCount++], sizeof(candidate[0]), "%s/%s",
                 roots[rootIndex], archiveName);
        snprintf(candidate[candidateCount++], sizeof(candidate[0]), "%s/dm1/%s",
                 roots[rootIndex], archiveName);
        for (candidateIndex = 0U; candidateIndex < candidateCount;
            ...
            for (language = 0; language < 2; ++language) {
                ...
                const FmtownsIsoEntry* graphics =
                    fmtowns_disc_find(&disc, language == 0
                                      ? "DATA/GRAPHICS.DAT"
                                      : "JDATA/GRAPHICS.DAT");
                const FmtownsIsoEntry* dungeon =
                    fmtowns_disc_find(&disc, language == 0
                                      ? "DATA/DUNGEON.DAT"
                                      : "JDATA/DUNGEON.DAT");
                ...
                char graphicsHash[33], dungeonHash[33];
                (void)directory;
                if (!graphics || !dungeon ||
                    fmtowns_disc_extract_alloc(image, imageSize,
                                               FMTOWNS_SECTOR_2048, graphics,
                                               &graphicsBytes, &graphicsSize) != 0 ||
                    fmtowns_disc_extract_alloc(image, imageSize,
                                               FMTOWNS_SECTOR_2048, dungeon,
                                               &dungeonBytes, &dungeonSize) != 0) {
                    free(graphicsBytes);
                    free(dungeonBytes);
                    continue;
                }
                m12_bytes_md5_hex(graphicsBytes, graphicsSize, graphicsHash);
                m12_bytes_md5_hex(dungeonBytes, dungeonSize, dungeonHash);
                accepted = strcmp(graphicsHash, graphicsMd5[language]) == 0 &&
                           strcmp(dungeonHash, dungeonMd5[language]) == 0;
                free(graphicsBytes);
                free(dungeonBytes);
                ...
                anyAccepted = 1;
                {
                    const char* versionId = language == 0 ? "fmtowns-en" : "fmtowns-ja";
                    size_t versionIndex;
                    for (versionIndex = 0U;
                         versionIndex < g_games[gameIndex].versionCount;
                         ++versionIndex) {
                        ...
                        version->matched = 1;
                        snprintf(version->matchedPath, sizeof(version->matchedPath),
                                 "%s::%s/GRAPHICS.DAT", candidate[candidateIndex],
                                 language == 0 ? "DATA" : "JDATA");
                        snprintf(version->matchedMd5, sizeof(version->matchedMd5),
                                 "%s", graphicsHash);
                        break;
                    }
...
static void m12_publish_dm1_fmtowns_required_files(M12_AssetStatus* status,
    ...
    size_t i, archiveLength;
    const char* directory;
    const char* dungeonHash;
    if (!status || gameIndex < 0 || gameIndex >= M12_ASSET_GAME_COUNT ||
        strcmp(g_games[gameIndex].gameId, "dm1") != 0) return;
    ...
    directory = strcmp(version->versionId, "fmtowns-en") == 0 ? "DATA" : "JDATA";
    dungeonHash = strcmp(version->versionId, "fmtowns-en") == 0
        ? "3dc0a932d0e0adfe59878f07c51700c5"
        : "fe098f70ce83cfe3f2333565093daf35";
    for (i = 0U; i < status->requiredFileCounts[gameIndex]; ++i) {
        ...
        snprintf(required->matchedPath, sizeof(required->matchedPath),
                 "%.*s::%s/%s", (int)archiveLength, version->matchedPath,
                 directory, member);
...
static void m12_publish_dm2_fmtowns_required_files(M12_AssetStatus* status,
    ...
    for (i = 0U; i < status->requiredFileCounts[gameIndex]; ++i) {
        ...
        if (strcmp(required->roleId, "graphics") == 0) {
            ...
            md5 = "027ff3b8ddc2c4c4cdda7ada0b0bc46c";
        } else if (strcmp(required->roleId, "dungeon") == 0) {
            member = "DATA/DUNGEON.DAT";
            md5 = "74c7549f174574201988bf936385841a";
        }
        if (!member) {
            continue;
        }
...
/* A user can retain the original F31 CD as an extracted ISO tree rather than
 * its ZIP/RAR container.  The generic hash walker sees the individual files,
 * but it cannot prove that CDATA and CJDATA share the F31 disc root and its
 * language-owned executables.  Bind that relation here before the generic
 * pass clears version rows, using the same graphics/dungeon hashes as the
 * archive admission path. */
static int m12_admit_csb_fmtowns_loose_tree(
    M12_AssetStatus* status, int gameIndex,
    const char roots[M12_SEARCH_ROOT_COUNT][M12_ASSET_DATA_DIR_CAPACITY],
    size_t rootCount) {
    static const char* const directories[] = {"CDATA", "CJDATA"};
    static const char* const versionIds[] = {"fmtowns-en", "fmtowns-ja"};
    static const char* const graphicsMd5s[] = {
        "405b757038eea3c263e60f240854d6de",
        "761d6fc588b31aeaaa9caf3725e111b9"
    };
    static const char* const dungeonMd5s[] = {
        "83c56cf1b779e7460a55c9299ebeb04b",
        "7ca51c17ef8bd542ca5f0273672ec1a5"
    };
    ...
    for (rootIndex = 0U; rootIndex < rootCount; ++rootIndex) {
        ...
        m12_copy_string(candidates[candidateCount++], sizeof(candidates[0]),
                        roots[rootIndex]);
        if (FSP_JoinPath(candidates[candidateCount], sizeof(candidates[0]),
                         roots[rootIndex], "csb") &&
            FSP_DirExists(candidates[candidateCount])) {
            ++candidateCount;
        }
        if (FSP_JoinPath(candidates[candidateCount], sizeof(candidates[0]),
                         roots[rootIndex], "csb/fmtowns_iso") &&
            FSP_DirExists(candidates[candidateCount])) {
            ++candidateCount;
        }
        for (candidateIndex = 0U; candidateIndex < candidateCount;
             ++candidateIndex) {
            size_t languageIndex;
            int allLanguages = 1;
            for (languageIndex = 0U; languageIndex < 2U; ++languageIndex) {
                ...
                for (versionIndex = 0U;
                    ...
                    M12_AssetVersionStatus* version =
                        &status->versions[gameIndex][versionIndex];
                    if (strcmp(version->versionId, versionIds[languageIndex]) != 0) {
                        continue;
                    }
                    version->matched = 1;
                    m12_copy_string(version->matchedPath,
                                    sizeof(version->matchedPath), graphicsPath);
                    m12_copy_string(version->matchedMd5,
                                    sizeof(version->matchedMd5), graphicsMd5);
                    break;
                }
            }
            if (allLanguages) {
...
static void m12_publish_csb_fmtowns_required_files(M12_AssetStatus* status,
    ...
    const char* directory;
    const char* dungeonMd5;
    char looseDungeonMd5[M12_ASSET_MD5_CAPACITY];
    size_t i;
    if (!status || gameIndex < 0 || gameIndex >= M12_ASSET_GAME_COUNT ||
    ...
    if (!separator) {
        ...
        if (!FSP_ParentDir(dataDirectory, sizeof(dataDirectory),
                           version->matchedPath) ||
            !FSP_JoinPath(dungeonPath, sizeof(dungeonPath), dataDirectory,
                          "DUNGEON.DAT") ||
            !m12_file_md5_hex(dungeonPath, looseDungeonMd5)) {
            return;
        }
        for (i = 0U; i < status->requiredFileCounts[gameIndex]; ++i) {
            M12_AssetRequiredFileStatus* required =
            ...
            if (strcmp(required->roleId, "graphics") == 0) {
                ...
                m12_copy_string(required->matchedHash,
                       ...
                       strcmp(looseDungeonMd5, strcmp(version->versionId,
                           "fmtowns-en") == 0
                           ? "83c56cf1b779e7460a55c9299ebeb04b"
                           : "7ca51c17ef8bd542ca5f0273672ec1a5") == 0) {
                required->matched = 1;
                m12_copy_string(required->matchedPath,
                                sizeof(required->matchedPath), dungeonPath);
                m12_copy_string(required->matchedHash,
                                sizeof(required->matchedHash), looseDungeonMd5);
            }
        }
        return;
    }
    if ((size_t)(separator - version->matchedPath) >= sizeof(archivePath)) return;
...
/* A manually extracted F31 CD still contains both CDATA and CJDATA below a
 * common root.  Returning that root leaks the language selection back to M11:
 * the first executable discovered is F31E, even when the caller explicitly
 * selected F31J.  Flatten only the authenticated language-owned DAT pair into
 * a private cache, while copying the retail programs and presentation owners
 * that are shared by the CD.  This is an exact file copy from original media,
 * never a replacement asset or a generated package. */
static int m12_materialize_csb_fmtowns_loose_runtime_cache(
    const M12_AssetVersionStatus* version, const char* gameCacheDir) {
    static const char* const rootFiles[] = {
        "AUTOEXEC.BAT", "RUN386.EXE", "DICUTY.COM", "ENDING.ANM",
        "CHTWJ.EXP", "OAK2USR.DIC", "UTILE.EXP", "TBIOS.BIN",
        "SWITCHTW.EXP", "T_OAK2.EXE", "DRIVE_R.IMG", "UTILJ.EXP",
        "CONFIG.SYS", "IO.SYS", "CHTWE.EXP", "ANIMTW.EXP",
        "TITLE.ANM", "TMENU.EXP", "TBIOS.SYS", "CONTROL.EXE",
        "TMENU.ICN", "STORY.ANM", NULL
    };
    static const char* const portraitFiles[] = {
        ...
        "MOPHUS.CMP", "ALEX.CMP", "HAWK.CMP", "LINFLAS.CMP", NULL
    };
    char dataDirectory[M12_ASSET_DATA_DIR_CAPACITY];
    ...
    if (!version || !version->versionId || !gameCacheDir ||
        (strcmp(version->versionId, "fmtowns-en") != 0 &&
         strcmp(version->versionId, "fmtowns-ja") != 0) ||
        m12_path_is_virtual_asset(version->matchedPath) ||
        !FSP_ParentDir(dataDirectory, sizeof(dataDirectory),
                       version->matchedPath) ||
    ...
    if (!FSP_JoinPath(destinationPath, sizeof(destinationPath), gameCacheDir,
                      "GRAPHICS.DAT") ||
        !m12_copy_file_to_path(version->matchedPath, destinationPath) ||
        !m12_file_md5_hex(destinationPath, md5) ||
        strcmp(md5, version->matchedMd5) != 0 ||
        !FSP_JoinPath(sourcePath, sizeof(sourcePath), dataDirectory,
                      "DUNGEON.DAT") ||
        !FSP_JoinPath(destinationPath, sizeof(destinationPath), gameCacheDir,
                      "DUNGEON.DAT") ||
    ...
    for (i = 0U; rootFiles[i] != NULL; ++i) {
        if (!FSP_JoinPath(sourcePath, sizeof(sourcePath), discRoot,
                          rootFiles[i]) ||
            !FSP_JoinPath(destinationPath, sizeof(destinationPath), gameCacheDir,
                          rootFiles[i]) ||
            !FSP_FileExists(sourcePath) ||
            !m12_copy_file_to_path(sourcePath, destinationPath)) {
            return 0;
        }
    }
    if (!FSP_JoinPath(portraitDirectory, sizeof(portraitDirectory), discRoot,
                      "PORTRAIT") ||
        !FSP_JoinPath(destinationPortraitDirectory,
                      sizeof(destinationPortraitDirectory), gameCacheDir,
                      "PORTRAIT") ||
        !FSP_CreateDirectoryRecursive(destinationPortraitDirectory)) {
        return 0;
    }
    for (i = 0U; portraitFiles[i] != NULL; ++i) {
        if (!FSP_JoinPath(sourcePath, sizeof(sourcePath), portraitDirectory,
                          portraitFiles[i]) ||
            !FSP_JoinPath(destinationPath, sizeof(destinationPath),
                          destinationPortraitDirectory, portraitFiles[i]) ||
            !FSP_FileExists(sourcePath) ||
            !m12_copy_file_to_path(sourcePath, destinationPath)) {
            return 0;
        }
...
static int m12_write_bytes(const char* path, const uint8_t* bytes, size_t size) {
    FILE* file;
...
static int m12_materialize_dm1_fmtowns_runtime_cache(
    const M12_AssetVersionStatus* version, const char* gameCacheDir) {
    const char* separator;
    char archivePath[M12_ASSET_DATA_DIR_CAPACITY];
    uint8_t* image = NULL;
    ...
    int i;
    if (!version || !version->versionId || !gameCacheDir) return 0;
    if (strcmp(version->versionId, "fmtowns-en") != 0 &&
        strcmp(version->versionId, "fmtowns-ja") != 0) return 0;
    separator = strstr(version->matchedPath, "::");
    if (!separator || (size_t)(separator - version->matchedPath) >= sizeof(archivePath))
        return 0;
    memcpy(archivePath, version->matchedPath,
           (size_t)(separator - version->matchedPath));
    archivePath[separator - version->matchedPath] = '\0';
    if (firestaff_zip_extract_by_suffix(archivePath, ".bin", &image, &imageSize) != 0 ||
        fmtowns_disc_probe(image, imageSize, FMTOWNS_SECTOR_2048, &disc) != 0) {
        free(image);
        return 0;
    }
    selectedDirectory = strcmp(version->versionId, "fmtowns-en") == 0 ? "DATA" : "JDATA";
    expectedGraphics = strcmp(version->versionId, "fmtowns-en") == 0
        ? "c10c512f63461ebe79b5ac365115b61b"
        : "edf47d7da5de8184604d6d80477ef01f";
    expectedDungeon = strcmp(version->versionId, "fmtowns-en") == 0
        ? "3dc0a932d0e0adfe59878f07c51700c5"
        : "fe098f70ce83cfe3f2333565093daf35";
    for (i = 0; i < disc.entry_count; ++i) {
        const FmtownsIsoEntry* entry = &disc.entries[i];
        const char* relative = entry->name;
        uint8_t* bytes = NULL;
        size_t size = 0U;
        char outPath[M12_ASSET_DATA_DIR_CAPACITY];
        char parent[M12_ASSET_DATA_DIR_CAPACITY];
        if (entry->is_directory || !entry->name[0]) continue;
        if (strncmp(relative, "DATA/", 5U) == 0 ||
            strncmp(relative, "JDATA/", 6U) == 0) {
            if (strncmp(relative, selectedDirectory, strlen(selectedDirectory)) != 0)
                continue;
        } else if (strchr(relative, '/') != NULL) {
            continue;
        }
        if (fmtowns_disc_extract_alloc(image, imageSize, FMTOWNS_SECTOR_2048,
                                       entry, &bytes, &size) != 0) continue;
        if (strncmp(relative, selectedDirectory, strlen(selectedDirectory)) == 0 &&
            relative[strlen(selectedDirectory)] == '/') {
            relative += strlen(selectedDirectory) + 1U;
        }
        if (!FSP_JoinPath(outPath, sizeof(outPath), gameCacheDir, relative)) {
            free(bytes);
            free(image);
            return 0;
        }
        if (strrchr(relative, '/') != NULL) {
            size_t prefix = (size_t)(strrchr(outPath, '/') - outPath);
            if (prefix >= sizeof(parent)) prefix = sizeof(parent) - 1U;
            memcpy(parent, outPath, prefix);
            parent[prefix] = '\0';
            if (!FSP_CreateDirectoryRecursive(parent)) {
                free(bytes);
                free(image);
                return 0;
            }
        }
        if (!m12_write_bytes(outPath, bytes, size)) {
            free(bytes);
            free(image);
            return 0;
        }
        if (strcmp(entry->name, "DATA/GRAPHICS.DAT") == 0 ||
            strcmp(entry->name, "JDATA/GRAPHICS.DAT") == 0) {
            char hash[33];
            m12_bytes_md5_hex(bytes, size, hash);
            if (strcmp(hash, expectedGraphics) != 0) {
                free(bytes); free(image); return 0;
            }
        }
        if (strcmp(entry->name, "DATA/DUNGEON.DAT") == 0 ||
            strcmp(entry->name, "JDATA/DUNGEON.DAT") == 0) {
            char hash[33];
            m12_bytes_md5_hex(bytes, size, hash);
            if (strcmp(hash, expectedDungeon) != 0) {
                free(bytes); free(image); return 0;
            }
        }
        free(bytes);
...
static int m12_materialize_runtime_cache_for_game(M12_AssetStatus* status,
    ...
    }
    {
        ...
        if (strcmp(gameId, "csb") == 0 && selectedVersion &&
             ...
             * disc root.  Materialize one selected language package instead
             * of returning that shared root: otherwise F31E is rediscovered
             * ahead of a selected F31J. */
            if (m12_materialize_csb_fmtowns_loose_runtime_cache(
...
int M12_AssetStatus_PrepareDM1RuntimeVersion(
    ...
    version = &status->versions[gameIndex][versionIndex];
    if (!version->matched || !version->matchedPath[0]) return 0;
    if (strcmp(versionId, "fmtowns-en") == 0 ||
        strcmp(versionId, "fmtowns-ja") == 0) {
        return M12_AssetStatus_MaterializeDM1FmtownsRuntimeVersion(
            status, versionId, outPath, outPathSize);
    }
    if ((M12_AssetStatus_GetVersionArchitecture("dm1", (size_t)versionIndex) ==
             M12_ARCH_ATARI_ST ||
         M12_AssetStatus_GetVersionArchitecture("dm1", (size_t)versionIndex) ==
             M12_ARCH_AMIGA) && m12_path_is_virtual_asset(version->matchedPath)) {
