...
int firestaff_7z_extract_single_lzma2_file(const char *path,uint8_t **out,size_t *out_size,char *name,size_t name_size) {
 ...
 if(!fs7z_byte(&r,&marker)||marker!=1||!fs7z_byte(&r,&marker)||marker!=4||!fs7z_pack(&r,&s)||!fs7z_unpack(&r,&s)||!fs7z_substreams(&r,&s)||!fs7z_byte(&r,&marker)||marker!=0||!fs7z_files(&r,name,name_size)||s.packed>off){free(a);return 0;}
