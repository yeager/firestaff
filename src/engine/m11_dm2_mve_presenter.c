                                       ...
                                       int failure_code)
{
    if (presenter) {
        presenter->failed = 1;
        presenter->failure_code = failure_code;
    }
}
    ...
    const DM2_V1_MveDisplayAudioBoundary *boundary)
{
    ...
    if (!presenter || !boundary || !boundary->valid) {
        m11_dm2_mve_presenter_fail(presenter, M11_DM2_MVE_FAILURE_BOUNDARY);
        return 0;
    }
    for (packet = 0u; packet < boundary->audio_packet_count; ++packet) {
        ...
        if (next != 1 || !pcm.valid) {
            m11_dm2_mve_presenter_fail(presenter,
                                       M11_DM2_MVE_FAILURE_PCM_SOURCE);
            return 0;
        }
        if (pcm.source_sequence !=
            (uint16_t)(boundary->first_audio_source_sequence + packet)) {
            m11_dm2_mve_presenter_fail(presenter,
                                       M11_DM2_MVE_FAILURE_PCM_SEQUENCE);
            return 0;
        }
        if (!dm2_v1_mve_audio_sdl_owner_queue(&presenter->audio, &pcm)) {
            m11_dm2_mve_presenter_fail(presenter,
                                       M11_DM2_MVE_FAILURE_AUDIO_SINK);
            return 0;
        }
    }
    return 1;
                                  ...
                                  uint64_t monotonic_now_us)
{
    ...
    if (monotonic_now_us < presenter->last_host_time_us ||
        ...
        m11_dm2_mve_presenter_fail(presenter, M11_DM2_MVE_FAILURE_CLOCK_OR_RANGE);
        return -1;
    }
    ...
    if (next != 1 || !frame.valid ||
        ...
        m11_dm2_mve_presenter_fail(
            presenter, next != 1 || !frame.valid ? M11_DM2_MVE_FAILURE_VIDEO_SOURCE
                                                  : M11_DM2_MVE_FAILURE_PRESENT_CALLBACK);
        return -1;
    }
