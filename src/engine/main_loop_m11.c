...
static int m11_map_window_pointer_to_game_source(
    ...
    if (!gameView || !outSourceX || !outSourceY ||
        !M11_Render_MapWindowToFramebuffer(windowX,
                                            windowY,
                                            &presentedX,
                                            &presentedY)) {
        return 0;
    }
    m11_map_presented_game_point_to_source(gameView, &presentedX, &presentedY);
    *outSourceX = presentedX;
    *outSourceY = presentedY;
    return 1;
}
...
static int m11_play_redmcsb_title_graphic_intro_if_available(
    ...
    for (sourceStep = 1U; sourceStep <= V1_TitleFrontend_GetSourceAnimationStepCount(); ++sourceStep) {
        ...
        /* ReDMCSB TITLE.C F0437:362-367 clears the title surface, fades
         * the curtain to black, and installs C13_DUNGEON followed by
         * C14_MASTER before the first zoom-loop M526_WaitVerticalBlank()
         * at line 386.  The frontend has always exposed that distinct
         * pre-VBlank phase in the C001 plan; consume it here instead of
         * leaving C12_PRESENTS active through the first zoom wait. */
        if (blitPlan.clearBeforeVblank) {
            if (M11_Render_PresentIndexedWithSpecialPalette(
                    framebuffer,
                    M11_FB_WIDTH,
                    M11_FB_HEIGHT,
                    blitPlan.specialPaletteBeforeVblank) != M11_RENDER_OK) {
                break;
            }
        }
...
int M11_PhaseA_Run(const M11_PhaseA_Options* opts) {
    ...
    if (gameView.active) {
        M11_Render_Present();
        m11_record_csb_presented_frame(&gameView);
