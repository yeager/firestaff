/*
 * theron_v22_modern_assets_pc34.c — Theron V2.2 Modern Graphics Fallback Pipeline
 *
 ...
 * THERON_V22_SHAPE_MISSING_PLACEHOLDER rather than crashing.
 */
 ...
 * ~/.firestaff/assets/theron/modern/ location. */
static char g_v22_manifest_path[FSP_PATH_MAX];

/* ── Retired missing-asset pixels ───────────────────────────────── */

/* THERON_V22_SHAPE_MISSING_PLACEHOLDER — returned by shape-fetching
...
#if 0 /* Retained only in history; missing assets now fail closed. */
static const uint32_t THERON_V22_SHAPE_MISSING_PLACEHOLDER_DATA[16 * 16] = {
...
/* ── Shape source enum is defined in theron_v22_modern_assets_pc34.h
...
static int theron_v22_file_exists(const char* path) {
...
/* Trimming helpers */
static void theron_v22_trim(char* dst, const char* src, size_t dstSize) {
...
static int theron_v22_read_line(FILE* fp, char* out, size_t outSize) {
    if (!fp || !out || outSize == 0U) return 0;
    out[0] = '\0';
    if (!fgets(out, (int)outSize, fp)) return 0;
    theron_v22_trim(out, out, outSize);
    return 1;
}

/* Extract a string value for a key from a JSON line of the form: "key": "value"
 * Returns 1 on success, 0 if not found. */
static int theron_v22_extract_string(const char* line, const char* key, char* out, size_t outSize) {
 ...
 * Returns 1 on success, 0 if not found. */
static int theron_v22_extract_int(const char* line, const char* key, int* out) {
...
static void __attribute__((unused)) theron_v22_skip_to_next_object (FILE* fp) {
    int depth = 0;
    char line[256];
...
void theron_v22_set_manifest_path(const char* dataDir) {
    ...
    /* Build: ~/.firestaff/assets/theron/modern/modern_asset_manifest.json
     * Walk up two levels from dataDir (e.g. data/theron -> data -> ~/.firestaff)
     * then append assets/theron/modern/. */
    ...
    FSP_JoinPath(modernDir, sizeof(modernDir), assetsRoot, "theron");
...
int theron_v22_validate_manifest(const char* manifest_path) {
    ...
    while (theron_v22_read_line(fp, line, sizeof(line))) {
        ...
        if (strchr(line, '[') == NULL && strchr(line, '{') == NULL) {
            ...
            if (!theron_v22_read_line(fp, line, sizeof(line))) break;
        }
        ...
        for (;;) {
            ...
            if (strchr(line, '{') != NULL || strchr(line, '"') != NULL) {
                ...
                int has_id = theron_v22_extract_string(line, "id", id_val, sizeof(id_val));
                int has_file = theron_v22_extract_string(line, "source_file", file_val, sizeof(file_val));
                int has_width = theron_v22_extract_int(line, "width", &width_val);
                int has_height = theron_v22_extract_int(line, "height", &height_val);
...
/* theron_v22_modern_assets_available — checks if the modern asset pack
 * is installed and at least critical shape categories are present.
 *
 * Checks:
 *   - ~/.firestaff/assets/theron/modern/modern_asset_manifest.json exists
...
int theron_v22_modern_assets_available(void) {
    ...
    while (theron_v22_read_line(fp, line, sizeof(line))) {
        ...
        for (int ci = 0; ci < 3; ++ci) {
            ...
            if (theron_v22_extract_string(line, "id", id_val, sizeof(id_val))) {
                found_critical[ci] = 1;
                current_cat = -1;
            } else {
...
/* theron_v22_set_installed — called at startup by M12_AssetStatus_Scan()
...
/* theron_v22_get_installed — returns whether V2.2 modern assets are installed */
int theron_v22_get_installed(void) {
    return g_v22_modern_assets_installed;
}

/* theron_v22_set_epx_cache_warm / theron_v22_get_epx_cache_warm — EPX cache
...
/* theron_v22_best_available_shape_source — returns the best available
    ...
    switch (presentation_mode_index) {
        case 3: /* M12_PRESENTATION_V22_MODERN */
            if (g_v22_modern_assets_installed) {
                return THERON_V22_SHAPE_SOURCE_V2_MODERN;
            }
        ...
        case 0:
        default:
            return THERON_V22_SHAPE_SOURCE_V1_ORIGINAL;
    }
}

/* Missing modern assets fail closed.  A real manifest entry must supply the
 * surface; this API never manufactures a checkerboard replacement. */
...
/* theron_v22_get_shape_path — given a category and asset id from the
...
int theron_v22_get_shape_path(const char* category, const char* asset_id,
    ...
    while (theron_v22_read_line(fp, line, sizeof(line))) {
        ...
        char val[256];
        if (theron_v22_extract_string(line, "id", val, sizeof(val))) {
            theron_v22_trim(resolved_id, val, sizeof(resolved_id));
        }
        if (theron_v22_extract_string(line, "source_file", val, sizeof(val))) {
            theron_v22_trim(resolved_file, val, sizeof(resolved_file));
        }
    }
    ...
    return theron_v22_file_exists(out_path) ? 1 : 0;
}
...
const char* theron_v22_shape_source_name(THERON_V22_ShapeSource src) {
    switch (src) {
        case THERON_V22_SHAPE_SOURCE_V1_ORIGINAL: return "V1_ORIGINAL";
        case THERON_V22_SHAPE_SOURCE_V2_FILTERED: return "V2_FILTERED";
        case THERON_V22_SHAPE_SOURCE_V2_UPSCALED: return "V2_UPSCALED";
        case THERON_V22_SHAPE_SOURCE_V2_MODERN:   return "V2_MODERN";
...
const char* theron_v22_source_evidence(void) {
    return "THQUEST.ASM T400 (Theron dungeon viewport); "
           "THQUEST.ASM T520 (Theron champion panel); "
           "THQUEST.ASM T600 (Theron outdoor viewport); "
           "THQUEST.ASM torchlight routine; "
