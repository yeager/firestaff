/*
 * theron_v2_hud_widget_assets_pc34.c — Theron V2 HUD Widget Asset Manifest + Gate
 *
 ...
 * sibling theron_v22_modern_assets_pc34 shape
 * (id + source_file + width + height), with one extra field:
 *
...
typedef struct {
    ...
    const char*            id;
    const char*            category;
} Theron_V2_HudWidgetSlotDesc;

static const Theron_V2_HudWidgetSlotDesc k_slot_table[THERON_V2_HUD_WIDGET_COUNT] = {
    { THERON_V2_HUD_WIDGET_COMPASS_ROSE,     "compass_rose",     "hud_widgets" },
    { THERON_V2_HUD_WIDGET_QUEST_ITEMS,      "quest_items",      "hud_widgets" },
    { THERON_V2_HUD_WIDGET_DUNGEON_PROGRESS, "dungeon_progress", "hud_widgets" },
    { THERON_V2_HUD_WIDGET_RELIC_COUNTER,    "relic_counter",    "hud_widgets" },
    { THERON_V2_HUD_WIDGET_RUNE_INDICATOR,   "rune_indicator",   "hud_widgets" },
    { THERON_V2_HUD_WIDGET_CHAMPION_BARS,    "champion_bars",    "hud_chrome"  },
    { THERON_V2_HUD_WIDGET_ACTION_STRIP,     "action_strip",     "hud_chrome"  }
};

/* ── Module state ──────────────────────────────────────────────── */
static char     g_manifest_path[FSP_PATH_MAX] = {0};
static int      g_installed = 0;  /* last gate classification: 1 if PARTIAL/COMPLETE */
static Theron_V2_HudWidgetGate g_last_gate = THERON_V2_HUD_WIDGET_GATE_NOT_PROBED;

/* ── Trimming / JSON helpers (parallels theron_v22_modern_assets_pc34) ── */
static void theron_v2_hwa_trim(char* dst, const char* src, size_t dstSize) {
...
static int theron_v2_hwa_file_exists(const char* path) {
...
static void theron_v2_hwa_buf_reset(void) {
    g_entry_buf_len = 0;
...
static void theron_v2_hwa_buf_append_char(char c) {
...
static int theron_v2_hwa_extract_string(const char* line, const char* key,
                                        char* out, size_t outSize);
static int theron_v2_hwa_extract_int(const char* line, const char* key,
                                     int* out);

static void theron_v2_hwa_extract_fields_from_buf(Theron_V2_HwaSlotRaw* out) {
    char val[256];
    if (theron_v2_hwa_extract_string(g_entry_buf, "id", val, sizeof(val))) {
        theron_v2_hwa_trim(out->id, val, sizeof(out->id));
        out->has_id = 1;
    }
    if (theron_v2_hwa_extract_string(g_entry_buf, "generator", val, sizeof(val))) {
        theron_v2_hwa_trim(out->generator, val, sizeof(out->generator));
        out->has_generator = 1;
    }
    if (theron_v2_hwa_extract_string(g_entry_buf, "source_file", val, sizeof(val))) {
        theron_v2_hwa_trim(out->source_file, val, sizeof(out->source_file));
        out->has_source_file = 1;
    }
    int w = 0, h = 0;
    if (theron_v2_hwa_extract_int(g_entry_buf, "width", &w)) {
        out->width = w;
        out->has_width = 1;
    }
    if (theron_v2_hwa_extract_int(g_entry_buf, "height", &h)) {
        out->height = h;
        out->has_height = 1;
    }
}
...
void theron_v2_hud_widget_assets_set_manifest_path(const char* dataDir) {
    ...
    /* ~/.firestaff/data/theron -> ~/.firestaff -> assets/theron/hud/hud_widget_manifest.json
    ...
    char theron_hud_dir[FSP_PATH_MAX];
    ...
    FSP_JoinPath(theron_hud_dir, sizeof(theron_hud_dir), assets_root, "theron");
    FSP_JoinPath(theron_hud_dir, sizeof(theron_hud_dir), theron_hud_dir, "hud");
    FSP_JoinPath(g_manifest_path, sizeof(g_manifest_path),
                 theron_hud_dir, "hud_widget_manifest.json");
}
...
static void theron_v2_hwa_slot_raw_init(Theron_V2_HwaSlotRaw* r) {
    memset(r, 0, sizeof(*r));
}

static int theron_v2_hwa_find_slot_in_manifest(const char* manifest_path,
                                               const char* slot_id,
                                               Theron_V2_HwaSlotRaw* out) {
    if (!manifest_path || manifest_path[0] == '\0' || !slot_id || !out) return 0;
    theron_v2_hwa_slot_raw_init(out);
            ...
            if (*c == '[') {
                ...
                if (in_entry) {
                    ...
                    if (brace_depth == 0) {
                        /* End of entry */
                        in_entry = 0;
                        Theron_V2_HwaSlotRaw raw;
                        theron_v2_hwa_slot_raw_init(&raw);
                        theron_v2_hwa_extract_fields_from_buf(&raw);
                ...
                if (in_entry) {
                    theron_v2_hwa_buf_append_char(*c);
                }
            }
        }
    }
 ...
 *   ~/.firestaff/assets/theron/hud/hud_widgets/compass_rose.png
 */
static int theron_v2_hwa_resolve_source_file(const char* manifest_path,
                                             const char* category,
    ...
    return theron_v2_hwa_file_exists(joined);
}
...
int theron_v2_hud_widget_assets_validate_manifest(const char* manifest_path) {
    ...
    if (p[0] == '\0') return -1;
    if (!theron_v2_hwa_file_exists(p)) return -1;
    ...
    Theron_V2_HwaSlotRaw raw;
    if (!theron_v2_hwa_find_slot_in_manifest(p, first_id, &raw)) {
...
Theron_V2_HudWidgetClass theron_v2_hud_widget_assets_classify_slot(
    ...
    if ((unsigned)slot >= (unsigned)THERON_V2_HUD_WIDGET_COUNT) {
        return THERON_V2_HUD_WIDGET_CLASS_UNKNOWN;
    }
    if (g_manifest_path[0] == '\0' ||
        !theron_v2_hwa_file_exists(g_manifest_path)) {
        return THERON_V2_HUD_WIDGET_CLASS_MISSING;
    }

    Theron_V2_HwaSlotRaw raw;
    if (!theron_v2_hwa_find_slot_in_manifest(g_manifest_path,
...
int theron_v2_hud_widget_assets_get_slot_info(Theron_V2_HudWidgetSlot slot,
    ...
    if ((unsigned)slot >= (unsigned)THERON_V2_HUD_WIDGET_COUNT) return 0;
    ...
    out->classification = theron_v2_hud_widget_assets_classify_slot(slot);
...
int theron_v2_hud_widget_assets_real_count(int* out_total) {
    ...
    if (g_manifest_path[0] != '\0' &&
        ...
        for (size_t i = 0; i < THERON_V2_HUD_WIDGET_COUNT; ++i) {
            ...
            if (cls != THERON_V2_HUD_WIDGET_CLASS_MISSING) {
                ++total;
                if (cls == THERON_V2_HUD_WIDGET_CLASS_REAL) ++real;
            }
        }
    }
...
Theron_V2_HudWidgetGate theron_v2_hud_widget_assets_gate(void) {
    ...
    if (g_manifest_path[0] == '\0' ||
        !theron_v2_hwa_file_exists(g_manifest_path)) {
        g_last_gate = THERON_V2_HUD_WIDGET_GATE_NO_MANIFEST;
        g_installed = 0;
        return g_last_gate;
    }

    int manifest_valid = theron_v2_hud_widget_assets_validate_manifest(NULL);
    if (manifest_valid < 0) {
        g_last_gate = THERON_V2_HUD_WIDGET_GATE_NO_MANIFEST;
        g_installed = 0;
        return g_last_gate;
    ...
    if (total == 0) {
         ...
         * promote the gate). */
        g_last_gate = THERON_V2_HUD_WIDGET_GATE_PLACEHOLDER;
        g_installed = 0;
        return g_last_gate;
    }

    if (real == total) {
        g_last_gate = THERON_V2_HUD_WIDGET_GATE_COMPLETE;
        g_installed = 1;
    } else if (real > 0) {
        g_last_gate = THERON_V2_HUD_WIDGET_GATE_PARTIAL;
        g_installed = 1;
    } else {
        g_last_gate = THERON_V2_HUD_WIDGET_GATE_PLACEHOLDER;
        g_installed = 0;
...
const char* theron_v2_hud_widget_assets_slot_name(Theron_V2_HudWidgetSlot slot) {
    if ((unsigned)slot >= (unsigned)THERON_V2_HUD_WIDGET_COUNT) return "UNKNOWN";
    return k_slot_table[slot].id;
}

const char* theron_v2_hud_widget_assets_class_name(Theron_V2_HudWidgetClass cls) {
    switch (cls) {
        case THERON_V2_HUD_WIDGET_CLASS_UNKNOWN:     return "UNKNOWN";
        case THERON_V2_HUD_WIDGET_CLASS_MISSING:     return "MISSING";
        case THERON_V2_HUD_WIDGET_CLASS_PLACEHOLDER: return "PLACEHOLDER";
        case THERON_V2_HUD_WIDGET_CLASS_PARTIAL:     return "PARTIAL";
        case THERON_V2_HUD_WIDGET_CLASS_REAL:        return "REAL";
...
const char* theron_v2_hud_widget_assets_gate_name(Theron_V2_HudWidgetGate gate) {
    switch (gate) {
        case THERON_V2_HUD_WIDGET_GATE_NOT_PROBED:  return "NOT_PROBED";
        case THERON_V2_HUD_WIDGET_GATE_NO_MANIFEST: return "NO_MANIFEST";
        case THERON_V2_HUD_WIDGET_GATE_PLACEHOLDER: return "PLACEHOLDER";
        case THERON_V2_HUD_WIDGET_GATE_PARTIAL:     return "PARTIAL";
        case THERON_V2_HUD_WIDGET_GATE_COMPLETE:    return "COMPLETE";
        default: return "INVALID";
    }
}
...
int theron_v2_hud_widget_assets_uses_placeholder(Theron_V2_HudWidgetSlot slot) {
    Theron_V2_HudWidgetClass cls = theron_v2_hud_widget_assets_classify_slot(slot);
    switch (cls) {
        case THERON_V2_HUD_WIDGET_CLASS_REAL:
            return 0;
        case THERON_V2_HUD_WIDGET_CLASS_PLACEHOLDER:
        case THERON_V2_HUD_WIDGET_CLASS_PARTIAL:
        case THERON_V2_HUD_WIDGET_CLASS_MISSING:
        case THERON_V2_HUD_WIDGET_CLASS_UNKNOWN:
        default:
            return 1;
    }
}
...
const char* theron_v2_hud_widget_assets_source_evidence(void) {
    return
        "Theron V2 HUD Widget Asset Manifest — Phase 3 placeholder-vs-real gate\n"
        ...
        "Source: ReDMCSB PANEL.C F0354 (champion status-box drawing, sibling)\n"
        ...
        "Manifest path: ~/.firestaff/assets/theron/hud/hud_widget_manifest.json\n"
        ...
        "V1 invariant: V1 UI chrome (theron_v1_ui_chrome) NEVER bypassed\n"
