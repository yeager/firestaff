 ...
 *
 * Theron V2.2 GPU render path: V22 modern-art IN-PLACE bitmap cache.
 *
 ...
 *   2. Output: ~/.firestaff/assets/theron/modern/v22_inplace_cache.bin
 *      (~6 MB for 29 assets at 256x256 max, lanczos-resampled)
 *
 * Runtime pipeline:
 *   theron_v22_inplace_draw_init() reads cache file, mmaps, builds in-memory
 *   hash table keyed by (category_hash, asset_id_hash).
 *   theron_v22_inplace_get_cell_bitmap(depth, lateral) looks up the V22 shape
 ...
 *   - Walls (any variant) -> wall_dungeon_stone_01
 ...
 * theron_v22_modern_assets_pc34.c (manifest path resolution),
 * theron_v22_shape_cache_pc34.c (sibling cache + Theron_V22_CellRect coords),
 * include/dm1_v2_shape_runtime_pc34.h (shape variant enum),
 * THQUEST.ASM T400/T520/T600 (Theron viewport).
 */

#include "theron_v22_inplace_draw_pc34.h"
#include "theron_v22_shape_cache_pc34.h"
#include "theron_v22_shape_cache_pc34.h" /* Theron_V22_CellRect + cache */
...
/* For first cut, all walls use wall_dungeon_stone_01 (the most common
 * carved stone). All creatures use creature_demon_01. Floors use
 * the tile pattern. */
static const char* v22_wall_asset_id  = "wall_dungeon_stone_01";
...
static const char* v22_inplace_get_cell_asset_id(int depth, int lateral) {
    if (!theron_v22_shape_cache_active(depth, lateral)) return NULL;
    const Theron_V22_ShapeRuntimeResult* r = theron_v22_shape_cache_get(depth, lateral);
    if (!r || !r->active) return NULL;

    /* Use Theron_V22_ShapeParams.type to pick asset. The shape type enum
     * (THERON_V22_SHAPE_FLOOR_PLAIN/CRACKED etc., THERON_V22_SHAPE_WALL_*,
     * THERON_V22_SHAPE_CREATURE_*) is the authoritative discriminator. */
    Theron_V22_ShapeType t = r->params.type;
    switch (t) {
        case THERON_V22_SHAPE_FLOOR_PLAIN:
        case THERON_V22_SHAPE_CEILING_PLAIN:
            return v22_floor_plain_id;
        case THERON_V22_SHAPE_FLOOR_CRACKED:
        case THERON_V22_SHAPE_FLOOR_MOSSY:
            return v22_floor_cracked_id;
        case THERON_V22_SHAPE_CREATURE:
        case THERON_V22_SHAPE_CREATURE_PROJECTILE:
            return v22_creature_asset_id;
        case THERON_V22_SHAPE_ITEM:
        case THERON_V22_SHAPE_ITEM_FLOOR:
        case THERON_V22_SHAPE_ITEM_PROJECTILE:
...
int theron_v22_inplace_draw_init(void) {
    ...
    {
        /* Re-use theron_v22_get_shape_path to find the modern dir.
         * The cache file lives next to modern_asset_manifest.json. */
        char manifest_path[FSP_PATH_MAX];
        /* theron_v22 doesn't expose its manifest path; reconstruct from data dir
         * using the same logic as theron_v22_set_manifest_path. For first cut,
         * hardcode the conventional path ~/.firestaff/assets/theron/modern/. */
...
/* Theron 4x3 cell rect coordinates (depth x lateral). Must match
 * theron_v22_kCellRects in theron_v22_shape_cache_pc34.c (exposed as
 * Theron_V22_CellRect in theron_v22_shape_cache_pc34.h). */
static const Theron_V22_CellRect kV22CellRects[3][3] = {
    /* depth 0 = D1 (closest) */ {
        {  8, 103, 69, 30 },
        { 78, 103, 61, 30 },
        {139, 103, 69, 30 }
...
int theron_v22_inplace_render_pass(unsigned char* framebuffer, int fbW, int fbH) {
    ...
    if (!theron_v22_inplace_draw_active()) return 0;
    if (!theron_v22_shape_cache_populated()) return 0;
    for (depth = 0; depth < 3; ++depth) {
        for (lateral = -1; lateral <= 1; ++lateral) {
            int w = 0, h = 0;
            const uint32_t* rgba =
                theron_v22_inplace_get_cell_bitmap(depth + 1, lateral, &w, &h);
            if (!rgba || w <= 0 || h <= 0) continue;
            const Theron_V22_CellRect* rect = &kV22CellRects[depth][lateral + 1];
...
const char* theron_v22_inplace_draw_source_evidence(void) {
    return "theron_v22_shape_cache_pc34.c (per-cell V22 shape cache); "
           "dm1_v2_modern_assets_pc34.c (manifest path resolution); "
           "theron_v22_shape_cache_pc34.c (sibling cache + Theron_V22_CellRect coords); "
           "theron_v22_modern_assets_pc34.c (manifest path resolution); "
           "theron_v22_shapes.c (Theron V22 shape book); "
           "THQUEST.ASM T400/T520/T600 (Theron viewport); "
           "THQUEST.ASM T400/T520/T600 (9-square layout); "
