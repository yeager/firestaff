    ...
    int c15Cell)
{
    ...
    if (world && world->things && world->things->loaded &&
        world->dungeon && world->dungeon->loaded &&
        world->things->rawThingData[THING_TYPE_EXPLOSION] &&
        world->things->thingCounts[THING_TYPE_EXPLOSION] > 0 &&
        world->things->explosions &&
        world->things->explosionCount > 0) {
...
cmd_attack_legacy_marker:
    ...
    case CMD_CAST_SPELL: {
        ...
        /* MENU.C F0412:1816-1826 rejects unknown spells before drawing XP,
         * then spends one sample before practice validation. M11 already
         * supplied XP through reserved2; do not repeat its sample. */
