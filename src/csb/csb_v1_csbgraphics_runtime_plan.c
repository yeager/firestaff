...
static const char s_source_evidence[] =
    "CSBgraphics.dat runtime plan: CSBWin/Graphics.cpp:1838 "
...
static int find_pair(const CSB_V1_CSBGraphicsRuntimePlan *plan,
...
static const CSB_V1_CSBGraphicsRuntimePlanEntry *
    ...
    CSB_V1_CSBGraphicsRuntimeCustomBackgroundLayer layer)
{
    ...
    for (i = 0u; i < plan->planned_count; ++i) {
        const CSB_V1_CSBGraphicsRuntimePlanEntry *entry = &plan->entries[i];
        if (entry->entry_index == bitmap_entry_index &&
            entry->mask_entry_index == mask_entry_index &&
            entry->route == CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_VIEWPORT_CUSTOM_BACKGROUND &&
...
static int known_geometry_for_entry(uint32_t entry_index,
                                    ...
                                    uint16_t *out_w,
                                    uint16_t *out_h,
                                    CSB_V1_CSBGraphicsRuntimeRoute *out_route)
{
    ...
    if (entry_index == CSB_GRAPHIC_INVENTORY &&
        decompressed_size ==
            (uint16_t)(CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_W *
                       CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_H)) {
        *out_w = CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_W;
        *out_h = CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_H;
        *out_route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_INVENTORY;
        return 1;
    }
    if (entry_index == CSB_GRAPHIC_PANEL_RESURRECT_REINCARNATE &&
        ...
        *out_w = CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_W;
        *out_h = CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_H;
        *out_route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_RESURRECT_PANEL;
        return 1;
    }
    if (route_is_viewport(entry_index) &&
        ...
        *out_w = CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_W;
        *out_h = CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_H;
        *out_route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_VIEWPORT_DERIVED;
        return 1;
    }
    return 0;
}

static int append_entry(CSB_V1_CSBGraphicsRuntimePlan *plan,
...
{
    CSB_V1_CSBGraphicsRuntimePlanEntry *entry;

    if (!plan || !span || width == 0u || height == 0u) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_ARGUMENT;
    }
    if (plan->planned_count >=
        CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_MAX_ENTRIES) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_FULL;
    }
    ...
    entry->needs_viewport_redraw =
        (route == CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_VIEWPORT_DERIVED) ? 1 : 0;
    entry->needs_hud_redraw =
        (route == CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_INVENTORY ||
         route == CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_RESURRECT_PANEL) ? 1 : 0;
    plan->ready = 1;
                                 ...
                                 CSB_V1_CSBGraphicsRuntimeRoute *out_route)
{
    ...
    *out_route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_NONE;
    ...
    case CSB_V1_CSBGRAPHICS_STARTUP_ASSET_HUD_INVENTORY:
        *out_width = CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_W;
        *out_height = CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_H;
        *out_route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_INVENTORY;
        return 1;
    case CSB_V1_CSBGRAPHICS_STARTUP_ASSET_HUD_RESURRECT_PANEL:
        *out_width = CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_W;
        *out_height = CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_H;
        *out_route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_RESURRECT_PANEL;
        return 1;
    default:
        return 0;
    ...
    CSB_V1_CSBGraphicsStartupPackage *out_package)
{
    ...
    CSB_V1_CSBGraphicsRuntimePlan original_plan;
    size_t i;
    int result = CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK;
    ...
    for (i = 0u; i < spec_count; ++i) {
        ...
        if (spec->role >= CSB_V1_CSBGRAPHICS_STARTUP_ASSET_COUNT ||
            ...
            result = CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_GEOMETRY;
            goto fail;
        }
    ...
    if (!package->valid) {
        result = CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_NO_SUPPORTED_ENTRIES;
        goto fail;
    }
    return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK;
    ...
    CSB_V1_CSBGraphicsStartupDecodedSurface *out_surface)
{
    ...
    if (!out_surface->pixels) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_APPLY;
    }
    ...
    size_t *out_word_count)
{
    ...
    if (rc != CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK || !bytes) {
        return NULL;
    }
    ...
    uint16_t **out_mask_words)
{
    ...
    *out_mask_words = NULL;
    ...
    if (expected_size < 8u) {
        free(bytes);
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_GEOMETRY;
    }
    mask_count = read_le32_bytes(bytes);
    if ((uint32_t)room_num >= mask_count ||
        ((size_t)room_num + 2u) * 4u > (size_t)expected_size) {
        free(bytes);
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_NO_SUPPORTED_ENTRIES;
    }
    ...
    size_t *out_skin_def_word_count)
{
    ...
    for (skin_num = 0u; skin_num < skin_count; ++skin_num) {
        ...
        free(bytes);
        return csb_v1_csbgraphics_runtime_plan_decode_custom_background_skin_def_for_skin(
            cache,
            skin_num,
            out_skin_def_words,
...
{
    CSB_V1_CSBGraphicsEntrySpan span;
    CSB_V1_CSBGraphicsRuntimeRoute route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_NONE;
    ...
    if (entry_index == CSB_GRAPHIC_INVENTORY) {
        ...
        route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_RESURRECT_PANEL;
    } else if (route_is_viewport(entry_index)) {
        route = CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_VIEWPORT_DERIVED;
    }
    if (route == CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_NONE) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_GEOMETRY;
    }
    ...
    CSB_V1_CSBGraphicsRuntimePlan *plan)
{
    ...
    if (rc == CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK) {
        ++added;
    } else if (rc == CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_FULL ||
               rc == CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_GEOMETRY) {
        return rc;
    } else {
        ++missing;
    }
    ...
    return added > 0 ? CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK
    ...
    CSB_V1_CSBGraphicsRuntimePlan *plan)
{
    ...
    if (!cache_ready(cache)) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_NO_CACHE;
    }
    copy_cache_metadata(cache, plan);
    ...
    for (i = 0u; i < sizeof(candidates) / sizeof(candidates[0]); ++i) {
        ...
        uint16_t height = 0u;
        CSB_V1_CSBGraphicsRuntimeRoute route =
            CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_NONE;
        int rc;
    ...
    CSB_V1_CSBGraphicsRuntimeBinding *out_binding)
{
    ...
    entry = csb_v1_csbgraphics_runtime_plan_find_entry(plan,
                                                           entry_index);
    if (!entry) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_NO_SUPPORTED_ENTRIES;
    }
    if (entry->deferred_masked_composite) {
        return CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_DEFERRED_COMPOSITE;
    }
    applied = csb_v1_csbgraphics_runtime_decode_entry_and_apply(
        cache->file_buffer,
        cache->file_size,
        entry->entry_index,
        ...
        framebuffer_stride,
        out_binding);
    return applied ? CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK
                   : CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_APPLY;
}
...
int csb_v1_csbgraphics_runtime_plan_apply_custom_background_room_layer(
    ...
    int viewport_width_pixels)
{
    const CSB_V1_CSBGraphicsRuntimePlanEntry *entry = NULL;
    uint32_t bitmap_entry_index;
    uint32_t mask_entry_index;
    ...
    return csb_v1_csbgraphics_runtime_plan_apply_custom_background_entry(
        plan,
        cache,
        entry->entry_index,
        mask_geometry,
    ...
    int viewport_width_pixels)
{
    ...
    rc = decode_custom_background_mask_for_room(cache,
                                                ...
                                                &mask_words);
    if (!bitmap_words ||
        rc != CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK ||
        !mask_words) {
        free(bitmap_words);
        free(mask_words);
...
{
    switch (result) {
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_OK:
        return "ok";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_ARGUMENT:
        return "argument";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_NO_CACHE:
        return "no_cache";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_NO_SUPPORTED_ENTRIES:
        return "no_supported_entries";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_FULL:
        return "full";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_GEOMETRY:
        return "geometry";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_APPLY:
        return "apply";
    case CSB_V1_CSBGRAPHICS_RUNTIME_PLAN_ERR_DEFERRED_COMPOSITE:
