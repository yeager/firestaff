...
/* ══════════════════════════════════════════════════════════════════════
 * V2.2 Minimap — Dungeon overview map
 *
 * Renders explored squares as colored cells:
 *   - Wall: dark gray
 *   - Corridor: light gray
 *   - Door: brown
 *   - Stairs: blue
 *   - Pit: red
 *   - Party position: white blinking dot
 *   - Unexplored: black
 * ══════════════════════════════════════════════════════════════════════ */

void v22_minimap_render_cell(uint32_t *pixels, int map_w, int map_h,
    int cell_x, int cell_y, int cell_size, uint32_t color)
{
    int px, py;
    if (!pixels || cell_x < 0 || cell_y < 0) return;
    for (py = 0; py < cell_size && (cell_y * cell_size + py) < map_h; py++) {
        for (px = 0; px < cell_size && (cell_x * cell_size + px) < map_w; px++) {
            int idx = (cell_y * cell_size + py) * map_w + (cell_x * cell_size + px);
            pixels[idx] = color;
        }
    }
}

uint32_t v22_minimap_square_color(int square_type, int explored) {
    if (!explored) return 0xFF000000; /* black = unexplored */
    switch (square_type) {
        case 0: return 0xFF404040; /* wall */
        case 1: return 0xFFA0A0A0; /* corridor */
        case 2: return 0xFF0000FF; /* stairs up */
        case 3: return 0xFF0000CC; /* stairs down */
        case 4: return 0xFFCC0000; /* pit */
        case 5: return 0xFF00CCCC; /* teleporter */
        case 16: return 0xFF806030; /* door front */
        case 17: return 0xFF806030; /* door side */
        default: return 0xFF606060; /* other */
    }
}
