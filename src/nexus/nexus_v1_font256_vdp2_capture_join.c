    ...
    Nexus_V1_Font256Vdp2CaptureJoinReceipt *out_receipt)
{
    ...
    if (!input || !input->capture_page ||
        !input->capture_character_generator ||
    ...
    page_offset = input->decoded->page_offset;
    page_size = input->decoded->page_size;
    ...
    if (page_size < 16U || cg_size < 16U || palette_size < 16U ||
        page_offset > (uint32_t)input->font256_s2d_size ||
        page_size > (uint32_t)input->font256_s2d_size - page_offset ||
        ...
        input->capture_page_size != (int)(page_size - 16U) ||
    ...
    if (receipt.character_generator_tile_count <= 0 ||
        ...
        memcmp(input->capture_page, input->font256_s2d + page_offset + 16U,
               page_size - 16U) != 0 ||
    ...
    receipt.page_span_join_verified = 1;
    receipt.character_generator_span_join_verified = 1;
    receipt.palette_span_join_verified = 1;
    /* A raw Page span join does not establish PND field meaning, text
     * encoding, character-code-to-tile mapping, placement, or layer
     * ownership. */
