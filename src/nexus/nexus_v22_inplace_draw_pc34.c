 ...
 *
 * Runtime pipeline:
 *   nexus_v22_inplace_draw_init() reads cache file, mmaps, builds in-memory
 *   hash table keyed by (category_hash, asset_id_hash).
 *   nexus_v22_inplace_get_cell_bitmap(depth, lateral) looks up the V22 shape
 ...
 *   - Walls (any variant) -> wall_saturn_tech_01
 *   - Floor PLAIN pattern  -> floor_saturn_metal_01
 *   - Floor CRACKED pattern -> floor_saturn_metal_01
 *   - Floor MOSSY pattern  -> floor_saturn_metal_01 (no mossy variant in v1.4.0)
 *   - Creatures (any)      -> creature_saturn_brigand_01
 *
 * Source-lock: nexus_v22_shape_cache_pc34.h (the cache),
 * nexus_v22_modern_assets_pc34.c (manifest path resolution),
 * nexus_v22_shape_cache_pc34.c (sibling cache + NEXUS_V22_CellRect coords),
 * include/dm1_v2_shape_runtime_pc34.h (shape variant enum),
 * SATURN_DMDF T400/T600 (composition order).
 */

#include "nexus_v22_inplace_draw_pc34.h"
#include "nexus_v22_shape_cache_pc34.h"
#include "nexus_v22_shape_cache_pc34.h" /* NEXUS_V22_CellRect + cache */
...
int nexus_v22_inplace_draw_init(void) {
    if (g_v22_inplace_active) return 1;

    /* Resolve cache path from manifest path */
    char cache_path[FSP_PATH_MAX];
    {
        /* Re-use nexus_v22_get_shape_path to find the modern dir.
         * The cache file lives next to modern_asset_manifest.json. */
        char manifest_path[FSP_PATH_MAX];
        /* nexus_v22 doesn't expose its manifest path; reconstruct from data dir
         * using the same logic as nexus_v22_set_manifest_path. For first cut,
         * hardcode the conventional path ~/.firestaff/assets/nexus/modern/. */
...
void nexus_v22_inplace_draw_shutdown(void) {
...
int nexus_v22_inplace_draw_active(void) {
    return g_v22_inplace_active;
}

const uint32_t* nexus_v22_inplace_get_cell_bitmap(int depth, int lateral,
                                                 int* out_w, int* out_h) {
    if (out_w) *out_w = 0;
    if (out_h) *out_h = 0;
...
/* Nexus 3x3 cell rect coordinates (depth x lateral). Must match
 * nexus_v22_kCellRects in nexus_v22_shape_cache_pc34.c (exposed as
 * NEXUS_V22_CellRect in nexus_v22_shape_cache_pc34.h). */
static const NEXUS_V22_CellRect kV22CellRects[3][3] = {
    /* depth 0 = D1 (closest) */ {
        {  8, 103, 69, 30 },
        { 78, 103, 61, 30 },
        {139, 103, 69, 30 }
...
int nexus_v22_inplace_render_pass(unsigned char* framebuffer, int fbW, int fbH) {
    int depth, lateral;
    int cells_painted = 0;
    if (!framebuffer || fbW <= 0 || fbH <= 0) return 0;
    if (!nexus_v22_inplace_draw_active()) return 0;
    if (!nexus_v22_shape_cache_populated()) return 0;
    for (depth = 0; depth < 3; ++depth) {
        for (lateral = -1; lateral <= 1; ++lateral) {
            int w = 0, h = 0;
            const uint32_t* rgba =
                nexus_v22_inplace_get_cell_bitmap(depth + 1, lateral, &w, &h);
            if (!rgba || w <= 0 || h <= 0) continue;
            const NEXUS_V22_CellRect* rect = &kV22CellRects[depth][lateral + 1];
...
const char* nexus_v22_inplace_draw_source_evidence(void) {
    return "nexus_v22_shape_cache_pc34.c (per-cell V22 shape cache); "
           "dm1_v2_modern_assets_pc34.c (manifest path resolution); "
           "nexus_v22_shape_cache_pc34.c (sibling cache + NEXUS_V22_CellRect coords); "
           "nexus_v22_modern_assets_pc34.c (manifest path resolution); "
           "nexus_v22_shapes.c (Nexus V22 minimal shape selector); "
           "SATURN_DMDF T400/T600 (Nexus viewport routing); "
           "SATURN_DMDF T400/T600 (9-square layout); "
