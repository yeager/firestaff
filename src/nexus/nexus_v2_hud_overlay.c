...
/* ── Action strip icon ──────────────────────────────────────────── */
static const char *g_action_icon_labels[NEXUS_V2_ACTION_COUNT] = {
    "ATK", "CST", "USE", "DRP", "MOV"
};

static void hud_draw_action_icon(uint8_t *fb, int stride, int x, int y,
    Nexus_V2_ActionIcon icon, bool active, bool flash, uint8_t base, uint8_t high)
{
    (void)active;
    if (flash) {
        hud_rect(fb, stride, x, y, NEXUS_ACTION_ICON_W, 28, high);
    } else {
        hud_rect(fb, stride, x, y, NEXUS_ACTION_ICON_W, 28, base);
    }
    /* Icon label */
    ...
    if (active) {
        for (int i = 0; i < NEXUS_ACTION_ICON_W; i++) {
    ...
    bool leader, bool spell_ready, uint8_t high)
{
    (void)spell_ready;
    /* HP bar (red) */
    int bar_w = NEXUS_CHAMP_BAR_W;
    int hp_w = (bar_w * hp_pct) / 100;
    if (hp_w < bar_w / 4) {
        float alpha = hud_health_pulse_alpha(champ_idx);
        uint8_t pulse_val = (uint8_t)(high * alpha);
        hud_rect(fb, stride, x, y, hp_w, NEXUS_CHAMP_BAR_H / 3, pulse_val);
    } else {
        hud_rect(fb, stride, x, y, hp_w, NEXUS_CHAMP_BAR_H / 3, high);
    }

    /* Stamina bar (green) */
    int st_w = (bar_w * stamina_pct) / 100;
    hud_rect(fb, stride, x, y + NEXUS_CHAMP_BAR_H / 3, st_w, NEXUS_CHAMP_BAR_H / 3, 10);

    /* Mana bar (blue) */
    int mn_w = (bar_w * mana_pct) / 100;
    hud_rect(fb, stride, x, y + (NEXUS_CHAMP_BAR_H * 2) / 3, mn_w, NEXUS_CHAMP_BAR_H / 3, 9);
...
/* ── Saturn panel indicator dots ───────────────────────────────── */
/* Small 3×3 indicator dots for menu, map, and party status */
static void hud_draw_panel_dot(uint8_t *fb, int stride, int x, int y, uint8_t val) {
    hud_rect(fb, stride, x, y, 3, 3, val);
}
...
void nexus_v2_hud_render(Nexus_V2_HudOverlay *h, uint8_t *fb, int stride, int h_res) {
    ...
    if (h->stats_bar_visible) {
        for (int i = 0; i < 4; i++) {
            Nexus_V2_HudChampionBar *cb = &h->champion_bars[i];
            int bx = NEXUS_CHAMP_BAR_X_START + i * (NEXUS_CHAMP_BAR_W + NEXUS_CHAMP_BAR_SPACING);
            hud_draw_champion_bar(fb, stride, bx, NEXUS_CHAMP_BAR_Y, i,
    ...
    if (h->action_strip.visible) {
        for (int i = 0; i < NEXUS_V2_ACTION_COUNT; i++) {
            int ax = NEXUS_ACTION_ICONS_X_START + i * (NEXUS_ACTION_ICON_W + 4);
            Nexus_V2_ActionIconState *st = &h->action_strip.icons[i];
            bool flash = h->hit_flash_active && st->active;
            hud_draw_action_icon(fb, stride, ax, NEXUS_ACTION_STRIP_Y,
                (Nexus_V2_ActionIcon)i, st->active, flash, base, high);
        }
    ...
    /* ── Top-right corner: Saturn panel indicators ─────────────── */
    /* Menu dot (hamburger icon — 3 horizontal lines) */
    if (h->panel.menu_indicator) {
        int mx = stride - 24;
        hud_rect(fb, stride, mx, 4, 10, 2, high);
        hud_rect(fb, stride, mx, 8, 10, 2, high);
        hud_rect(fb, stride, mx, 12, 10, 2, high);
    }
    /* Map dot (small square) */
    if (h->panel.map_indicator) {
        int mx2 = stride - 12;
        hud_draw_panel_dot(fb, stride, mx2, 6, high);
    }
    /* Party dot (diamond shape) */
    if (h->panel.party_indicator) {
        int px = stride - 12;
        hud_plot(fb, stride, px, 20, high);
        hud_plot(fb, stride, px + 1, 19, high);
        hud_plot(fb, stride, px + 2, 20, high);
        hud_plot(fb, stride, px + 1, 21, high);
    }
}
