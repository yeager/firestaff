    ...
    uint32_t record_index, Nexus_V1_Structure1FAlcoveAdmissionReceipt *out_receipt)
{
    ...
    family = &directory->families[NEXUS_V1_DGN_STRUCTURE1F_ALCOVES];
    if (package_fnv1a64 != identity->fnv1a64 ||
        package_fnv1a64 != directory->package_fnv1a64 ||
        family->source_tag != 0x20U || family->record_size != 12U ||
    ...
    receipt.valid = receipt.directory_bound = receipt.alcove_record_bound = 1;
