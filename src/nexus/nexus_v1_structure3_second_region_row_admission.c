...
{
    Nexus_V1_Structure3SecondRegionRowAdmissionReceipt receipt;
    ...
    if (!identity || !dgn_data || dgn_size <= 0 || !entry || !entry->valid ||
        ...
        entry->second_region_offset != entry->first_region_offset +
            entry->first_region_length ||
    ...
    row_offset = (uint64_t)entry->target_offset + entry->second_region_offset +
    ...
    receipt.valid = receipt.entry_bound = receipt.second_region_bound = 1;
