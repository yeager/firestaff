...
/* Canonical TITLE.BIN CNFD payload facts, derived from the SHA-256-
 * attested retail asset. These are provenance bindings of the same class
 * as the existing TITLE.BIN DGT2 constants: head dimensions and flag
 * words, the 32-byte post-head prefix, and the packed width*height/2
 * plane, plus the observed contiguous tail-covering sub-chain. They
 * assign no colour, palette, image, pixel, or presentation semantics. */
static const uint16_t k_cnfd_width[NEXUS_V1_TITLE_CNFD_COUNT] = {
    ...
    0x50U, 0x50U, 0x18U, 0x18U, 0x20U, 0x20U, 0x28U, 0x28U,
    0xe8U
};
static const uint16_t k_cnfd_height[NEXUS_V1_TITLE_CNFD_COUNT] = {
    ...
    0x0fU, 0x11U, 0x0dU, 0x0dU, 0x0eU, 0x0eU, 0x0fU, 0x11U,
    0x25U
};
static const uint16_t k_cnfd_flag[NEXUS_V1_TITLE_CNFD_COUNT] = {
    NEXUS_V1_TITLE_CNFD_FLAG_A, NEXUS_V1_TITLE_CNFD_FLAG_B,
    NEXUS_V1_TITLE_CNFD_FLAG_B, NEXUS_V1_TITLE_CNFD_FLAG_B,
    NEXUS_V1_TITLE_CNFD_FLAG_B, NEXUS_V1_TITLE_CNFD_FLAG_B,
    NEXUS_V1_TITLE_CNFD_FLAG_A, NEXUS_V1_TITLE_CNFD_FLAG_B,
    NEXUS_V1_TITLE_CNFD_FLAG_B, NEXUS_V1_TITLE_CNFD_FLAG_B,
    NEXUS_V1_TITLE_CNFD_FLAG_B, NEXUS_V1_TITLE_CNFD_FLAG_B,
...
{
    Nexus_V1_TitleCnfdRecordReceipt receipt;
    ...
    if (cnfd_index >= NEXUS_V1_TITLE_CNFD_COUNT) {
        *out_receipt = receipt;
        return 0;
    }
    if (!nexus_v1_title_res_record_admit(source_bytes, source_size, identity,
            NEXUS_V1_TITLE_CNFD_FIRST_ENTRY_INDEX + cnfd_index, &base) ||
        base.entry_class != NEXUS_V1_TITLE_RES_CLASS_CNFD ||
        base.entry_id != cnfd_index ||
        base.record_length < NEXUS_V1_TITLE_CNFD_HEAD_BYTES +
            NEXUS_V1_TITLE_CNFD_PREFIX_BYTES) {
        *out_receipt = receipt;
        return 0;
    }
    record = source_bytes + base.record_offset;
    if (be16(record + 10U) != k_cnfd_width[cnfd_index] ||
        be16(record + 12U) != k_cnfd_height[cnfd_index] ||
        be16(record + 14U) != k_cnfd_flag[cnfd_index]) {
        *out_receipt = receipt;
        return 0;
    }
    if (((uint32_t)k_cnfd_width[cnfd_index] *
         (uint32_t)k_cnfd_height[cnfd_index]) & 1U) {
        *out_receipt = receipt;
        return 0;
    }
    plane_length = (uint32_t)k_cnfd_width[cnfd_index] *
                   (uint32_t)k_cnfd_height[cnfd_index] / 2U;
    expected_length = NEXUS_V1_TITLE_CNFD_HEAD_BYTES +
        NEXUS_V1_TITLE_CNFD_PREFIX_BYTES + plane_length;
    ...
    receipt.valid = receipt.source_identity_bound =
        receipt.res_directory_bound = receipt.cnfd_head_bound =
    ...
    receipt.width = k_cnfd_width[cnfd_index];
    receipt.height = k_cnfd_height[cnfd_index];
    receipt.flag_word = be16(record + 14U);
    receipt.prefix_offset = base.record_offset + NEXUS_V1_TITLE_CNFD_HEAD_BYTES;
    ...
    Nexus_V1_TitleCnfdCorpusReceipt *out_receipt)
{
    ...
    if (!out_receipt) return 0;
    memset(&receipt, 0, sizeof(receipt));
    for (index = 0U; index < NEXUS_V1_TITLE_CNFD_COUNT; ++index) {
        if (!nexus_v1_title_cnfd_record_admit(source_bytes, source_size,
    ...
    if (receipt.chain_offset != NEXUS_V1_TITLE_CNFD_CHAIN_OFFSET ||
        chain_end != (uint64_t)NEXUS_V1_TITLE_CNFD_CHAIN_END) {
        memset(&receipt, 0, sizeof(receipt));
        *out_receipt = receipt;
        return 0;
    }
    receipt.valid = receipt.source_identity_bound =
        receipt.res_directory_bound = receipt.all_cnfd_bound = 1;
    receipt.contiguous_chain_observed = 1;
    receipt.chain_covers_source_tail =
    ...
    receipt.cnfd_count = NEXUS_V1_TITLE_CNFD_COUNT;
...
{
    uint32_t index;
    if (!iterator || !receipt || !receipt->valid || !receipt->all_cnfd_bound ||
        ...
        receipt->cnfd_count != NEXUS_V1_TITLE_CNFD_COUNT) return -1;
    for (index = 0U; index < NEXUS_V1_TITLE_CNFD_COUNT; ++index) {
...
{
    const Nexus_V1_TitleCnfdRecordReceipt *record;
    if (!iterator || !out_span || !iterator->receipt.valid) return -1;
    if (iterator->emitted >= NEXUS_V1_TITLE_CNFD_COUNT) return 0;
