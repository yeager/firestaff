/*
 * nexus_v22_modern_assets_pc34.c — Nexus V2.2 Modern Graphics Fallback Pipeline
 *
 ...
 * NEXUS_V22_SHAPE_MISSING_PLACEHOLDER rather than crashing.
 */

#include "nexus_v22_modern_assets_pc34.h"
 ...
 * ~/.firestaff/assets/nexus/modern/ location. */
static char g_v22_manifest_path[FSP_PATH_MAX];

/* ── Placeholder for missing assets ──────────────────────────────── */

/* NEXUS_V22_SHAPE_MISSING_PLACEHOLDER — returned by shape-fetching
...
static const uint32_t NEXUS_V22_SHAPE_MISSING_PLACEHOLDER_DATA[16 * 16] = {
...
/* ── Shape source enum is defined in nexus_v22_modern_assets_pc34.h
...
static int nexus_v22_file_exists(const char* path) {
...
/* Trimming helpers */
static void nexus_v22_trim(char* dst, const char* src, size_t dstSize) {
...
static int nexus_v22_read_line(FILE* fp, char* out, size_t outSize) {
    if (!fp || !out || outSize == 0U) return 0;
    out[0] = '\0';
    if (!fgets(out, (int)outSize, fp)) return 0;
    nexus_v22_trim(out, out, outSize);
    return 1;
}

/* Extract a string value for a key from a JSON line of the form: "key": "value"
 * Returns 1 on success, 0 if not found. */
static int nexus_v22_extract_string(const char* line, const char* key, char* out, size_t outSize) {
 ...
 * Returns 1 on success, 0 if not found. */
static int nexus_v22_extract_int(const char* line, const char* key, int* out) {
...
static void __attribute__((unused)) nexus_v22_skip_to_next_object (FILE* fp) {
    int depth = 0;
    char line[256];
...
/* nexus_v22_set_manifest_path — set the path to the modern asset manifest.
 * Called by M12_AssetStatus_Scan() after resolving the data directory.
 * The manifest path is derived as: ~/.firestaff/assets/nexus/modern/
 * modern_asset_manifest.json (per docs/v2_2_asset_manifest.md and
 * docs/v2_2_asset_provenance_schema.md).
 *
 * dataDir is the Nexus game data directory path (e.g. ~/.firestaff/data/nexus).
 * We walk up two levels to reach ~/.firestaff, then append
 * assets/nexus/modern/modern_asset_manifest.json.
 *
 * CRITICAL: We must use FSP_ParentDir to strip the last path segment,
 * NOT FSP_JoinPath with "..". The ".." join preserves the directory
 * name in the path string (e.g. "nexus/../" doesn't simplify to just the
 * parent), so using it would produce a manifest path that contains the
 * dataDir name (e.g. ".../nexus/../assets/...") which never resolves to
 * the actual file. FSP_ParentDir strips the last segment cleanly.
 *
 * Example: dataDir="/home/user/.firestaff/data/nexus"
 *   ParentDir → "/home/user/.firestaff/data"
 *   ParentDir → "/home/user/.firestaff"
 *   assets/nexus/modern → correct modern assets directory.
 *
 * Source: FSP_ParentDir finds the last separator and truncates there. */
void nexus_v22_set_manifest_path(const char* dataDir) {
    if (!dataDir || dataDir[0] == '\0') {
        g_v22_manifest_path[0] = '\0';
        return;
    }
    /* Build: ~/.firestaff/assets/nexus/modern/modern_asset_manifest.json
     * Walk up two levels from dataDir (e.g. data/nexus -> data -> ~/.firestaff)
     * then append assets/nexus/modern/. */
    ...
    FSP_JoinPath(modernDir, sizeof(modernDir), assetsRoot, "nexus");
...
/* nexus_v22_validate_manifest — validates the JSON manifest.
 *
 * Checks:
...
int nexus_v22_validate_manifest(const char* manifest_path) {
    FILE* fp;
    char line[256];
    ...
    while (nexus_v22_read_line(fp, line, sizeof(line))) {
        ...
        if (strchr(line, '[') == NULL && strchr(line, '{') == NULL) {
            ...
            if (!nexus_v22_read_line(fp, line, sizeof(line))) break;
        }
        ...
        for (;;) {
            ...
            if (strchr(line, '{') != NULL || strchr(line, '"') != NULL) {
                ...
                int has_id = nexus_v22_extract_string(line, "id", id_val, sizeof(id_val));
                int has_file = nexus_v22_extract_string(line, "source_file", file_val, sizeof(file_val));
                int has_width = nexus_v22_extract_int(line, "width", &width_val);
                int has_height = nexus_v22_extract_int(line, "height", &height_val);
...
/* nexus_v22_modern_assets_available — checks if the modern asset pack
 * is installed and at least critical shape categories are present.
 *
 * Checks:
 *   - ~/.firestaff/assets/nexus/modern/modern_asset_manifest.json exists
...
int nexus_v22_modern_assets_available(void) {
    if (g_v22_manifest_path[0] == '\0') return 0;
    if (!nexus_v22_file_exists(g_v22_manifest_path)) return 0;
    ...
    while (nexus_v22_read_line(fp, line, sizeof(line))) {
        ...
        for (int ci = 0; ci < 3; ++ci) {
            ...
            if (nexus_v22_extract_string(line, "id", id_val, sizeof(id_val))) {
                found_critical[ci] = 1;
                current_cat = -1;
            } else {
...
/* nexus_v22_set_installed — called at startup by M12_AssetStatus_Scan()
 * after detecting whether the modern asset pack is present. */
void nexus_v22_set_installed(int installed) {
    g_v22_modern_assets_installed = installed ? 1 : 0;
}

/* nexus_v22_get_installed — returns whether V2.2 modern assets are installed */
int nexus_v22_get_installed(void) {
    return g_v22_modern_assets_installed;
}

/* nexus_v22_set_epx_cache_warm / nexus_v22_get_epx_cache_warm — EPX cache
 * state for V2.1 upscale. When UPSCALED mode is selected but the EPX
 * cache is cold, the system falls back to FILTERED (V2.0). */
void nexus_v22_set_epx_cache_warm(int warm) {
    g_epx_cache_warm = warm ? 1 : 0;
}

int nexus_v22_get_epx_cache_warm(void) {
    return g_epx_cache_warm;
}

/* nexus_v22_best_available_shape_source — returns the best available
    ...
    switch (presentation_mode_index) {
        case 3: /* M12_PRESENTATION_V22_MODERN */
            if (g_v22_modern_assets_installed) {
                return NEXUS_V22_SHAPE_SOURCE_V2_MODERN;
            }
        ...
        case 0:
        default:
            return NEXUS_V22_SHAPE_SOURCE_V1_ORIGINAL;
    }
}

/* nexus_v22_get_missing_placeholder — returns the missing-asset
 * placeholder surface (16×16 RGBA). Never returns NULL. */
const uint32_t* nexus_v22_get_missing_placeholder(int* out_w, int* out_h) {
    if (out_w) *out_w = 16;
    if (out_h) *out_h = 16;
    return NEXUS_V22_SHAPE_MISSING_PLACEHOLDER_DATA;
}

/* nexus_v22_get_shape_path — given a category and asset id from the
 * manifest, returns the full filesystem path to the asset file.
 * Returns 1 on success, 0 if not found or manifest not available. */
int nexus_v22_get_shape_path(const char* category, const char* asset_id,
    ...
    while (nexus_v22_read_line(fp, line, sizeof(line))) {
        ...
        char val[256];
        if (nexus_v22_extract_string(line, "id", val, sizeof(val))) {
            nexus_v22_trim(resolved_id, val, sizeof(resolved_id));
        }
        if (nexus_v22_extract_string(line, "source_file", val, sizeof(val))) {
            nexus_v22_trim(resolved_file, val, sizeof(resolved_file));
        }
    }
    ...
    return nexus_v22_file_exists(out_path) ? 1 : 0;
}

/* nexus_v22_shape_source_name — human-readable name for a shape source */
const char* nexus_v22_shape_source_name(NEXUS_V22_ShapeSource src) {
    switch (src) {
        case NEXUS_V22_SHAPE_SOURCE_V1_ORIGINAL: return "V1_ORIGINAL";
        case NEXUS_V22_SHAPE_SOURCE_V2_FILTERED: return "V2_FILTERED";
        case NEXUS_V22_SHAPE_SOURCE_V2_UPSCALED: return "V2_UPSCALED";
        case NEXUS_V22_SHAPE_SOURCE_V2_MODERN:   return "V2_MODERN";
        default: return "UNKNOWN";
    }
}

/* nexus_v22_source_evidence — citation string for source-lock tests. */
const char* nexus_v22_source_evidence(void) {
    return "SATURN_DMDF T400 (Nexus dungeon viewport); "
           "SATURN_DMDF T520 (Nexus champion panel); "
           "SATURN_DMDF T600 (Nexus outdoor viewport); "
           "SATURN_DMDF torchlight routine; "
           "Saturn VDP1 + VDP2 (Saturn VDP1/VDP2 graphics); "
