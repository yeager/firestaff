...
int nexus_v1_creature_spawn(Nexus_V1_CreatureManager *mgr, int type_idx, int x, int y, int dir) {
    ...
    /* ReDMCSB: GROUP.C F0183 lines ~414-424 refuses to assign an
     * active-group slot after the active pool is exhausted. The fixed
     * active-pool boundary and the actor-type reference are hard rejects
     * (a bad type index would read garbage stats), but malformed spatial
     * fields from external Nexus DGN actor records are clamped/normalized
     * below rather than dropped, so a single bad coordinate does not lose
     * an otherwise-valid actor. */
