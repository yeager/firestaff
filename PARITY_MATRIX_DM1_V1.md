...
| Map overlay | Local ReDMCSB PC source has `NEWMAP.C`, but it is map-transition plumbing (`F0003_MAIN_ProcessNewPartyMap_CPSE`: music track, active groups, current-map graphics, dungeon-view palette), not a player automap/overlay renderer. No source-backed full-screen map UI has been identified in the DM1 V1 runtime. | Firestaff has an invented `mapOverlayActive` / `m11_draw_fullscreen_map(...)` debug/convenience overlay toggled by `M12_MENU_INPUT_MAP_TOGGLE` / `M` key, with internal probes around toggle behavior. | `KNOWN_DIFF` — Firestaff convenience overlay, not claimed DM1 V1 parity surface. | Hide/disable from default V1 parity mode or keep strictly debug-only; do not count map overlay as original UI until contrary source/runtime evidence appears. |
...
| `KNOWN_DIFF` | 13 — prior palette + typography + Firestaff-invented UI chrome, the invented/convenience map overlay, plus the now-narrowed inventory-screen and ornament rows. Viewport −28×−18 drift from pass 33/40 was superseded by all-graphics phases 76–81; champion status-box +8 px stride retired in pass 41. |
| `UNPROVEN` | ~34 — unchanged by pass 102; the map overlay row moved from blocked/unknown to an explicit `KNOWN_DIFF`. |
...
**Bottom line (updated 2026-04-25, pass 102):** Firestaff has measured ownership progress (passes 29–32), measured visual drift (passes 33–34), a complete text-vs-graphics enumeration (pass 35), source-faithful champion status-box spacing (pass 41), source-matched DM1 viewport rectangle bounds via the all-graphics phases, source-bound action/inventory object icons plus C011 spell label cells, a narrowed inventory-screen row backed by deterministic capture evidence, narrowed ornament metadata/draw-path coverage, and the invented map overlay is now explicitly classified as `KNOWN_DIFF`.  **This still does not complete DM1/V1 parity**; remaining work is viewport content/draw-order parity, original screenshot overlays, audio/timing, and non-current surfaces such as pointer/held-object icons and exact inventory/spell-panel/ornament placement overlays.
