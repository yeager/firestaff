...
| Status | Count |
|--------|-------|
| `MATCHED` | 11 — GRAPHICS.DAT decode coverage, placeholder cataloging, title-side UI asset mapping, Champion portraits (pass 34), Spell-panel asset identity (pass 34), M9 gate infra, M10 suite infra, M11 suite infra, Submenu matrix infra, Champion status-box stride + slot width V1 source-anchored (pass 41), viewport rectangle bounds (all-graphics phases 76–81). |
| `KNOWN_DIFF` | 10 — prior palette + typography + Firestaff-invented UI chrome. Viewport −28×−18 drift from pass 33/40 was superseded by all-graphics phases 76–81; champion status-box +8 px stride retired in pass 41. |
...
**Bottom line (updated 2026-04-25):** Firestaff has measured ownership progress (passes 29–32), measured visual drift (passes 33–34), a complete text-vs-graphics enumeration (pass 35), source-faithful champion status-box spacing (pass 41), and now source-matched DM1 viewport rectangle bounds via the all-graphics phases.  **This still does not complete DM1/V1 parity**; remaining work is viewport content/draw-order parity, right-column/action UI polish, audio/timing, and original screenshot overlay evidence.
