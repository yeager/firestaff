...
| Viewport region bounds | DEFS.H: `C112_BYTE_WIDTH_VIEWPORT`=112 (224px at 4bpp, line 2478), `C136_HEIGHT_VIEWPORT`=136 (line 2484). Graphic #0 (`C000_DERIVED_BITMAP_VIEWPORT`) is 224×136 in manifest, confirming exact match. Viewport uses color indices 16–31 via `G8177_c_ViewportColorIndexOffset=0x10`. | Composed backdrop + viewport patch verified stable in `m7_reachability_b`. Graphic #0 extraction matches 224×136. | `UNPROVEN` — exact pixel placement on 320×200 screen not measured | Measure viewport X,Y origin from Graphic562 box data or runtime capture. See §E7. |
