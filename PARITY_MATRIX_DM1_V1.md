...
| Area | Original/Reference Evidence | Firestaff Current State | Status | Next Action |
|------|---------------------------|------------------------|--------|-------------|
| Sound trigger points | ReDMCSB I34E sound-index namespace mapped to SND3 item indices in Pass 52 (`DEFS.H`, `DATA.C`, `PASS52_AUDIO_FINDINGS.md`) | Sound-event → SND3 mapping table landed (`sound_event_snd3_map_v1.[ch]`); 5/5 probe invariants PASS; runtime trigger cadence still not captured against original audio | `KNOWN_DIFF` (narrowed) | Wire mapped decoded SND3 events into runtime, then capture cadence/overlap |
