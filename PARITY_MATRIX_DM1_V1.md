...
| TITLE animation bank | Greatstone DM PC 3.4 TITLE reference lists 59 items: `AN`, `BR`, `P8`, first `PL`, first `EN` + 36 `DL`, second `PL`, second `EN` + 15 `DL`, then `DO`; local original `TITLE` is 12002 bytes. Pass 56 parser/player probes verify 2 EN + 51 DL = 53 original 320×200 frames, palette break segmentation 37 + 16 frames, and `DO` stop semantics. Optional Greatstone source-PNG probe checks the 56 downloaded reference PNGs (P8/PL + EN/DL only) and 53 frame dimensions. | Loader/player landed in `title_dat_loader_v1.[ch]` with focused probes/evidence. Runtime/frontend drawing is not wired yet, and EN/DL pixel decode/compositing is not claimed beyond source item progression/dimensions. | `KNOWN_DIFF` (narrowed) | Decode/render EN/DL payload pixels into V1 title frontend and capture timing/cadence before claiming full title animation parity. |
