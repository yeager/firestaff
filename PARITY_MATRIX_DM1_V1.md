...
| Dialog/endgame overlays | ReDMCSB `DIALOG.C:F0427_DIALOG_Draw` expands the original dialog-box graphic into the viewport, prints `V3.4` in `C450_ZONE_DIALOG_VERSION`, patches 1/2/4-choice layouts with dialog patch zones, centers up to two message strings, and uses source choice zones/colours. `ENDGAME.C` has separate source flow. | Firestaff has compat stubs (`dialog_frontend_pc34_compat.*`, `endgame_frontend_pc34_compat.*`) and runtime flags. Pass 107 suppresses the most obvious placeholder/debug labels in default V1; passes 109–110 now draw the source `C000_GRAPHIC_DIALOG_BOX` backdrop (GRAPHICS.DAT graphic 17, 224×136) at viewport origin and print `V3.4` at reconstructed C450 `(192,40)`. Remaining dialog composition is still incomplete: no source choice patch zones/input flow and message centering/wrapping is not source-metric accurate yet. Endgame remains placeholder. | `KNOWN_DIFF` (narrowed) — source dialog backdrop + version-zone text are wired, but full dialog/endgame visual parity remains open. | Wire source dialog patch/choice zones and message centering; then capture original dialog/endgame frames for overlay comparison. |
