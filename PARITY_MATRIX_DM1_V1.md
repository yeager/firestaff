...
| Menu screen | M9 beta harness reaches menu state; submenu matrix exists. Pass 68 candidate records startup-menu candidate asset metric windows from GRAPHICS.DAT: A `304..319`, B-left `360..367`, B-core `368..383`, B-right `384..391`, B-wide `360..391`. | Menu interaction verified via M9 verify gate; submenu drift removed. Pass 68 candidate adds `run_firestaff_v1_title_menu_cadence_layout_probe.sh`, which verifies the menu candidate graphic windows exist and records their dimensions/serial-width sums for overlay groundwork. | `UNPROVEN` (narrowed) — source-backed menu asset windows are identified, but 320×200 original placement/layout is not measured against original. | Capture original menu frame and overlay against the candidate-B menu window metrics; then bind exact placement. |
