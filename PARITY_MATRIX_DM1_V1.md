...
| Sound samples (content) — GRAPHICS.DAT SND3 SFX bank | 33 SND3 items (indices 671-675, 677-685, 687-693, 701-712) at 6000 Hz, per dmweb; Greatstone Sound 00..32 labels; real-file decode verified Pass 51 (`PASS51_AUDIO_FINDINGS.md`, `parity-evidence/pass51_v1_graphics_dat_snd3_probe.txt`) | Format + decoder landed (`graphics_dat_snd3_loader_v1.[ch]`); 6/6 probe invariants PASS; **not** mapped to sound events or wired into runtime yet | `KNOWN_DIFF` (narrowed) | Build sound-event → SND3 index mapping, then wire decoded SFX into runtime with asset-gated fallback |
