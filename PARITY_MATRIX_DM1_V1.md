...
| TITLE animation bank | Greatstone DM PC 3.4 TITLE reference lists 59 items: `AN`, `BR`, `P8`, first `PL`, first `EN` + 36 `DL`, second `PL`, second `EN` + 15 `DL`, then `DO`; local original `TITLE` is 12002 bytes. Pass 56 parser/player probes verify 2 EN + 51 DL = 53 original 320×200 frames, palette break segmentation 37 + 16 frames, and `DO` stop semantics. Pass 57 decodes original TITLE IMG1 EN/DL payloads, composites DL transparent skips over the 320×200 V1 canvas, applies PL palettes, and matches all 53 rendered RGB frames byte-for-byte against local Greatstone source PNG references. | Loader/player plus bounded original-data renderer landed in `title_dat_loader_v1.[ch]` with focused probes/evidence. Runtime/frontend drawing is not wired yet; frame timing/cadence and title-menu handoff are not claimed. | `KNOWN_DIFF` (narrowed) | Wire pass-57 renderer into V1 title frontend and capture/source-prove timing/cadence before claiming full title animation parity. |
