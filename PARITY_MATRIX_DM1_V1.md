...
| Text-tag fallbacks | `TEXT.C`/`TEXT2.C`/`PANEL.C`/`CHAMPION.C`/`SPELDRAW.C` distinguish text-engine vs bar-graph vs graphic | Pass 43 landed source-faithful V1 HP/stamina/mana bar graphs (`CHAMDRAW.C` / recovered ZONES.H geometry, source color table `{7,11,8,14}`); spell-panel rune labels still render as text instead of `C011_GRAPHIC_MENU_SPELL_AREA_LINES` | `KNOWN_DIFF` — narrowed by pass 43; champion bar-graph half is landed, rune-label half remains. See `parity-evidence/pass43_bar_graphs.md` | Retire champion-bar-graph half; route rune labels through C011 graphic blit in pass 44. |
