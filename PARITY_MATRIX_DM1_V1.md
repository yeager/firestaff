...
| Dialog/endgame overlays | ReDMCSB `DIALOG.C:F0427_DIALOG_Draw` expands the original dialog-box graphic into the viewport, prints `V3.4` in `C450_ZONE_DIALOG_VERSION`, patches 1/2/4-choice layouts with dialog patch zones, centers up to two message strings, and uses source choice zones/colours. `ENDGAME.C` has separate source flow. | Firestaff has compat stubs (`dialog_frontend_pc34_compat.*`, `endgame_frontend_pc34_compat.*`) and runtime flags. Pass 107 suppresses the most obvious placeholder/debug labels in default V1; passes 109–110 draw the source `C000_GRAPHIC_DIALOG_BOX` backdrop (GRAPHICS.DAT graphic 17, 224×136) at viewport origin and print `V3.4` at reconstructed C450 `(192,40)`; pass 112 centers message text across the source viewport region using text measurement. Remaining dialog composition is still incomplete: no source choice patch zones/input flow and exact C469/C471 vertical placement/line splitting is not fully source-metric accurate yet. Endgame remains placeholder. | `KNOWN_DIFF` (narrowed) — source dialog backdrop + version-zone text + centered message text are wired, but full dialog/endgame visual parity remains open. | Wire source dialog patch/choice zones and exact source message-zone placement/splitting; then capture original dialog/endgame frames for overlay comparison. |
...
**Bottom line (updated 2026-04-25, pass 113):** Firestaff has measured ownership progress (passes 29–32), measured visual drift (passes 33–34), a complete text-vs-graphics enumeration (pass 35), source-faithful champion status-box spacing (pass 41), source-matched DM1 viewport rectangle bounds via the all-graphics phases, source-bound action/inventory object icons plus C011 spell label cells, a narrowed inventory-screen row backed by deterministic capture evidence, narrowed ornament metadata/draw-path coverage, and the invented map overlay is explicitly `KNOWN_DIFF` but now debug-only and unreachable in normal V1 parity play. Dialog visuals are now narrowed further with source backdrop + `V3.4` version-zone text + centered message text, but full dialog/endgame parity is still open. **This still does not complete DM1/V1 parity**; remaining work is viewport content/draw-order parity, original screenshot overlays, audio/timing, remaining source dialog/endgame visuals, and non-current surfaces such as pointer/held-object icons and exact inventory/spell-panel/ornament placement overlays.
