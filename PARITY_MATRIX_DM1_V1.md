...
| `KNOWN_DIFF` | 12 — prior palette + typography + Firestaff-invented UI chrome, plus the now-narrowed inventory-screen and ornament rows. Viewport −28×−18 drift from pass 33/40 was superseded by all-graphics phases 76–81; champion status-box +8 px stride retired in pass 41. |
| `UNPROVEN` | ~34 — reduced from ~35 after the ornament row moved from generic unknown to `KNOWN_DIFF (narrowed)` based on source-backed ornament metadata/draw-path gates. |
...
**Bottom line (updated 2026-04-25, pass 101):** Firestaff has measured ownership progress (passes 29–32), measured visual drift (passes 33–34), a complete text-vs-graphics enumeration (pass 35), source-faithful champion status-box spacing (pass 41), source-matched DM1 viewport rectangle bounds via the all-graphics phases, source-bound action/inventory object icons plus C011 spell label cells, a narrowed inventory-screen row backed by deterministic capture evidence, and narrowed ornament metadata/draw-path coverage.  **This still does not complete DM1/V1 parity**; remaining work is viewport content/draw-order parity, original screenshot overlays, audio/timing, and non-current surfaces such as pointer/held-object icons and exact inventory/spell-panel/ornament placement overlays.
