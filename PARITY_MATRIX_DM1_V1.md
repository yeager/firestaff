...
| Dialog/endgame overlays | ReDMCSB `DIALOG.C:F0427_DIALOG_Draw` expands the original dialog-box graphic into the viewport, prints `V3.4`, patches 1/2/4-choice layouts with dialog patch zones, centers up to two message strings, and uses source choice zones/colours. `ENDGAME.C` has separate source flow. | Firestaff has compat stubs (`dialog_frontend_pc34_compat.*`, `endgame_frontend_pc34_compat.*`) and runtime flags, but current visible overlays are invented/simple surfaces (`TEXT PLAQUE`, `QUEST COMPLETE`, tick text, “PRESS ANY KEY...”) drawn directly in `m11_game_view.c`, not the source dialog-box graphic/zone pipeline. | `KNOWN_DIFF` — functional placeholder overlays, not DM1 V1 visual parity. | Wire the source dialog/endgame frontend graphics or hide placeholders from parity claims; then capture original dialog/endgame frames for overlay comparison. |
