...
static int try_candidate(
    ...
    struct MemoryGraphicsDatMenuRenderEffectResult_Compat* renderResult,
    ...
    ok = F0479_MEMORY_RunMenuRenderEffectMini_Compat(
        graphicsDatPath,
        &fileState,
        dialogGraphicIndex,
...
int main(int argc, char** argv) {
    ...
    for (dialogIndex = dialogStart; dialogIndex < dialogLimit; ++dialogIndex) {
        for (viewportIndex = 0; viewportIndex < viewportLimit; ++viewportIndex) {
            ...
            attempts++;
            if (!try_candidate(graphicsDatPath, dialogIndex, viewportIndex, &renderResult, &header)) {
                continue;
            }
            printf("ok\n");
            ...
            printf("initialSelectionIndex=%u\n", renderResult.menuState.initialSelectionIndex);
            printf("finalSelectionIndex=%u\n", renderResult.menuState.finalSelectionIndex);
            printf("renderVariantCount=%u\n", renderResult.renderVariantCount);
            printf("selectedRenderVariant=%u\n", renderResult.selectedRenderVariant);
            printf("highlightedGraphicIndex=%u\n", renderResult.highlightedGraphicIndex);
            printf("handledTickCommandCount=%u\n", renderResult.menuState.dispatch.inputQueue.typedQueue.handledTickCommandCount);
            F0479_MEMORY_FreeMenuRenderEffectMini_Compat(&renderResult);
            F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
            return 0;
        }
    }

    fprintf(stderr, "failed: no menu-render-effect candidate succeeded in %u attempts\n", attempts);
    F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
    return 1;
}
