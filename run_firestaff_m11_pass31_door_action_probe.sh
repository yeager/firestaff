...
OUT_DIR=${1:-$HERE/verification-m11/pass31-door-action}
mkdir -p "$OUT_DIR"

PROBE_BIN="$HERE/firestaff_m11_pass31_door_action_probe_bin"
LOG="$OUT_DIR/pass31_door_action_probe.log"

cc -std=c99 -Wall -Wextra -O1 \
    -I "$HERE" \
    -o "$PROBE_BIN" \
    "$HERE/firestaff_m11_pass31_door_action_probe.c" \
...
echo "Pass 31 door action probe: $SUMMARY"
