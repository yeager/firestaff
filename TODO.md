...
- 🐛 DM1 V1 D0L2/D0R2 stairs/pit dispatch parity worker `firestaff-parity/1780882306_dm1_v1_d0l2_d0r2_stairs_pit_dispatch` exited with status `failed` at 03:43:35 (734s) without producing a commit. The three sibling workers (DM1 coding chest teleport-survival, CSB parity D3L2/D3R2 F0115, DM1 parity D1R2 wall) all committed and integrated successfully. Need to investigate the parity worker's failure transcript to see whether it hit a ReDMCSB anchor that does not exist or a build/test conflict with the D0L/D0R wall work that landed earlier, and either retry the gate or close it as `BLOCKED_PARITY_D0L2_D0R2_STAIRS_PIT_DISPATCH_DEFERRED` until the upstream blocker is resolved.
