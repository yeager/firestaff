...
- 🐛 2026-06-16 csb_v2_texture_upscale_pc34 test-bug: `t_epx_2x` (and downstream 9square/panel/V22 EPX checks) expect nearest-2x output from `csb_v2_upscale_epx()` but the actual EPX rule returns the source pixel when neighbour conditions are not met (`(A==B && A!=C && B!=D) ? B : P` is the only branch that picks B). For src={10,20,10,20}, dst[1] is 10, not 20. The same test-bug is latent in `dm1_v2_upscale_epx()` (CSB and DM1 V2.1 share the rule). Local verification: `ctest -R csb_v2_texture_upscale` 23/29 (6 fail: EPX top-left block, EPX top-right block, EPX row 1 left, 9square EPX 2x, panel EPX 2x, EPX ran under V22). Fix: either change the test expectations to `{10,10,10,10, 10,10,10,10, 10,10,10,10, 20,20,20,20}` (per the actual EPX rule on this pattern) or change the EPX rule to nearest-2x when the EPX neighbour predicate fails. The second is the safer CSB-PC34 choice because the original CSBWin/Viewport.cpp EPX path (cited in the source header) follows the same "no match -> P" rule.
