...
- 🐛 P1 visual bugs needing capture/repro: missing or incorrect viewport walls and champion Z-order/floating. The known Hall champion-mirror visibility route is covered by `dm1_v1_champion_mirror_visibility_runtime`, and the east-facing Hall D1C/no-floating side slice is covered by `dm1_v1_champion_mirror_zorder_runtime`; keep broader mirror/Z-order capture evidence open until more routes are pixel-proved.
...
- 🔧 Scanner performance around Theron: avoid rehashing the full default data root on every direct launch when the selected Theron hash/path is already known from startup/menu state.
