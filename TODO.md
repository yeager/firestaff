...
- 🔧 Original DOS capture parity: five specific paired evidence sets are blocked. Details and honest status labels at `docs/parity/DM1_V1_CAPTURE_GAP_EVIDENCE.md`. Minimum runbook at `docs/parity/DM1_V1_ORIGINAL_CAPTURE_RUNBOOK.md`.
  ...
  - Champion-panel: Firestaff V1 captures exist (party_hud_four_champions_vga.ppm, party_hud_statusbox_gfx_vga.ppm), `dm1_v1_champion_panel_pixels_runtime` covers a deterministic Firestaff-side real-asset HUD/status-box pixel slice, and `dm1_v1_champion_panel_status_states_runtime` extends it with the C008 dead status box, C032 POISONED label, C037/C038/C039 shield border stack, C015 damage indicator, status hand icon zone, and V1 name text glyph pixels; no paired original DM1 PC 3.4 champion panel screenshot exists.
...
- 🔧 Viewport rendering hardening: broad wall/floor/ceiling/door/ornament/pit/stair/source routes are source-locked, and D2C/D2L, D2L2/D2R2, D1C/D1R, D0L, and D0L/D0R parity pixels now have concrete gates; remaining work is broader pixel-polish evidence (D3L2/D3R2) and capture-backed bug closure.
- 🔧 Inventory/chest polish beyond source-locked routes: core slot/body/chest/backpack/source routes, compact-close edge cases, and C537/C538/C539 real-asset icon pixels are implemented; remaining work is broader chest runtime detail coverage and pixel-polish evidence.
...
- 🔧 Phase 3 - Rendering parity hardening: D3/D2 wall tables, bitmap selection, grid routing, CSB-only D3L2/D3R2 and D2L2/D2R2 draw-order/frame gates, F0107 back-wall ornament routing, F0108/F0111 route gates, F0108 floor-blit metadata gates, F0111 door panel zone/clip gates, F0115 projectile metadata gate, and initial viewport gates exist; remaining work includes actual ornament blits, F0115 creature/item/explosion proof, F0111 runtime door panel bitmap blits, and `CustomBackgrounds`.
