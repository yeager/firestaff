...
- 🐛 P1 visual bugs needing capture/repro: missing or incorrect viewport walls and champion Z-order/floating. The known Hall champion-mirror visibility route is covered by `dm1_v1_champion_mirror_visibility_runtime`, north/south/east Hall D1C plus west-facing no-floating slices are covered by `dm1_v1_champion_mirror_zorder_runtime`, and the C040 resurrect/reincarnate panel pixel/overlap/select-miss/reincarnate-click/rest-guard/inventory-toggle/spell-rune/portrait-reselect/status-box-click/save-input/C159 champion-icon guard routes are covered by `dm1_v1_champion_mirror_candidate_panel_runtime`; keep broader mirror/Z-order/candidate capture evidence open until more routes are pixel-proved.
...
- 🔧 Phase 3 - Rendering parity hardening: D3/D2 wall tables, bitmap selection, grid routing, CSB-only D3L2/D3R2 and D2L2/D2R2 draw-order/frame gates, F0107 back-wall ornament routing/blit/side-effect gates, F0108/F0111 route gates, F0108 floor-blit metadata gates, F0111 door panel zone/clip/partially-open-zone/runtime bitmap/destroyed-mask blit gates, F0115 projectile metadata/runtime synthetic blit, creature/item/explosion metadata gates, teleporter-field route gates, `CustomBackgrounds` room-slot/bitmap-application source-lock gates, the `CustomBackgrounds` viewport backdrop mask/enumeration source-lock gate, and initial viewport gates exist; remaining work is broader capture-backed bug closure and real-asset viewport integration evidence.
