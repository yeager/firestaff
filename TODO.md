...
- 🔧 DM2 New Game: ett NEW GAME-val bygger nu först den verkliga, tomma
  ...
  dörrornamentlistorna följer också med samma karta och byggs nu i den
  verkliga M11 NEW GAME-vägen. Varje efterföljande mirror-klick kan
...
- ✅ **DM1-SWSH-PSG-ORIGINAL-AUDIO:** 2026-08-08 — PCM renderer replaced with
  cycle-accurate YM2149 PSG emulation: proper 17-bit LFSR noise generator
  clocked at master_clock/(16*NP) via fixed-point 16.16 fractional accumulators,
  hardware envelope generator clocked at master_clock/(256*EP) with shape 0x09
  (single decay 15→0 hold) and 0x0D (decay-attack repeat), envelope counter
  reset on reg 0D write per real YM2149 behavior. All 34 swoosh tests pass.
  Remaining: original Atari ST capture for waveform regression comparison.
