...
- 🔧 Phase 3 - Rendering parity hardening: D3/D2 wall tables, bitmap selection, grid routing, CSB-only D3L2/D3R2 and D2L2/D2R2 draw-order/frame gates, D3L2/D3R2 and D2L2/D2R2 wall gates, F0107 back-wall ornament routing/blit/side-effect gates, F0108/F0111 route gates, F0108 floor-blit metadata and footprint-recursion source-lock gates, F0111 door panel zone/clip/partially-open-zone/runtime bitmap/destroyed-mask blit gates, F0115 projectile metadata/runtime synthetic blit, creature/item/explosion metadata gates, teleporter-field route gates, `CustomBackgrounds` room-slot/bitmap-application source-lock gates, the `CustomBackgrounds` viewport backdrop mask/enumeration source-lock gate, the `CustomBackgrounds` first backdrop source-lock gate (room 0 / rel fwd=3 side=-2, non-overlap with second backdrop), and initial viewport gates exist; remaining work is broader capture-backed bug closure and real-asset viewport integration evidence.
