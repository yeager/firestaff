...
- ✅ 2026-08-11 CSB FM Towns real user-save corpus boundary: added the opt-in
  ...
  materializes its verified map-0 `(13,13)` state. The M11 regression now
  starts that resumed F31J session, runs F0433/F7052/F7062 writeback against
  its own canonical slot, then reopens it through F0435; a MINI.DAT session
  cannot overwrite the distinct slot. Both checks retain the source file's
...
- ✅ 2026-08-11 CSB FM Towns native CLI launch regression: added the opt-in
  ...
  selected CSB entry reaches the native F31 handoff without treating
  boot-probe's intentional direct-launch mode as menu coverage. The new
  `--csb-fmtowns-ja` selector binds only the matched `fmtowns-ja` cache row;
  the same test proves that F31J does not silently fall back to F31E. It
