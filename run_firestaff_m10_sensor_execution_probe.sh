...
OUT_DIR=${2:-$HERE/verification-m10/sensor-execution}
ROOT=$HERE

mkdir -p "$OUT_DIR"

PROBE_BIN="$ROOT/firestaff_m10_sensor_execution_probe_bin"

cc -Wall -Wextra -O1 \
    -o "$PROBE_BIN" \
    "$ROOT/firestaff_m10_sensor_execution_probe.c" \
...
echo "Sensor execution probe complete. Output: $OUT_DIR"
