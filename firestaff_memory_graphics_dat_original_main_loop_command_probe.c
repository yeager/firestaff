...
static int try_candidate(
    ...
    struct MemoryGraphicsDatMainLoopCommandResult_Compat* commandResult,
    ...
    ok = F0479_MEMORY_RunMainLoopCommandMini_Compat(
        graphicsDatPath,
        &fileState,
        dialogGraphicIndex,
...
int main(int argc, char** argv) {
    ...
    for (dialogIndex = dialogStart; dialogIndex < dialogLimit; ++dialogIndex) {
        for (viewportIndex = 0; viewportIndex < viewportLimit; ++viewportIndex) {
            ...
            attempts++;
            if (!try_candidate(graphicsDatPath, dialogIndex, viewportIndex, command, &commandResult, &header)) {
                continue;
            }
            printf("ok\n");
            ...
            printf("command=%d\n", (int)commandResult.command);
            printf("commandStage=%d\n", (int)commandResult.stage);
            printf("commandReady=%d\n", commandResult.commandReady);
            printf("commandHandled=%d\n", commandResult.commandHandled);
            printf("processedCommandCount=%u\n", commandResult.processedCommandCount);
            printf("startupTickCompleted=%d\n", commandResult.startupTick.startupTickCompleted);
            printf("mainLoopEntered=%d\n", commandResult.startupTick.mainLoop.mainLoopEntered);
            F0479_MEMORY_FreeMainLoopCommandMini_Compat(&commandResult);
            F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
            return 0;
        }
    }

    fprintf(stderr, "failed: no main-loop command candidate succeeded in %u attempts\n", attempts);
    F0479_MEMORY_FreeGraphicsDatHeader_Compat(&header);
    return 1;
}
