...
| Macintosh large retail | **Partial native route** | Authentic English retail ZIP is read in RAM. HFS, big-endian dungeon data, New Game, bounded wall input, and an accepted real-GDAT M11 frame with zero fallback draws pass. Its 256-row `PalIRGB`/`dtPalette16` pair remains distinct from the Amiga physical 16-colour palette route. A native QuickTime reader admits exact original `moov`/`mdat` sample spans for all four retail films, and `Swoosh.MooV` plays its 16-bit Animation RLE/`raw ` PCM lane without dependencies. Cinepak-based `Title.MooV` still requires optional FFmpeg, so the normal dependency-free build must not present the title/movie route as verified. | Add bounded built-in Cinepak and `twos` PCM decoding; then complete Mac GAME_LOAD/Resume, dynamic pointer/drag owner, MIDI timing and full pixel/audio parity. No authentic Mac save is present. |
