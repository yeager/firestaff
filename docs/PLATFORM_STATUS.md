...
|---|---|---|---|
| PC DOS 3.4 | **Playable** | V1 startup, menus, viewport, HUD, input, combat and saves use matching original data. The start-menu receipt reports the admitted IMG3 decoder only after the original `GRAPHICS.DAT` and `DUNGEON.DAT` pair bind. | Broader original-vs-Firestaff capture and V2 finished material. |
| PC DOS French SFX/RAR2 | **Playable after manual extraction** | The supplied original compressed package is detected and reports that RAR2 is unsupported. Its manually unpacked authentic `EUDATA` reaches `dm1-runtime` through direct CLI and start menu; the actual launcher receipt admits the matching `IMG3` source decoder only after the original graphics and dungeon pair bind. | Firestaff will not decode or extract RAR2 itself. |
| Atari ST | **Verified route** | Native ST/STX media is read in RAM. The supplied English 1.0a, English 1.2, German 1.2 and French 1.3 archive routes reach the bounded runtime through direct CLI and start menu; each selected `GRAPHICS.DAT` is hash-locked. The menu receipt reports the admitted `DMCSB1` Atari decoder only after its original graphics and dungeon pair bind. | Complete gameplay parity and original-media capture. |
