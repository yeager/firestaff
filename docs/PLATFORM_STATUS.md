...
| Macintosh small First Chapter demo | **Partial source route** | Authentic demo ZIP is read in RAM and reaches its static source startup menu with the truncated dungeon and 16-entry roster retained. | The real-media New Game/GAME_LOAD handoff is not yet verified: its current source action does not publish a runtime session. Dynamic Mac Control/Event ownership and Resume also remain closed because the authoritative ZIP has no application fork. |
