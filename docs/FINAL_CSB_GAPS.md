...
| # | Title | Status | Source |
|---|-------|--------|--------|
| 1 | VBL handler fix (BUG0_03) | **FIXED — source-modelled and live-runtime bound.** `csb_v1_atari_st_vblank_deliver` mirrors `BASE.C:E0017` / `CHANGE7_01_FIX`: palette setup starts on every received VBlank, re-entrant arrivals increment the pending count, and the outer handler drains the count at priority 3. M11 advances that handler both during the original `ANIMATE.SCR` title route and after FTLCODE enters the dungeon; its palette-start callback installs only the source-selected C232 light palette. `csb_v1_atari_st_vblank` proves retained concurrent arrivals, `csb_v1_atari_stx_native_cli_boot` covers the authentic STX title/runtime/menu input route, and `csb_v1_atari_st_m12_m11_real_media_handoff` proves the real M12→M11 handoff. Pixel/audio capture comparisons remain separately open in `TODO-csb.md`. | BASE.C / VBLANK.C, CHANGE7_01_FIX |
