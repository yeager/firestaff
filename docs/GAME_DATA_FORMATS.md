...
| FM Towns `CSBGAME.DAT` / `.BAK` | F31 save image: a C5 header, five checksummed save parts, active groups, champions/party, events, timeline, four portraits and a dungeon tail. `CDATA/MINI.DAT` and `CJDATA/MINI.DAT` are bootstrap images, not user saves. | Verified F0435-style read/resume and backup recovery are bound. F0433 output remains closed: the live runtime does not retain every raw field and no authentic before/after save pair exists. |
