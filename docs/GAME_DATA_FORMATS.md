...
| FM Towns `CSBGAME.DAT` / `.BAK` | F31 save image: a C5 header, five checksummed save parts, active groups, champions/party, events, timeline, four portraits and a dungeon tail. `CDATA/MINI.DAT` and `CJDATA/MINI.DAT` are bootstrap images, not user saves. | The reader and canonical-slot backup recovery are fail-closed. The current external EN/JP files pass header and part checks but pair a two-map Prison tail with an impossible saved map index, so they are rejected before runtime admission. F0433 output remains closed. |
| CSBWin `csbgame*.dat` | GAMEBLOCK body variants and a legacy saved-dungeon stream. Firestaff can authenticate the body, timer widths and tail records, then materialise a private candidate dungeon. | Read-only candidate preparation is bound. Publishing a CSBWin dungeon requires one atomic owner transaction for dungeon, party, timers and inventory, so resume remains closed. |
| Utility portraits | Amiga Utility ADF, Atari Utility MSA and FM Towns C06 portrait files are separately identified media. FM Towns `.CMP` admission checks its original header fields before use. | Supported import/editor slices are bound; a full native file-picker and save transaction are not claimed. |
