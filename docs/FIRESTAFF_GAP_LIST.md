...
| V2 modern UI overlay polish (inventory/champion/rune/action) | PARTIAL / OPEN-LARGE — 2026-06-21: V2 HUD interaction mirrors the source-owned V1 champion/action/rune routes, including spell parent/caster/rune/cast/recant (`C100`, `C101`..`C109`) via `dm1_v2_hud_interaction_pc34`; the HUD overlay now adds a data-free champion/action/rune presentation-state API plus deterministic pixel assertions for champion bars, leader/spell-ready cues, action highlight/flash decay, rune/cast/recant/caster strips, clipping, clear, and invalid-input behavior. CTest/source-lock group PASS 5/5: HUD overlay, HUD overlay source lock, HUD interaction, HUD interaction source lock, and UI overlay affordance routes. Remaining: finished art/assets, fit/animation quality, inventory-panel visual polish, and broader real runtime screenshot/pixel gates. |
