...
| CSB X68000 (jp) | PARTIAL — raw 2DHD HDM root access is CTest-gated through `csb_v1_x68k_hdm`: the local v3.1 cracked HDM proves `GRAPHICS.DAT` (373,583 bytes), `DUNGEON.DAT` (2,066 bytes), `ENTER.SNG`, `AUTOEXEC.BAT` and `CHAOS_ST.X` extraction through Human68k FAT12 chains. The joined `csb_v1_x68k_hdm_source_media_receipt` validates the same image's graphics, two-map dungeon and initial pose (level 0, x=9, y=0, direction 2), format-1 MIDI (9 tracks, 480 TPQN, 3,205 events), AUTOEXEC command order (`CK`, `VIDSET`, `CHAOS_STRIKES_BACK`) and the `CHAOS_ST.X` Human68k `HU` layout (64-byte header, 9,020 text bytes). `csb_v1_x68k_graphics_handoff` verifies the complete X68000 `GRAPHICS.DAT` DMCSB2 table as 732 bounded direct source records and decodes original post 13 to its native 96×41 indexed-pixel receipt. `M11_AssetLoader_InitCsbX68kFromHdm` extracts `GRAPHICS.DAT` from the raw HDM into a distinct native cache and can render that same post without letting the shared format claim Amiga identity; this is not an X68000 boot profile and does not establish media authenticity. The receipt and program disassembly are input evidence only: neither runs the cracked program nor infers that its startup command resolves to a particular root file. DMWeb's original-media, `CK.R` crack, 24/31 kHz presentation, Champion Editor/portrait-loading and Ctrl-S / Opt.1 / Opt.2 boundaries still need classification or runtime evidence. |
