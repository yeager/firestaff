...
| CSB PC-98 3.1 (jp) | PARTIAL / RECEIPTED — the local original and cracked HDM corpus both classify as `pc98-2hd-raw` CSB from actual FAT12 root slots (`CJDATA` + `CSBGAME`), with the PC-98 1024-byte-sector geometry verified. Original-versus-cracked remains intentionally unknown until FAT-chain extraction reaches the executable payload; public original media is documented as missing its copy-protection sector. Remaining: extracted-file hashes, 8-bit/16-bit and three-light-level receipts, keypad/Alt-S input, Champion Editor/portrait path, and native runtime launch. |
