...
| **QuickTime `MooV` / `.mov` (DM2 Macintosh)** | greatstone d_articles_mac.html / dmweb Animations | PARTIAL — the retail HFS reader now keeps authentic data and resource forks in RAM and extracts each present `moov` resource without flattening. The four present movies are source-bound; `Story.MooV` is absent. Original QuickTime 1.x/2.x playback, Cinepak (`cvid`) and Animation (`rle`) decoding, timing, audio, and presentation remain open. |
