...
| **Hint Oracle text and graphics archives (CSB)** | dmweb Hint Oracle Files / ReDMCSB HINTHTC.C,HINTLZW.C,HINTGRAP.C,HINTSCR.C,EXPAND.C | PARTIAL / OPEN-BOUNDED — Hash-verified HTC parsing, caching, page decoding and structured UI binding are present for the five dmweb-documented variants: English R1/R2/R3, Amiga French and Amiga German. `csb_hint_oracle_htc_real_scan_and_load_md5()` and `csb_hint_oracle_ui_panel_load_md5()` select an exact registered source MD5; they reject arbitrary values and never silently fall back to another Utility Disk. `csb_hint_oracle_dat_real_scan_and_load()` applies the same hash-only rule to HCSB.DAT (EN R1/RX/ST2, EN R2/R3, FR, DE), including archive-member reads without copying game data into the library. The real ST 2.0/2.1 asset is admitted by `708e113c869ab922633e885aa72a3c77`, exposes four exact segments (100/29146/1497/32 bytes), and now yields the Oracle's 320×200 4-bit pixels plus its 16-color RGB4 palette. The latter follows `HINT001.C`'s exact 3-bit→4-bit conversion of segment 3, not a host-selected palette. Unit tests cover fills, literals, malformed streams and palette mapping; the real probe verifies the original asset. Graphical UI, M11/M12 routing, original-frame capture and pixel parity remain open. |
