...
| Paired original creature-chain screenshot | PARTIAL — 2026-06-21: ran `scripts/dm1_v1_original_capture.py --pair 04_creature` in this session. DOSBox launched, window found, selector sequence (GRAPHICS=1 / SOUND=1 / CONTROL=4) executed, 7 forward-step captures produced (`0e6eeb...`, `643b0f...`, `11b29f...`, `6d94c3...`). Capture artifacts at `/tmp/dm1_test_capture/04_creature/` (size 320x200 + 224x136 viewport crops + 1024x800 DOSBox-window crops). Caveat: pass80 classifier marked all 7 captures `entrance_menu` not `dungeon_gameplay`, indicating the selector sequence did not progress into the dungeon in this session — the captures are of the title/entrance sequence, not the actual dungeon creature encounter. Real creature-chain captures still require a session where the selector-to-dungeon hand-off completes. The corrected level-1 route per pass1058 (kp5/kp8/kp4/kp6) plus the runbook's host-mouse-click requirement remain the next step. Downstream pass86 classifier + semantic-promotion gates are ready and waiting. |
...
| Dungeon model/mechanics | csb_gap_dungeon.md | PARTIAL — 2026-06-21 PC launch boundary now has a positive real-data gate: `csb_v1_pc_real_asset_launch` verifies canonical PC CSB assets scan by hash, enter `csb_v1_boot_enter_game()`, load `DUNGEON.DAT` into the runtime-owned dungeon singleton, select map 0, tick once, and clean up. Core PC runtime/input/movement/system slices are now CTest-registered through command-chain, input-queue, movement step/rotation, runtime tick, queue overflow, reincarnation, projectile-speed, Grey Lord, DECOMPDU, version-checker, monster-generator, chaos cast, and DSA trigger gates. Remaining dungeon/mechanics work is deeper end-to-end gameplay parity, save-path hardening beyond current smoke gates, viewport/UI runtime evidence, and playability, not the PC asset handoff. |
