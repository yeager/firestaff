...
**Nexus status correction, 2026-08-07:** The older BPX/BPK row above still
describes PRS3 as an unidentified compression algorithm. That is no longer
accurate for the verified retail `MENU.BPK`: `test_nexus_v1_bpk_archive` now
decodes all 162 real PRS3 surfaces and checks each result against its declared
pixel count. The remaining gap is Saturn presentation ownership: VDP1 command
order, VDP2/CLUT binding, menu placement and screen identity. No decoded host
pixels are promoted to production drawing until those observations exist.
