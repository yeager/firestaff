...
| DM2 Macintosh 1.0 (en/jp/demo) — uses QuickTime .moov | PARTIAL / TWO US EN VERSIONS BOOT-GATED — the large retail ZIP (`mac-en-retail`) and smaller First Chapter demo ZIP (`mac-en-demo`) now read their authentic HFS/StuffIt data in RAM and pass separate hash-paired boot tests; the demo uses the 6,535-byte truncated big-endian File_header/map member. The English Macintosh keyboard/menu table is now source-locked and CTest-gated (`dm2_v1_mac_input`), with queue-compatible actions forwarded to the existing command boundary and Mac-only wall/menu actions kept explicit. Japanese and French remain preservation inputs. DMWeb's Macintosh edition page identifies JP, USA, and USA demo as redump BIN/CUE CD images plus CD-content archives. Remaining: native Mac dispatcher/held-input, audio/video owners, balloon help, save corpus, plus JP/FR runtime proof. |
