...
| **AMG sound format (CSB utility disk)** | dmweb Data Files / local original CSB Amiga Utility Disk corpus | FIXED for documented CSB Utility Disk sound-effect files — `firestaff_amg_decode_unit` and `csb_v1_amg_sound` parse the authenticated SND2 shape (`TELE2`, `SWIPE`, `MAGEXPLO`, `EXPLOS1`, `DRAGON`): big-endian sample count, signed 8-bit mono PCM, then 0..3 trailing bytes. Local originals prove payload/trailing pairs `1506/1`, `995/2`, `3107/2`, `3970/1`, and `4003/2`; this corrects the duplicate parser's former fictitious control-word interpretation. PAL/NTSC rate helpers remain source-cited. Runtime playback/rate binding and non-SND2 `NAKED.AMG` are not claimed. |
| **MVE (Interplay, DM2 PC)** | dmweb Animations | OPEN-LARGE — DOS-stub + Interplay MVE binary |
| **QuickTime `MooV` / `.mov` (DM2 Macintosh)** | greatstone d_articles_mac.html / dmweb Animations | PARTIAL — the retail HFS reader keeps authentic data and resource forks in RAM, extracts each present `moov` resource without flattening, and builds a validated in-memory `moov`+`mdat` view for the four present movies (`0x19`; `Story.MooV` is absent). The private view preserves source ownership and handles the authentic four-byte prefix before `Swoosh.MooV`'s `mdat`. The optional FFmpeg-backed in-memory route rebases the private QuickTime chunk offsets and decodes all four present movies' authentic video/PCM paths; real-asset decoder and M11 startup gates pass, and no converted movie is written. M11 now holds each decoded frame for its authentic positive QuickTime duration, and the real-media gate rejects non-monotonic timestamps or zero-duration frames. The small First Chapter demo separately passes an M11 New Game gate from its authentic static route and truncated File_header; it does not borrow retail movie pixels. Exact original QuickTime 1.x/2.x native presentation ownership remains open. The same boot profile retains the authentic Mac `Music`, `General.sounds`, and `Weapon.sounds` resource forks; a bounded format-1 `snd ` parser validates their 2/19/12 resource sets and resource 10001 is hash-checked and transported through the SDL host mixer. Complete resource selection, gameplay timing, and MIDI/music scheduling remain open. |
...
| Gap | Doc | Status |
|---|---|---|
| Champions per-stat parity | csb_gap_champions.md | PARTIAL — 2026-06-26 data-free `test_csb_v1_champion_per_stat_parity_pc34_compat` (CTest `csb_v1_champion_per_stat_parity`, 67/67 PASS) pins every per-stat field of the CHANGE7_24 reincarnation rule against `csb_v1_champion_reincarnate()` in `src/csb/csb_v1_character_pc34_compat.c`: HP/Mana/Stamina halve, six non-Luck stats reduced by 1/8th of current value with minimum-floor clamp, Luck exempt, all 16 skill slots cleared, NEEDS_RENAME attribute set, DEAD attribute cleared, ActionIndex reset to REST, per-champion `reincarnateStatPenalty` divisor (default 8, plus 4/16 and zero-divisor fallback), C160 resurrect-vs-C161 reincarnate panel contract, and alive-champion no-op contract. Companion data-free CTests also cover the older `ChampionState_Compat` reincarnation penalty shim and the seeded `randomPoints` + `reincarnateStatPenalty` interaction with F0309/F0306, including exact `randomPoints=12` and `25` LCG per-stat hit distributions. Remaining: live real-asset F0282 panel integration gate (requires a real CSB Utility Disk `CSBGAME` save + M11 input handler wiring the C040 panel into the champion-mirror sensor). |
...
| Graphics + ornament blits (F0108, F0115, F0111, CustomBackgrounds) | csb_gap_graphics.md | PARTIAL / OPEN-LARGE — 2026-06-21 CTest now covers seven data-free CSB viewport/source-lock slices: first CustomBackgrounds backdrop, room-slot backdrop-1, D1C F0108 floor/ceiling ornament, D1C F0115 thing pass, D3C F0107/F0108 first-backdrop composition, D3L/D3R sidewall backdrops, and D2C F0107 wall-ornament plus F0111 door-front layering. Existing `csb_v1_viewport_phase3_rendering`, inventory-grid, and viewport-inventory mouse gates remain green. 2026-06-29 hardens the existing PC real-asset ornament gate: `csb_v1_pc_real_asset_ornament_blit` now bounds the selected GRAPHICS.DAT bitmap payload, writes a JSON provenance manifest beside the deterministic PPM/SHA capture, and reads the manifest back to assert the schema, verified PC 3.4 GRAPHICS.DAT MD5, capture SHA256, bitmap index/dimensions, D1C floor-band capture rows, F0108/F0115 source anchors, tally fields, and non-claims are present. Remaining: broader viewport/HUD captures, original/pixel parity evidence, and M11 CSBgraphics.dat override binding. |
...
| DM2 Macintosh 1.0 (en/jp/demo) — uses QuickTime .moov | PARTIAL / TWO US EN VERSIONS NEW-GAME-GATED — the large retail ZIP (`mac-en-retail`) and smaller First Chapter demo ZIP (`mac-en-demo`) read their authentic HFS/StuffIt data in RAM and pass separate hash-paired boot and New Game tests; the demo uses the 6,535-byte truncated big-endian File_header/map member and its real 16-entry mirror roster. The English Macintosh keyboard/menu table is source-locked and CTest-gated (`dm2_v1_mac_input`), and an admitted `DM2_PLATFORM_MAC_EN` profile takes precedence over PC aliases in the M11/SDL route for movement, champion/leader inventory, freeze, wake, save and quit. Both editions now pass `dm2_v1_mac_sfx_queue_real`: immediate GAME_LOAD sound rows resolve `xsndptr2 w_00` to authenticated GDAT `w_05` samples and reach the playback backend from the original ZIP in RAM. Retail `MooV` data forks, complete HFS resource forks, and `moov` payloads are retained by the boot profile with exact presence/size receipts for Title, Swoosh, Credits and Ending; Story is absent from the image. Mac wall-button columns now reach a source-owned activation gate for original door-button targets and standalone visible DB3 `0x17`/`0x18`/`0x1A` wall mechanisms; local-action chains rotate through the source path while remote item/target owners remain fail-closed. Japanese and French remain preservation inputs. DMWeb's Macintosh edition page identifies JP, USA, and USA demo as redump BIN/CUE CD images plus CD-content archives. Remaining: complete original QuickTime timing/presentation ownership, remote 0x17/0x1A item/target owners and remaining Mac menu owners, full MIDI scheduling, balloon help, save corpus, and JP/FR runtime proof. |
...
- `tools/verify_dm2_v1_original_overlay_capture_source_lock.py` — read-only source-lock verifier against SKULLWIN (10 anchors across dm2global.h, c_gfx_main.cpp, c_gfx_main.h, c_gui_vp.cpp, c_tmouse.h, types.h, c_input.cpp, c_gui_draw.cpp). SKULLWIN auto-discovered from `~/.openclaw/data/firestaff-dm2-sources/...`, `/Volumes/Extern-disk/openclaw-data/firestaff/skproject-source/SKULLWIN`, or repo-relative fallbacks. CTest-gated as `dm2_v1_original_overlay_capture_source_lock`. 2026-06-29 adds an optional pair-manifest gate for `dm2_original_firestaff_pair_manifest.tsv`: missing means OPEN/no claim, malformed tracked rows fail, and promotion requires same-state `dungeon_gameplay` original+Firestaff 224x136 viewport SHA-256s.
