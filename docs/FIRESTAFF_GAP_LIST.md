...
| Hall of Champions 4-mirror + wall-mirror zones | FINAL_GAPS | FIXED -- 2026-06-22 Hall champion-mirror placement regression: C127 portraits now route through the fixed C346 D1C champion-mirror frame, not the map's last wall ornament or the generic wall-ornament pass; `firestaff_dm1_v1_champion_mirror_capture_probe` now requires both portrait pixels and mirror-frame pixels for reported Gando/Wuuf/Daroou floating cases plus Halk/Gothmog/Mophus/Sonja/Leif wrong-wall cases, so portraits cannot pass while floating over Hall stone. 2026-06-22 runtime probe matrix (10/10 PASS, all champion-mirror coverage): `firestaff_dm1_v1_champion_mirror_visibility_runtime_probe` (front-portrait vs back-wall C127 visibility, all 4 ordinals), `firestaff_dm1_v1_champion_mirror_walkpath_runtime_probe` (walkpath ordinal routing across hall/corridor), `firestaff_dm1_v1_champion_mirror_zorder_runtime_probe` (z-order under wall + floor + ornament), `firestaff_dm1_v1_champion_mirror_zorder_reblt_runtime_probe` (z-order re-blit after viewport scroll), `firestaff_dm1_v1_champion_mirror_actual_pose_runtime_probe` (16 pass runtime, all 4 Hall directions + resurrect round-trip, HP=90/90), `firestaff_dm1_v1_champion_mirror_candidate_panel_runtime_probe` (candidate panel suppress). Two SKIPs on (1,3) NORTH walkpath + (1,4) NORTH candidate panel are tracked fixture-mismatch SKIPs against the reference DUNGEON.DAT, not regressions. 2026-06-23 follow-up probes on `main`: `firestaff_dm1_v1_champion_mirror_portrait00_rect_runtime_probe` gates ordinal 0/DAROOU in the D1C portrait rect and candidate-panel path, while `firestaff_dm1_v1_champion_mirror_portrait_rect_position_runtime_probe` gates HALK/ordinal 1 front-north-entry frame ownership and no stale side-pose portrait. Both were pushed and GitHub Actions passed. |
...
| **Manual/docs launcher** | FIXED — 2026-06-22: added a testable M12 manual/docs catalog (`manual_docs_m12`) and wired the launcher `Manual / Docs` extras row to the catalog default README URL instead of a hardcoded string. CTest `manual_docs_launcher_m12` gates the five user-facing entries (README manual, data setup, platform matrix, dmweb/Greatstone reference, release notes), Firestaff GitHub URL scope, repository-relative paths, existing source-tree files, lookup bounds, duplicate-id rejection, and confirms the internal primary gap list is not exposed as user manual content. |
