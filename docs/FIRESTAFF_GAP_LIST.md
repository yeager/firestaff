...
| DM2 Macintosh 1.0 (en/jp/demo) — uses QuickTime .moov | PARTIAL / TWO US EN VERSIONS BOOT-GATED — the large retail ZIP (`mac-en-retail`) and smaller First Chapter demo ZIP (`mac-en-demo`) now read their authentic HFS/StuffIt data in RAM and pass separate hash-paired boot tests; the demo uses the 6,535-byte truncated big-endian File_header/map member. The English Macintosh keyboard/menu table is source-locked and CTest-gated (`dm2_v1_mac_input`), and an admitted `DM2_PLATFORM_MAC_EN` profile now takes precedence over PC aliases in the M11/SDL route for movement, champion/leader inventory, freeze, wake, save and quit. Mac-only wall actions remain explicit and fail closed. Japanese and French remain preservation inputs. DMWeb's Macintosh edition page identifies JP, USA, and USA demo as redump BIN/CUE CD images plus CD-content archives. Remaining: native Mac held-input, wall/menu owners, audio/video owners, balloon help, save corpus, plus JP/FR runtime proof. |
