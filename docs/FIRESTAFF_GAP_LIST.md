...
| **HTC hint oracle text format (CSB)** | dmweb Hint Oracle Files / ReDMCSB HINTHTC.C,HINTLZW.C | PARTIAL / OPEN-BOUNDED — 2026-06-22 added data-free `csb_hint_oracle_htc_unit`: read-only HCSB.HTC parser for big-endian format word/dungeon id/header skip, location records, hint records, page compressed-length pool, content slicing, level/X/Y matching including the 255/255 any-XY rule, and bounded HTC LZW decompression with the ReDMCSB 0x90 repeat marker. **2026-06-25 real-asset scan/cache handoff:** `csb_hint_oracle_htc_real_scan` + `firestaff_csb_v1_hint_oracle_real_htc_scan_probe` (CTest `csb_v1_hint_oracle_real_htc_scan`, label `tier2;csb;hint_oracle;real_data;skip_safe`) hash-discovers a real Utility Disk HCSB.HTC in the user's data tree, materializes virtual container paths into `~/.firestaff/asset-cache/csbbin/`, owns the parsed view, and round-trips hint-name + first-page decompression against the file. **2026-06-26 runtime-adjacent UI/binding gate:** `csb_hint_oracle_ui_runtime_binding` (include + src/csb) + `firestaff_csb_v1_hint_oracle_ui_runtime_binding_probe.c` (CTest `csb_v1_hint_oracle_ui_runtime_binding`, label `tier2;csb;hint_oracle;real_data;skip_safe`) prove a decoded HCSB.HTC page or page-slice can reach a Firestaff-facing oracle/hint surface: `format_report()` writes a multi-line diagnostic/oracle report (matched MD5/label + format word 2 + dungeon id 13 + location/hint/page counts + hint 0 first-page binding smoke + level-0 (255,255) wildcard binding smoke); `format_hint(hint_index)` writes the hint name + first-page decoded text; `resolve_location(level, x, y)` resolves a (level, x, y) into the first matching hint's name + first-page decoded text. Data-free unit `csb_hint_oracle_ui_runtime_binding_unit` 6/6 PASS (synthetic in-memory fixture, no real asset). Real-asset probe 32/32 invariants PASS on the local Atari ST 2.x HCSB.HTC: `format_hint(0)` writes the parsed hint 0 "FULYA PIT" header + the first-page text starting `"This is where Lord Chaos mined the deadly Mana-absorbing Corbum.."`; `resolve_location(level=0, 255, 255)` resolves to hint 197 "CREATURES" and writes the first-page text starting `"Learn well the shapes and forms dread Chaos make"`; second scan + binding is byte-identical to the first (deterministic). The binding surface is intentionally small: it does not draw a graphical Hint Oracle overlay, it does not bind into the M11 game loop or M12 launch flow, and it does not claim parity for every Utility Disk release variant — the known-hash list in `csb_hint_oracle_htc_real_scan` still gates which HCSB.HTCs we accept; the binding surface itself is variant-agnostic. Verified end-to-end against `~/.firestaff/data/csb-atari-st-2x/HCSB.HTC` (Atari ST 2.x PP 2009-02-22 hard-disk variant, md5 `8ce69b54cf255a15e98e909bb45b9742`, 66172 bytes). **2026-06-28 variant catalog + classification gate:** `csb_hint_oracle_htc_variant` (include + src/csb) + `firestaff_csb_v1_hint_oracle_htc_variant_probe.c` (CTest `csb_v1_hint_oracle_htc_variant`, label `tier2;csb;hint_oracle;real_data;skip_safe`) carry the 5 documented dmweb Hint Oracle Files variants (R1 EN / R2 EN / R3 EN / Amiga FR / Amiga GE) as a source-cited catalog with per-variant MD5 + size + location/hint/page counts + language + use-high-glyphs flag. The classifier goes MD5 first (preferred, case-insensitive) and falls back to a parsed-content classification (counts + size) so a future operator-staged R2/R3/GE HCSB.HTC classifies without a code change; the size gate disambiguates the R2 EN / FR / GE count tie (5036/219/512). `csb_hint_oracle_htc_variant_drift()` exposes observed vs. expected size / location / hint / page counts so a drift between catalog contract and observed parse is visible at the call site. Data-free unit `csb_hint_oracle_htc_variant_unit` 82/82 PASS (catalog identity + MD5 classification + parsed-content classification including tied-count resolution + cache classification + drift detail + diagnostic-name stability). Real-asset probe 22/22 invariants PASS on the local Atari ST 2.x HCSB.HTC: catalog carries exactly 5 documented variants, MD5 → variant lookup resolves the locally-staged Atari ST 2.x MD5 to RELEASE_1_EN with release_name='R1' and language='EN', and `drift.matches=1` (size 66,172 / 4,663 locations / 210 hints / 492 pages all match the catalog exactly). The variant surface is intentionally narrow: it does NOT draw the Hint Oracle graphical overlay, does NOT add the documented R2/R3/FR/GE HCSB.HTCs to the known-hash list (those still need a locally-staged copy before they appear in `csb_hint_oracle_htc_real_scan.g_known_hashes[]`), and does NOT claim UI/runtime parity beyond what `csb_hint_oracle_ui_runtime_binding` already provides. The known-hash list in `csb_hint_oracle_htc_real_scan` still gates which HCSB.HTCs we accept on disk; the variant catalog gates which variant name a matched file carries. Verified end-to-end against `~/.firestaff/data/csb-atari-st-2x/HCSB.HTC` (Atari ST 2.x PP 2009-02-22 hard-disk variant, md5 `8ce69b54cf255a15e98e909bb45b9742`, 66172 bytes). Skip-safe: hosts without a known HCSB.HTC exit 0 with a SKIP message after the catalog-identity checks have run. **2026-06-29 graphical overlay boundary gate:** `csb_hint_oracle_graphical_overlay` + `firestaff_csb_v1_hint_oracle_graphical_overlay_probe.c` (CTest `csb_v1_hint_oracle_graphical_overlay`, labels `tier2;csb;hint_oracle;real_data;skip_safe;render`) draw a framed 8-bit framebuffer overlay from staged decoded Hint Oracle text and, when a known HCSB.HTC is present, render hint 0 through the existing real cache/decode path; data-free unit `csb_hint_oracle_graphical_overlay_unit` covers geometry, clipping, not-loaded/out-of-range safety, and synthetic decoded-page rendering. This is a render boundary only: no M11/M12 live flow, no original Utility Disk pixel parity, no screenshot promotion, and no variant-list change. Remaining: bind the graphical overlay into the real M11/M12 Hint Oracle flow, locally stage a real R2/R3/FR/GE HCSB.HTC to promote the documented catalog hashes into the `csb_hint_oracle_htc_real_scan` known-hash list, and capture a real Hint Oracle screen using a verified HCSB.HTC before this row can move past OPEN-BOUNDED. |
...
| DM2 Macintosh 1.0 (en/jp/demo) — uses QuickTime .moov | PARTIAL / TWO US EN VERSIONS BOOT-GATED — the large retail ZIP (`mac-en-retail`) and smaller First Chapter demo ZIP (`mac-en-demo`) now read their authentic HFS/StuffIt data in RAM and pass separate hash-paired boot tests; the demo uses the 6,535-byte truncated big-endian File_header/map member. The English Macintosh keyboard/menu table is source-locked and CTest-gated (`dm2_v1_mac_input`), and an admitted `DM2_PLATFORM_MAC_EN` profile now takes precedence over PC aliases in the M11/SDL route for movement, champion/leader inventory, freeze, wake, save and quit. Retail `MooV` data forks, complete HFS resource forks, and `moov` payloads are now retained by the boot profile with exact presence/size receipts for Title, Swoosh, Credits and Ending; Story is absent from the image. Mac-only wall actions remain explicit and fail closed. Japanese and French remain preservation inputs. DMWeb's Macintosh edition page identifies JP, USA, and USA demo as redump BIN/CUE CD images plus CD-content archives. Remaining: original QuickTime playback/codec timing, native Mac held-input, wall/menu owners, MIDI/audio, balloon help, save corpus, plus JP/FR runtime proof. |
