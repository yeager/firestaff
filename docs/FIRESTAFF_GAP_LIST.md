...
| CSB X68000 (jp) | PARTIAL — raw 2DHD HDM root access is CTest-gated through `csb_v1_x68k_hdm`: the local v3.1 cracked HDM proves `GRAPHICS.DAT` (373,583 bytes) and `DUNGEON.DAT` (2,066 bytes) extraction through Human68k FAT12 chains. `csb_v1_x68k_graphics_handoff` verifies the complete X68000 `GRAPHICS.DAT` DMCSB2 table as 732 bounded direct source records and decodes original post 13 to its native 96×41 indexed-pixel receipt. `M11_AssetLoader_InitCsbX68kFromHdm` now extracts `GRAPHICS.DAT` from the raw HDM into a distinct native cache and can render that same post without letting the shared format claim Amiga identity; this is not an X68000 bootprofile and does not establish media authenticity. `csb_v1_x68k_dungeon_handoff` feeds `DUNGEON.DAT` directly into the existing big-endian source-dungeon loader and verifies its two maps plus initial party pose (level 0, x=9, y=0, direction 2). `csb_v1_x68k_enter_sng` reads the same HDM's real `ENTER.SNG` as format-1 MIDI (9 tracks, 480 TPQN, 3,205 events) without manufacturing playback. DMWeb's original-media, `CK.R` crack, 24/31 kHz presentation, Champion Editor/portrait-loading and Ctrl-S / Opt.1 / Opt.2 boundaries still need classification or runtime evidence. |
