...
| CSB X68000 (jp) | PARTIAL — raw 2DHD HDM root access is CTest-gated through `csb_v1_x68k_hdm`: the local v3.1 cracked HDM proves `GRAPHICS.DAT` (373,583 bytes), `DUNGEON.DAT` (2,066 bytes), `ENTER.SNG`, `AUTOEXEC.BAT` and `CHAOS_ST.X` extraction through Human68k FAT12 chains. The joined `csb_v1_x68k_hdm_source_media_receipt` validates the same image's graphics, two-map dungeon and initial pose (level 0, x=9, y=0, direction 2), format-1 MIDI (9 tracks, 480 TPQN, 3,205 events), AUTOEXEC command order (`CK`, `VIDSET`, `CHAOS_STRIKES_BACK`) and the `CHAOS_ST.X` Human68k `HU` layout (64-byte header, 9,020 text bytes). `csb_v1_x68k_startup_handoff_admit` now joins that receipt to an owned X68000 M11 graphics cache and an actual source-dungeon load; its real-media test proves the 732-record cache, two maps and initial pose together. It does not retain the HDM, run `CHAOS_ST.X`, reuse Amiga identity or claim an M11 game-view boot profile. The receipt and program disassembly are input evidence only: neither runs the cracked program nor infers that its startup command resolves to a particular root file. DMWeb's original-media, `CK.R` crack, 24/31 kHz presentation, Champion Editor/portrait-loading and Ctrl-S / Opt.1 / Opt.2 boundaries still need classification or runtime evidence. |
