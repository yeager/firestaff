...
| **QuickTime `MooV` / `.mov` (DM2 Macintosh)** | greatstone d_articles_mac.html / dmweb Animations | PARTIAL — the retail HFS reader keeps authentic data and resource forks in RAM, extracts each present `moov` resource without flattening, and builds a validated in-memory `moov`+`mdat` view for the four present movies (`0x19`; `Story.MooV` is absent). The optional FFmpeg-backed in-memory route now rebases the private QuickTime chunk offsets, decodes authentic Cinepak (`cvid`) frames and PCM audio, and owns the M11 Mac startup surface until EOF; no converted movie is written. Exact original QuickTime 1.x/2.x timing and Animation (`rle`) coverage remain open. The same boot profile retains the authentic Mac `Music`, `General.sounds`, and `Weapon.sounds` resource forks; a bounded format-1 `snd ` parser validates their 2/19/12 resource sets and exact sample ranges, and resource 10001 is hash-checked and transported through the SDL host mixer. Complete resource selection, gameplay timing, and MIDI/music scheduling remain open. |
