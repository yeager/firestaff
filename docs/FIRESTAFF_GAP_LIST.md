...
| **Hint Oracle text and graphics archives (CSB)** | dmweb Hint Oracle Files / ReDMCSB HINTHTC.C,HINTLZW.C,HINTGRAP.C,HINTSCR.C,HINTTEXT.C,EXPAND.C | PARTIAL / OPEN-BOUNDED — Hash-verified HTC parsing, caching, page decoding and structured UI binding are present for the five dmweb-documented variants: English R1/R2/R3, Amiga French and Amiga German. `csb_hint_oracle_htc_real_scan_and_load_md5()` and `csb_hint_oracle_ui_panel_load_md5()` select an exact registered source MD5; they reject arbitrary values and never silently fall back to another Utility Disk. `csb_hint_oracle_dat_real_scan_and_load()` applies the same hash-only rule to HCSB.DAT (EN R1/RX/ST2, EN R2/R3, FR, DE), including archive-member reads without copying game data into the library. The real ST 2.0/2.1 asset is admitted by `708e113c869ab922633e885aa72a3c77`; `CSB_HintOracleGraphicsSurface` owns the same authenticated archive plus the Oracle's 320×200 4-bit pixels, 256×27 indexed font raster and 16-color RGB4 palette. The palette follows `HINT001.C`'s exact 3-bit→4-bit conversion, not a host-selected palette; the font dimensions and 8×9-glyph source are locked to `HINTTEXT.C`. It now also consumes HINTSCR/HINT001's C26 control pair through HINTTEXT/F0129 (`font index 9 → 1` in the real ST data, transparent index 12 unchanged). `CSB_HintOracleSession` recovers HINTHINT.C's authored-order exact/wildcard selection, seven-row cap, one-based page state and non-wrapping LAST/NEXT; `CSB_HintOracleAtariSaveSession` feeds it only the existing checked Atari MINI.DAT GAMEBLOCK2 map/X/Y receipt. Graphical UI, M11/M12 routing, original text/page layout, original-frame capture and pixel parity remain open. |
