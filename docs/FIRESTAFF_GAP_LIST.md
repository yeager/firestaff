...
| **Hint Oracle text and graphics archives (CSB)** | dmweb Hint Oracle Files / ReDMCSB HINTHTC.C,HINTLZW.C,HINTGRAP.C,HINTSCR.C,HINTTEXT.C,EXPAND.C | PARTIAL / OPEN-BOUNDED — Hash-verified HTC parsing, caching, page decoding and structured UI binding are present for the five dmweb-documented variants: English R1/R2/R3, Amiga French and Amiga German. `csb_hint_oracle_htc_real_scan_and_load_md5()` and `csb_hint_oracle_ui_panel_load_md5()` select an exact registered source MD5; they reject arbitrary values and never silently fall back to another Utility Disk. `csb_hint_oracle_dat_real_scan_and_load()` applies the same hash-only rule to HCSB.DAT (EN R1/RX/ST2, EN R2/R3, FR, DE), including archive-member reads without copying game data into the library. `CSB_HintOracleAtariRuntime` now joins the known Atari R1 pair (HTC `8ce69b54cf255a15e98e909bb45b9742`, DAT `708e113c869ab922633e885aa72a3c77`) with only the checked MINI.DAT GAMEBLOCK2 position receipt, source-order seven-row selection and all four original 320×200 states: LOAD prompt, list, no-clue acknowledgement and hint page. Its opt-in real-media regression decodes the staged save and renders every state; no host frame/font/palette fallback is admitted. Other release/language DAT/HTC pairs remain separate until their pairing is source-verified. M11/M12 routing, original click layout/cadence, original-frame capture and pixel parity remain open. |
