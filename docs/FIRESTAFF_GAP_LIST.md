...
| Gap | Source | Status |
|---|---|---|
| IMG1/IMG2 RLE 16-color image decoder | ReDMCSB, sck, DMWeb DM Apple IIGS/FM Towns edition pages | FIXED (`image_backend_pc34_compat.c` `IMG3_Compat_ExpandFromSource`); remaining Apple IIGS proof is PO/ProDOS media handoff for 2.0/2.1, while remaining FM Towns proof is CD-image handoff, EN/JP hash classification, and real IMG2 asset receipts |
| IMG3/IMG4 4bpp local-palette image | dmweb Data Files | FIXED (same code path) |
| **IMG5 4bpp chunked image (Amiga, SNES)** | greatstone d_items.html / DMWeb DM Amiga edition page | **FIXED in v2.9.2** (`firestaff_img5_decode.c`, commit `216b0b67`); remaining Amiga proof is real-media handoff across DM Amiga 2.0/2.1/2.2/3.6, not the bounded decoder itself |
...
| **FTL container format (Amiga, X68000, MegaCD)** | greatstone d_ftl.html / DMWeb DM Amiga edition page | PARTIAL / OPEN-BOUNDED — `firestaff_ftl_container_unit` PASS 1/1 on 2026-06-22; parses the 20-byte common header, 12-byte hunk headers, BSS/DATA/CODE hunk discovery, BSS metadata, and verifies documented common/BSS/DATA/uncompressed-CODE checksums. DMWeb's DM Amiga page adds the practical media/protection boundary for this work: official SPS IPF 3.6, unofficial 2.0/2.2 IPFs, protected original ADFs, cracked FR/DE ADFs, hard-disk patches, and Meynaf's Atari ST-to-Amiga port must be classified separately. Remaining: HUNK_DATA zero-run decompression, HUNK_CODE 0x5223 decompression/checksum verification, mapfile item extraction, real FTL corpus gates, and runtime asset loading. |
