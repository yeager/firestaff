...
| **Hint Oracle text and graphics archives (CSB)** | dmweb Hint Oracle Files / ReDMCSB HINTHTC.C,HINTLZW.C,HINTGRAP.C,HINTSCR.C | PARTIAL / OPEN-BOUNDED — Hash-verified HTC parsing, caching, page decoding and structured UI binding are present for the five dmweb-documented variants: English R1/R2/R3, Amiga French and Amiga German. `csb_hint_oracle_htc_real_scan_and_load_md5()` and `csb_hint_oracle_ui_panel_load_md5()` select an exact registered source MD5; they reject arbitrary values and never silently fall back to another Utility Disk. Real-data probe `firestaff_csb_v1_hint_oracle_real_htc_scan_probe` passes 47 checks against the external original corpus, loading all five exact variants with their registered sizes and parsed counts (R1 EN 210/492; R2 EN 219/512; R3 EN 219/513; FR 219/512; GE 219/512). `HCSB.DAT` now has a strict read-only archive index: duplicated big-endian 16-bit tables must agree and the declared segment spans must cover the file exactly. The real ST 2.0/2.1 asset passes with four segments (100/29146/1497/32 bytes), matching ReDMCSB's `F1872_LoadGraphics()` boundary `2 + count * 4`; `HINTSCR.C` identifies the first three as integer glyphs, Oracle bitmap and font. This remains resource/text binding only: no source-owned graphic-stream decoder, Oracle graphical UI, M11/M12 route, original-frame capture, or pixel-parity claim exists. |
