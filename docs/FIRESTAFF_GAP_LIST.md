...
| Gap | Doc | Status |
|---|---|---|
| Champions per-stat parity | csb_gap_champions.md | PARTIAL / SOURCE-RUNTIME COVERED — ReDMCSB `REVIVE.C:F0282` is package-specific: PC I34 keeps vitals/statistics before the twelve live F0027 increments; Atari ST halves vitals and applies the one-eighth non-Luck reduction; Amiga/FM Towns quarter vitals and apply that same statistic rule. `csb_v1_reincarnation_media_branch_pc34_compat` pins all four package families, including random Luck increments, while `csb_v1_m11_prison_runtime_hud_pc34` exercises a real PC3.4 source C161 candidate against its live G0349 stream. The older character compatibility test remains a narrow API contract, not evidence for every package. Remaining: long real-play death → C127 → C161 rename → save → cold-resume captures on supplied Amiga and FM Towns media. |
