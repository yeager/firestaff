...
proves spans `100`, `29146`, `1497` and `32` bytes. Segments 1 and 2 now
follow the source-owned IMG2 expansion path, producing the Oracle's 320×200
indexed surface and its 256×27 indexed font raster; segment 3 follows
`HINT001.C`'s exact Atari RGB3-to-RGB4 conversion. This remains a resource
boundary: graphical UI routing, segment-0 integer consumption, original
text/page layout and frame/pixel parity are still open.
...
| **DM1 PC-9801 HDM/floppy preservation** | DMWeb DM PC-9801 edition page | INTENTIONALLY UNSUPPORTED — retain read-only HDM/floppy classification and provenance for the Japanese v2.0 line, but do not add extraction, cache, startup, game-view, input or emulator/runtime support. |
| **DM1 X68000 HDM/floppy media import** | DMWeb DM X68000 edition page + DMWeb copy-protection page (Sharp X68000 section) | OPEN-BOUNDED / PARTIAL-EVIDENCE — DMWeb identifies the Japanese v3.0 X68000 line, 1990-01-26 release, 31 kHz screenshot set, HDM original image that cannot boot without copy-protection sectors, cracked image, and blank save disk. **2026-06-25 X68000 media classifier added:** `firestaff_x68k_media_classify_unit` now covers geometry constants, empty input, too-small input, single-side / full-disk / oversize size classes, blank save-disk detection, `HPR-0007` Track 1 Side 1 Sector 9 sentinel detection, FTL-payload magic detection at offset 0, configurable explicit-window FTL magic candidate counting, FTL handoff fits / overflow, unprotected-disk flag, and NULL safety. **2026-06-25 X68000 <-> FTL container handoff added:** `firestaff_x68k_ftl_handoff_unit` PASS 5/5 cross-module test that builds a synthetic FTL container with a chosen `data_area1_memory_size`, parses it with `firestaff_ftl_container`, and asks the X68k HDM classifier whether the declared area_1 fits inside the DMWeb-documented 1232 KB 2DHD disk. **2026-06-26 real-media receipt + cross-module FTL-handoff added:** `firestaff_x68k_media_receipt_real_corpus_probe` verifies the local public DMFiles DM1 X68000 v3.0 HDM geometry receipt, and `firestaff_x68k_ftl_handoff_real_corpus_probe` now compares the default 32 KiB classifier scan, `FIRESTAFF_X68K_SCAN_WINDOW_FULL`, and `FirestaffX68kMedia_CountFTLMagicCandidates(..., 0, hdm_size)` before parsing embedded resources via `FirestaffFtlContainer_Parse`; it finds 2 parseable FTL containers at offsets 73,728 (0x12000) and 460,800 (0x70800), verifies the first FTL's BSS `data_area1_memory_size` (24,720 bytes) fits the on-disk HDM media class, and cross-checks the FTL-handoff helper's documented boundary cases. **2026-06-29 receipt-class gate added:** the classifier covers conservative receipt classes for FTL payloads, blank save disks, live protection-sector captures, off-axis sentinel-only media, nonblank/no-sentinel media, and partial-side dumps; the real-media receipt probe reports `receipt_class=off_axis_sentinel_only`, `total_hpr=1`, `off_axis_hpr=1`, `first_off_axis=464078`, proving the embedded backup/label `HPR-0007` string is not promoted to live protection-sector evidence. Known limitation now bounded: the legacy `FirestaffX68kMedia_Classify()` field still scans only the first 32 KiB for single-resource `.FTL` sniffing, while `FirestaffX68kMedia_ClassifyEx(..., scan_window_bytes, ...)` and the explicit-window helper cover full-HDM receipt scans without pretending every raw 0x6160 hit is parseable. Remaining: HDM/floppy extraction on a real preserved master, original-vs-cracked/save-disk classification, real `GRAPHICS.DAT`/`DUNGEON.DAT` extraction and payload receipts, FTL payload decode/runtime asset loading, 31 kHz presentation evidence, X68000 keypad/arrow input gates, and emulator/runtime launch proof. Copy-protection/sector gaps remain honest: public DMFiles HDMs lack the `HPR-0007` sentinel and the only operational check is documented as broken on DM and CSB alike (DMWeb Sharp X68000 section). |
...
| **Hint Oracle text and graphics archives (CSB)** | dmweb Hint Oracle Files / ReDMCSB HINTHTC.C,HINTLZW.C,HINTGRAP.C,HINTSCR.C,HINTTEXT.C,EXPAND.C | PARTIAL / OPEN-BOUNDED — Hash-verified HTC parsing, caching, page decoding and structured UI binding are present for the five dmweb-documented variants: English R1/R2/R3, Amiga French and Amiga German. `csb_hint_oracle_htc_real_scan_and_load_md5()` and `csb_hint_oracle_ui_panel_load_md5()` select an exact registered source MD5; they reject arbitrary values and never silently fall back to another Utility Disk. `csb_hint_oracle_dat_real_scan_and_load()` applies the same hash-only rule to HCSB.DAT (EN R1/RX/ST2, EN R2/R3, FR, DE), including archive-member reads without copying game data into the library. `CSB_HintOracleAtariRuntime` joins the Atari R1 pair (HTC `8ce69b54cf255a15e98e909bb45b9742`, DAT `708e113c869ab922633e885aa72a3c77`) with a checked MINI.DAT GAMEBLOCK2 position receipt, source-order seven-row selection and all four original 320×200 states. It is exposed through both M12's `CSB UTILITY DISK — HINT ORACLE` post and explicit CLI `--csb-hint-oracle`; both fail closed unless the R1 pair and native MINI.DAT pass. The opt-in real-media regression renders every state; no host frame/font/palette fallback is admitted. Other release/language DAT/HTC pairs remain separate until their pairing is source-verified. Original click capture/cadence, original-frame capture and pixel parity remain open. |
...
| **AMG sound format (CSB utility disk)** | dmweb Data Files / ReDMCSB `SWSHSND.C:F0908_InitSound` / local original CSB Amiga Utility Disk corpus | FIXED for documented CSB Utility Disk sound-effect files — `firestaff_amg_decode_unit` parses the bounded SND2 shape (`TELE2`, `SWIPE`, `MAGEXPLO`, `EXPLOS1`, `DRAGON`): big-endian sample count, signed 8-bit mono PCM, then 0..3 trailing bytes. The separate skip-safe `csb_v1_amiga_amg_real_media` gate reads those five files as `ZIP→ADF→AMG` virtual members in RAM, and locks their original payload/trailing pairs `1506/1`, `995/2`, `3107/2`, `3970/1`, and `4003/2`; this corrects the duplicate parser's former fictitious control-word interpretation. PAL/NTSC rate helpers remain source-cited. Runtime playback/rate binding and non-SND2 `NAKED.AMG` are not claimed. |
| **MVE (Interplay, DM2 PC)** | dmweb Animations | OPEN-LARGE — DOS-stub + Interplay MVE binary |
| **QuickTime `MooV` / `.mov` (DM2 Macintosh retail)** | greatstone d_articles_mac.html / dmweb Animations | PARTIAL — the retail HFS reader keeps authentic data and resource forks in RAM, extracts each present `moov` resource without flattening, and builds a validated in-memory `moov`+`mdat` view for the four present movies (`0x19`; `Story.MooV` is absent). The private view preserves source ownership and handles the authentic four-byte prefix before `Swoosh.MooV`'s `mdat`. The optional FFmpeg-backed in-memory route rebases the private QuickTime chunk offsets and decodes all four present movies' authentic video/PCM paths; real-asset decoder and M11 startup gates pass, and no converted movie is written. M11 holds each decoded frame for its authentic positive QuickTime duration, and the real-media gate rejects non-monotonic timestamps or zero-duration frames. Exact original QuickTime 1.x/2.x native presentation ownership remains open. The same boot profile retains the authentic Mac `Music`, `General.sounds`, and `Weapon.sounds` resource forks; a bounded format-1 `snd ` parser validates their 2/19/12 resource sets and resource 10001 is hash-checked and transported through the SDL host mixer. Complete resource selection, gameplay timing, and MIDI/music scheduling remain open. |
...
| **S2D (Nexus font files)** | locally verified | PARTIAL — parser exists, has a bounded bitmap-to-indexed-framebuffer renderer, a bounded real-on-disk SEGA SATURN SCR section-table parser + helpers, and a bounded section→glyph-range map + runtime text-layout cursor that walks ASCII strings through the existing renderer. **2026-06-21 SEGA SATURN SCR parser determinism probe added:** `firestaff_nexus_v1_saturn_font_determinism_probe` (commit `b2157a62`, ctest `nexus_v1_saturn_font_determinism`) covers load/free/get_glyph + dimension inference (16x16 / 12x12 / 8xN buckets) + NULL-safety + 50-repetition determinism. **2026-06-22 render gate added:** `firestaff_nexus_v1_saturn_font_render_probe` (ctest `nexus_v1_saturn_font_render`) expands parser-exposed 1bpp glyph bytes, draws into a synthetic indexed framebuffer, and verifies glyph dimensions, expansion, clipping, transparent/background behavior, deterministic framebuffer hash, NULL/bounds handling, plus an optional local `FONT256.S2D` parser-to-render handoff when the verified 25,012-byte asset is present. **2026-06-25 SCR section-table parser added:** `firestaff_nexus_v1_saturn_font_scr_sections_probe` (ctest `nexus_v1_saturn_font_scr_sections`, 55/55 PASS) walks the 32-entry SEGA SATURN SCR section table at offset 0x20 of an SEGA SATURN SCR payload via `nexus_v1_font_load_sections()` + `nexus_v1_font_section_count()` + `nexus_v1_font_get_section()` + `nexus_v1_font_section_table_index()` + `nexus_v1_font_section_in_bounds()`. Synthesizes a small SCR fixture to lock NULL-safety, too-small-buffer rejection, invalid-magic rejection, all-reserved/zero entry skipping, single-populated-entry handling, out-of-bounds section skipping, table-overlap skipping, four-populated FONT256.S2D-shape layout (indices 0/2/4/6), in-bounds validation, original-index preservation across skips, and a fully-bound four-populated real-asset chain (entries 0x0120+0x2010 / 0x2130+0x3c90 / 0x5dc0+0x0210 / 0x5fd0+0x01e4 form a contiguous non-overlapping partition that ends at file_size). When `~/.firestaff/data/nexus/FONT256.S2D` is present the optional gate locks all four real section windows and the [0]->[1]->[2]->[3] chain end at 0x61b4 inside the verified 25,012-byte asset; entries 28..31 (non-zero but out-of-bounds tail padding the flat 1bpp loader silently consumes) are skipped by the parser without rejecting the whole table. **2026-06-28 section→glyph-range map + bounded runtime text-layout cursor added:** `nexus_v1_s2d_text_layout` (`include/nexus_v1_s2d_text_layout.h`, `src/nexus/nexus_v1_s2d_text_layout.c`) builds a `Nexus_V1_S2D_SectionGlyphMap` from the parsed sections (each populated section holds a contiguous `(char_start, char_count)` slice of the character table, capped against `bytes_per_glyph` and the SCR header `char_count`), routes glyph indices via `nexus_v1_s2d_glyph_range_lookup()`, and exposes a `Nexus_V1_S2D_TextLayout` cursor that walks ASCII strings through the existing `nexus_v1_font_draw_glyph_indexed()` (advance by `char_width + letter_spacing_x`, `'\n'` breaks, `'\t'` steps, `max_chars` cap, `chars_skipped` for out-of-coverage). CTest `nexus_v1_s2d_runtime_text_layout` (99/99 PASS) covers NULL-safety, empty/single/four-populated synthetic map invariants, layout cursor + reset + free + per-glyph advance + multi-line + `max_chars` + out-of-coverage skip, deterministic FNV-1a framebuffer hash across two `"NEXUS"` runs, and a real-asset `FONT256.S2D` skip-safe path that drives `"NEXUS"` through the real parser→map→layout chain 3 times and asserts bit-identical FNV-1a framebuffer hash. Remaining: decode which bytes inside each populated section hold the actual glyph payload (the flat 1bpp loader still owns that decode), support real Shift-JIS double-byte characters / proportional advance / vertical text, and capture an actual Nexus screen using the real font before calling this FIXED. |
| **TAI/SAL/MAP (Nexus level data)** | locally verified | PARTIAL — loaders exist; TLINK metadata and rendering sparse |
| **BPX/BPK (Nexus compressed archives)** | locally verified | PARTIAL — `nexus_v1_bpk_archive` is CTest-gated and locks the observed Nexus `MENU.BPK` BPPK/BMPD directory shape: `BPPK` outer size, `BMPD` section header, 163 big-endian candidate offsets, 162 `PRS3` payload markers, one raw payload, monotonic bounds checks, and local MENU.BPK receipt checks when the file is present. `nexus_v1_bpx_bpk_archive_boundary` also recognizes the verified `nexus/MENU.BPK` size/hash marker, keeps unknown real `.BPK/.BPX` payloads unsupported, and locks a bounded synthetic `BPX0` table contract for entry bounds, lookup, stored-entry extraction, and unsupported-compression reporting. **2026-06-27 byte-level boundary inspection (pass1082) added:** three new `nexus_v1_bpk_archive` APIs (`get_entry_prefix` / `inspect_prs3` / `mode_distribution`) walk the verified real MENU.BPK structure without claiming PRS3 decompression — 20-byte prefix (width @ 12..14 BE u16, height @ 15, mode tag @ 19), `PRS3` magic + 0x00000001 version + pixel-count sub-header, and the strongest single signal that the reverse-engineered prefix layout is right: every one of the 162 PRS3-bearing entries has `width * height == prs3_pixel_count`. Mode distribution {6:14, 14:62, 22:39, 30:47, 10:1} (sum = 163); entry[0] is a unique directory trailer (mode tag 10) whose raw bytes 0..4 = 0x00015940 (= offset[161]) and 4..8 = 0x00015980 (= offset[162]). `nexus_v1_bpx_prs3_parse()` adds a stronger synthetic BPX3 stream contract that mirrors the real MENU.BPK layout (16-byte name + BE u16 width + mode tag + BE u8 height + BE u32 pixel_count + BE u32 payload offset) without claiming PRS3 decompression. Skip-safe CTest `nexus_v1_menumenu_bpk_inspect` (49/49 PASS) verifies the synthetic BPX3 contract + optional real MENU.BPK receipt. **2026-06-28 surface-class + BPX3 directory-trailer contract (pass1083) added:** three new `nexus_v1_bpk_archive` lookup APIs (`mode_to_surface_class` / `mode_to_bpp` / `surface_estimate`) describe the unpacked surface shape each PRS3-bearing entry would decode to (INDEXED_8BPP / RGB565 / RGB888 / RGBA8888 / DIRECTORY_TRAILER enum + bytes-per-pixel + rowstride + surface_bytes = width * height * bpp) without claiming PRS3 decompression; `nexus_v1_bpx_prs3_parse()` now also recognises a synthetic BPX3 directory-trailer entry (mode tag 10, zero width/height/payload_offset, no PRS3 magic) tagged with `NEXUS_V1_BPX_BPK_METHOD_DIRECTORY_TRAILER`. CTest `nexus_v1_bpk_surface_class` (38 PASS) and skip-safe CTest `nexus_v1_bpk_surface_class_probe` (522 PASS on the local `~/.firestaff/data/nexus/MENU.BPK`) cross-check 14 indexed / 62 RGB565 / 39 RGB888 / 47 RGBA8888 entries with every rowstride == width * bpp and every surface_bytes == width * height * bpp. Remaining: identify the PRS3 compression algorithm from real MENU.BPK bytes / executable disassembly, decode the four bpp/mode-tag values into actual palette/indexed/RGB565/RGB888/RGBA surfaces, hand the decoded payloads into a renderable Nexus menu graphics pipeline (atmospheric HUD/textures + first Nexus screen capture with the real MENU.BPK), and wire the directory-trailer info into launcher metadata. |
...
**Nexus status correction, 2026-08-08:** The older BPX/BPK row above still
describes PRS3 as an unidentified compression algorithm. The bounded DMWeb
byte decoder now passes `test_nexus_v1_bpk_archive` against the verified retail
`MENU.BPK` and decodes all 162 PRS3 surfaces to their declared pixel counts.
The remaining gap is Saturn presentation ownership: VDP1 command order,
VDP2/CLUT binding, menu placement and screen identity. No decoded host pixels
are promoted to production drawing until those observations exist.
...
| Savegame format (DM1, CSB) | ReDMCSB + dmweb | FIXED for DM1 (`dm1_v1_save_load.c`), PARTIAL for CSB — native Atari `MINI.DAT` is now admitted end-to-end through the regular M12 Continue row and F0435 (`csb_v1_atari_mini_menu_resume_archive`). A stock CSBWin `csbgame2.dat` also reaches F0435, the live C0128/C232 frame, movement and inventory through the start menu and CLI. CSBWin write-back remains fail closed: Firestaff can only make a provenance-checked byte copy or bounded decoder-core export until its saved dungeon, timer queue and EXPOOL/DSA continuation sections have a complete source-verified writer. |
...
| Custom dungeon import (DM1 dungeon.dat, CSBWin dmsave/csbgame) | dmweb Custom Dungeons page | PARTIAL / BLOCKED-DATA — 2026-06-22: M12 has a disk-backed `custom_dungeon_m12_real_path` CTest for `data/custom/<name>/DUNGEON.DAT` discovery, mixed-case community filenames, header validation, sorted listing, invalid-entry rejection, and valid-entry selection. `custom_dungeon_import` also covers the existing M12 launcher scanner and DM1 V1 engine scanner with synthetic `DUNGEON.DAT` fixtures: valid header/map-count parsing, alphabetical launcher list, case-insensitive `dungeon.dat`/`graphics.dat` discovery, compressed-signature rejection, too-small rejection, valid-entry selection, and invalid-entry refusal. **2026-06-27 CSBWin `dmsave`/`csbgame` loader-boundary contract gate landed:** `csb_v1_csbwin_save_loader_boundary_pc34_compat` (14 documented shapes: 3 accept + 11 reject; per-shape `csb_v1_import_csb_save_buffer()` loader-boundary verdict). **2026-06-29 staged-file discovery/readiness gate landed:** the same module now classifies `csbgame.dat`, `csbgame.bak`, `dmsave.dat`, and `dmsave.bak` basenames case-insensitively, maps bytes to the documented save shape, reports the actual loader verdict, and emits `should_attempt_import` plus stable rejection labels for DM1 raw saves, CSBWin 512-byte saves, DSA-marked saves, bad versions, bad magic, and truncation. CTest `csb_v1_csbwin_save_loader_boundary_pc34_compat_unit` PASS 112/112 + skip-safe real-asset probe `csb_v1_csbwin_save_loader_boundary` PASS. Remaining blocker: real community/custom dungeon corpus handoff, the CSBWin 512-byte XOR-pad obfuscation-key decoder, the DM1→CSB raw-save conversion path, DSA section import, CSBgraphics.dat payload decode/runtime binding, and the end-to-end CSBWin save importer wiring. |
...
| Inventory route parity for all item types | `docs/dm1_gap_inventory_items.md` | FIXED -- pass1070 audit verifies equip slots, backpack/chest routes, consumables, mouth/eye routes, panel-slot routing, object interaction, and M11 inventory runtime/pixel gates together. The F0352/F0353 eye lifecycle is explicitly covered: releasing C071 restores F0347's normal action-hand panel, closes the transient eye-opened chest, and reopens an action-hand container through ordinary F0333 so the C145 icon is restored. |
...
| Gap | Doc | Status |
|---|---|---|
| Champions per-stat parity | csb_gap_champions.md | PARTIAL / SOURCE-RUNTIME COVERED — ReDMCSB `REVIVE.C:F0282` is package-specific: Atari ST halves vitals and applies the one-eighth non-Luck reduction; Amiga/FM Towns quarter vitals and apply that same statistic rule. Compatibility fixtures remain source-reference tests only and do not represent a DOS/Windows edition. Remaining: long real-play death → C127 → C161 rename → save → cold-resume captures on supplied Atari ST, Amiga and FM Towns media. |
| Combat mechanics | csb_gap_combat.md | PARTIAL |
| Dungeon model/mechanics | csb_gap_dungeon.md | PARTIAL — native mechanics are source-locked against ReDMCSB/CSBWin, but those references are not original DOS/Windows editions. Runtime evidence must use authenticated Atari ST, Amiga or FM Towns packages. Remaining work is deeper end-to-end gameplay parity, real save compatibility artifacts, viewport/UI runtime evidence, and playability. |
| Graphics + ornament blits (F0108, F0115, F0111, CustomBackgrounds) | csb_gap_graphics.md | PARTIAL / OPEN-LARGE — CTest covers seven data-free CSB viewport/source-lock slices: first CustomBackgrounds backdrop, room-slot backdrop-1, D1C F0108 floor/ceiling ornament, D1C F0115 thing pass, D3C F0107/F0108 first-backdrop composition, D3L/D3R sidewall backdrops, and D2C F0107 wall-ornament plus F0111 door-front layering. Former PC-labelled ornament fixtures are not platform evidence. Remaining: broader viewport/HUD captures and original/pixel parity evidence from Atari ST, Amiga or FM Towns. |
| Full mechanics parity | csb_gap_mechanics.md | OPEN-LARGE — 2026-08-31 ReDMCSB `HINTLOAD.C`/`HINTHTC.C` audit closed the archive-admission gap for the supplied Atari Utility disk: `csb_v1_hint_oracle_real_htc_scan`, `csb_v1_hint_oracle_ui_runtime_binding`, `csb_v1_hint_oracle_ui_panel`, and `csb_v1_hint_oracle_htc_variant` now read `Utility.stx::HCSB.HTC` directly in RAM, authenticate and parse its authored page tables, then bind decoded text to the panel-facing API. This is format and UI-data binding only; original click/cadence behavior and pixel capture parity remain open. |
...
| CSB PC-9801 3.1 (jp) | INTENTIONALLY UNSUPPORTED — local original and cracked HDM media may retain read-only `pc98-2hd-raw` preservation receipts from FAT12 root slots (`CJDATA` + `CSBGAME`), but no cache, startup handoff, game view, input, portrait path or native runtime route may be added. |
| CSB X68000 (jp) | INTENTIONALLY UNSUPPORTED — `csb-extras/legacy-jp-x68000/` is retained solely as preservation reference; no CSB reader, cache, startup handoff or M11 route is provided for this platform |
| CSBWin (unofficial port by Paul Stevens) | REFERENCE ONLY — useful for source and format comparison; never a playable Windows/DOS platform or game-data identity in Firestaff |
...
| CSBWin custom resource handling (csbgraphics.dat + dmsave + csbgame) | PARTIAL — csbgraphics.dat side: the bounded index classifier, real-scan/cache, exact `LocateNthGraphic(n)` payload-span helper and `csb_v1_csbgraphics_dat_decode_entry()` are present. The decoder sends precisely one declared LZW payload to the ReDMCSB-compatible graphics decoder and accepts it only when its output byte count exactly equals the entry's declared decompressed size; empty/mismatched/truncated entries fail closed. `csb_v1_csbgraphics_runtime_binding` can then apply a caller-declared indexed bitmap only at a source-locked runtime destination. CTests cover big-endian and `0x8001` little-endian headers, spans, bounded LZW decode, zero entries and rejection paths; real scan remains skip-safe because CSBgraphics.dat is a user-authored CSBWin product with no canonical built-in hash. dmsave/csbgame side: loader-boundary and on-disk shape gates cover the documented accepts/rejects. Remaining work: a real user-staged CSBgraphics.dat registration/receipt, source-owned bitmap geometry/palette admission for each override, M11 viewport override ownership, CSBWin 512-byte XOR-pad obfuscation-key decoder, DM1→CSB raw-save conversion, and end-to-end importer wiring that feeds both halves into `csb_v1_import_csb_save_buffer`. |
...
| **Original-overlay proof** | PARTIAL — the canonical PC 1.0 EN DOS archive now produces an authentic original `dungeon_gameplay` frame through `DM2.BAT → EREGCARD → IBMIOP → SKULL.EXE` in the local DOSBox harness, and Firestaff produces a native real-asset/no-fallback dungeon frame from that same ZIP. The local capture files are deliberately untracked because they contain copyrighted game imagery. They are not yet a pair: the exact original party tuple, frame/tick, state-transition trace, palette treatment, and 224×136 crop have not been matched to the Firestaff frame. Therefore no overlay, pixel-parity, or visual-completion claim is made. `scripts/dosbox_dm2_original_overlay_capture.sh`, `tools/verify_dm2_v1_original_overlay_capture_source_lock.py`, and `firestaff_dm2_v1_original_overlay_capture_scaffold_probe` provide the source/capture gate. `dm2_original_firestaff_pair_manifest.tsv` remains absent by design; promotion requires same-state `dungeon_gameplay` original+Firestaff 224×136 SHA-256 rows. |
| **Timer dispatch** | FIXED — 2026-08-01: all 22 skproject timer types (c_tim_proc.cpp:3980 DM2_PROCEED_TIMERS) now have registered handlers in dm2_v1_runtime.c. 14 new fail-closed handlers added: 0x0C (PROCESS_TIMER_0C), 0x0D (RESURRECTION), 0x0E (PROCESS_TIMER_0E), 0x15 (PROCESS_SOUND), 0x19 (PROCESS_CLOUD), 0x1E (STEP_MISSILE), 0x3D (PROCESS_TIMER_3D), 0x46 (LIGHT), 0x47 (HERO_ENCH_FLAG), 0x48 (ENCH_POWER), 0x4B (POISON), 0x5A (ORNATE_NOISE), 0x5D (MOVE_RECORD_ROTATE), 0x5E (ALLOC_NEW_CREATURE). All consume fail-closed until their respective ownership chains are source-bound. |
...
| Per-cell modern-art swap in DM2 V1 (T560 indoor, T600 outdoor) | OPEN-BOUNDED — 2026-06-27: new `dm2_v22_viewport_swap_pc34.{c,h}` provides the bounded per-cell swap API + render pass (indoor T560 4×3 grid + outdoor T600 sky/horizon/ground). `Dm2_V22_ShapeType` 24-entry enum (walls + floors + creatures + doors + fields + outdoor shapes + SHAPE_NONE sentinel) + `dm2_v22_shape_for_cell()` discriminator + `dm2_v22_asset_id_for_shape()` resolver. New CTest `dm2_v22_inplace_render_probe` was driven by a synthetic manifest under a probe-controlled HOME (no real GRAPHICS.DAT/PNG receipts needed). V1 source ownership preserved: the render pass is a no-op when V22 modern pack is missing, presentation mode is not V22_MODERN, the cache file is not loaded, or the cache is not populated. Companion fix: `dm2_v22_inplace_draw_pc34.c` previously hardcoded `~/.firestaff/assets/dm1/modern/...` (copy/paste leftover from the CSB template) — now mirrors `dm2_v22_set_manifest_path()` in `dm2_v22_modern_assets_pc34.c`. **2026-06-29 T560 indoor route gate:** `DM2_V22_T560IndoorRoute` now exposes the nine indoor D0..D2 x L/C/R route descriptors (stable `T560_Dx_[LCR]` names, raw cell type, discriminator shape, category/asset id, rect + clipped rect, active/no-op state), the probe is 33/33 PASS, V22-disabled routes remain inspectable but do not paint, and indoor-vs-outdoor cache-type mismatches are rejected. Source-locked against SKULL.ASM T520/T560/T600 + ReDMCSB DUNVIEW.C:2962-3070 + DUNVIEW.C:4351-4382 F0112. Remaining: (a) wire `dm2_v22_viewport_swap_render()` into the DM2 V2 viewport draw path (currently `dm2_v2_viewport_renderer.c` only manages smooth animation state, not the actual framebuffer paint), (b) authoring real DM2 PBR hero art via gpt-image-2 batch + real-asset receipts in `~/.firestaff/assets/dm2/modern/`, (c) outdoor T600 route-depth follow-up beyond the existing 3-cell synthetic paint, and (d) per-mode pixel/material verification gates (V1 vs V2.0 vs V2.1 vs V2.2) to confirm the per-cell swap is observable end-to-end. |
...
route is verified for bounded New Game/media/input, not complete Macintosh
gameplay. Source `0x04` wall/floor timers are fail-closed
...
| MNS creature/spell rendering | PARTIAL — 2026-06-27: `firestaff_nexus_v1_mns_multi_model_bounds_probe` is CTest-gated as `nexus_v1_mns_multi_model_bounds_{synthetic,real_corpus,saturn_ja_iso}` and PASS 3/3. Synthetic path locks slot-fill (1 + 8 models), `NEXUS_MAX_MODELS=64` cap, bad-magic / too-small-buffer failure-isolation across the slot pool, and `nexus_v1_shutdown` zeroing `model_count` + nulling every slot's heap allocations. Real-data path drives the documented 30-creature MNS subset (ANTMAN/BIGWORM/BORKETH/CHAOS/DRA_ZOM/D_GOLD/D_RED/D_SILVER/GHOST/GIGGLER/GOLEM/GRN_DRA/H_HOUND/LAS_MON/LORD_RIB/MINI_DRA/MUMMY/OBAKE/OITU/RAT/RED_DRA/ROCKPILE/SCORPION/SCREAMER/SN_FLOOR/SN_WALL/S_SHIELD/S_SWORD/VEXIRK/WORM) through `nexus_v1_load_model` and asserts monotonic index growth + per-slot DMDF magic + matching per-slot `name` + `model_count == loaded_count` + cap invariant + shutdown cleanup. 2026-06-29 follow-up: the same real-data path now repeats the subset through a fresh engine init/shutdown cycle and requires the accepted/skipped names, returned indices, `model_count`, DMDF magic, and parsed vertex/face counts to match via a local FNV-1a digest. 2026-08-05: the dedicated MNS decoder retains the real 33-joint SCORPION and 37-joint ROCKPILE skeletons; all 30 retail MNS files and 452 source texture records pass `test_nexus_v1_mns`. Per-MNS geometry bounds, vertex/UV extents, and face-index validation stay with `firestaff_nexus_v1_model_frame_gate_probe`. Remaining: runtime creature render against the parsed slot pool, VDP1 texture/palette ownership, and the render-time handoff that turns a parsed model into a visible creature. |
| S2D font loader/renderer | PARTIAL — parser, bounded indexed-framebuffer renderer, bounded real-on-disk SEGA SATURN SCR section-table parser (`nexus_v1_saturn_font_scr_sections`, 55/55 PASS), and bounded section→glyph-range map + runtime text-layout cursor (`nexus_v1_s2d_runtime_text_layout`, 99/99 PASS) are CTest-gated; real Nexus screen render capture and full Shift-JIS double-byte / proportional advance support remain open. |
| TLINK/TAI/SAL/MAP runtime | PARTIAL |
| Save/load (.sav) | PARTIAL |
| V1 mechanics | PARTIAL |
| **Real Saturn asset handoff (NEXUS.BIN/ISO)** | EXTRACTED + VERIFIED + PHASE-LAUNCH GATED — 2026-06-25: `nexus-extras/saturn-ja/Dungeon Master Nexus (Japan) (Track 1).bin::DM.BIN` matches canonical DM.BIN hash; the Track 1 .bin (not a re-muxed merged ISO) is the actual source the engine opens through `nexus_iso_open_cue`. `firestaff_nexus_v1_track1_phase_launch_probe` is now CTest-gated as `nexus_v1_track1_phase_launch_{synthetic,extracted_root,saturn_ja_iso}` and PASS 3/3 on 2026-06-25: synthetic-only path covers the engine-init / DGN-parser / DMDF-parser / BPK-parser / Saturn-font-parser / determinism contract for phases 0-7 (32 PASS); `extracted_root` (53 PASS) and `saturn_ja_iso` (56 PASS) additionally drive the live `nexus_v1_init → nexus_v1_read_file → nexus_v1_load_level(0) → nexus_v1_tick × 5 → nexus_v1_load_model("SCORPION.MNS") → nexus_v1_shutdown` pipeline against the verified Track 1 media, including DMDF parser handoff on a real 3D model, real FONT256.S2D parser handoff (256 chars, 16×16), MENU.BPK BPPK magic, and Track 1 BIN ISO-contract checks (`nexus_iso_is_nexus` recognises DM.BIN + LEV00.DGN; `nexus_iso_find("DM.BIN")` returns a >=64 KiB payload). **2026-06-29 screen-capture readiness gate added:** `firestaff_nexus_v1_track1_screen_capture_readiness_probe` is CTest-wired for skip-safe, extracted-root, and Saturn-JA-ISO paths; it writes an operator-local JSON manifest and, when real assets are staged, a 320x200 PPM readiness receipt after verifying the combined `DM.BIN` / `FONT256.S2D` / `LEV00.DGN` / `SCORPION.MNS` handoff and stamping a real-font glyph into the runtime framebuffer. This is provenance/readiness only: the MNS model is loaded but not rendered as a creature, the PPM is not a reviewed public screenshot, and no original Saturn pixel-parity claim is made. Demo and EN/FR fan-translation media lines (separate from the JP retail disc) remain tracked for separate classification. DMWeb's Saturn pages identify the official JP retail disc (Victor Interactive Software catalog `T-9111G`, 1998-03-26), a separate JP demo, CD audio tracks 02-09, optional EN/FR fan-translation disc images, and public-proof material from the Japan retail page: screenshots, 48-page booklet, physical scans, and a two-part Japanese playthrough. Remaining: classify demo and EN/FR fan-translation media separately from the JP retail Track 1 source, review/promote an actual Nexus screen capture using the real Track 1 assets before promoting this beyond phase-launch gated, and wire the parsed MNS model pool into visible creature rendering with real VDP1 BITMAP/palette decode. |
...
| Light-overflow gameplay bug | PARTIAL — 2026-06-25 added a data-free Nexus V1 light-overflow data model + bug-classification API (`include/nexus_v1_light_overflow.h`, `src/nexus/nexus_v1_light_overflow.c`) plus a CTest probe (`firestaff_nexus_v1_light_overflow_probe`, ctest `nexus_v1_light_overflow`, 61/61 invariants PASS). 2026-06-28 added a bounded M11/runtime cast hook (`include/nexus_v1_light_runtime.h`, `src/nexus/nexus_v1_light_runtime.c`) and a CTest probe (`firestaff_nexus_v1_light_runtime_probe`, ctest `nexus_v1_light_runtime`, 81/81 invariants PASS) that owns one `Nexus_V1_LightTimeline` + one `Nexus_V1_LightState` per active game, preserves emulate-vs-guard mode distinction via `guard_rejects`, surfaces `CAST_REJECTED` via `should_guard()` when guard mode rejects at the cap, round-trips MagicalLightAmount + cast/decay/dropped counters + in-use slot array through an `NGLT` v1 blob (1640 bytes) that drops into the existing `FNXS` data section without modifying the FNXS header, and rejects mode-mismatched saves cleanly without leaking state. The data model mirrors ReDMCSB WIP20210206 `TIMELINE.C F0238` (BUG0_18 silent drop when the timeline hits `G0369_EventMaximumCount`), `TIMELINE.C F0257_ProcessEvent70_Light` (recursive weaker-event chain at `GameTime+4`), `MENU.C F0404_MENUS_CreateEvent70_Light` and `MENU.C:1926-1942` (Light/Torch/Darkness dispatch), `DATA.C:359` (16-entry `LightPowerToLightAmount` table), `CHAMPION.C:27` (`MagicalLightAmount` field), and `LOADSAVE.C:2041` (EventMaximumCount=100 base cap). DMWeb evidence (`editions/sega-saturn/` light-overflow symptom and `solutions/cheats-and-hacks/` permanent-spell-effect exploitation) and the dmweb cheats/hacks page both confirm BUG0_18 as the root cause. The classification hook (`Nexus_V1_LightOverflowKind` enum) returns one of `NONE` / `TIMELINE_FULL_PERMANENT_LIGHT` / `LIGHT_BLEED_NEGATIVE` / `CAST_REJECTED` so the future M11 runtime can either **emulate** (default — source-faithful V1 path that reproduces the dmweb-documented "permanent Light" / "dungeon into darkness" symptoms) or **guard** (set `guard_rejects=1` on the timeline to refuse casts at the cap, suitable for replay/capture contexts). Remaining: Saturn runtime asset handoff (DM.BIN, FONT256.S2D, MNS models), live M11 spellcast dispatch handoff that calls `nexus_v1_light_runtime_apply_cast()` per champion/mana check, real-asset decay-tick evidence, real `FNXS` save/load round-trip evidence against a staged operator file, and extending `nexus_v1_load_full_from_path()` to recognize + decode the embedded `NGLT` blob at the documented offset. None of those block the bounded classification contract or the runtime wire-in. |
...
| **DMWeb PC Engine / TurboGrafx edition provenance** | FIXED / EVIDENCE-BOUNDED — 2026-06-25: DMWeb's edition page separates JP, USA, and PCEWorks bootleg records; lists JP/USA redump BIN/CUE, CloneCD, and DMFiles ISO/OGG sources; identifies Track 02 as the data track; records tracks 01/03/04 as shared music with Japanese or English speech; lists JP tracks 05-18; and notes the JP Track 17 static-noise defect fixed in the US version. 2026-06-29: `theron_v1_track02_bank` now verifies all three raw US and all three raw JP audio-bank marker tuples (sentinel, id word, and offsets) when matching user-staged Track 02 BINs are present. Remaining work is BIN/CUE-vs-ISO/OGG classification, intro/outro/screenshot receipts, and actual ADPCM/CD-DA decode or playback proof, while keeping the bootleg out of canonical data. |
...
| Runtime handoff for every option | PARTIAL — CSB is catalogued for its original Atari ST, Amiga and FM Towns editions. Remaining gaps are per-game polished flows, richer CSB viewport/HUD/gameplay proof, and original capture parity. |
| **Save export/import** | FIXED / PARTIAL-PER-GAME — 2026-06-22: added M12 save-browser export/import helpers and CTest `save_browser_export_import_m12`, covering selected `firestaff-*.sav` export to a backup directory, byte-preserving import back into the data directory, destination no-overwrite behavior, invalid filename rejection, reported destination paths, and NULL-state rejection. M12 settings also expose bounded Export/Import Save Manifest actions that write/read `firestaff-save-export-manifest.json`, validate the manifest type, reject invalid path strings, preserve existing config on failed import, and CTest-cover config/API plus settings-row reachability via `m12_json_export_import` and `m12_startup_menu`. 2026-06-29 adds `save_byte_manifest_m12`, a versioned per-game save-byte manifest gate for Firestaff-native DM1 saves: the manifest records `runtime_save_bytes_included = 1`, `FSDM1SV1` format id, byte count, and whole-file CRC; import verifies the DM1 header version/declared size/body CRC, rejects corrupt payloads, and refuses overwrites before copying bytes. Remaining per-game work: original DM1 save conversion/interoperability, CSB/DM2/Nexus/Theron save-byte manifests, version migration policy, and real original/community save compatibility artifacts. |
...
| **Cloud sync** | OPEN-LARGE — bounded M12 opt-in/no-op boundary exists (2026-06-28) and the launcher settings JSON schema now round-trips `cloud_sync_enabled`, `cloud_sync_policy`, and `cloud_sync_dir` (2026-06-29). Still no real cloud provider, multi-device encryption, conflict UI, or scheduled push/pull runtime integration. |
...
| Gap | Status |
|---|---|
| Screen reader integration | FIXED — 2026-06-27: `firestaff_m12_launcher_screen_reader_manifest_probe` now PASS 69/69 (up from 40/40) + `firestaff_accessibility_manifest_unit` PASS 39/39. Launcher state manifest now covers main view (game cards), settings (tabs + rows), missing-data / status popup, plus the four cell-by-cell extras views — bestiary, item encyclopedia, screenshot gallery, and museum of lore — with stable element IDs (`GAME_CARD_DM1..THERON`, `MENU_SETTINGS`, `MENU_MUSEUM`, `TAB_*`, `ROW_*`, `POPUP_*`, `BESTIARY_CAT_*` / `BESTIARY_ROW_*`, `ITEM_CAT_*` / `ITEM_ROW_*`, `SCREENSHOT_ROW_*`, `MUSEUM_CATEGORY_*` / `MUSEUM_BULLET_*`). New element types in `firestaff_accessibility.h` (`FS_AX_CATEGORY_TAB`, `FS_AX_BESTIARY_ROW`, `FS_AX_ITEM_ENCYCLOPEDIA_ROW`, `FS_AX_SCREENSHOT_THUMB`, `FS_AX_MUSEUM_CATEGORY`, `FS_AX_MUSEUM_BULLET`); new private emit functions in `menu_startup_a11y_m12.c` (`emit_bestiary_view` / `emit_item_encyclopedia_view` / `emit_screenshot_gallery_view` / `emit_museum_view`); new public getters in `include/menu_startup_m12.h` (`M12_Museum_GetCategoryTitle` / `M12_Museum_GetCategoryPageCount` / `M12_Museum_GetBullet`) so the museum's private static data is reachable without leaking the table. Probe subtests H/I/J/K cover bestiary category tabs + per-creature rows with `selected` value, item-encyclopedia category tabs + per-item rows with a per-category leak check, screenshot-gallery thumbnails with filename label + `selected` value, and museum sections + page-driven bullet content (page 0 surfaces `1987 FTL GAMES`, page 2 surfaces `PRESERVATION NOTES` for the DUNGEON MASTER section). M11 game-view manifest covers gameplay / inventory / map / dialog / entrance-mirror / endgame states, and `m11_gameplay_screen_reader_manifest_probe` gates open-chest zones, classifier precedence, atomic-write/no-launcher-leak invariants, redraw idempotence, and framebuffer-contained bounds; the probe also fixed the endgame manifest coordinate offset so plaque/mirror/portrait bounds match the victory overlay. **Hardened 2026-07-01 (fallthrough views navigation-anchor gate):** subtest N in `firestaff_m12_launcher_screen_reader_manifest_probe` (now PASS 175/175, up from 91/91) pins the documented "out of scope" fallthrough contract for the 12 launcher views that intentionally fall through to `emit_main_view` as a navigation anchor: data-validator / audio-settings / accessibility / theme / save-browser / input-remap / custom-dungeon / campaign / spell-reference / map-viewer / touch-layout / presentation-preview. For each view the subtest asserts (a) the envelope `gameState` matches the active view (`launcher_<viewName>`), (b) the `GAME_CARD_DM1` / `MENU_SETTINGS` / `MENU_MUSEUM` / `GAME_CARD_THERON` navigation anchors are emitted, (c) the framebuffer dims stay pinned to 480x270, and (d) no foreign view markers leak into the manifest (forbidden-marker table covers `ROW_*`, `TAB_*`, `CHANGELOG_LINE_*`, `MANUAL_DOC_*`, `BESTIARY_CAT_*`, `BESTIARY_ROW_*`, `ITEM_CAT_*`, `ITEM_ROW_*`, `SCREENSHOT_ROW_*`, `MUSEUM_CATEGORY_*`, `MUSEUM_BULLET_*`, `POPUP_MESSAGE`, `POPUP_OK`, `POPUP_LINE*`). A future dedicated emitter for any of these 12 views only needs to assert that the new view-specific markers appear AND the fallthrough markers disappear. Still open: real assistive-technology smoke evidence beyond JSON shape (the 12 fallthrough views now have the navigation-anchor contract machine-checked; dedicated emitters for any of them remain a per-view follow-up). |
| High-contrast launcher remap | FIXED |
| In-game overlay coverage | FIXED — 2026-06-27: `m11_high_contrast_overlay_pc34_compat` module exposes the M11 chrome-remap gate (`SetActive/IsActive/RemapPresentedColor/ApplyActiveRGBA/GetManifest`); `m12_apply_loaded_config` in `src/ui/menu_startup_m12.c` pushes `config.highContrast` into `M11_HighContrast_SetActive()` next to the existing `M11_UIScale_SetPercent` push. `m11_draw_text` and `m11_draw_text_original` route through a `m11_chrome_remap_style()` helper so HUD/dialog/log/rune/hit-flash/death-overlay text picks up the restricted palette when the gate is on. Raw 320x200 dungeon-viewport pixels + HUD panel C040/C017 blits are deliberately preserved bit-exact (they go through separate bitmap-blit paths, never through `m11_draw_text`). CTest `m11_high_contrast_overlay_pc34_compat` PASS 369/369 (default-off identity for V1 fidelity, restricted-palette remap when on, region apply with `excludeMask` viewport fence, manifest string contract, launcher / game palette parity). Headless probe `firestaff_m11_high_contrast_overlay_runtime_probe` PASS 14/14 (launcher→M11→viewport-fence state machine). Labels `tier2;m11;accessibility;high-contrast`. Source-lock: none — pure accessibility glue that mirrors the existing `m12_presented_color()` contract in `src/ui/menu_startup_m12.c`; ReDMCSB does not cover high-contrast palette remap. **Hardened 2026-06-28 (rectangle-fence apply helper):** added `M11_HighContrast_ApplyActiveRGBAExceptRect()` so a broad chrome-overlay pass can skip an explicit dungeon-viewport rectangle (e.g. `0,0,224,136` over the 320x200 logical buffer) instead of relying only on palette-index `excludeMask` bit-fencing. The dungeon viewport is now fenced by both axes: pixels inside the preserve rectangle are NEVER touched, even when their palette index is muted and would otherwise collapse to BLACK; pixels outside the preserve rectangle still follow the restricted-palette remap. The new helper is wired into the existing `m11_high_contrast_apply_active_region` static implementation so the original `M11_HighContrast_ApplyActiveRGBA()` retains its `excludeMask`-only contract. Manifest `chrome_rect_remap_with_viewport_fence` token added so the gate can be audited by name. CTest `m11_high_contrast_overlay_pc34_compat` now PASS 379/379 (10 new assertions for the rectangle-fence variant: broad remap reports chrome changes, preserve rectangle stays byte-identical, chrome outside the rectangle follows the remap table, zero-area preserve rect disables the rectangle fence and falls back to excludeMask, negative preserve dimension disables the rectangle fence, gate off leaves every pixel alone even with a real preserve rect, manifest coverage entry). Headless probe `firestaff_m11_high_contrast_overlay_runtime_probe` now PASS 19/19 (5 new invariants: rect-fenced broad remap touches HUD chrome strip, rect-fenced broad remap preserves dungeon viewport pixels, rect-fenced broad remap collapses chrome strip to BLACK, zero-area preserve rect disables the rectangle fence and falls back to excludeMask, rect-fence helper preserves checker pattern inside the dungeon viewport). Existing launcher/game palette parity + toggle-round-trip + manifest-contract invariants all still PASS. |
| In-game overlay helper safety | FIXED — 2026-06-29: `m11_high_contrast_apply_active_region()` now clips both apply rectangles and preserve rectangles with widened arithmetic before converting back to framebuffer coordinates. This keeps the public high-contrast helpers defined for extreme synthetic/tooling inputs (`INT_MAX` widths/heights, huge negative/off-right rectangles, off-screen preserve rectangles) without changing the accessibility scope or viewport-fence behavior. CTest `m11_high_contrast_overlay_pc34_compat` now PASS 422/422; `firestaff_m11_high_contrast_overlay_runtime_probe` remains PASS 19/19. |
...
25. Screen reader integration. — FIXED 2026-06-27/29 + fallthrough-anchor gate 2026-07-01: launcher-state manifest covers main view / settings / missing-data popup plus the four cell-by-cell extras views (bestiary, item encyclopedia, screenshot gallery, museum of lore) with stable element IDs (probe PASS 175/175, up from 69/69), and M11 gameplay manifest coverage now has a headless probe for gameplay / inventory / map / dialog / entrance mirror / endgame / open chest surfaces. Subtest N pins the navigation-anchor contract for the 12 fallthrough launcher views (data-validator / audio-settings / accessibility / theme / save-browser / input-remap / custom-dungeon / campaign / spell-reference / map-viewer / touch-layout / presentation-preview) — each view emits `GAME_CARD_DM1` / `MENU_SETTINGS` / `MENU_MUSEUM` / `GAME_CARD_THERON`, uses the active-view `gameState` prefix, and never leaks a foreign view's markers. Remaining follow-up: manual-docs + changelog already have dedicated emitters; the 12 fallthrough views still intentionally fall through to main-view emission as a navigation anchor (now machine-checked), but each one is a candidate for a dedicated emitter if its UI gains a per-cell or per-row surface; plus real assistive-technology smoke evidence beyond JSON shape.
...
- `scripts/dosbox_dm2_original_overlay_capture.sh` — DM2 PC 1.0 EN DOSBox capture harness with deterministic DOS4GW/4DOS config (memsize=63, fixed cycles=3000, no dynamic core, svga_paradise), Swift (macOS) and xdotool (X11/Linux) route injectors, labelled shot tokens, 320x200 rawshot normalization (including 640x400 2x DOSBox downsample to 320x200), and the retail-RAW4 `RECT_7` 224x136 viewport crop at (0, 40). Refuses to inject an unvalidated DM2 route. Outputs land in `verification-screens/passH2313-dm2-original-overlays/` with deterministic naming and SHA-256 receipts.
- `tools/verify_dm2_v1_original_overlay_capture_source_lock.py` — read-only source-lock verifier against SKULLWIN (10 anchors across dm2global.h, c_gfx_main.cpp, c_gfx_main.h, c_gui_vp.cpp, c_tmouse.h, types.h, c_input.cpp, c_gui_draw.cpp). SKULLWIN is auto-discovered from the historical machine-specific paths, repo-relative fallbacks, or an ignored read-only `reference/skproject/SKULLWIN` checkout. It is a build reference only, never a runtime or release dependency. CTest-gated as `dm2_v1_original_overlay_capture_source_lock`. 2026-06-29 adds an optional pair-manifest gate for `dm2_original_firestaff_pair_manifest.tsv`: missing means OPEN/no claim, malformed tracked rows fail, and promotion requires same-state `dungeon_gameplay` original+Firestaff 224x136 viewport SHA-256s.
- `probes/firestaff_dm2_v1_original_overlay_capture_scaffold_probe.c` + CMakeLists wiring — data-free source-lock probe covering screen size 320x200, backbuffer 224x136 = 0xe0x0x88, mouse+command queue lengths 10/10, c_evententry {b,x,y} layout, surface=64000B/backbuffer=30464B, right-panel geometry, HUD text fit, click-route disjointness, shot-label vocabulary, route-token inventory, and the pair-manifest schema. CTest `dm2_v1_original_overlay_capture_scaffold_probe` PASS.
