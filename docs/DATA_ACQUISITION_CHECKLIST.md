...
| X68000 Japanese | 🟡 `291e1bc6…` | 🟡 | The v3.1 JP cracked HDM is read directly through its Human68k FAT12 root: `GRAPHICS.DAT`, `DUNGEON.DAT` and the format-1 `ENTER.SNG` MIDI sequence have real-media receipts. The X68000 graphics handoff verifies all 732 `GRAPHICS.DAT` source records as direct DMCSB2 spans under an X68000-specific receipt; `DUNGEON.DAT` reaches the existing big-endian source-dungeon loader with two verified maps and the original initial party pose. DMWeb's original image without copy-protection sectors, `CK.R` crack, blank save disk, 24/31 kHz presentation, Champion Editor/portrait-loading and Ctrl-S / Opt.1 / Opt.2 input still need canonical classification or runtime evidence. |
