...
| CSB V1 | 20% | 20/100 | `inventory_and_launch_blocker` | Positive front-door render smoke and source-locked QuickPlay/load-route boundaries exist, but true CSB runtime/capture gates remain blocked; current launch evidence is a deliberate runtime blocker, not playable CSB.<br>Do not infer CSB runtime/render/gameplay parity from DM1. |
...
| `core_input_movement` | 13/15 | pass296/pass299 plus pass372/pass406 source-lock Firestaff input/keypad route through C001..C006 command queue, movement timing/pipeline, target-square legality, and first viewport redraw; residual is original DOS keyboard-buffer transcript/overlay proof. |
...
| `launch_smoke` | 2/10 | `csb_v1_launch_blocker_m12` proves the hash-matched CSB launcher path renders options/blocker surfaces and stays blocked, while `csb_v1_quickplay_load_route_source_lock` source-locks the CSB reference QuickPlay/load route as playback/load setup only: diagnostics identify CSB/version, but no runtime launch request or valid intent is produced. |
