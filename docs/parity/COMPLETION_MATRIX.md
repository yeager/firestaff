...
| `launch_smoke` | 9/10 | DM1 V1 launch/menu/title/runtime paths are repeatedly CTest- and probe-gated. On 2026-08-27 the real-media matrix passed both direct CLI and start-menu boots for DOS PC 3.4, French DOS unpacked media, Atari ST English/German, Amiga and FM Towns; the archived media is consumed in memory and requires neither an emulator nor a BIOS. Release data defaults were restored after the v3.0.3 path fix. |
