...
|---|---:|---:|---|---|
| DM1 V1 | 78% | 78/100 | `playable_verified_partial` | Representative original-vs-Firestaff overlay parity remains incomplete. Native archive startup is now separately proven for both the canonical PC 3.4 ZIP and the supplied nested DOS-EN layout; the missing scope is original runtime overlay/capture evidence, not archive ownership. |
...
| CSB V1 | 46% | 46/100 | `runtime_boundary_verified_partial` | CSB has authentic native startup/runtime and start-menu evidence on Atari ST, Amiga and FM Towns. The Atari route and both FM Towns editions verify their complete initial input matrices from independent source sessions; Amiga proves its source title-to-runtime handoff and first movement. The French preservation ZIP verifies its original protected STX sector ordering through the same route. End-to-end gameplay/render capture proof is still being hardened. |
...
| CSB V1 | Authentic native Atari STX and nested-ZIP, including the supplied French preservation ZIP → protected STX route, Amiga ADF-archive, and English/Japanese FM Towns-CD startup paths now prove title → runtime and start-menu handoff. Atari and FM Towns independently cover their complete initial input matrices; Amiga covers the source title-to-runtime handoff and first movement. Additional evidence includes CSB save runtime boundary; command-chain/input/runtime tick slices; dungeon header/loader/world gates; utility/CMP import; DSA trigger; save import path; viewport source-lock slices and CustomBackgrounds coverage; hidden-code skip tables. |
...
| DM2 V1 | Authentic DOS MVE → SKULL → New Game CLI, Amiga title/runtime, FM Towns title/New Game/champion/runtime, and Mac retail boot; the separately supplied Mac First Chapter demo is fail-closed and cannot select a retail sibling; door/button toggle boundary; minimap level-transition/save-load gate; weather/timer save round-trip; projectile-vs-creature collision; creature death/drop observer; pressure plate and trigger gates; focused combat/shop probes. |
...
| `launch_smoke` | 9/10 | DM1 V1 launch/menu/title/runtime paths are repeatedly CTest- and probe-gated. On 2026-08-29 the real-media matrix passed direct CLI and the single-path start-menu → Enter → first movement route for both DOS PC 3.4 ZIP layouts, Atari ST English/German, Amiga and FM Towns; French DOS unpacked media retains its separate native route. Archived media is consumed in memory and requires neither an emulator nor a BIOS. |
...
| `launch_smoke` | 9/10 | Authentic native Atari STX and nested-ZIP paths, including the French preservation ZIP → STX route, pass the single start-menu → title → runtime → first-movement route. Amiga ADF-archive and English/Japanese FM Towns-CD paths prove title → runtime → first movement plus their native start-menu handoff. The Amiga and FM Towns CTest entries bind their canonical user archives automatically when present, rather than depending on an ambient corpus variable. CSB's non-existent PC/DOS platform is fail-closed. Hash-matched launch intent, blocked-launch UI, quickplay/load-route source locks, and profile/diagnostic gates remain covered. |
