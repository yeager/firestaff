...
| `launch_smoke` | 9/10 | DM1 V1 launch/menu/title/runtime paths are repeatedly CTest- and probe-gated. On 2026-08-27 the real-media matrix passed direct CLI and the single-path start-menu → Enter → first movement route for DOS PC 3.4, Atari ST English/German, Amiga and FM Towns; French DOS unpacked media retains its separate native route. Archived media is consumed in memory and requires neither an emulator nor a BIOS. Release data defaults were restored after the v3.0.3 path fix. |
...
| `launch_smoke` | 9/10 | Authentic native Atari STX and nested-ZIP paths pass the single start-menu → title → runtime → first-movement route. Amiga ADF-archive and English/Japanese FM Towns-CD paths prove title → runtime → first movement plus their native start-menu handoff. The Amiga and FM Towns CTest entries bind their canonical user archives automatically when present, rather than depending on an ambient corpus variable. CSB's non-existent PC/DOS platform is fail-closed. Hash-matched launch intent, blocked-launch UI, quickplay/load-route source locks, and profile/diagnostic gates remain covered. |
