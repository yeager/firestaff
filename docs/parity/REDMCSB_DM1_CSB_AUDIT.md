...
| DM1 PC 3.4 sound-event arbitration data | `DATA.C:1264-1310` MEDIA719/MEDIA712 defines all 35 I34E rows; `SOUND.C:F0064/F0065` consumes each row's period, priority and loud/soft distances | Replaced guessed host values with the exact source rows and rejects invalid event indices before M11 presentation. A packed-media gate binds every event to its authentic SND3 item and decodes the complete source bank in memory. | `dm1_v1_sound_music_source_lock`; `dm1_v1_audio_source_closure_real_pc34` |
...
- The PC 3.4 sound-event identity and source metadata are closed for all 35
  events: the exact I34E periods/priorities/distances are verified and every
  mapped SND3 payload decodes from the selected packed ZIP. This does not by
  itself claim original multi-event cadence or overlap capture parity.
