...
- **Status:** Pass 179 consolidates the big original-faithful evidence lane without claiming pixel parity. `tools/pass179_dm1_v1_original_evidence_capture_integration_gate.py` verifies 10 ReDMCSB source contracts for new-game party state, relative movement, movement legality/cooldown, game-loop draw/command cadence, viewport state/draw request, floor/ceiling base, wall/door blits, object/creature/projectile draw stack, viewport presentation, and pass175 C080 queue boundaries. Pass 208 narrows the remaining FIRES runtime debugger boundary by proving the stock PC34/I34E LZEXE loader entry and the exact source seams for `command_accepted`, `movement_applied`, `party_coordinates_committed`, and `viewport_present`.
- **Evidence:** `parity-evidence/pass179_dm1_v1_original_evidence_capture_integration_gate.md`, `parity-evidence/verification/pass179_dm1_v1_original_evidence_capture_integration_gate/manifest.json`, `parity-evidence/pass208_dm1_v1_fires_loader_csip_source_map_gate.md`, and `parity-evidence/verification/pass208_dm1_v1_fires_loader_csip_source_map_gate/manifest.json`.
- **Current truth:** capture-overlay tooling is recovered/source-locked but not semantically overlay-ready; pass175's stale Firestaff no-delta blocker is retired, while original runtime remains blocked on a decompressed FIRES/runtime source-symbol map. Stock FIRES starts in the LZEXE loader at relative `1665:000e` from DOS load-image base (`PSP+0x10`), but this is not yet the decompressed game-code CS:IP for F0359/F0380/F0377/F0280 or movement/viewport seams.
- **Next unblocker:** produce a decompressed stock FIRES memory dump or ReDMCSB I34E `FIRES.MAP`, bind it to the DOS load base and LZEXE post-decompression transfer point, then set debugger breakpoints for command acceptance, movement application, coordinate commit, and viewport presentation before any original-vs-Firestaff pixel overlay claim.

## Current original-route blocker — **SOURCE-LOCKED / ORIGINAL-RUNTIME INPUT BLOCKED (pass 180, 2026-05-10)**
- **Area:** `VISUAL` / original-runtime reference capture
- **Status:** Still blocked on a usable original PC 3.4 party/champion runtime capture, but the semantic source route is now source-locked. Pass 180 verifies ReDMCSB new-game/forward-vector contracts plus GreatStone map data for start `(3,2)` facing east, floor corridor `(3..9,2)`, and Elija portrait wall `(10,2)` west side. Pass 113's direct-start N2 DOSBox probes remain invalid as references because they repeat dungeon frames with blank right-column/spell control regions instead of moving/recruiting/opening controls.
- **Evidence:** `parity-evidence/pass180_dm1_v1_elija_route_source_gate.md`, `parity-evidence/verification/pass180_dm1_v1_elija_route_source_gate/manifest.json`, `tools/pass180_dm1_v1_elija_route_source_gate.py`, plus prior `parity-evidence/pass113_original_party_state_followup.md`, `parity-evidence/pass113_original_party_state_probe.md`, and `tools/pass113_original_party_state_probe.py`.
- **Next unblocker:** solve the original-runtime input/address binding that proves a non-static route capture reaching `(9,2)` facing east and recruit/open Elija; keep `tools/pass113_original_party_state_probe.py` as the semantic readiness gate before any HUD/spell/inventory overlay references. Do not promote direct-start/no-party captures as parity references.
