...
| 6 | Build CSB representative visual/audio comparisons | CSB Atari, Amiga, FM Towns | Original HUD, viewport, title, door and audio captures with native comparison gates. The supplied Amiga ADF now has a RAM-only IMG1 receipt across HUD, viewport, pit, stair, ornament and door source families; composed capture comparisons remain blocked. | **Blocked by missing captures** |
...
| 8 | Extend DM2 retail gameplay receipts | DM2 DOS, Amiga, FM Towns, Mac | Real-media dialog/input ordering, AI/drop, audio and save/resume paths captured and natively regression-tested. CTest binds the supplied DOS, Amiga and Mac retail archives to native title, startup, New Game, Amiga LZX/resource, movie, audio and input receipts; broader cross-platform gameplay remains open. | Partly actionable; corpus limited |
| 9 | Prove DM1 FM Towns audio and input | DM1 FM Towns | BIN/CUE in-memory track selection, CD-audio timing receipts and FM Towns-specific input-route tests. CTest derives actual CDDA ranges and retail TMenu poll/initialization/TBIOS entry bytes from ZIP→CUE→BIN→ISO9660 in RAM; live TownsOS event delivery remains open. | Partly actionable with supplied media |
