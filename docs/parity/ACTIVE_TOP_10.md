...
| 6 | Build CSB representative visual/audio comparisons | CSB Atari, Amiga, FM Towns | Original HUD, viewport, title, door and audio captures with native comparison gates. Native CLI and start-menu routes now pass against supplied Atari STX, Amiga A31 and FM Towns retail archives; the FM Towns check also captures the real C28 + C002/C003 closed Entrance composition in Original, Modern and Custom and rejects the broad-red regression. The supplied Amiga ADF has a RAM-only IMG1 receipt across HUD, viewport, pit, stair, ornament and door source families; the FM Towns source ZIP supplies its CUE/IMG, ISO files, all 30 CDDA ranges, and native M11 startup for both language editions directly in RAM. Original emulator HUD/door/audio comparison captures remain blocked. | **Blocked by missing captures** |
