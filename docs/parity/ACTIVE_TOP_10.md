...
| 9 | Prove DM1 FM Towns audio and input | DM1 FM Towns | BIN/CUE in-memory track selection, CD-audio timing receipts and FM Towns-specific input-route tests. CTest derives actual CDDA ranges, TMenu poll/initialization/TBIOS entry bytes, and the RLE graphics-library corpus from ZIP→CUE→BIN→ISO9660 in RAM; live TownsOS event delivery remains open. | Partly actionable with supplied media |
