...
| HoC forward/back input delta | LOCAL VERIFIED (unpaired) | Local ignored original-media receipt | A fresh PC 3.4 DOSBox-X session reached the original C407 Hall state, then captured `start`, source-space forward-click, and keypad-back rows. All three healthy 320x200 frames classify as `dungeon_gameplay`; their raw hashes are distinct, which proves that both input actions changed the original framebuffer. This is an input-delivery and reference-capture receipt only: the no-party state does not prove champion HUD, inventory, pickup, coordinates, or Firestaff pixel parity. |
...
| Original Eye hold/release transition | LOCAL VERIFIED (unpaired) | Local ignored capture receipt | The corrected PC 3.4 route preserves its preceding mirror row and the control-plan labels `wuuf_inventory → wuuf_eye_held → wuuf_eye_released`. The resulting candidate is visually Azizi Yohari rather than independently verified WUUF, so the labels are retained solely as route-token provenance. Replaying this authentic route with the source Eye hit area `(20,53)` produces a 8,142-pixel held-panel delta and restores every checked static panel pixel on release; the canonical `DUNGEON.DAT` and `GRAPHICS.DAT` hashes bind the result. The earlier `(24,77)` attempt remains rejected as cursor-only. This proves the observed Eye panel transition only, not a scroll, pickup, a WUUF coordinate, or a Firestaff pixel pair. |
...
| Champion-panel | OK | IMPAIRED Firestaff-only | PARTIAL OK (pass1053 candidate/resurrect panel) | MISSING | Full four-champion HUD/status-panel pairing and a validated scroll transition are still missing |
...
| Labelled Eye receipt | 2026-09-13 | local evidence | Corrected replay of the authentic PC 3.4 route with the source Eye hit area validates the held/released transition. Its control-plan labels mention WUUF, but its observed candidate is Azizi Yohari, so it is not WUUF-route evidence. The earlier wrong-coordinate attempt is retained only as a rejected diagnostic. No published pixels or local path |
