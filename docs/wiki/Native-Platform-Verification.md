...
| CSB Atari ST | The supplied standalone STX plus English and French preservation ZIP→ZIP→STX packages are admitted in memory. A bounded original TOS 1.62 run reaches the `PRISON / RESUME` Entrance from the protected campaign disk. `Utility.stx::MINI.DAT` is admitted as its own source member and `csb_v1_atari_st_animation_assets_real` verifies title pages, source SND1 cue layout, VBlank ordering and FTLCODE handoff without extracting either disk. The Hint Oracle gates read `Utility.stx::HCSB.HTC` in RAM and bind its authored pages to the panel API. | An authenticated campaign/DSA `CSBGAME.DAT` (or source-equivalent original save) is required before claiming an original dungeon, HUD, door, movement or audio capture. Broader campaign and live Hint Oracle click parity remain open. |
...
| DM2 FM Towns | The supplied Japanese disc is admitted in memory and an original emulator trace proves pre-title, FTL, castle title, emulator-directed input and a first-dungeon transition from one media-immutable session. Firestaff's native title/New Game path is covered separately. | The captured nominal menu checkpoint was still title animation; original menu, loading, HUD, palette/register and audio receipts are still required before visual or audio parity can be claimed or the reported startup defects can be corrected from evidence. |
