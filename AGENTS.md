...
1. `CMakeLists.txt` — `project(Firestaff VERSION x.y.z)` (the single source of truth)
2. `include/firestaff_version.h` — **generated by CMake `configure_file`** from `include/firestaff_version.h.in` into `build/generated/include/`; never edit or commit the generated output. Consumed by `firestaff_main_m11.c` (`--version`), `src/ui/menu_startup_m12.c`, and `src/shared/changelog_m12.c` (`M12_Changelog_VersionString()`).
3. `src/shared/changelog_m12.c` — the human-written changelog lines; each release must add a top-level `Vx.y.z` entry matching the CMake version (enforced by `test_m12_version_changelog_consistency`).
...
2. Add the matching top-level `Vx.y.z` entry in `src/shared/changelog_m12.c`
3. Update `RELEASE_NOTES.md`
4. Commit, push, tag with `vX.Y.Z`
