...
OUT=ROOT/'parity-evidence/verification/pass316_dm1_v1_f0380_f0097_direct_probe_attempt'
REPORT=ROOT/'parity-evidence/pass316_dm1_v1_f0380_f0097_direct_probe_attempt.md'
...
def set_stage(sess,stage,events):
    ...
    if stage=='queue_watch':
        ...
        cmds=[f"BP {ADDR['F0380_COMMAND_ProcessQueue_CPSC']}", f"BP {ADDR['F0128_DUNGEONVIEW_Draw_CPSF']}", f"BP {ADDR['F0097_DUNGEONVIEW_DrawViewport']}"]
    else:
        cmds=[]
...
def debugger_loop(sess,stop_evt,run_evt,snaps,hits,events):
    ...
    while not stop_evt.is_set():
        ...
        if sig and sig not in seen:
            ...
            # pass316: keep direct code breakpoints armed; no staged BPM switching
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='firestaff-pass316-') as td:
        ...
        sess=f'pass316-{os.getpid()}'; stop_evt=threading.Event(); run_evt=threading.Event()
        try:
    ...
    blocker=None if status=='PROVEN_RUNTIME_HOOK' else 'Code-only probe captured the existing F0128 hit but not a direct F0380 dequeue hit at 22F4:0699 or F0097 viewport-present hit at 2809:1E31. This remains a blocked attempt, not a runtime-hook promotion.'
    manifest={'schema':'pass316_dm1_v1_f0380_f0097_direct_probe_attempt.raw_probe.v1','timestamp_utc':datetime.now(timezone.utc).isoformat(),'status':status,'source_audit':audit,'addresses':ADDR,'route':args.route,'method':'pass316 code-only debugger window after pass315: arm F0380 22F4:0699, F0128 23AD:40FE, and F0097 2809:1E31, then drive the pass278 guarded xdotool route','dosbox_debug':'/usr/bin/dosbox-debug','dosbox_window_pid':dosbox_pid,'debugger_events':events[:220],'memory_breakpoint_changes':memchg[:140],'debugger_hits_captured':hits[:180],'proof_predicates':required,'blocker':blocker,'transcript':'parity-evidence/verification/pass316_dm1_v1_f0380_f0097_direct_probe_attempt/dosbox_debug_noise_reduced.clean.txt','route_keylog':'parity-evidence/verification/pass316_dm1_v1_f0380_f0097_direct_probe_attempt/route_noise_reduced_keylog.json','promotion_rule':'Promote only an explicit debugger stop for F0380 at 22F4:0699 or F0097 at 2809:1E31; do not reinterpret setup echo/collision text as a direct runtime hit.'}
    (OUT/'manifest.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n')
    REPORT.write_text(f'''# Pass316 — DM1 V1 F0380/F0097 direct runtime probe attempt
...
- Armed code breakpoints for F0380 (`22F4:0699`), F0128 (`23AD:40FE`), and F0097 (`2809:1E31`).
- Drove the existing pass278 guarded DOSBox-debug/tmux/Xvfb/xdotool route for 75 seconds.

## Evidence

- Manifest: `parity-evidence/verification/pass316_dm1_v1_f0380_f0097_direct_probe_attempt/manifest.json`
- Transcript: `parity-evidence/verification/pass316_dm1_v1_f0380_f0097_direct_probe_attempt/dosbox_debug_noise_reduced.clean.txt`
- Route log: `parity-evidence/verification/pass316_dm1_v1_f0380_f0097_direct_probe_attempt/route_noise_reduced_keylog.json`
