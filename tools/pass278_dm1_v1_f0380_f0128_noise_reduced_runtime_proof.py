...
OUT=ROOT/'parity-evidence/verification/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof'
REPORT=ROOT/'parity-evidence/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof.md'
ORIG=Path.home()/'.openclaw/data/firestaff-original-games/DM/_extracted/dm-pc34/DungeonMasterPC34'
SOURCE_ROOT=Path.home()/'.openclaw/data/firestaff-redmcsb-source/ReDMCSB_WIP20210206/Toolchains/Common/Source'
ADDR={'G0432_as_CommandQueue':'2C20:3E7A','G0308_i_PartyDirection':'2C20:3C92','G0306_i_PartyMapX':'2C20:3C94','G0307_i_PartyMapY':'2C20:3CE0','F0380_COMMAND_ProcessQueue_CPSC':'22F4:0699','F0128_DUNGEONVIEW_Draw_CPSF':'23AD:40FE'}
...
def set_stage(sess,stage,events):
    ...
    if stage=='queue_watch':
        ...
        cmds=[f"BP {ADDR['F0380_COMMAND_ProcessQueue_CPSC']}", f"BP {ADDR['F0128_DUNGEONVIEW_Draw_CPSF']}"]
    else:
        cmds=[]
    for c in cmds: send_cmd(sess,c,events)
    send_cmd(sess,'BPLIST',events)
def debugger_loop(sess,stop_evt,run_evt,snaps,hits,events):
    seen=set(); stage='queue_watch'; states={'g0432_seen':False,'tuple_seen':False}; set_stage(sess,stage,events); send(sess,'Escape','O','t'); run_evt.set()
    last_continue=0.0
    while not stop_evt.is_set():
        ...
        if sig and sig not in seen:
            ...
            if kind=='g0432_bpm' and not states['g0432_seen']:
                states['g0432_seen']=True; stage='queue_to_tuple'; set_stage(sess,stage,events)
            if kind=='party_tuple_bpm' and not states['tuple_seen']:
                states['tuple_seen']=True; stage='code_only'; set_stage(sess,stage,events)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='firestaff-pass278-') as td:
        ...
        sess=f'pass278-{os.getpid()}'; stop_evt=threading.Event(); run_evt=threading.Event()
        try:
    ...
    blocker=None if status=='PROVEN_RUNTIME_HOOK' else 'Staged BPM disable/re-enable reduced queue/index noise and captured the queue write plus party tuple mutation under controlled keys, but the DOSBox-debug transcript still lacks a proven F0380 dequeue hit and/or F0128 post-mutation draw hit. This is an exact blocker, not a runtime-hook claim.'
    manifest={'schema':'pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof.v1','timestamp_utc':datetime.now(timezone.utc).isoformat(),'status':status,'source_audit':audit,'addresses':ADDR,'route':args.route,'method':'staged debugger windows: initial G0432+party BPM with F0380 BP, then after G0432 switch to F0380/F0128 BP plus party BPM, then after tuple mutation switch to F0380/F0128 code BP only; longer input-readiness route with guarded xdotool keys','dosbox_debug':'/usr/bin/dosbox-debug','dosbox_window_pid':dosbox_pid,'debugger_events':events[:220],'memory_breakpoint_changes':memchg[:140],'debugger_hits_captured':hits[:180],'proof_predicates':required,'blocker':blocker,'transcript':'parity-evidence/verification/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof/dosbox_debug_noise_reduced.clean.txt','route_keylog':'parity-evidence/verification/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof/route_noise_reduced_keylog.json','promotion_rule':'Claim runtime hook only if key input writes G0432, F0380 dequeues, movement/turn mutates G0308 or G0306/G0307, and F0128 consumes resulting tuple.'}
    (OUT/'manifest.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n')
    REPORT.write_text(f'''# Pass278 — DM1 V1 F0380/F0128 noise-reduced runtime proof\n\nStatus: `{status}`\n\n## What this pass tried\n\n- Started with a ReDMCSB source audit of `COMMAND.C`, `MOVESENS.C`, and `DUNVIEW.C`.\n- Used pass273 runtime addresses and pass276 artifacts as inputs.\n- Reduced debugger noise with staged windows: queue write watch -> queue-to-tuple watch -> F0380/F0128 code BP-only.\n- Drove a longer controlled route after debugger-run readiness.\n\n## Evidence\n\n- Manifest: `parity-evidence/verification/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof/manifest.json`\n- Transcript: `parity-evidence/verification/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof/dosbox_debug_noise_reduced.clean.txt`\n- Route log: `parity-evidence/verification/pass278_dm1_v1_f0380_f0128_noise_reduced_runtime_proof/route_noise_reduced_keylog.json`\n\n## Proof predicates\n\n```json\n{json.dumps(required, indent=2, sort_keys=True)}\n```\n\n## Decision\n\n{('Runtime hook proven by the strict chain predicates.' if status=='PROVEN_RUNTIME_HOOK' else blocker)}\n''',encoding='utf-8')
