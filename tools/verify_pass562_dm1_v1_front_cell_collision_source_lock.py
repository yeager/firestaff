...
def main():
    ...
    core_span=sp(core_s,process,order(process,["dm1_v1_apply_pre_step_stamina_plan(party, &staminaPlan, outResult);","if (!F0702_MOVEMENT_TryMove_Compat(dungeon, party, action, &outResult->movement)) {","DM1_V1_MovementCommandCore_BlockedResolutionPlanPc34Compat(","dm1_v1_apply_blocked_resolution_plan(outResult, &blockPlan);","DM1_V1_InputCommandQueue_DiscardAllInputPc34Compat(queue);","if (F0708_MOVEMENT_IsPartyStepBlockedByGroup_Compat(dungeon, things, party, action)) {","party, action, outResult->movement.resultCode, 1, &blockPlan);","dm1_v1_apply_successful_step_plan(party, &stepPlan, outResult);"],"firestaff core"))
