#!/usr/bin/env python3
"""Nexus V1 runtime screenshot readiness gate.

Runs real Firestaff Nexus launches when operator-supplied Track 1 data is
available and records runtime-probe/BMP receipts as metadata only. This is a
readiness gate for a future reviewed Nexus screenshot; it does not promote any
image bytes into public screenshot directories.
"""
...
PASS = "nexus_v1_runtime_screenshot_readiness"
...
DEFAULT_CASES = (
    {
        "id": "canonical_extracted_root",
        "label": "Nexus canonical extracted Track 1 root",
        "path": Path.home() / ".firestaff" / "data" / "nexus",
    },
    {
        "id": "saturn_ja_track1_iso",
        "label": "Nexus Saturn JA Track 1 ISO/CUE root",
...
def bmp_info(path: Path) -> dict[str, Any]:
    ...
    if len(data) < 54 or data[:2] != b"BM":
        return {"valid": False, "width": 0, "height": 0, "bit_count": 0, "non_black_pixels": 0}
    ...
    if width <= 0 or height <= 0 or bit_count != 24:
        return {"valid": False, "width": width, "height": height, "bit_count": bit_count, "non_black_pixels": 0}
    row_bytes = width * 3
...
def run_case(firestaff: Path, case: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"firestaff-{PASS}-{case['id']}-") as td:
        ...
        row["marker_found"] = (
            "Nexus launcher: loaded level 0" in combined
...
def write_outputs(result: dict[str, Any]) -> None:
    ...
    for row in result.get("cases", []):
        probe = row.get("probe") or {}
        if row.get("uses_iso_source"):
            source_kind = "iso"
        elif row.get("uses_extracted_source"):
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema": "firestaff.parity.nexus_v1_runtime_screenshot_readiness.v1",
        ...
        "non_claims": [
            ...
            "No claim of full Nexus runtime playability or final text/3D/BPK rendering parity.",
        ],
    }
