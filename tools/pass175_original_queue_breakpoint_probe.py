...
BREAKPOINTS = [
    {"name": "mouse interrupt / enqueue candidate", "symbol": "F0365_COMMAND_ProcessClick_CPSC", "source": "COMMAND.C:1458-1662", "condition": "after click x=111,y=82, expect L1109_i_Command == C080 and queue write into G0432_as_CommandQueue with X=111,Y=82 (screen-relative PC coordinates).", "logs": ["P0725_i_X", "P0726_i_Y", "P0727_i_ButtonsStatus", "L1109_i_Command", "G0433_i_CommandQueueFirstIndex", "G0434_i_CommandQueueLastIndex"]},
...
def main() -> int:
    ...
    manifest = {"schema": "pass175_original_queue_breakpoint_probe.v1", "classification": "blocked/debugger-required" if not dbg_available else "ready/debugger-available", "next_blocker": "Need an in-process debugger-capable DOSBox/DOSBox-X run to break at F0365/F0380/F0377/F0280 and classify whether C080 is absent before the queue, present but not dispatched, reaches F0377 but misses C05/front-wall sensor, or reaches F0280.", "source_root": str(SOURCE_ROOT), "source_citations": citations, "pass162": load_pass162(), "debugger": {"classification": "stock-dosbox-no-debugger" if not dbg_available else "debugger-binary-present", "bins": debugger_bins, "dosbox_version": dosbox}, "breakpoints": BREAKPOINTS, "non_goals": ["do not try more portrait coordinates", "do not push", "do not treat visual dungeon_gameplay hash as party/control readiness"]}
