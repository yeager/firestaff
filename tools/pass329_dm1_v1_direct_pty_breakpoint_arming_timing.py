...
def run_one(strategy: str, seconds: int) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix='firestaff-pass329-') as td:
        stage = Path(td) / 'dos'
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not missing:
        ...
        if not strategies[0].get('directHits', {}).get('f0128_23AD_40FE'):
            strategies.append(run_one('arm_at_stable_load_menu_prompt', args.seconds))
    ...
    if any_seq:
        ...
        status = 'BLOCKED_PASS329_LOAD_READY_MARKER_MISSING'
    else:
        stages = [s.get('stage') for s in strategies]
        if 'breakpoint retention' in stages:
            status = 'BLOCKED_PASS329_BREAKPOINT_NOT_RETAINED_ACROSS_LOAD'
        elif 'route input after arming' in stages:
            status = 'BLOCKED_PASS329_ROUTE_INPUT_AFTER_ARMING_FAILED'
...
def main() -> int:
    ...
    if args.no_runtime:
        audit = source_audit()
        manifest = {'schema': PASS + '.v1', 'timestampUtc': datetime.now(timezone.utc).isoformat(), 'status': 'BLOCKED_PASS329_LOAD_READY_MARKER_MISSING', 'sourceAudit': audit, 'runtimeProbe': {'ran': False, 'blocker': '--no-runtime'}, 'blocker': '--no-runtime'}
    else:
        manifest = build(args)
    OUT.mkdir(parents=True, exist_ok=True)
    ...
    REPORT.write_text('\n'.join(['# Pass329 — DM1 V1 direct-PTY breakpoint arming timing probe', '', f"Status: `{manifest['status']}`", '', '## ReDMCSB anchors', ''] + [f"- `{r['file']}` {r['id']}: `{r['anchors']}`" for r in manifest['sourceAudit']] + ['', '## Runtime decision', '', f"- Strategies: `{[s.get('strategy') for s in manifest.get('runtimeProbe', {}).get('strategies', [])]}`", f"- Direct hits: `{[s.get('directHits') for s in manifest.get('runtimeProbe', {}).get('strategies', [])]}`", f"- Blocker: `{manifest.get('blocker')}`", '', 'Manifest: `parity-evidence/verification/pass329_dm1_v1_direct_pty_breakpoint_arming_timing/manifest.json`']) + '\n', encoding='utf-8')
