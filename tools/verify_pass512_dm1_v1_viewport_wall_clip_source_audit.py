...
def main() -> int:
    ...
    firestaff = [
        source_window("local_clip_gate_contract", ROOT / "src/dm1/dm1_v1_viewport_3d_pc34_compat.c", "765-818", ["DM1_ViewportBlitClipGate dm1_viewport_3d_resolve_wall_blit_clip_gate", "gate.source_lines = \"DUNVIEW.C:3053-3058,3198-3204; COORD.C:2390-2409; IMAGE3.C:866-889\";", "int src_x = frame->blit_x;", "int src_y = frame->blit_y;", "if (dst_x < 0) { src_x -= dst_x; width += dst_x; dst_x = 0; }", "if (src_x + width > source_width) width = source_width - src_x;", "gate.src_x = (int16_t)src_x;", "gate.height = (int16_t)height;"]),
        source_window("local_transparent_wall_rows_use_clip_gate", ROOT / "src/dm1/dm1_v1_viewport_3d_pc34_compat.c", "517-541", ["void dm1_viewport_3d_draw_wall(", "dm1_viewport_3d_resolve_wall_blit_clip_gate", "const uint8_t *src_row = wall_bitmap + (gate.src_y + y) * bw + gate.src_x;", "uint8_t *dst_row = vp + (gate.dst_y + y) * vp_stride + gate.dst_x;", "if (pixel != COLOR_TRANSPARENT)"]),
        source_window("local_opaque_wall_rows_use_clip_gate", ROOT / "src/dm1/dm1_v1_viewport_3d_pc34_compat.c", "551-569", ["void dm1_viewport_3d_draw_wall_opaque(", "dm1_viewport_3d_resolve_wall_blit_clip_gate", "const uint8_t *src_row = wall_bitmap + (gate.src_y + y) * bw + gate.src_x;", "memcpy(dst_row, src_row, (size_t)gate.width);"]),
        source_window("local_clip_tests_cover_source_and_viewport_occlusion", ROOT / "tests/test_dm1_v1_viewport_3d_pc34_compat.c", "876-952", ["static void test_wall_source_row_clip_occlusion_gate(void)", "check_int(\"wall_clip_gate.151713.src_x\", gate.src_x, 4);", "check_int(\"wall_clip_gate.occluded_source_row\", gate.visible ? 1 : 0, 0);", "static void test_wall_draw_uses_clip_gate_source_offsets(void)", "check_int(\"wall_clip_draw.source_offset_next\"", "check_int(\"wall_clip_draw.opaque_copies_transparent_color\""]),
    ]
