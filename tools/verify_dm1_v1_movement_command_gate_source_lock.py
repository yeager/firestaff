...
FIRESTAFF_FILES = [
    ...
    {'path': ROOT / 'test_dm1_v1_input_command_queue_pc34_compat.c', 'claim': 'input queue probe covers keypad, PC-34 normalized arrow rows, shifted arrows, pending mouse replay, turn gate bypass, movement gate retention, and four-command queue capacity', 'needles': ['enqueue keypad forward', 'locked mouse becomes pending', 'movement gate set', 'turn bypasses movement disabled gate', 'pc34 table left arrow turns left', 'pc34 table up arrow moves forward', 'pc34 io2 shifted up arrow normalizes to forward', 'pc34 shifted backward arrow strafes right', 'redmcsb queue holds four commands']},
