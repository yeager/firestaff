#!/usr/bin/env python3
"""pass803 DM1 V1 ordered-cells-to-attack contract."""
...
SRC = ROOT / "src/dm1/dm1_v1_ordered_cells_to_attack_pc34_compat.c"
HDR = ROOT / "include/firestaff/dm1/v1/ordered_cells_to_attack_pc34_compat.h"
TEST = ROOT / "tests/test_dm1_v1_ordered_cells_to_attack_pc34_compat.c"
...
LOCAL_NEEDLES = [
    "dm1_v1_ordered_cells_to_attack_table_pc34",
    "dm1_v1_ordered_cells_to_attack_pc34",
    "dm1_v1_ordered_cells_to_attack_dispatch_pc34",
    "dm1_v1_ordered_cells_to_attack_row_is_permutation_pc34",
    "kRowCount",
    "kColCount",
    "kDirSouth",
    "kDirEast",
...
CMAKE_NEEDLES = [
    "test_dm1_v1_ordered_cells_to_attack_pc34_compat",
    "src/dm1/dm1_v1_ordered_cells_to_attack_pc34_compat.c",
    "NAME dm1_v1_ordered_cells_to_attack_pc34_compat",
    f"verify_{PASS}",
]

REDMCSB_WINDOWS = {
    "DATA.C": [
        (29, "G0023_aac_Graphic562_OrderedCellsToAttack"),
        (234, "G0023_aac_Graphic562_OrderedCellsToAttack"),
        (243, "East from position Southeast or Southwest"),
        (878, "G0023_aac_Graphic562_OrderedCellsToAttack"),
    ],
    "PROJEXPL.C": [
        (1302, "G0023_aac_Graphic562_OrderedCellsToAttack"),
    ],
    "DEFS.H": [
        (7644, "F0228_GROUP_GetDirectionsWhereDestinationIsVisibleFromSource"),
    ],
}
...
def write_outputs(
    ...
    manifest = {
        ...
        "scope": (
            ...
            "G0023_aac_Graphic562_OrderedCellsToAttack[8][4] — the "
            "creature-AI attack-direction priority table. Each of the "
            "8 rows maps to an (attack-direction, attacker-position) "
            "pair and holds 4 directional offsets to try in order. "
            "Read site: PROJEXPL.C:1302 F0229_GROUP_SetOrderedCellsToAttack "
            "calls CopyBytes(G0023[Index], P0487_pc_OrderedCellsToAttack, "
            "M543_BYTE_COUNT_INT(4)) where Index is derived from "
        ...
        "anchorDriftTodo": [
            "Re-pin DATA.C:234-243 + DATA.C:878-887 + PROJEXPL.C:1302 "
            "+ DEFS.H:7644 line numbers when the local ReDMCSB tree "
            "updates."
        ]
        if drift
        else [],
    ...
    lines = [
        ...
        "- Gate: Graphics.dat item 562 init var G0023_aac_Graphic562_"
        "OrderedCellsToAttack[8][4] = 8 rows of 4 directions each. "
        "Each row is a permutation of {0, 1, 2, 3} (SOUTH/EAST/NORTH/"
        "WEST) representing the attack-direction priority list for one "
        "(attack-direction, attacker-position) tuple. Read site: "
        "PROJEXPL.C:1302 F0229_GROUP_SetOrderedCellsToAttack copies "
        "the row to a 4-byte buffer.",
        "- Runtime assertion floor: 222 assertions in `tests/test_dm1_v1_"
        "ordered_cells_to_attack_pc34_compat.c`.",
    ...
    if drift:
        lines.extend(["", "## TODO", ""])
        lines.append(
            "- Anchor drift note: re-pin DATA.C:234-243, DATA.C:878-"
            "887, PROJEXPL.C:1302, DEFS.H:7644 in next source refresh."
        )
...
def main() -> int:
    ...
    runs = [
        run([str(build_dir / "test_dm1_v1_ordered_cells_to_attack_pc34_compat")])
    ]
