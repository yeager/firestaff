#!/usr/bin/env python3
"""Pass317: verify the N2 late-F0380/F0097 runtime probe outcome.

This is intentionally conservative.  It records the exact late-F0380 breakpoint attempt after pass316, but promotes no new runtime seam unless the probe manifest
...
PROBE_DIR = ROOT / "parity-evidence/verification/pass317_dm1_v1_f0380_f0097_direct_probe_attempt"
PROBE_MANIFEST = PROBE_DIR / "manifest.json"
PROBE_TRANSCRIPT = PROBE_DIR / "dosbox_debug_noise_reduced.clean.txt"
OUT_JSON = ROOT / "parity-evidence/verification/pass317_dm1_v1_f0380_f0097_direct_probe_attempt.json"
OUT_MD = ROOT / "parity-evidence/pass317_dm1_v1_f0380_f0097_direct_probe_attempt.md"
...
def main() -> int:
    ...
    status = "PROMOTED_DIRECT_RUNTIME_HIT" if (f0380_hit or f0097_hit) and all(r["ok"] for r in source) else "BLOCKED_DIRECT_F0380_F0097_AFTER_LATE_F0380_PROBE"
    manifest = {
        "schema": "pass317_dm1_v1_f0380_f0097_direct_probe_attempt.v1",
        "status": status,
        "attempted_command": "python3 tools/pass317_dm1_v1_late_f0380_f0097_direct_probe_attempt.py --seconds 70",
        "addresses": ADDR,
        "source_audit": source,
