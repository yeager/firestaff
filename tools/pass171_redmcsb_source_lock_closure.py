...
def main():
 ...
 lines=['# Pass171 — ReDMCSB source-lock closure','','## Result','',f"- status: **{status}**",f"- pass168 total pass groups: **{total}**",f"- pass168 directly source-locked: **{direct}**",f"- pass169 resolved `needs-redmcsb-anchor`: **{gap}**",f"- pass170 resolved `source-mentioned-unresolved`: **{mentioned}**",f"- closed: **{closed}/{total}**",f"- remaining: **{total-closed}**",'','## Inputs','',f'- `{P168.relative_to(REPO)}`',f'- `{P169.relative_to(REPO)}`',f'- `{P170.relative_to(REPO)}`','','## Interpretation','', 'The full historical pass corpus covered by pass168 now has a ReDMCSB source-lock path: direct anchors for 211 groups, explicit addendum anchors for the 11 weak/missing groups, and lane-level source anchors for the 46 source-mentioned unresolved groups.']
