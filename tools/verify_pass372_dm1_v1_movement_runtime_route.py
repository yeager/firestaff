...
def main():
    ...
    cmake_lines=lines(ROOT/'CMakeLists.txt','466-467'); cmake_ok='add_executable(test_dm1_v2_movement_command_adapter_pc34' in cmake_lines and 'test_dm1_v2_movement_command_adapter_pc34.c' in cmake_lines
    checks.append({"kind":"cmakelists_466_467_audit","file":"CMakeLists.txt","lines":"466-467","ok":cmake_ok,"observed":compact(cmake_lines)})
    ...
    out += ["","## Residual scope","",f"- Firestaff runtime route: {residual['firestaffRuntimeRoute']}",f"- Original DOS residual: {residual['originalDosResidual']}","- Route tokens audited: `/down`, `/left`, `/right`, and combined quality token `/down/left/right` remain covered by M12 movement input -> C001..C006 queue dispatch.","- `CMakeLists.txt:466` / `CMakeLists.txt:467` audited as unrelated DM1 V2 movement-command-adapter test wiring; no DM1 V1 route change needed there.",f"- Decision: {residual['decision']}","","## Gates","",f"- `ctest --test-dir build -R '{ctest_re}' --output-on-failure` ok=`{ctest_ok}`",f"- Manifest: `parity-evidence/verification/{PASS}/manifest.json`"]
    REPORT.write_text('\n'.join(out)+'\n')
