...
p=subprocess.run([str(Path(os.environ.get("FIRESTAFF_BUILD_DIR", str(ROOT/"build")))/"test_dm1_v1_viewport_3d_pc34_compat")],cwd=ROOT,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
ok=all(r["status"]=="PASS" for r in rows+lrows) and p.returncode==0
m={"schema":"pass518_dm1_v1_d1_side_field_occlusion_source_lock.v1","status":"passed" if ok else "failed","statusToken":"PASS518_DM1_V1_D1_SIDE_FIELD_OCCLUSION_SOURCE_LOCKED" if ok else "FAILED_PASS518_DM1_V1_D1_SIDE_FIELD_OCCLUSION_SOURCE_LOCK","redmcsbChecks":rows,"firestaffChecks":lrows,"verificationRuns":[{"command":[str(Path(os.environ.get("FIRESTAFF_BUILD_DIR", str(ROOT/"build")))/"test_dm1_v1_viewport_3d_pc34_compat")],"returncode":p.returncode,"passed":p.returncode==0,"outputTail":"\n".join(p.stdout.splitlines()[-10:])}],"nonClaims":["no input or movement queue edits","no original DOS pixel parity claim","no DANNESBURK use"]}
