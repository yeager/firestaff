...
locals=[("src/dm1/dm1_v1_viewport_3d_pc34_compat.c","DM1_VIEW_SQUARE_D1L, 0x0032"),("src/dm1/dm1_v1_viewport_3d_pc34_compat.c","DM1_VIEW_SQUARE_D1R, 0x0041"),("tests/test_dm1_v1_viewport_3d_pc34_compat.c","floor_field_order_spec_count(), 15")]
lrows=[{"file":f,"needle":n,"status":"PASS" if n in (ROOT/f).read_text(errors="replace") else "FAIL"} for f,n in locals]
p=subprocess.run([str(resolve_build_executable(ROOT,"test_dm1_v1_viewport_3d_pc34_compat",ROOT/"build"))],cwd=ROOT,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
ok=all(r["status"]=="PASS" for r in rows+lrows) and p.returncode==0
m={"schema":"pass518_dm1_v1_d1_side_field_occlusion_source_lock.v1","status":"passed" if ok else "failed","statusToken":"PASS518_DM1_V1_D1_SIDE_FIELD_OCCLUSION_SOURCE_LOCKED" if ok else "FAILED_PASS518_DM1_V1_D1_SIDE_FIELD_OCCLUSION_SOURCE_LOCK","redmcsbChecks":rows,"firestaffChecks":lrows,"verificationRuns":[{"command":[str(resolve_build_executable(ROOT,"test_dm1_v1_viewport_3d_pc34_compat",ROOT/"build"))],"returncode":p.returncode,"passed":p.returncode==0,"outputTail":"\n".join(p.stdout.splitlines()[-10:])}],"nonClaims":["no input or movement queue edits","no original DOS pixel parity claim","no DANNESBURK use"]}
