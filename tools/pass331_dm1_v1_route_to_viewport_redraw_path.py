...
def source_audit() -> list[dict[str, Any]]:
    specs = [
        ("COMMAND.C", "F0380 command dequeue/dispatch", [
            ...
            "void F0361_COMMAND_ProcessKeyPress", "G0459_as_Graphic561_SecondaryKeyboardInput_Movement",
        ]),
        ...
        if chunk:
            ...
            if "(Running)" in c and "->" in c.split("(Running)",1)[-1]:
                ...
                if addr == ADDR["F0128_DUNGEONVIEW_Draw_CPSF"] and not f0128_seen:
                    f0128_seen=True; dbg(child, f"BP {ADDR[F0097_DUNGEONVIEW_DrawViewport]}", cmdlog, transcript)
...
def run_probe(seconds: int) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="firestaff-pass331-") as td:
        ...
        try:
            ...
            if ADDR["F0097_DUNGEONVIEW_DrawViewport"] in [s.get("addr") for s in stops]: status="PASS_ROUTE_REACHES_F0128_F0097_SEQUENCE"
            elif "F0128_DUNGEONVIEW_Draw_CPSF" in labels: status="PASS_ROUTE_REACHES_F0128_REDRAW"
            elif ("F0361_COMMAND_ProcessKeyPress" in labels) and ("F0380_COMMAND_ProcessQueue_CPSC" not in labels): status="BLOCKED_PASS331_ROUTE_KEYS_NOT_COMMAND_QUEUE"
            elif ("F0380_COMMAND_ProcessQueue_CPSC" in labels) and not (set(labels) & {"F0365_COMMAND_ProcessTypes1To2_TurnParty","F0366_COMMAND_ProcessTypes3To6_MoveParty"}): status="BLOCKED_PASS331_ROUTE_KEYS_NOT_COMMAND_QUEUE"
            elif (set(labels) & {"F0365_COMMAND_ProcessTypes1To2_TurnParty","F0366_COMMAND_ProcessTypes3To6_MoveParty"}) and "F0267_MOVE_GetMoveResult_CPSCE" not in labels: status="BLOCKED_PASS331_ROUTE_STUCK_OUTSIDE_DUNGEON"
            elif "F0267_MOVE_GetMoveResult_CPSCE" in labels: status="BLOCKED_PASS331_COMMAND_NO_REDRAW_DIRTY_FLAG"
            else: status="BLOCKED_PASS331_ROUTE_KEYS_NOT_COMMAND_QUEUE"
            return {"ran": True, "durationSeconds":round(time.time()-start,3), "boundedSeconds":seconds, "routeLog":routelog, "commandLog":cmdlog, "stops":stops, "sawRunning":saw_running, "status":status, "stage":None if status.startswith("PASS") else "command queue mapping", "blocker":None if status.startswith("PASS") else status}
        finally:
            try:
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if missing: status="BLOCKED_PASS331_ROUTE_KEYS_NOT_COMMAND_QUEUE"
    elif args.no_runtime: status="BLOCKED_PASS331_ROUTE_KEYS_NOT_COMMAND_QUEUE"
    else: status=probe.get("status", "BLOCKED_PASS331_ROUTE_KEYS_NOT_COMMAND_QUEUE")
    return {"schema":PASS+".v1", "timestampUtc":datetime.now(timezone.utc).isoformat(), "status":status, "hypothesis":"route keys are delivered but not mapped to DM command queue", "sourceAudit":audit, "addresses":ADDR, "runtimeProbe":{"ran": bool(probe), "missingTools":missing, "boundedSecondsPerProbe":args.seconds, "probes":[probe] if probe else []}, "blocker": None if status.startswith("PASS") else (probe or {}).get("blocker", "runtime unavailable"), "notPromotedBy":["BPLIST","BP command echo","tmux/capture-pane"]}
...
def main() -> int:
    ...
    report_lines=["# Pass331 — DM1 V1 route-to-viewport-redraw path", "", "Status: `" + manifest["status"] + "`", "", "## ReDMCSB anchors"]
    report_lines += ["- `" + r["file"] + "` " + r["id"] + ": `" + str(r["anchors"]) + "`" for r in manifest["sourceAudit"]]
    report_lines += ["", "## Runtime decision", "", "- Hypothesis: `" + manifest["hypothesis"] + "`", "- Stops: `" + str(stop_summary) + "`", "- Blocker: `" + str(manifest.get("blocker")) + "`", "", "Manifest: `parity-evidence/verification/pass331_dm1_v1_route_to_viewport_redraw_path/manifest.json`"]
    REPORT.write_text("\n".join(report_lines)+"\n", encoding="utf-8")
    return 0
