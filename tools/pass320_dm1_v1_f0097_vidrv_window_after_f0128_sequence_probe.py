...
OUT=ROOT/'parity-evidence/verification/pass320_dm1_v1_f0097_vidrv_window_after_f0128_sequence_probe'
...
def stopped_signature(text):
    # Pass320 treated BPLIST/setup echoes as stops. Pass320 is stricter.
    hits=[]
    for line in text.splitlines()[-120:]:
        stripped=line.strip()
        low=stripped.lower()
        if not stripped or stripped.startswith('->') or low.startswith('breakpoint list') or 'breakpoint at' in low or re.match(r'^[0-9]+\.\s+BP\s+', stripped, re.I):
            continue
        if re.match(r'^BP\s+[0-9A-F]{4}:[0-9A-F]{4}\b', stripped, re.I):
            hits.append(stripped)
        elif re.match(r'^(23AD:40FE|2809:1E31|2809:1EBD|2809:1EEE|2809:1EFF)\s+',stripped,re.I):
            hits.append(stripped)
        elif re.search(r'\b(code\s+)?breakpoint\s*:?\s*[0-9A-F]{4}:[0-9A-F]{4}', stripped, re.I):
            hits.append(stripped)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='firestaff-pass320-') as td:
        ...
        sess=f'pass320-{os.getpid()}'; stop_evt=threading.Event(); run_evt=threading.Event()
        try:
    ...
    status='F0097_VIDRV_WINDOW_HIT_VERIFIED' if f0128 and f0097 else ('BLOCKED_F0097_WINDOW_ARMED_AFTER_F0128_NO_HIT' if f0128 else 'BLOCKED_F0128_GATE_NOT_RECAPTURED_STRICT_STOP_FILTER')
    manifest={'schema':'pass320_dm1_v1_f0097_vidrv_window_after_f0128_sequence_probe.raw_probe.v1','timestamp_utc':datetime.now(timezone.utc).isoformat(),'status':status,'source_audit':audit,'addresses':ADDR,'route':args.route,'method':'Pass320 robust sequencing: arm only F0128 first; ignore BPLIST/setup echoes as stops; only after a real BP stop arm exact VIDRV call 2809:1EFF plus nearby F0097 offsets and continue guarded route.','debugger_events':events[:240],'debugger_hits_captured':hits[:80],'direct_hits':{'f0128_23AD_40FE':f0128,'f0097_window':bool(f0097),'f0097_hit_kinds':[h.get('kind') for h in f0097]},'bounded_sanitized_transcript_excerpt':bounded_excerpt(transcript),'blocker':None if status.endswith('VERIFIED') else ('Candidate window was armed correctly after a strict F0128 stop, but exact 2809:1EFF and nearby offsets never hit.' if f0128 else 'Pass320 strict stop filter did not recapture F0128. Pass318/older scripts confuse BPLIST setup echoes with stops; the next blocker is debugger stop/control sequencing before the post-F0128 F0097 window can be trusted.'),'promotion_rule':'Promote viewport_present only for an explicit F0097-window debugger stop after prior F0128 stop.'}
