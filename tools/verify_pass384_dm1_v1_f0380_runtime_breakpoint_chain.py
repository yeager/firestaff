#!/usr/bin/env python3
"""Pass384 verifier: DM1 V1 F0380/F0365/F0366 runtime breakpoint chain.

This is a bounded original-runtime probe. It promotes only strict post-Running
...
PASS = "pass384_dm1_v1_f0380_runtime_breakpoint_chain"
...
def classify_stop(post: str) -> dict[str, Any]:
    ...
    for name in ["F0380_COMMAND_ProcessQueue_CPSC", "F0365_COMMAND_ProcessTypes1To2_TurnParty", "F0366_COMMAND_ProcessTypes3To6_MoveParty", "F0128_DUNGEONVIEW_Draw_CPSF"]:
        if addr == ADDR[name]:
            kind = name
    return {"addr": addr, "kind": kind, "memoryBreakpoints": mem[-4:], "postRunningCodeLines": code_lines(c)[-10:], "postRunningExcerpt": c[-2500:]}
...
def runtime_probe(seconds: int, route: str) -> dict[str, Any]:
    ...
    start = time.time()
    arm_time = None
    with tempfile.TemporaryDirectory(prefix="firestaff-pass384-") as td:
        stage = Path(td) / "dos"
        ...
        try:
            ...
            try:
                ...
                xvfb.kill()
            OUT.mkdir(parents=True, exist_ok=True)
            (OUT / "pass384_runtime.clean.txt").write_text(clean("".join(transcript))[-400000:] + "\n", encoding="utf-8")
...
def summarize(runtime: dict[str, Any]) -> tuple[str, dict[str, Any], str | None]:
    ...
    if all(predicates.values()):
        return "PASS384_F0380_DEQUEUE_G0321_EXIT_NEXT_F0128_PROVEN", predicates, None
    if predicates["sawRunning"] and predicates["routeInputAfterArming"] and predicates["breakpointsRetainedAtArm"]:
        ...
        return "BLOCKED_PASS384_RUNTIME_CHAIN_NOT_PROVEN", predicates, "strict post-Running runtime chain incomplete; missing " + ", ".join(missing)
    return "FAIL_PASS384_RUNTIME_CONTROL_OR_ARMING_INCOMPLETE", predicates, "runtime control/arming predicates failed"
...
def main() -> int:
    ...
    status, predicates, blocker = summarize(runtime) if runtime.get("ran") else ("BLOCKED_PASS384_RUNTIME_NOT_RUN", {}, runtime.get("blocker"))
    if not all(r["ok"] for r in source):
        status = "FAIL_PASS384_SOURCE_AUDIT_FAILED"
    ...
    manifest = {
        ...
        "runtimeArtifacts": {
            "transcript": f"parity-evidence/verification/{PASS}/pass384_runtime.clean.txt",
            "routeKeylog": f"parity-evidence/verification/{PASS}/pass384_route_keylog.json",
            "commandLog": f"parity-evidence/verification/{PASS}/pass384_command_log.json",
        },
        "proofPredicates": predicates,
        "blocker": blocker,
        "promotionRule": "Promote only if strict post-Running stops prove F0380 plus F0365/F0366, G0321 writes nonzero, and a later F0128 stop in the same bounded post-load run. BPLIST/setup echoes and forced pause samples are excluded.",
        "notPromotedBy": ["BPLIST", "BP/BPM command echo", "forced pause", "route keylog alone", "source-only address binding"],
    }
    (OUT / "manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (OUT / "pass384_route_keylog.json").write_text(json.dumps(runtime.get("routeLog", []), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (OUT / "pass384_command_log.json").write_text(json.dumps(runtime.get("commandLog", []), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT.write_text("\n".join([
        ...
        "## Decision",
        "",
        "Strict runtime chain proven." if status.startswith("PASS384") else f"Blocked: {blocker}",
        "",
        "## Runtime predicates",
        "",
        ...
        *[f"- `{k}`: `{v}`" for k, v in ADDR.items()],
        ...
        f"- Transcript: `parity-evidence/verification/{PASS}/pass384_runtime.clean.txt`",
        f"- Route keylog: `parity-evidence/verification/{PASS}/pass384_route_keylog.json`",
        f"- Command log: `parity-evidence/verification/{PASS}/pass384_command_log.json`",
    ]) + "\n", encoding="utf-8")
    ...
    return 0 if status.startswith("PASS384") or status.startswith("BLOCKED_PASS384") else 1
