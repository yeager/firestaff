...
static int test_allocator_orchestrator_hit_bypasses_orchestrator_and_allocator(void) {
    const char* path = "./test_graphics_dat_allocator_orchestrator.bin";
    struct MemoryGraphicsDatState_Compat fileState = {0};
    struct MemoryGraphicsDatRuntimeState_Compat runtimeState;
    struct MemoryGraphicsDatAllocatorOrchestratorResult_Compat result;
    ...
    gApplyCallCount = 0;
    gPrepareCallCount = 0;
    if (!write_format1_file(path)) return 0;
    if (!F0479_MEMORY_InitializeGraphicsDatState_Compat(path, &fileState, &runtimeState)) return 0;
    if (F0489_MEMORY_GetOrLoadBitmapWithAllocatorOrchestrator_Compat(path, &runtimeState, &fileState, 1, loadedGraphic, indices, derivedIndices, blockOffsets, blocks, 1, spareBlock, &usageState, headers, 1, segments, 2, &allocatorState, &defragState, &firstUnusedBlock, &splitRemainder, &result) != existingBlock->bitmap) {
        F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    if (result.orchestrator.segmentCount != 0 || result.normalizedRequestedSize != 0 || allocatorState.cacheBytesAvailable != 50) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    if (existingBlock->usageCount != 1 || gBuildCallCount || gApplyCallCount || gPrepareCallCount) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 1;
}

static int test_allocator_orchestrator_miss_runs_orchestrator_and_allocator(void) {
    const char* path = "./test_graphics_dat_allocator_orchestrator.bin";
    struct MemoryGraphicsDatState_Compat fileState = {0};
    struct MemoryGraphicsDatRuntimeState_Compat runtimeState;
    struct MemoryGraphicsDatAllocatorOrchestratorResult_Compat result;
    ...
    if (!write_format1_file(path)) return 0;
    if (!F0479_MEMORY_InitializeGraphicsDatState_Compat(path, &fileState, &runtimeState)) return 0;
    if (F0489_MEMORY_GetOrLoadBitmapWithAllocatorOrchestrator_Compat(path, &runtimeState, &fileState, 0, loadedGraphic, indices, derivedIndices, blockOffsets, blocks, 2, newBlock, &usageState, headers, 2, segments, 3, &allocatorState, &defragState, &firstUnusedBlock, &splitRemainder, &result) != newBlock->bitmap) {
        F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    if (result.orchestrator.segmentCount != 2 || result.orchestrator.movedBlockCount != 1 || result.orchestrator.compactedTopOffset != 26) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    if (result.normalizedRequestedSize != 14 || !result.allocator.defragmentRequested || !result.defragApplied) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    if (allocatorState.cacheBytesAvailable != 86 || allocatorState.cacheMemoryTopOffset != 26 || firstUnusedBlock != 0 || blockOffsets[0] != 0) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    if (gBuildCallCount != 1 || gApplyCallCount != 0 || gPrepareCallCount != 1 || indices[0] != 1) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; }
    for (i = 0; i < 3; i++) { if (loadedGraphic[i] != (unsigned char)(0x10 + i) || newBlock->bitmap[i] != loadedGraphic[i]) { F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 0; } }
    F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState); return 1;
}

int main(void) {
    if (!test_allocator_orchestrator_hit_bypasses_orchestrator_and_allocator()) { fprintf(stderr, "test_allocator_orchestrator_hit_bypasses_orchestrator_and_allocator failed\n"); return 1; }
    if (!test_allocator_orchestrator_miss_runs_orchestrator_and_allocator()) { fprintf(stderr, "test_allocator_orchestrator_miss_runs_orchestrator_and_allocator failed\n"); return 1; }
    printf("ok\n");
    return 0;
}
