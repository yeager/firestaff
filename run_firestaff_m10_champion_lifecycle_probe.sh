...
OUT_DIR=${2:-$HERE/verification-m10/champion-lifecycle}
ROOT=$HERE

mkdir -p "$OUT_DIR"

PROBE_BIN="$ROOT/firestaff_m10_champion_lifecycle_probe_bin"

cc -Wall -Wextra -O1 \
    -I "$ROOT" \
    -o "$PROBE_BIN" \
    "$ROOT/firestaff_m10_champion_lifecycle_probe.c" \
