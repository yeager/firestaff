/* CSBWin SaveGame.cpp global-variable raw DB11 recovery regression. */
...
int main(void)
{
    enum { record_index = 7u, record_id = (5u << 24) | (4u << 16) | record_index,
    ...
    check(csb_v1_runtime_recover_csbwin_global_variables_record(
          ...
          "CSBWin recovers raw globals without importing the runtime bank");
    check(csb_v1_runtime_recover_csbwin_global_variables_record(
              &profile, 999u, recovered) == 0,
          "CSBWin rejects a global-variable index beyond SaveGame source range");
    profile.csbwin_appended_tail[0] ^= 1u;
    check(csb_v1_runtime_recover_csbwin_global_variables_record(
              &profile, record_index, recovered) == 0,
          "CSBWin rejects a drifted global-variable receipt");
    ...
    check(csb_v1_runtime_recover_csbwin_global_variables_record(
              &profile, record_index, recovered) == 0,
          "CSBWin rejects duplicate global-variable records");
    csb_v1_runtime_cleanup(&profile);
    ...
    check(csb_v1_runtime_recover_csbwin_global_variables_record(
              &profile, record_index, recovered) == 0,
          "CSBWin rejects a short global-variable record");
    csb_v1_runtime_cleanup(&profile);
    ...
    check(csb_v1_runtime_recover_csbwin_global_variables_record(
              &profile, record_index, recovered) == 0,
          "CSBWin rejects an absent global-variable record without defaulting");
    csb_v1_runtime_cleanup(&profile);
