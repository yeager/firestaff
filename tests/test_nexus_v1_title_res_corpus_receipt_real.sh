#!/usr/bin/env bash
# TITLE.BIN RES* corpus real-data wrapper. Skips with code 77 unless the
# pinned TITLE.BIN matches the expected byte size and SHA-256.
set -euo pipefail

if [ "$#" -ne 1 ]; then
    echo "usage: $0 <test_nexus_v1_title_res_corpus_receipt binary>" >&2
    exit 2
fi

cue="${FIRESTAFF_NEXUS_CUE:-$HOME/.firestaff/data/nexus/Dungeon Master Nexus (Japan).cue}"
if [ -f "$cue" ]; then
    exec "$1" "$cue::TITLE.BIN"
fi
