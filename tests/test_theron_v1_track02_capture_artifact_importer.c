...
static int write_bundle(const char *path, const Theron_V1Track02CaptureTargetPlan *plan, const char *trace_md5, int complete)
{
    FILE *file = fopen(path, "wb"); size_t i;
    if (!file || fprintf(file, "THERON_TRACK02_CAPTURE_ARTIFACT_BUNDLE_V1\ntrack02_md5=%s\nmednafen_trace_md5=%s\ncampaign_route=0\n", plan->targets[0].track02_md5, trace_md5) < 0) return 0;
...
int main(void)
{
    ...
    Theron_V1Track02CaptureArtifactRuntimeAdmissionReceipt receipt; char trace_md5[33], bundle_md5[33];
    plan_fixture(&plan);
    if (!write_trace(trace_path) || !m12_file_md5_hex(trace_path, trace_md5) || !write_bundle(bundle_path, &plan, trace_md5, 1) || !m12_file_md5_hex(bundle_path, bundle_md5)) return 1;
    request = (Theron_V1Track02CaptureArtifactImportRequest){bundle_path, bundle_md5, trace_path, trace_md5};
    if (!theron_v1_track02_capture_artifact_import(&plan, &request, &receipt) || receipt.status != THERON_V1_TRACK02_CAPTURE_ARTIFACT_READY || !receipt.opaque_envelope_verified || !receipt.opaque_runtime_ready || receipt.render_allowed || receipt.pixel_decode_allowed || receipt.level_object_semantics_allowed) return 2;
    ...
    if (!write_bundle(bundle_path, &plan, trace_md5, 0) || !m12_file_md5_hex(bundle_path, bundle_md5)) return 4;
    request.expected_bundle_md5 = bundle_md5;
