 ...
 *
 * Data-free parser checks.  The byte fixture mirrors the documented
 * DOS-EN layout; test_dm2_v1_songlist_dat_real_data is the original-media
 * proof and must not be replaced by this fixture.
 */
...
static const uint8_t songlist_fixture[63] = {
...
static void test_parse_valid(void) {
    ...
    assert(dm2_v1_songlist_dat_parse(&sl, songlist_fixture, 63) == 1);
    assert(sl.valid == 1);
}

static void test_map_0_track(void) {
    DM2_V1_SonglistDat sl;
    dm2_v1_songlist_dat_parse(&sl, songlist_fixture, 63);
...
static void test_out_of_range(void) {
    ...
    assert(dm2_v1_songlist_dat_track_for_map(&sl, 63) == -1);
    assert(dm2_v1_songlist_dat_track_for_map(&sl, 100) == -1);
}

static void test_null_safety(void) {
    DM2_V1_SonglistDat sl;
    assert(dm2_v1_songlist_dat_parse(NULL, songlist_fixture, 63) == 0);
    assert(dm2_v1_songlist_dat_parse(&sl, NULL, 63) == 0);
    assert(dm2_v1_songlist_dat_parse(&sl, songlist_fixture, 10) == 0);
    assert(dm2_v1_songlist_dat_track_for_map(NULL, 0) == -1);
}

static void test_rejects_invalid_selector(void) {
    uint8_t malformed[DM2_SONGLIST_FILE_SIZE];
    DM2_V1_SonglistDat sl;
    memcpy(malformed, songlist_fixture, sizeof(malformed));
    malformed[45] = 29;
    assert(dm2_v1_songlist_dat_parse(&sl, malformed, sizeof(malformed)) == 0);
}
...
int main(void) {
    ...
    printf("Source-shaped 63-byte SONGLIST.DAT fixture\n\n");
    ...
    test_rejects_invalid_selector();
                              printf("  invalid_selector     PASS\n");
