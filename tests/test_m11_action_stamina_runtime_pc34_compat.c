...
static void test_projectile_creature_impact_at_zero_zero_applies_damage(void) {
    ...
    rawGroupData[6] = 0xBEu;
    rawGroupData[7] = 0xEFu;
...
static void test_fuse_complete_fluxcage_sets_m11_game_won_gate(void) {
    ...
    ASSERT_EQ(M11_GameView_GetEndgameFinalPresentationReady(&state), 0,
              "FUSE complete final presentation query starts false");
    ...
    ASSERT_EQ(M11_GameView_GetEndgameFinalPresentationReady(&state), 1,
              "FUSE complete final presentation query flips when delay drains");
...
static void test_endgame_restart_controls_respect_restart_allowed(void) {
    ...
    {
        DM1_V1_EndgameRectPc34 restartRect;
        DM1_V1_EndgameRectPc34 quitRect;
        (void)dm1_v1_endgame_restart_box_pc34(1, &restartRect);
        (void)dm1_v1_endgame_quit_box_pc34(1, &quitRect);
        restartX = restartRect.x;
        restartY = restartRect.y;
        restartW = restartRect.w;
        restartH = restartRect.h;
        quitX = quitRect.x;
        quitY = quitRect.y;
        quitW = quitRect.w;
        quitH = quitRect.h;
