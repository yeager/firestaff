/* CSBWin Character.cpp EDT_Character raw-record recovery regression. */
...
int main(void)
{
    enum { fingerprint = 0x4521u, record_index = 3u,
           record_id = (8u << 24) | (record_index << 16) | fingerprint,
           block_bytes = CSB_V1_CSBWIN_EXPOOL_BLOCK_BYTES };
    uint32_t source[25];
    uint8_t tail[2u * block_bytes];
    uint32_t recovered[25] = { 0u };
    CSB_V1_RuntimeProfile profile;
    uint32_t receipt;
    unsigned int word;

    for (word = 0u; word < 25u; ++word) source[word] = 0x1c000000u + word;
    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, source, 27u);
    prepare(&profile, tail, sizeof(tail));
    receipt = profile.csbwin_appended_tail_fnv1a;
    check(csb_v1_runtime_recover_csbwin_wing_record(
              &profile, fingerprint, record_index, recovered) == 1 &&
              memcmp(recovered, source, sizeof(source)) == 0 &&
              profile.csbwin_appended_tail_fnv1a == receipt,
          "CSBWin recovers one raw wing record without assembling a character");
    check(csb_v1_runtime_recover_csbwin_wing_record(
              &profile, fingerprint, 8u, recovered) == 0,
          "CSBWin rejects a wing record index outside Character source range");
    profile.csbwin_appended_tail[0] ^= 1u;
    check(csb_v1_runtime_recover_csbwin_wing_record(
              &profile, fingerprint, record_index, recovered) == 0,
          "CSBWin rejects a drifted wing record receipt");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, source, 27u);
    add_record(tail, 1u, record_id, source, 27u);
    prepare(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_wing_record(
              &profile, fingerprint, record_index, recovered) == 0,
          "CSBWin rejects duplicate wing records");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, source, 26u);
    prepare(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_wing_record(
              &profile, fingerprint, record_index, recovered) == 0,
          "CSBWin rejects a short wing record");
    csb_v1_runtime_cleanup(&profile);
    ...
    check(csb_v1_runtime_recover_csbwin_wing_record(
              &profile, fingerprint, record_index, recovered) == 0,
          "CSBWin rejects an absent wing record without defaulting");
    csb_v1_runtime_cleanup(&profile);
