...
int main(int argc, char **argv)
{
    ...
    free(bytes);
    receipt.committed = -1;
    if (dm2_v1_dungeon_materialize_g1_runtime_map_doors(&dungeon, 0,
                                                         &receipt) ||
        receipt.committed != -1) {
        dm2_v1_dungeon_free(&dungeon);
        fputs("FAIL: File_header entered legacy G1 door path\n", stderr);
        return 1;
    }
    dm2_v1_dungeon_free(&dungeon);
    puts("PASS: real File_header leaves legacy G1 door path unavailable");
    return 0;
}
