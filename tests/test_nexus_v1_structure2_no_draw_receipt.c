...
int main(void) {
    ...
    if (nexus_v1_prepare_dgn_material_plan(&engine, 11, 29, 0) != NULL ||
        ...
        fprintf(stderr, "Structure2 source gate did not produce a no-draw plan\n");
        return 1;
    }
    ...
    if (nexus_viewport_last_dgn_render_receipt(&viewport, &render) != 0 ||
        ...
        fprintf(stderr, "Structure2 source gate did not produce a no-draw viewport receipt\n");
        return 1;
    }

    printf("Nexus Structure2 no-draw receipt: OK\n");
    return 0;
}
