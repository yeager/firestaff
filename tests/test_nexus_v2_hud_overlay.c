 ...
 * renders into the framebuffer, and produces deterministic state
...
int main(void) {
    ...
    check("compass: pixels written", compass_pixels > 0);
    ...
    check("gold counter: pixels written", gold_pixels > 0);
    ...
    check("depth: pixels written", depth_pixels > 0);
    ...
    check("hidden render (opacity=0): render completes", 1);
    ...
    h.hit_flash_timer = 3;
    nexus_v2_hud_render(&h, fb, FB_W, FB_H);
    check("render decrements flash timer", h.hit_flash_timer == 2);
    nexus_v2_hud_render(&h, fb, FB_W, FB_H);
    check("render decrements flash timer again", h.hit_flash_timer == 1);
    nexus_v2_hud_render(&h, fb, FB_W, FB_H);
    check("render decrements flash timer to zero", h.hit_flash_timer == 0 && h.hit_flash_active == false);
