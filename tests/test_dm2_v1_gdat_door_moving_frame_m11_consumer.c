...
int main(void)
{
    ...
    int golden, table_reject, movement_reject;

    memset(framebuffer, 0xa5, sizeof(framebuffer));
    dm2_v1_viewport_init(&viewport, framebuffer, DM2_VP_WIDTH);
    viewport.gdat_scene_control_ready = 1; viewport.gdat_scene_control_hash = 10;
    viewport.gdat_scene_colorkey = 2; viewport.gdat_scene_movement_active = 1;
    ...
    command->view_square = DM2_SQ_D1C; command->field = 0x07;
    command->movement_active = 1; command->mirror_flip = 1;
    ...
    command->rect_x = 12; command->rect_y = 22; command->rect_number = 5089;
    command->material_source_bytes = raw; command->material_source_byte_count = sizeof(raw);
    ...
    composition.valid = composition.no_draw = 1; composition.identity_hash = 9;
    composition.session_identity = composition.data_epoch = 1;
    composition.scene_command_hash = 10; composition.door_command_hash = 7;
    ...
    command->field = 0x08;
    table_reject = !dm2_v1_gdat_door_side_frame_m11_consume(
        &receipt, &plan, &composition, &viewport) &&
        memcmp(before, framebuffer, sizeof(before)) == 0;
    command->field = 0x07;
    viewport.gdat_scene_movement_active = 0;
    movement_reject = !dm2_v1_gdat_door_side_frame_m11_consume(
        &receipt, &plan, &composition, &viewport);
    printf("golden=%d table=%d movement=%d\n", golden, table_reject,
           movement_reject);
    return golden && table_reject && movement_reject ? 0 : 1;
}
