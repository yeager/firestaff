...
int main(void)
{
    ...
    check(csb_v1_runtime_materialize_csbwin_timer_queue(&profile) == 1 &&
              csb_v1_runtime_tick_v1(&profile) == 1 && raw[102] == 1u &&
              profile.timeline_queue.eventCount == 0,
          "DSA-free TT_STONEROOM retains its positioned DB2 owner");
    ...
    check(csb_v1_runtime_materialize_csbwin_timer_queue(&profile) == 1 &&
              csb_v1_runtime_tick_v1(&profile) == 1 && raw[102] == 0u &&
              profile.csbwin_global_variables[1] == 0x55aau,
          "DSA-owned TT_STONEROOM blocks generic text mutation after receipt");
    ...
    /* ProcessTT_TELEPORTER changes bit 3 after ActivateDSA. With no Thing
     * list and the party elsewhere, its WiggleEverything call is a source
     * no-op and the restored TIMER owns the exact cell update. */
    memset(&profile.timeline_queue, 0, sizeof(profile.timeline_queue));
    memset(profile.csbwin_timeline_event_queue_slot, 0xff,
           sizeof(profile.csbwin_timeline_event_queue_slot));
    raw[80] = 0xa0u;
    ...
    profile.csbwin_timers[0].time = profile.game_time;
    check(csb_v1_runtime_materialize_csbwin_timer_queue(&profile) == 1 &&
              csb_v1_runtime_tick_v1(&profile) == 1 && raw[80] == 0xa8u &&
              profile.timeline_queue.eventCount == 0,
          "DSA-free empty teleporter applies its source-owned cell flag");

    /* A type-47 owner may execute its authenticated pure-stack receipt, but
     * its unowned cell mutation must not fall into the generic M10 C08 path. */
    memset(&profile.timeline_queue, 0, sizeof(profile.timeline_queue));
    memset(profile.csbwin_timeline_event_queue_slot, 0xff,
           sizeof(profile.csbwin_timeline_event_queue_slot));
    raw[80] = 0xb0u;
    ...
    profile.csbwin_timers[0].time = profile.game_time;
    check(csb_v1_runtime_materialize_csbwin_timer_queue(&profile) == 1 &&
              csb_v1_runtime_tick_v1(&profile) == 1 && raw[80] == 0xb0u &&
              profile.csbwin_global_variables[1] == 0x55aau,
          "DSA-owned teleporter blocks generic cell mutation after its receipt");
    ...
    /* Timer.cpp DeleteTimer clears the dispatched handle and replaces its
     * queue entry with the final handle. SetTimer then uses firstAvailTimer,
     * assigns m_timerSequence, and appends the successor before heap repair.
     * Exercise the non-identical old/new handle path rather than accepting a
     * convenient in-place watchdog rewrite. */
    memset(&profile.timeline_queue, 0, sizeof(profile.timeline_queue));
    memset(profile.csbwin_timeline_event_queue_slot, 0xff,
           sizeof(profile.csbwin_timeline_event_queue_slot));
    ...
    profile.csbwin_timer_sequence = 7u;
    ...
    profile.csbwin_timers[1].function = DM1_EVENT_WATCHDOG;
    ...
    check(csb_v1_runtime_materialize_csbwin_timer_queue(&profile) == 1 &&
              ...
              profile.timeline_queue.eventCount == 1 &&
              profile.csbwin_timer_queue[0] == 0u &&
              profile.csbwin_timers[0].function == DM1_EVENT_WATCHDOG &&
              profile.csbwin_timers[0].source_index == 0u &&
              profile.csbwin_timers[0].sequence == 7u &&
              profile.csbwin_timers[0].time == profile.game_time + 299u &&
    ...
    /* ProcessTT_DOOR changes the consumed TIMER to TT_1 and immediately
     * passes it to SetTimer. The conversion remains a source pool handoff,
     * rather than rewriting the dispatched handle in place. */
    memset(&profile.timeline_queue, 0, sizeof(profile.timeline_queue));
    memset(profile.csbwin_timeline_event_queue_slot, 0xff,
           sizeof(profile.csbwin_timeline_event_queue_slot));
    memset(profile.csbwin_timers, 0, sizeof(profile.csbwin_timers));
    ...
    profile.csbwin_timer_queue[0] = 1u;
    check(csb_v1_runtime_materialize_csbwin_timer_queue(&profile) == 1 &&
              csb_v1_runtime_tick_v1(&profile) == 1 && raw[80u] == 0x82u &&
              profile.timeline_queue.eventCount == 1 &&
              profile.csbwin_timer_queue[0] == 0u &&
              profile.csbwin_timers[0].function == 1u &&
              profile.csbwin_timers[0].source_index == 0u &&
          ...
          "TT_DOOR conversion uses Timer.cpp pool ownership");
