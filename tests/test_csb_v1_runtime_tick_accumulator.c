...
static void test_c37_group_approach_creates_empty_destination_thing_list(void)
{
    ...
    CHECK(profile.active_group_state_count == 1u &&
          ...
          "C37 empty-destination move creates native active-group state with F0205 one-step turn");
...
/* ReDMCSB GROUP.C F0209:2040-2044,2154-2240: C29 starts with one absolute
 * M004 direction and escapes danger through the ordinary F0202/F0267 move
 * transaction, rather than entering the C37 perception state machine. */
static void test_c29_danger_reaction_uses_absolute_escape_direction(void)
{
    CSB_V1_RuntimeProfile profile;
    CSB_V1_DungeonData dungeon;
    uint8_t raw[144];
    ...
    raw[75] = 0xffu;
    test_put_le16(raw, 76, 40u);
    test_put_le16(raw, 84, 0u);   /* C0 wander. */
    ...
    event.b_mapY = 0;
    CHECK(csb_v1_runtime_add_timeline_event(&profile, &event) >= 0,
          "C29 fixture queues the danger reaction");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C29 fixture dispatches the danger reaction");
    CHECK(test_get_le16(raw, 66) == 0xfffeu &&
...
static void test_c30_projectile_reaction_behind_facing_turns_without_moving(void)
{
    ...
    expected_random_state =
        1u * UINT32_C(0xbb40e62d) + UINT32_C(11);
    expected_random_state =
        expected_random_state * UINT32_C(0xbb40e62d) + UINT32_C(11);
    memset(&event, 0, sizeof(event));
    event.type = DM1_EVENT_GROUP_REACTION_HIT_BY_PROJECTILE;
...
static void test_c37_group_approach_turns_moved_group_per_creature(void)
{
    ...
    printf("\n-- CSB C37 moved-group per-creature turn --\n");
    ...
    test_put_le16(raw, 70, 0xfffeu);
    test_put_le16(raw, 72, 0u);
    raw[74] = 9u;    /* Mummy: ordinary two-creature group. */
    raw[75] = 0x04u; /* creature 0 cell 0, creature 1 cell 1. */
    test_put_le16(raw, 76, 40u);
    test_put_le16(raw, 78, 40u);
    test_put_le16(raw, 84, (uint16_t)((1u << 5) | 7u)); /* C7, dir north. */
    ...
    ev.b_mapY = 0;
    CHECK(csb_v1_runtime_add_timeline_event(&profile, &ev) >= 0,
          "C37 moved-group turn fixture queues the approach event");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C37 moved-group turn fixture dispatches the approach event");

    directions = profile.active_group_state[0].directions;
    CHECK(test_get_le16(raw, 66) == 0xfffeu &&
              test_get_le16(raw, 68) == (uint16_t)(4u << 10),
          "C37 moved-group turn fixture moves the group to the target square");
    CHECK(((test_get_le16(raw, 84) >> 8) & 0x03u) == 2u,
          "C37 moved-group turn fixture writes raw C04 target-facing direction");
    CHECK(profile.active_group_state_count == 1u &&
              profile.active_group_state[0].valid &&
              profile.active_group_state[0].map_x == 0 &&
              profile.active_group_state[0].map_y == 1 &&
...
static void test_c37_group_approach_uses_stored_target_without_party_sight(void)
{
    ...
    printf("\n-- CSB C37 approach stored target without party sight --\n");
    ...
    raw[75] = 0xffu;
    test_put_le16(raw, 76, 40u);
    test_put_le16(raw, 84, (uint16_t)((1u << 5) | 7u));
    ...
    profile.active_group_state[0].home_map_x = 0;
    profile.active_group_state[0].home_map_y = 0;
    profile.active_group_state[0].target_map_x = 0;
    profile.active_group_state[0].target_map_y = 2;
    ...
    ev.b_mapY = 0;
    CHECK(csb_v1_runtime_add_timeline_event(&profile, &ev) >= 0,
          "C37 stored-target fixture queues the approach event");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C37 stored-target fixture dispatches the approach event");
    CHECK(test_get_le16(raw, 66) == 0xfffeu &&
              test_get_le16(raw, 68) == (uint16_t)(4u << 10),
          "C37 stored-target fixture moves toward prior visible target");
    CHECK(profile.active_group_state_count == 1u &&
              profile.active_group_state[0].valid &&
              profile.active_group_state[0].map_x == 0 &&
              profile.active_group_state[0].map_y == 1 &&
              ...
              profile.active_group_state[0].target_map_x == 0 &&
              profile.active_group_state[0].target_map_y == 2,
          "C37 stored-target fixture preserves ActiveGroup.TargetMapX/Y");
}
...
static void test_c37_blocked_wall_mirrors_half_square_pair_direction(void)
{
    ...
    printf("\n-- CSB C37 blocked-wall half-square direction mirror --\n");
    ...
    raw[real_format_square_offset(0, 0)] =
        (uint8_t)((1u << 5) | 0x10u);
    raw[real_format_square_offset(0, 1)] = 0u;
    test_put_le16(raw, 60 + 0 * 2, 0);
    test_put_le16(raw, 60 + 1 * 2, 1);
    test_put_le16(raw, 66, (uint16_t)(4u << 10));
    test_put_le16(raw, 68, 0xfffeu);
    test_put_le16(raw, 70, 0u);
    raw[72] = CREATURE_TYPE_PAIN_RAT;
    raw[73] = 0x04u;
    test_put_le16(raw, 74, 40u);
    test_put_le16(raw, 76, 40u);
    test_put_le16(raw, 82, (uint16_t)((1u << 5) | 7u));
    ...
    ev.b_mapY = 0;
    CHECK(csb_v1_runtime_add_timeline_event(&profile, &ev) >= 0,
          "C37 half-square fixture queues the approach event");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C37 half-square fixture dispatches the approach event");
    CHECK(profile.active_group_state_count == 1u &&
              profile.active_group_state[0].valid,
          "C37 half-square fixture creates native active-group state");
    directions = profile.active_group_state[0].directions;
    CHECK(((directions & 0x0003u) == ((directions >> 2) & 0x0003u)) &&
              (((directions & 0x0003u) == 1u) ||
               ((directions & 0x0003u) == 3u)),
          "C37 half-square fixture mirrors the F0205 clamp to the paired creature");
}

static void test_c37_half_square_direction_debounces_same_tick(void)
{
    CSB_V1_RuntimeProfile profile;
    CSB_V1_DungeonData dungeon;
    uint8_t raw[128];
    ...
    printf("\n-- CSB C37 half-square same-tick direction debounce --\n");
    ...
    ev.b_mapY = 0;
    CHECK(csb_v1_runtime_add_timeline_event(&profile, &ev) >= 0,
          "C37 half-square debounce fixture queues first event");
    CHECK(csb_v1_runtime_add_timeline_event(&profile, &ev) >= 0,
          "C37 half-square debounce fixture queues second same-tick event");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C37 half-square debounce fixture dispatches same-tick events");
    directions = profile.active_group_state[0].directions;
    CHECK(profile.active_group_state_count == 1u &&
              profile.half_square_direction_debounce_valid &&
              profile.half_square_direction_debounce_time == 0u &&
              profile.half_square_direction_debounce_group ==
                  (uint16_t)(4u << 10),
          "C37 half-square debounce records source-style group/time marker");
    CHECK(((directions & 0x0003u) == ((directions >> 2) & 0x0003u)) &&
              (((directions & 0x0003u) == 1u) ||
               ((directions & 0x0003u) == 3u)),
...
static void test_f0190_death_transaction_rolls_back_bad_slot_chain(void)
{
    CSB_V1_RuntimeProfile profile;
    CSB_V1_DungeonData dungeon;
    uint8_t raw[192];
    uint8_t before[sizeof(raw)];
    struct ExplosionCreateInput_Compat input;
    struct TimelineEvent_Compat first_advance;
    int slot = -1;

    printf("\n-- CSB F0190 transactional death rollback --\n");
    make_real_format_square_event_dungeon(&dungeon, raw, sizeof(raw));
    dungeon.square_first_thing_base = 66;
    dungeon.square_first_thing_count = 1;
    dungeon.thing_data_bases[4] = 82;
    ...
    dungeon.thing_type_counts[5] = 1;
    raw[real_format_square_offset(1, 1)] = (uint8_t)((1u << 5) | 0x10u);
    test_put_le16(raw, 60 + 1 * 2, 0u);
    test_put_le16(raw, 66, (uint16_t)(4u << 10));
    test_put_le16(raw, 82, 0xfffeu);
    test_put_le16(raw, 84, (uint16_t)(5u << 10));
    raw[86] = 9u;
    raw[87] = 0xffu;
    test_put_le16(raw, 88, 1u);
    test_put_le16(raw, 96, 0u);
    /* A C05 record whose Next points outside its type pool.  F0188 reaches
     * the invalid successor only after it has staged the first floor drop. */
    test_put_le16(raw, 98, 0x1fffu);
    ...
    input.creatorProjectileSlot = -1;
    CHECK(F0821_EXPLOSION_Create_Compat(
              &input, &profile.explosions, &slot, &first_advance) == 1 &&
              slot == 0,
          "F0190 rollback fixture creates a source C25 impact");
    queue_explosion_advance_event(&profile, &first_advance);
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1 &&
              csb_v1_runtime_tick_v1(&profile) == 1,
          "F0190 rollback fixture reaches the lethal C25 boundary");
    CHECK(memcmp(raw, before, sizeof(raw)) == 0,
          "F0190 rejects malformed C04 Slot ownership without partial drops");
...
static void test_runtime_save_roundtrips_projectiles_and_explosions(void)
{
    ...
    profile.active_group_state[0].target_map_x = 4;
    profile.active_group_state[0].target_map_y = 6;
    ...
    CHECK(loaded.active_group_state_count == 1u &&
              ...
              loaded.active_group_state[0].target_map_x == 4 &&
              loaded.active_group_state[0].target_map_y == 6 &&
...
static void test_timeline_wall_gate_and_generator_sensor_mutations(void)
{
    ...
    profile.dungeon_handle = &dungeon;
    {
        ...
        projectile_input.subtype = PROJECTILE_SUBTYPE_KINETIC_ARROW;
        projectile_input.ownerKind = PROJECTILE_OWNER_LAUNCHER;
        projectile_input.ownerIndex = 31;
        projectile_input.mapIndex = 0;
        projectile_input.mapX = 1;
        projectile_input.mapY = 0;
        projectile_input.cell = 0;
        ...
        CHECK(F0810_PROJECTILE_Create_Compat(
                  ...
                  &first_move) == 1 &&
                  projectile_slot == 0,
              "C49 C004 object-sensor fixture creates a kinetic projectile");
        queue_projectile_move_event(&profile, &first_move);
    }
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C49 C004 object-sensor fixture advances to first move tick");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C49 C004 object-sensor wall-impact event dispatches");
    CHECK(profile.projectiles.count == 0 &&
              test_get_le16(raw, 66) == (uint16_t)(3u << 10) &&
              test_get_le16(raw, 68) == (uint16_t)(5u << 10),
          "C49 C004 object-sensor materializes the object behind the sensor");
    CHECK(count_queued_event_type(&profile, DM1_EVENT_FAKEWALL) == 1,
          "C49 C004 object-sensor queues the target fakewall event");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "C49 C004 queued fakewall event fires on the next tick");
    CHECK((raw[real_format_square_offset(2, 0)] & 0x04u) == 0x04u,
          "C49 C004 object-sensor opens the target fakewall");
...
int main(void)
{
    ...
    test_f0190_death_transaction_rolls_back_bad_slot_chain();
