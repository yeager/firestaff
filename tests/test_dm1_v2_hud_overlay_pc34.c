...
int main(void) {
    ...
    v2_hud_clear_presentation_state();
    v2_hud_set_opacity(200);
    v2_hud_set_champion_overlay_state(2, 33, 66, 99, true, true);
    v2_hud_set_champion_overlay_state(-1, 100, 100, 100, true, true);
    v2_hud_set_champion_overlay_state(4, 100, 100, 100, true, true);
    v2_hud_set_action_overlay_state(1, 1, 6);
    v2_hud_set_rune_overlay_state((uint8_t)(1u << 3), 3, true, false, true);

    memset(fb, 0, sizeof(fb));
    v2_hud_render(fb, w, h);
    CHECK(fb[6 * w + 158] == 200);          /* champion 2 HP fill at 33% */
    CHECK(fb[6 * w + 159] == 100);          /* champion 2 HP clamp boundary */
    CHECK(fb[11 * w + 172] == 150);         /* stamina fill at 66% */
    CHECK(fb[16 * w + 186] == 133);         /* mana fill at 99% */
    CHECK(fb[1 * w + 190] == 255);          /* leader cue */
    CHECK(fb[7 * w + 193] == 255);          /* spell-ready cue */
    CHECK(fb[78 * w + 252] == 255);         /* active action flash strip */
    CHECK(fb[86 * w + 256] == 255);         /* active action icon */
    CHECK(fb[52 * w + 272] == 255);         /* active rune 3 */
    CHECK(fb[52 * w + 248] == 100);         /* inactive rune 1 */
    CHECK(fb[64 * w + 236] == 255);         /* cast-ready control */
    CHECK(fb[64 * w + 300] == 100);         /* recant disabled control */
    ...
    v2_hud_set_action_overlay_state(99, 99, 0);
    memset(fb, 0, sizeof(fb));
    v2_hud_render(fb, w, h);
    CHECK(fb[86 * w + 256] == 50);          /* invalid action clears highlight */
