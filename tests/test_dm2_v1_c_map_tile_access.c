...
static size_t build_fixture(uint8_t *buf, size_t cap)
{
    ...
    const size_t db0_base = stack_base + 4u;
    const size_t raw_map_base = db0_base + 8u;
    uint8_t *desc;
...
int main(void)
{
    ...
    CHECK(size == 82u, "synthetic c_map fixture is complete");
    ...
    CHECK(record == dungeon.raw_data + 72 && object_id == 0x0001u &&
          record_offset == 72,
