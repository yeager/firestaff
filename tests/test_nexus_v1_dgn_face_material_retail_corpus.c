...
int main(void)
{
    ...
    for (level_index = 0; level_index < 16; ++level_index) {
        char path[1024];
        char matched_path[ASSET_PATH_MAX];
        uint8_t *data;
        int size;
        int binding_count = 0;
        ...
        CHECK(data != NULL && expected_dgn_md5(level_index) &&
                  asset_find_by_md5(data_dir, expected_dgn_md5(level_index),
                                    matched_path, sizeof(matched_path), 0) == 1 &&
                  strcmp(matched_path, path) == 0,
        ...
        input.geometry_source_bound = level.geometry_info.mesh_ready;
        input.geometry_material_face_count = binding_count;
        input.geometry_can_submit_geometry = level.geometry_info.mesh_ready;
