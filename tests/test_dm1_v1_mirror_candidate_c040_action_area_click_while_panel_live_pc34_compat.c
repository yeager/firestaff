...
static void test_source_evidence_is_pinned(void)
{
    ...
    CHECK(evidence != 0);
    CHECK(strstr(evidence, "F0380:2309-2311") != 0);
    CHECK(strstr(evidence, "F0371") != 0);
    CHECK(strstr(evidence, "F0280:124-132") != 0);
    CHECK(strstr(evidence, "F0282:744-806") != 0);
    CHECK(strstr(evidence, "C111") != 0);
}

static void test_spec_is_stable(void)
{
    const DM1_V1_MirrorCandidateC040ActionAreaClickWhilePanelLiveSpecPc34 *spec =
    ...
    CHECK(spec->actionAreaClick == 111);
    CHECK(spec->nonOverlap != 0);
    CHECK(strstr(spec->nonOverlap, "action-area-click-while-c040-live") != 0);
    CHECK(strstr(spec->nonOverlap, "C100 spell-area") != 0);
    CHECK(strstr(spec->nonOverlap, "Disjoint from pass786") != 0);
}
...
{
    DM1_V1_MirrorCandidateC040ActionAreaClickWhilePanelLiveResultPc34 r;
    int ok =
    ...
    CHECK(r.f0371CallsAfterClear == 1);
    CHECK(r.rejectedAfterClear == 3);
    CHECK(r.panelContentAfterClear == 0);
    CHECK(r.candidateOrdinalAfterClear == 0);
}

static void test_run_gates_action_area_while_panel_live(void)
{
    DM1_V1_MirrorCandidateC040ActionAreaClickWhilePanelLiveResultPc34 r;
    int ok;
    dm1_v1_mirror_candidate_c040_action_area_click_while_panel_live_run_pc34(
        &r);
    ok = (r.f0371CallsWhileLive == 0 &&
...
int main(void)
{
    ...
    printf("dm1_v1_mirror_candidate_c040_action_area_click_while_panel_live: "
