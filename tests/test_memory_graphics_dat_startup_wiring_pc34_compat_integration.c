...
static int test_startup_wiring_marks_first_frame_prerequisites_ready(void) {
    const char* path = "./test_graphics_dat_startup_wiring.bin";
    struct MemoryGraphicsDatState_Compat fileState = {0};
    struct MemoryGraphicsDatStartupWiringResult_Compat wiring;
    ...
    if (!wiring.runtimeAssetsReady || !wiring.persistentSpecialsReady || !wiring.viewportPathReady || !wiring.firstFramePrerequisitesReady) {
        F0479_MEMORY_FreeStartupWiringMini_Compat(&wiring);
        return 0;
    }
    if (!wiring.boot.succeeded || wiring.boot.stage != MEMORY_GRAPHICS_DAT_BOOT_STAGE_COMPLETE) {
        F0479_MEMORY_FreeStartupWiringMini_Compat(&wiring);
        return 0;
    }
    if (wiring.boot.startup.specials.dialogBoxGraphic == 0 || wiring.boot.startup.viewportSelection.compressedByteCount != 3) {
        F0479_MEMORY_FreeStartupWiringMini_Compat(&wiring);
        return 0;
    }
...
int main(void) {
    if (!test_startup_wiring_marks_first_frame_prerequisites_ready()) {
        fprintf(stderr, "test_startup_wiring_marks_first_frame_prerequisites_ready failed\n");
        return 1;
    }
    printf("ok\n");
    return 0;
