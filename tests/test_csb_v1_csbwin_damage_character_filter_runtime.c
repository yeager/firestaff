/* CSBWin Character.cpp DamageCharacter DamageCharFilter regression. */
...
int main(void)
{
    ...
    const uint16_t actuator_thing = (uint16_t)(CSB_V1_THING_TYPE_ACTUATOR << 10);
    const uint32_t key = (CSB_V1_EXPOOL_EDT_SPECIAL_LOCATIONS << 24) |
        CSB_V1_EXPOOL_ESL_DAMAGE_CHAR_FILTER;
    const uint32_t bucket = 32u + ((key * 0xbb40e62du) >> 27);
    /* EX_LOAD integer zero then EX_STORE source parameter E. The runner
     * carries the source count separately, so E is payload selector 3. */
    ...
    profile.dungeon_handle = &dungeon; profile.party_state.ChampionCount = 1;
    profile.party_state.Champions[0].Fingerprint = 0x4a3cu;
    ...
    check(csb_v1_runtime_execute_csbwin_damage_character_filter(
              &profile, 0, 23, 0x0014u, 3u, &final_damage) == 1 &&
              final_damage == 0,
          "DamageCharFilter consumes the authenticated seven-word ABI and suppresses C38 damage");
    profile.csbwin_appended_tail[0] ^= 0x01u; final_damage = -1;
    check(csb_v1_runtime_execute_csbwin_damage_character_filter(
              &profile, 0, 23, 0x0014u, 3u, &final_damage) == 0 &&
              final_damage == -1,
          "altered FNV-owned Extended Features tail blocks DamageCharFilter execution");
    return failures ? 1 : 0;
}
