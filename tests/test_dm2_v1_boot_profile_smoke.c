...
static void test_startup_launch_alloc_real_assets_when_available(void)
{
    ...
    CHECK(complete_support_result == 1 &&
              ...
              complete_support.save_corpus_original_game_load_owner_complete == 0 &&
...
static void test_fmtowns_zip_real_media(void)
{
    ...
    /* A former complete session cannot survive the source-owned New Game
     * reset. LOAD_NEW_DUNGEON clears party and leader hand before the later
     * GAME_LOAD stages restore a new record/hero/timer graph. */
    p.source_game_load_session_ready = 1;
    {
        ...
        CHECK(dm2_v1_boot_load_new_dungeon(&p, &new_dungeon) == 1 &&
                  ...
                  !p.source_game_load_session_ready,
              "FM Towns New Game reload clears prior session readiness before original GAME_LOAD");
    }
    dm2_v1_boot_cleanup(&p);
}
...
static void test_fmtowns_zip_direct_data_root(void)
{
    ...
    if (!root || !root[0]) {
        printf("  SKIP: FIRESTAFF_DM2_FMTOWNS_DIRECT_ROOT not set\n");
        return;
    }
...
/* ── source evidence --─ */
    ...
    test_fmtowns_zip_real_media();
    printf("\n--- test_fmtowns_zip_direct_data_root ---\n");
    test_fmtowns_zip_direct_data_root();
