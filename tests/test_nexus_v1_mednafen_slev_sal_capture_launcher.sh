...
args=(--mednafen /usr/bin/true --bios "$tmp/bios.bin" --bios-sha256 "$bios_hash" --bios-region us --disc "$tmp/game.cue" --disc-sha256 "$disc_hash" --capture "$tmp/capture.slevsal" --manifest "$tmp/plan.txt" --route-epoch 7 --package-fnv 1020304050607080 --card-fnv 8877665544332211 --task-trace-fnv 0123456789abcdef --task-source-fnv fedcba9876543210 --sal-descriptor-fnv 13579bdf2468ace0 --map-table-fnv 0f1e2d3c4b5a6978 --sddrvs-fnv 89abcdef01234567)
...
grep -Fx 'task_source_fnv1a64=fedcba9876543210' "$tmp/plan.txt" >/dev/null
