...
static uint8_t *read_file(const char *path,size_t *out_size) { FILE *f; long n; uint8_t *b; *out_size=0; f=fopen(path,"rb"); if(!f||fseek(f,0,SEEK_END)||(n=ftell(f))<=0||fseek(f,0,SEEK_SET)){if(f)fclose(f);return NULL;} b=(uint8_t *)malloc((size_t)n); if(!b||fread(b,1,(size_t)n,f)!=(size_t)n){free(b);fclose(f);return NULL;} fclose(f);*out_size=(size_t)n;return b; }
int main(int argc,char **argv)
{
    ...
    memset(&identity,0,sizeof(identity)); identity.sha256_verified=1; identity.sha256_hex=NEXUS_V1_FONT256_S2D_SHA256; identity.source_fnv1a64=fnv1a64(bytes,size);
