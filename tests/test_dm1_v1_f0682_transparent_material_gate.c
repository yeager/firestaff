...
int main(void)
{
    ...
    for (index = 0; index < (int)(sizeof(graphics) / sizeof(graphics[0])); ++index) {
        ...
        if (!dm1_v1_f0682_transparent_material_receipt_pc34(
                &surface, 0, NULL, 0, &receipt) || !receipt.valid ||
            receipt.graphicIndex != graphics[index] || receipt.transparentColor != 0) goto fail;
        if (graphics[index] == 488) {
            memcpy(alteredPalette, dm1_v1_f0663_smoke_palette_changes_pc34(),
                   sizeof(alteredPalette));
            if (!dm1_v1_f0682_transparent_material_receipt_pc34(
                    &surface, 0, dm1_v1_f0663_smoke_palette_changes_pc34(),
                    DM1_V1_F0663_PALETTE_COUNT_PC34, &receipt)) goto fail;
            alteredPalette[7] ^= 1u;
            if (dm1_v1_f0682_transparent_material_receipt_pc34(
                    &surface, 0, alteredPalette, sizeof(alteredPalette), &receipt)) goto fail;
        } else if (dm1_v1_f0682_transparent_material_receipt_pc34(
                       &surface, 0, dm1_v1_f0663_smoke_palette_changes_pc34(),
                       DM1_V1_F0663_PALETTE_COUNT_PC34, &receipt)) goto fail;
        ++surface.pixelsFNV1a;
        if (dm1_v1_f0682_transparent_material_receipt_pc34(
                &surface, 0, NULL, 0, &receipt)) goto fail;
    }
    M11_AssetLoader_Shutdown(&loader);
    puts("ok: real PC34 F0682 transparent material gate");
    return 0;
fail:
    M11_AssetLoader_Shutdown(&loader);
    return 1;
