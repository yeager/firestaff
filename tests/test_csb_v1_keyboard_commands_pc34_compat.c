...
int main(void)
{
    ...
    ok &= expect_int("CSB disk menu exposes save-path choices only", view.dialogChoiceCount, 3);
    ...
    ok &= expect_int("movement ignored while disk menu is open",
                     M11_GameView_HandleInput(&view, M12_MENU_INPUT_UP),
                     M11_GAME_INPUT_REDRAW);
    ok &= expect_int("CSB disk menu keyboard wraps to save and quit", view.csbDiskMenuSelectedChoice, 3);
