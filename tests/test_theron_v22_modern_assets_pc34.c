/*
 * test_theron_v22_modern_assets_pc34.c — Theron V2.2 Modern Graphics asset pipeline
 *
 * Focused tests for the Theron V2.2 modern-assets module:
 *   - Path resolution from dataDir → ~/.firestaff/assets/theron/modern/
 ...
 *
 * Parallel to test_dm1_v22_verification.c (but Theron-scoped, smaller).
...
static void test_manifest_path_from_data_dir(void) {
    /* dataDir = ~/.firestaff/data/theron → manifest = ~/.firestaff/assets/theron/modern/modern_asset_manifest.json */
    theron_v22_set_manifest_path("/home/user/.firestaff/data/theron");
    const char* expected_suffix = "/.firestaff/assets/theron/modern/modern_asset_manifest.json";
...
static void test_installed_flag(void) {
    theron_v22_set_installed(0);
    CHECK(theron_v22_get_installed() == 0, "installed=0 round-trip");
    theron_v22_set_installed(1);
    CHECK(theron_v22_get_installed() == 1, "installed=1 round-trip");
    theron_v22_set_installed(42);  /* truthy */
    CHECK(theron_v22_get_installed() == 1, "installed=42 → 1 (clamped)");
    theron_v22_set_installed(0);
}

static void test_epx_cache_flag(void) {
    theron_v22_set_epx_cache_warm(0);
    CHECK(theron_v22_get_epx_cache_warm() == 0, "epx warm=0 round-trip");
    theron_v22_set_epx_cache_warm(1);
    CHECK(theron_v22_get_epx_cache_warm() == 1, "epx warm=1 round-trip");
    theron_v22_set_epx_cache_warm(0);
}
...
static void test_best_available_shape_source(void) {
    ...
    CHECK(theron_v22_best_available_shape_source(0) == THERON_V22_SHAPE_SOURCE_V1_ORIGINAL,
          "mode 0 (V1) → V1_ORIGINAL");

    CHECK(theron_v22_best_available_shape_source(1) == THERON_V22_SHAPE_SOURCE_V2_FILTERED,
          "mode 1 (V2.0) → V2_FILTERED");

    CHECK(theron_v22_best_available_shape_source(2) == THERON_V22_SHAPE_SOURCE_V2_FILTERED,
          "mode 2 (V2.1) cold cache → V2_FILTERED (fallback)");

    theron_v22_set_epx_cache_warm(1);
    CHECK(theron_v22_best_available_shape_source(2) == THERON_V22_SHAPE_SOURCE_V2_UPSCALED,
    ...
    theron_v22_set_installed(0);
    theron_v22_set_epx_cache_warm(0);
    CHECK(theron_v22_best_available_shape_source(3) == THERON_V22_SHAPE_SOURCE_V2_FILTERED,
          "mode 3 (V2.2) no install, cold cache → V2_FILTERED (fallback)");

    theron_v22_set_installed(1);
    CHECK(theron_v22_best_available_shape_source(3) == THERON_V22_SHAPE_SOURCE_V2_MODERN,
          "mode 3 (V2.2) installed → V2_MODERN");

    /* Unknown mode → V1_ORIGINAL */
    CHECK(theron_v22_best_available_shape_source(99) == THERON_V22_SHAPE_SOURCE_V1_ORIGINAL,
...
static void test_shape_source_name(void) {
    CHECK(strcmp(theron_v22_shape_source_name(THERON_V22_SHAPE_SOURCE_V1_ORIGINAL), "V1_ORIGINAL") == 0,
          "name V1_ORIGINAL");
    CHECK(strcmp(theron_v22_shape_source_name(THERON_V22_SHAPE_SOURCE_V2_FILTERED), "V2_FILTERED") == 0,
          "name V2_FILTERED");
    CHECK(strcmp(theron_v22_shape_source_name(THERON_V22_SHAPE_SOURCE_V2_UPSCALED), "V2_UPSCALED") == 0,
          "name V2_UPSCALED");
    CHECK(strcmp(theron_v22_shape_source_name(THERON_V22_SHAPE_SOURCE_V2_MODERN), "V2_MODERN") == 0,
          "name V2_MODERN");
    CHECK(strcmp(theron_v22_shape_source_name(99), "UNKNOWN") == 0,
          "name unknown");
}
...
static void test_source_evidence(void) {
    const char* ev = theron_v22_source_evidence();
    CHECK(ev != NULL, "evidence non-null");
    CHECK(strlen(ev) > 0, "evidence non-empty");
    CHECK(strstr(ev, "ReDMCSB") != NULL || strstr(ev, "THQUEST") != NULL,
...
int main(void) {
    ...
    if (failures > 0) {
        printf("theron_v22_modern_assets_pc34: FAIL\n");
        return 1;
    }
    printf("theron_v22_modern_assets_pc34: PASS\n");
    return 0;
}
