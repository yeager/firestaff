/* CSBWin SaveGame.cpp DeleteDuplicateTimers raw recovery regression. */
...
{
    enum {
        record_id = (5u << 24) | (1u << 16),
        block_bytes = CSB_V1_CSBWIN_EXPOOL_BLOCK_BYTES
    };
    uint8_t tail[2u * block_bytes];
    CSB_V1_RuntimeProfile profile;
    uint32_t delete_duplicates = 0u;

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, 0u, 3u);
    prepare_profile(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_delete_duplicate_timers(
              &profile, &delete_duplicates) == 1 && delete_duplicates == 0u,
          "CSBWin recovers the raw DeleteDuplicateTimers word");

    profile.csbwin_appended_tail[0] ^= 1u;
    check(csb_v1_runtime_recover_csbwin_delete_duplicate_timers(
              &profile, &delete_duplicates) == 0 && delete_duplicates == 0u,
          "CSBWin rejects a drifted DeleteDuplicateTimers receipt");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, 0u, 3u);
    add_record(tail, 1u, record_id, 0u, 3u);
    prepare_profile(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_delete_duplicate_timers(
              &profile, &delete_duplicates) == 0 && delete_duplicates == 0u,
          "CSBWin rejects duplicate live DeleteDuplicateTimers owners");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, 0u, 4u);
    prepare_profile(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_delete_duplicate_timers(
              &profile, &delete_duplicates) == 0 && delete_duplicates == 0u,
          "CSBWin rejects a malformed DeleteDuplicateTimers record size");
    csb_v1_runtime_cleanup(&profile);
    ...
    check(csb_v1_runtime_recover_csbwin_delete_duplicate_timers(
              &profile, &delete_duplicates) == 0 && delete_duplicates == 0u,
          "CSBWin rejects an absent DeleteDuplicateTimers owner without defaulting");
    csb_v1_runtime_cleanup(&profile);
