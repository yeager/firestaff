/*
 * test_nexus_v22_modern_assets_pc34.c — Nexus V2.2 Modern Graphics asset pipeline
 *
 * Focused tests for the Nexus V2.2 modern-assets module:
 *   - Path resolution from dataDir → ~/.firestaff/assets/nexus/modern/
 ...
 *
 * Parallel to test_dm1_v22_verification.c (but Nexus-scoped, smaller).
...
static void test_installed_flag(void) {
    nexus_v22_set_installed(0);
    CHECK(nexus_v22_get_installed() == 0, "installed=0 round-trip");
    nexus_v22_set_installed(1);
    CHECK(nexus_v22_get_installed() == 1, "installed=1 round-trip");
    nexus_v22_set_installed(42);  /* truthy */
    CHECK(nexus_v22_get_installed() == 1, "installed=42 → 1 (clamped)");
    nexus_v22_set_installed(0);
}

static void test_epx_cache_flag(void) {
    nexus_v22_set_epx_cache_warm(0);
    CHECK(nexus_v22_get_epx_cache_warm() == 0, "epx warm=0 round-trip");
    nexus_v22_set_epx_cache_warm(1);
    CHECK(nexus_v22_get_epx_cache_warm() == 1, "epx warm=1 round-trip");
    nexus_v22_set_epx_cache_warm(0);
}
...
static void test_best_available_shape_source(void) {
    ...
    CHECK(nexus_v22_best_available_shape_source(0) == NEXUS_V22_SHAPE_SOURCE_V1_ORIGINAL,
          "mode 0 (V1) → V1_ORIGINAL");

    CHECK(nexus_v22_best_available_shape_source(1) == NEXUS_V22_SHAPE_SOURCE_V2_FILTERED,
          "mode 1 (V2.0) → V2_FILTERED");

    CHECK(nexus_v22_best_available_shape_source(2) == NEXUS_V22_SHAPE_SOURCE_V2_FILTERED,
          "mode 2 (V2.1) cold cache → V2_FILTERED (fallback)");

    nexus_v22_set_epx_cache_warm(1);
    CHECK(nexus_v22_best_available_shape_source(2) == NEXUS_V22_SHAPE_SOURCE_V2_UPSCALED,
    ...
    nexus_v22_set_installed(0);
    nexus_v22_set_epx_cache_warm(0);
    CHECK(nexus_v22_best_available_shape_source(3) == NEXUS_V22_SHAPE_SOURCE_V2_FILTERED,
          "mode 3 (V2.2) no install, cold cache → V2_FILTERED (fallback)");

    nexus_v22_set_installed(1);
    CHECK(nexus_v22_best_available_shape_source(3) == NEXUS_V22_SHAPE_SOURCE_V2_MODERN,
          "mode 3 (V2.2) installed → V2_MODERN");

    /* Unknown mode → V1_ORIGINAL */
    CHECK(nexus_v22_best_available_shape_source(99) == NEXUS_V22_SHAPE_SOURCE_V1_ORIGINAL,
...
static void test_shape_source_name(void) {
    CHECK(strcmp(nexus_v22_shape_source_name(NEXUS_V22_SHAPE_SOURCE_V1_ORIGINAL), "V1_ORIGINAL") == 0,
          "name V1_ORIGINAL");
    CHECK(strcmp(nexus_v22_shape_source_name(NEXUS_V22_SHAPE_SOURCE_V2_FILTERED), "V2_FILTERED") == 0,
          "name V2_FILTERED");
    CHECK(strcmp(nexus_v22_shape_source_name(NEXUS_V22_SHAPE_SOURCE_V2_UPSCALED), "V2_UPSCALED") == 0,
          "name V2_UPSCALED");
    CHECK(strcmp(nexus_v22_shape_source_name(NEXUS_V22_SHAPE_SOURCE_V2_MODERN), "V2_MODERN") == 0,
          "name V2_MODERN");
    CHECK(strcmp(nexus_v22_shape_source_name(99), "UNKNOWN") == 0,
          "name unknown");
}
...
static void test_source_evidence(void) {
    ...
    CHECK(strstr(ev, "ReDMCSB") != NULL || strstr(ev, "THQUEST") != NULL || strstr(ev, "SATURN") != NULL,
...
int main(void) {
    ...
    printf("nexus_v22_modern_assets_pc34: checks=%d failures=%d\n", checks, failures);
    if (failures > 0) {
        printf("nexus_v22_modern_assets_pc34: FAIL\n");
        return 1;
    }
    printf("nexus_v22_modern_assets_pc34: PASS\n");
    return 0;
}
