...
static int test_boot_init_mini_runs_startup_chain_to_completion(void) {
    const char* path = "./test_graphics_dat_boot.bin";
    ...
    if (!boot.succeeded || boot.stage != MEMORY_GRAPHICS_DAT_BOOT_STAGE_COMPLETE) {
        F0479_MEMORY_FreeBootInitMini_Compat(&boot);
        return 0;
    }
    if (!boot.startup.runtimeInitialized || !boot.startup.dialogPreloaded || !boot.startup.viewportLoaded) {
        F0479_MEMORY_FreeBootInitMini_Compat(&boot);
        return 0;
    }
    if (boot.startup.specials.dialogBoxGraphic == 0 || boot.startup.viewportSelection.compressedByteCount != 3) {
        F0479_MEMORY_FreeBootInitMini_Compat(&boot);
        return 0;
    }
...
int main(void) {
    if (!test_boot_init_mini_runs_startup_chain_to_completion()) {
        fprintf(stderr, "test_boot_init_mini_runs_startup_chain_to_completion failed\n");
        return 1;
    }
    printf("ok\n");
    return 0;
