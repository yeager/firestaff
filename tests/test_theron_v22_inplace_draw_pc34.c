/*
 * test_theron_v22_inplace_draw_pc34.c — Theron V2.2 in-place bitmap cache + render pass
 *
 ...
 *   - theron_v22_inplace_render_pass() draws bitmaps into a framebuffer
 *   - Source evidence citation
 *
 * Test does NOT modify any V1/V2 state — read-only verification.
 * Skips cache-dependent assertions when ~/.firestaff/assets/theron/modern/
...
static void test_init_shutdown(void) {
    int active_before = theron_v22_inplace_draw_active();
    int r1 = theron_v22_inplace_draw_init();
    int r2 = theron_v22_inplace_draw_init();
    CHECK(r1 == r2, "init is idempotent (same return on repeat call)");
    int active_after = theron_v22_inplace_draw_active();
    CHECK((active_after == 0) || (active_after == 1), "active is 0 or 1");
    theron_v22_inplace_draw_shutdown();
    CHECK(theron_v22_inplace_draw_active() == 0, "active==0 after shutdown");
    int r3 = theron_v22_inplace_draw_init();
...
static void test_get_cell_bitmap_no_cache(void) {
    theron_v22_inplace_draw_shutdown();
    int w = -1, h = -1;
    const uint32_t* p = theron_v22_inplace_get_cell_bitmap(1, 0, &w, &h);
...
static void test_get_cell_asset_id_no_cache(void) {
    theron_v22_inplace_draw_shutdown();
    const char* aid = theron_v22_inplace_get_cell_asset_id(1, 0);
    CHECK(aid == NULL, "no cache -> asset_id NULL");
}

static void test_source_evidence(void) {
    const char* ev = theron_v22_inplace_draw_source_evidence();
    CHECK(ev != NULL, "evidence non-null");
    CHECK(strlen(ev) > 0, "evidence non-empty");
    CHECK(strstr(ev, "ReDMCSB") != NULL || strstr(ev, "theron_v22") != NULL || strstr(ev, "THQUEST") != NULL,
          "evidence cites source");
}

static void test_cache_load_path(void) {
    int r = theron_v22_inplace_draw_init();
    int active = theron_v22_inplace_draw_active();
    if (r == 1) {
...
static void test_double_shutdown_safe(void) {
    theron_v22_inplace_draw_init();
    theron_v22_inplace_draw_shutdown();
    theron_v22_inplace_draw_shutdown();
    CHECK(theron_v22_inplace_draw_active() == 0, "double shutdown safe");
}
...
static void test_render_pass_safe_when_no_cache(void) {
    ...
    int painted = theron_v22_inplace_render_pass(fb, 320, 200);
...
static void test_render_pass_safe_with_null_args(void) {
    theron_v22_inplace_draw_init();
    int painted = theron_v22_inplace_render_pass(NULL, 320, 200);
    CHECK(painted == 0, "NULL fb -> 0 cells painted");
    unsigned char fb[10];
    memset(fb, 0, sizeof(fb));
    painted = theron_v22_inplace_render_pass(fb, 0, 200);
    CHECK(painted == 0, "zero width -> 0 cells painted");
    painted = theron_v22_inplace_render_pass(fb, 320, 0);
...
static void test_render_pass_safe_when_no_shape_cache(void) {
    ...
    theron_v22_inplace_draw_init();
    if (!theron_v22_inplace_draw_active()) {
...
int main(void) {
    ...
    printf("theron_v22_inplace_draw_pc34: checks=%d failures=%d\n", checks, failures);
    if (failures > 0) {
        printf("theron_v22_inplace_draw_pc34: FAIL\n");
        return 1;
    }
    printf("theron_v22_inplace_draw_pc34: PASS\n");
    return 0;
}
