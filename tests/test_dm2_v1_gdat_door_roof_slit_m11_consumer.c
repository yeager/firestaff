...
int main(void)
{
    ...
    int golden, scene_reject, source_reject;
    ...
    command->view_square = DM2_SQ_D1C; command->field = 0x12;
    ...
    command->rect_x = 12; command->rect_y = 22; command->rect_number = 0x2f2;
    ...
    composition.identity_hash = 9; composition.session_identity = composition.data_epoch = 1;
    ...
    golden = dm2_v1_gdat_door_roof_slit_m11_receipt_build(
                 &plan, 0, &composition, &viewport, &receipt) &&
        dm2_v1_gdat_door_roof_slit_m11_consume(
    ...
    scene_reject = !dm2_v1_gdat_door_roof_slit_m11_consume(
        &receipt, &plan, &composition, &viewport) &&
        memcmp(before, framebuffer, sizeof(before)) == 0;
    --viewport.gdat_scene_colorkey;
    command->field = 0x13;
    source_reject = !dm2_v1_gdat_door_roof_slit_m11_receipt_build(
        &plan, 0, &composition, &viewport, &receipt);
    printf("golden=%d scene=%d source=%d\n", golden, scene_reject, source_reject);
    return golden && scene_reject && source_reject ? 0 : 1;
}
