...
int main(void) {
    ...
    printf("PASS: md5=%s commands=%u requested=%u raw_sectors=%u bindings=%u "
           ...
           receipt.adpcm_ram_write_count, receipt.destination_candidate_count,
           receipt.origin_ram_receipt_count);
    if (receipt.adpcm_fifo_read_count != receipt.adpcm_ram_write_count) {
        fprintf(stderr, "FAIL: ADPCM FIFO/RAM transport counts diverged\n");
        return 1;
    }
