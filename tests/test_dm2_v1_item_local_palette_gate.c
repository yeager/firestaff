...
static int fetch_item_map_chip(void *user,
                               int gdat_index,
...
static int fetch_item_local_palette(void *user,
                                    int gdat_index,
                                    ...
                                    uint32_t *out_hash)
{
    ...
    for (int i = 0; i < 16; ++i) out_palette16[i] = (uint8_t)(0x70 + i);
    if (out_hash) *out_hash = 0x49544550u;
    return 0;
}
...
static void setup_item(DM2_V1_ViewportState *viewport,
                       uint8_t *framebuffer,
                       FetchTrace *trace,
                       int bind_palette)
{
    ...
    dm2_v1_viewport_set_asset_provider(viewport, fetch_item_map_chip, trace);
    if (bind_palette) {
        dm2_v1_viewport_set_asset_palette_provider(
            viewport, fetch_item_local_palette, trace);
    }
...
int main(void)
{
    ...
    setup_item(&viewport, framebuffer, &trace, 1);
    dm2_v1_render_items(&viewport);
    CHECK("floor object map-chip consumes its matching source IMG3 palette",
          trace.asset_fetches == 1 && trace.palette_fetches == 1 &&
              ...
              framebuffer_contains(framebuffer, 0x71u) &&
    ...
    memset(&trace, 0, sizeof(trace));
    setup_item(&viewport, framebuffer, &trace, 0);
    dm2_v1_render_items(&viewport);
    CHECK("source-required floor object map-chip fails closed without its palette",
    ...
    printf("DM2 item local palette gate: %d/%d passed\n", passed, checks);
