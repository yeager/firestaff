...
int main(void)
{
    static const uint8_t row[12] = {
        0x21U, 14U, 0U, 0x42U, 0x31U, 0xffU,
        0x80U, 0x27U, 0xa1U, 0xb2U, 0xc3U, 0xd4U
    };
    static const char row_md5[] = "684c9f64413880bc6dc089d4763406f1";
    char path[] = "/tmp/firestaff-nexus-structure1f-wall-decoration-XXXXXX";
    ...
    directory.families[NEXUS_V1_DGN_STRUCTURE1F_WALL_DECORATIONS].record_size = sizeof(row);
    directory.families[NEXUS_V1_DGN_STRUCTURE1F_WALL_DECORATIONS].record_length = sizeof(row);
    directory.families[NEXUS_V1_DGN_STRUCTURE1F_WALL_DECORATIONS].record_fnv1a64 = identity.fnv1a64;

    if (!nexus_v1_structure1f_wall_decoration_admit(
            &identity, row, sizeof(row), &directory, 0U, &receipt) ||
        !receipt.valid || receipt.source_tag != 0x21U ||
        receipt.raw_face_selector != 14U || receipt.raw_structure1a_index != 0x0042U ||
    ...
    if (nexus_v1_structure1f_wall_decoration_admit(
        ...
        nexus_v1_structure1f_wall_decoration_admit(
            &identity, row, sizeof(row), &directory, 1U, &receipt) || receipt.valid) {
        unlink(path);
        return 1;
    }
    directory.families[NEXUS_V1_DGN_STRUCTURE1F_WALL_DECORATIONS].record_length--;
    if (nexus_v1_structure1f_wall_decoration_admit(
    ...
    unlink(path);
    puts("Structure1F wall-decoration fixture admission: PASS");
    return 0;
}
