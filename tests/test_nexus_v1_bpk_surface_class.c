...
static void test_prs3_candidate_evidence(void) {
    ...
    expect(nexus_v1_bpk_archive_prs3_candidate_evidence_with_bit_order(
               ...
               rows, 2U, &summary) == 0 &&
               summary.complete_exact == 0U,
           "PRS3 LSB-first traversal on MSB-only fixture produces no exact match");
    expect(nexus_v1_bpk_archive_prs3_candidate_evidence_with_bit_order(
...
static void test_synthetic_surface_estimate(void) {
    ...
    /* Entry 2 is PRS3: mode 14 is opaque flags, not RGB565. DMWeb output is
     * indexed 8bpp: rowstride=8, surface=32. */
    ...
    expect(entries[1].mode == NEXUS_V1_BPK_MODE_16BPP &&
           ...
           "second surface entry is 8x4 PRS3 indexed output");
    ...
    expect(entries[1].layout.surface_class ==
               NEXUS_V1_BPK_SURFACE_INDEXED_8BPP,
           "second surface entry class is INDEXED_8BPP");

    /* Entry 3 is PRS3: mode 22 remains opaque flags; output is indexed 8bpp. */
    ...
    expect(entries[2].mode == NEXUS_V1_BPK_MODE_24BPP &&
           ...
           "third surface entry is 2x3 PRS3 indexed output");
    ...
    expect(entries[2].layout.surface_class ==
               NEXUS_V1_BPK_SURFACE_INDEXED_8BPP,
           "third surface entry class is INDEXED_8BPP");

    /* Total PRS3 output bytes: 16 + 32 + 6 = 54. */
    expect(summary.total_surface_bytes == 54U,
           "synthetic archive total PRS3 surface bytes = 54");
}
...
static void test_runtime_render_receipt_blocks_prs3(void) {
    ...
    expect(receipt.expected_surface_bytes == 54U,
           "runtime receipt: expected PRS3 surface bytes = 54");
...
static void test_runtime_decode_receipt_routes(void) {
    ...
    expect(receipt.surface_entries == 3U &&
           ...
           "decode receipt blocks synthetic entries without valid compressed size");
    ...
    expect(receipt.route == NEXUS_V1_BPK_DECODE_ROUTE_READY_DECODED,
           "decode receipt promotes literal PRS3 archive to decoded");
    expect(receipt.prs3_decode_successes == 1U &&
           ...
           "decode receipt decodes the literal PRS3 surface");
...
static void test_runtime_upload_plan_routes(void) {
    ...
    expect(receipt.surface_entries == 3U &&
           ...
           "upload plan exposes PRS3 blockers for synthetic entries");
expect(rows[0].entry_index == 1U &&
               rows[0].status ==
...
static void test_optional_local_menu_bpk(void) {
    ...
    expect(indexed == 162U,
           "local MENU.BPK: all 162 PRS3 entries are indexed 8bpp");
...
expect(decode_receipt.route == NEXUS_V1_BPK_DECODE_ROUTE_READY_DECODED,
    ...
    expect(decode_receipt.decode_blocked == 0,
           "local MENU.BPK decode receipt unblocks rendering");
    ...
    expect(upload_receipt.surface_entries == 162U &&
           ...
           "local MENU.BPK upload plan decodes all PRS3 surfaces");
    ...
    expect(upload_rows[0].entry_index == 1U &&
           ...
           "local MENU.BPK upload row is decoded and ready");
