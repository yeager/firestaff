...
static void test_real_menu_bpk_and_structure2_intake(void)
{
    ...
    static const char *menu_bpk_md5[] = {
        "c2776768ff25287c79013a1452253ca0", /* legacy extracted revision */
        "a6f2272a4f6cb3c6b3b33012bc5b15ed", /* English Saturn ISO */
        "fcf8a00fbb92593ed9ae908f8e285cda"  /* French Saturn ISO */
    };
    ...
    if (!default_nexus_dir(data_dir, sizeof(data_dir)) ||
        ...
        !(asset_file_matches_md5(menu_path, menu_bpk_md5[0]) ||
          asset_file_matches_md5(menu_path, menu_bpk_md5[1]) ||
          asset_file_matches_md5(menu_path, menu_bpk_md5[2])) ||
    ...
    CHECK(nexus_v1_prs3_structure2_intake_admit(&input, &receipt) == 0,
          "real MENU.BPK PRS3 admits bytes before the DGN Structure2 gate");
    CHECK(receipt.status == NEXUS_V1_PRS3_STRUCTURE2_INTAKE_BLOCKED_STRUCTURE2 &&
              strcmp(nexus_v1_prs3_structure2_intake_status_name(receipt.status),
                     "blocked-structure2") == 0,
          "intake reaches the independent Structure2 source gate");
    ...
    CHECK(receipt.prs3_framing_bound &&
              ...
              receipt.prs3_decoder_promoted &&
              receipt.can_decode_prs3,
          "PRS3 stream framing and byte decoder are source-bound");
    ...
    CHECK(!receipt.structure2_pixel_span_proven &&
          ...
          "real intake stays no-draw for the unbound Structure2, palette, render, and fallback");
    ...
    CHECK(nexus_v1_prs3_structure2_intake_admit(&input, &receipt) == 0 &&
          ...
          "unverified Structure2 source remains blocked after PRS3 byte admission");
