...
/* CHD remains unavailable until Firestaff owns a native reader.  An installed
 * host converter must never alter the launcher state or become a runtime
 * dependency. */
static void check_unsupported_chd_has_no_tool_popup(void) {
    M12_StartupMenuState state;
    char dataRoot[M12_ASSET_DATA_DIR_CAPACITY];
    char ignoredMd5[M12_ASSET_MD5_CAPACITY];
    ...
    M12_StartupMenu_InitWithDataDir(&state, dataRoot, NULL);
    CHECK(state.view == M12_MENU_VIEW_MESSAGE);
    CHECK(has_text_prefix(state.messageLine1, "NO GAME DATA FOUND"));
...
int main(void) {
    ...
    check_unsupported_chd_has_no_tool_popup();
