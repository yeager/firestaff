/* ReDMCSB MOVESENS.C F0267/F0276 C003 -> F0272/F0268 corpus route. */
...
static int is_remote_c003_only(const CSB_V1_DungeonData *dungeon,
                               int level, int x, int y, int direction)
{
    int thing = csb_v1_dungeon_get_first_thing(dungeon, level, x, y);
    int guard;
    int found = 0;
    ...
    for (guard = 0; guard < 128 && thing != 0xfffe && thing != 0xffff;
        ...
        if (type == 3 && size >= 8) {
            ...
            /* One non-Revert, non-local original C003 makes F0272/F0268
             * observable without fabricating a timeline event. */
            if ((type_data & 0x007f) != DM1_SENSOR_FLOOR_PARTY ||
                ((type_data >> 7) != 0 &&
                 (type_data >> 7) != (unsigned short)((direction & 3) + 1)) ||
...
static int find_c003_party_route(const CSB_V1_DungeonData *dungeon,
                                 C003OriginalRoute *route)
{
            ...
            for (y = 0; y < dungeon->level_heights[level]; ++y) {
                ...
                for (direction = 0; direction < 4; ++direction) {
                    ...
                    if (square_type(dungeon, level, source_x, source_y) != 1 ||
                        ...
                        !is_remote_c003_only(dungeon, level, x, y, direction)) {
                        continue;
                    }
...
int main(void)
{
    ...
    if (!find_c003_party_route(&dungeon, &route)) {
        puts("SKIP: original Dungeon.dat has no isolated C003 party route");
    ...
    if (!result.movement_step_applied ||
        ...
        fputs("FAIL: original C003 did not publish F0276 through F0272/F0268\n", stderr);
        csb_v1_dungeon_free(&dungeon);
        return 1;
    }

    puts("ok: original C003 party route publishes F0276 through F0272/F0268");
    csb_v1_dungeon_free(&dungeon);
    return 0;
}
