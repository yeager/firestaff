    ...
    int *out_width, int *out_height)
{
    ...
    if (graphic_index == 87 && fixture->d1_side_pixels) {
        *out_pixels = fixture->d1_side_pixels;
        *out_width = 32;
        *out_height = 94;
        return 1;
    }
    if (graphic_index == 91 && fixture->d1_top_pixels) {
...
static void test_redmcsb_f0115_object_c2500_geometry(void)
{
    ...
    for (size_t i = 0; i < sizeof(expected_square_rows) /
                            sizeof(expected_square_rows[0]); ++i) {
        char id[96];
        snprintf(id, sizeof(id), "F0115.object.view_square.%zu", i);
        check_int(id,
                  dm1_viewport_3d_f0115_view_square_index(
                      expected_square_rows[i][0], expected_square_rows[i][1]),
                  expected_square_rows[i][2]);
        snprintf(id, sizeof(id), "F0115.object.g2028_row.%zu", i);
        check_int(id,
    ...
    check_int("F0115.object.g2028_row.invalid_d2_far_left",
              dm1_viewport_3d_f0115_c2500_c2900_row(2, -2), -1);
    check_int("F0115.object.g2028_row.invalid_d2_far_right",
              dm1_viewport_3d_f0115_c2500_c2900_row(2, 2), -1);
...
static void test_d3c_door_frame_uses_source_graphic558_geometry(void)
{
    uint8_t viewport[DM1_VIEWPORT_WIDTH * DM1_VIEWPORT_HEIGHT];
    uint8_t pixels[32 * 44];
    uint8_t grid[8 * 8];
    IndexedGraphicProviderFixture provider;
    DM1_Viewport3DState state;

    /* G0166/G0167: {64,95,27,70,16,44,0,0} / {128,159,27,70,16,44,0,0}.
     * F0104/F0105 consume M657 (record 89) and C722/C723 directly. */
    memset(viewport, 0x5a, sizeof(viewport));
    memset(pixels, 10, sizeof(pixels));
    memset(grid, DM1_VP_ELEMENT_DOOR_FRONT, sizeof(grid));
    pixels[0] = 0x31;
    pixels[31] = 0x42;
    provider.graphic_index = 89;
    ...
    dm1_viewport_3d_draw_frame(&state, 0, 4, 4);
    check_int("d3c_door_frame.source_left_native",
              viewport[27 * DM1_VIEWPORT_WIDTH + 64], 0x31);
    check_int("d3c_door_frame.source_right_flipped",
              viewport[27 * DM1_VIEWPORT_WIDTH + 128], 0x42);
    check_int("d3c_door_frame.source_c10_transparent",
...
static void test_d2_side_door_frame_uses_source_graphic558_geometry(void)
{
    uint8_t viewport[DM1_VIEWPORT_WIDTH * DM1_VIEWPORT_HEIGHT];
    uint8_t pixels[96 * 3];
    uint8_t grid[8 * 8];
    ...
    /* G0173/G0175 crop the shared M660/G2115 source into the D2L/D2R
     * 60x3 top strips at x=0 and x=164. */
    memset(viewport, 0x5a, sizeof(viewport));
    memset(pixels, 10, sizeof(pixels));
    memset(grid, DM1_VP_ELEMENT_DOOR_FRONT, sizeof(grid));
    pixels[0] = 0x31;
    pixels[59] = 0x42;
    provider.graphic_index = 92;
    provider.pixels = pixels;
    provider.width = 96;
    provider.height = 3;
    dm1_viewport_3d_set_wall_frame_bitmaps(NULL);
    ...
    dm1_viewport_3d_draw_frame(&state, 0, 4, 4);
    check_int("d2_side_door_frame.source_left_native",
              viewport[22 * DM1_VIEWPORT_WIDTH], 0x31);
    check_int("d2_side_door_frame.source_right_native",
              viewport[22 * DM1_VIEWPORT_WIDTH + 164], 0x31);
    check_int("d2_side_door_frame.source_c10_transparent",
              viewport[22 * DM1_VIEWPORT_WIDTH + 1], 0x5a);
}

static void test_d1c_door_frame_uses_source_graphic558_geometry(void)
{
    uint8_t viewport[DM1_VIEWPORT_WIDTH * DM1_VIEWPORT_HEIGHT];
    uint8_t side_pixels[32 * 94];
    uint8_t top_pixels[128 * 4];
    uint8_t grid[8 * 8];
    DoorFrameGraphicProviderFixture provider;
    DM1_Viewport3DState state;

    /* G0177 is M659/G2112's 128x4 top bar; G0170/G0171 are the
     * native/mirrored M655/G2117 32x94 side pair. */
    memset(viewport, 0x5a, sizeof(viewport));
    memset(side_pixels, 10, sizeof(side_pixels));
    memset(top_pixels, 10, sizeof(top_pixels));
    memset(grid, DM1_VP_ELEMENT_DOOR_FRONT, sizeof(grid));
    ...
    side_pixels[31] = 0x42;
    top_pixels[0] = 0x53;
    provider.d1_side_pixels = side_pixels;
    provider.d1_top_pixels = top_pixels;
    dm1_viewport_3d_set_wall_frame_bitmaps(NULL);
    dm1_viewport_3d_init(&state, viewport, DM1_VIEWPORT_WIDTH);
    dm1_viewport_3d_load_wall_set(&state, 0, 0);
    state.floor_ceiling_dirty = false;
    ...
    dm1_viewport_3d_draw_frame(&state, 0, 4, 4);
    check_int("d1c_door_frame.source_top_native",
              viewport[14 * DM1_VIEWPORT_WIDTH + 48], 0x53);
    check_int("d1c_door_frame.source_left_native",
              viewport[14 * DM1_VIEWPORT_WIDTH + 43], 0x31);
    check_int("d1c_door_frame.source_right_flipped",
              viewport[14 * DM1_VIEWPORT_WIDTH + 149], 0x42);
    check_int("d1c_door_frame.source_c10_transparent",
...
static void test_side_lane_clear_contract(void)
{
    ...
    check_int("F0128.side_lane.near_always_clear",
              dm1_viewport_3d_side_lane_clear_for_rel_pc34(1, -1, 0u), 1);
    check_int("F0128.side_lane.d2_requires_d1_open",
              dm1_viewport_3d_side_lane_clear_for_rel_pc34(2, -1, 0x1u), 1);
    check_int("F0128.side_lane.d2_blocks_on_d1_wall",
              dm1_viewport_3d_side_lane_clear_for_rel_pc34(2, -1, 0x0u), 0);
    check_int("F0128.side_lane.d3_requires_d1_d2_open",
              ...
              dm1_viewport_3d_side_lane_clear_for_rel_pc34(3, 1, 0x1u), 0);
    {
        ...
        check_int("F0128.lane_visibility.left_d3_visible_by_source_order",
