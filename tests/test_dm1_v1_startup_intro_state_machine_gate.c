...
static void check_dm1_launch_path_bypass_contract(void) {
    ...
    uint8_t corpus_save_bytes[4096];
    size_t corpus_save_size = 0u;
    int corpus_ready = 0;
    ...
    expect_i("DM1 HoC host probe carries real Mac asset capture route",
             dm1_v1_startup_hoc_full_graphics_host_probe_receipt_pc34(
                 ...
                 &hoc_host_probe_apply,
                 &hoc_host_probe_consumer) &&
                 hoc_host_probe_apply.ready &&
                 hoc_host_probe_apply.host_capture_route_matches &&
    ...
    if (mkdtemp(corpus_dir)) {
        ...
        if (fp) {
            corpus_ready =
                corpus_ready &&
                fwrite(corpus_save_bytes, 1u, corpus_save_size, fp) ==
                corpus_save_size;
            fclose(fp);
        }
