...
static void test_table_values(void)
{
    ...
    CHECK(n == 60);
    for (i = 0; i < 60; ++i) {
        CHECK(dm1_v1_g0194_get_pc34(i) == kExpected[i]);
    }
    for (i = 0; i < 60; ++i) {
        CHECK(t[i] >= 0);
        CHECK(t[i] <= 255);
...
static void test_lookup_function(void)
{
    int i;
    for (i = 0; i < 60; ++i) {
        CHECK(dm1_v1_g0194_get_pc34(i) >= 0);
        CHECK(dm1_v1_g0194_get_pc34(i) <= 255);
    }
    CHECK(dm1_v1_g0194_get_pc34(-1) == -1);
    CHECK(dm1_v1_g0194_get_pc34(60) == -1);
    CHECK(dm1_v1_g0194_get_pc34(999) == -1);
}
...
int main(void)
{
    ...
    test_run_accepted();
    printf("dm1_v1_g0194: %d/%d assertions passed\n",
