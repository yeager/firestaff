...
int main(void) {
    ...
    ok &= expect_eq("interactive_ignores_1200ms_auto_enter",
                    ENTRANCE_Compat_ShouldAutoEnterForTimeout(0, 1200, 1201), 0);
    ok &= expect_eq("interactive_still_waits_after_headless_timeout",
                    ENTRANCE_Compat_ShouldAutoEnterForTimeout(0, 1200, 6000), 0);
    ...
    ok &= expect_eq("headless_waits_before_short_timeout",
                    ENTRANCE_Compat_ShouldAutoEnterForTimeout(1, 1200, 1200), 0);
    ok &= expect_eq("headless_enters_after_short_timeout",
                    ENTRANCE_Compat_ShouldAutoEnterForTimeout(1, 1200, 1201), 1);
    ok &= expect_eq("headless_waits_before_default_timeout",
                    ENTRANCE_Compat_ShouldAutoEnterForTimeout(1, 0, 5000), 0);
    ok &= expect_eq("headless_enters_after_default_timeout",
                    ENTRANCE_Compat_ShouldAutoEnterForTimeout(1, 0, 5001), 1);
