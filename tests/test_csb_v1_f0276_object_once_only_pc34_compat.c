...
int main(void)
{
    ...
    CHECK(F0810_PROJECTILE_Create_Compat(&input, &profile.projectiles,
          ...
          "C49 projectile owns an object for C004 OnceOnly placement");
    ...
    CHECK((get_le16(raw, 74) & 0x007fu) == 0u,
          "triggered C004 OnceOnly sensor is disabled through F0272");
    CHECK(profile.timeline_queue.eventCount == 1 &&
          ...
          "OnceOnly C004 still publishes its first remote square event");
