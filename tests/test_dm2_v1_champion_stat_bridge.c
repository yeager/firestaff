...
static void test_four_heroes_source_bar_color(void)
{
    ...
    DM2_V1_ChampionStatBridgeReceipt r;
    assert(dm2_v1_champion_stat_bridge_compute(ins, NULL, 4, 9, &r));
    assert(r.champion_count == 4);
    assert(r.champions[0].hp_bar_color == 9);
    assert(r.champions[1].hp_bar_color == 9);
    assert(r.champions[2].hp_bar_color == 9);
    assert(r.champions[3].hp_bar_color == 9);
}
...
static void test_redraw_with_prev(void)
{
    ...
    DM2_V1_ChampionStatBridgeReceipt r;
    assert(dm2_v1_champion_stat_bridge_compute(&in, &prev, 1, 9, &r));
    assert(!r.champions[0].redraw_hp);
...
static void test_missing_gdat_bar_color_blocks(void)
{
    DM2_V1_ChampionStatInput in = {
        .cur_hp = 50, .max_hp = 100,
        .cur_stamina = 100, .max_stamina = 100,
...
int main(void)
{
    ...
    test_gdat_override();
    test_null_inputs();
    test_missing_gdat_bar_color_blocks();
