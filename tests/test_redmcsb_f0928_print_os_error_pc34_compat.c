#include "redmcsb_f0928_print_os_error_pc34_compat.h"
...
/* Source-faithful F0933 dependency used to isolate the F0928 adapter. */
...
typedef struct {
    ...
    bool key_states[3];
} redmcsb_f0928_capture_pc34_compat;

static void capture_write(void *context, const char *text)
{
    redmcsb_f0928_capture_pc34_compat *capture = context;

    capture->writes[capture->write_count++] = text;
}
...
int main(void)
{
    redmcsb_f0928_capture_pc34_compat capture = {
        { NULL, NULL, NULL }, 0U, 0U, 0U, { false, false, true }
    };
    ...
    assert(redmcsb_f0928_print_os_error_pc34_compat(
               -INT16_C(1), &callbacks) == -INT16_C(1));
    assert(capture.write_count == 3U);
    assert(strcmp(capture.writes[0], "Primative OS error:\a 0x") == 0);
    ...
    evidence = redmcsb_f0928_print_os_error_source_evidence_pc34();
    assert(strstr(evidence, "PRIM1.C:423-445") != NULL);
    assert(strstr(evidence, "primError") != NULL);
    return 0;
}
