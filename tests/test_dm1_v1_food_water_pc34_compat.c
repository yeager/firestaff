 ...
 *   - include/dm1_v1_food_water_pc34_compat.h — DM1_V1_FOOD_DECAY_PER_TICK_PC34,
 *     DM1_V1_WATER_DECAY_PER_TICK_PC34, DM1_V1_FOOD_WATER_MAX_CHAMPIONS_PC34, DM1_V1_FoodWaterStatePc34.
 *
...
static void test_init(void)
{
     ...
     * [0, DM1_V1_FOOD_WATER_MAX_CHAMPIONS_PC34]. */
    DM1_V1_FoodWaterStatePc34 s;
    memset(&s, 0x7A, sizeof(s));
    DM1_V1_FoodWater_InitPc34Compat(&s, 4);
    ...
    /* Clamp count to DM1_V1_FOOD_WATER_MAX_CHAMPIONS_PC34. */
    DM1_V1_FoodWaterStatePc34 sBig;
    memset(&sBig, 0, sizeof(sBig));
    DM1_V1_FoodWater_InitPc34Compat(&sBig, 99);
    check_int("init.clamp_high", sBig.count, DM1_V1_FOOD_WATER_MAX_CHAMPIONS_PC34);

    /* Negative count clamps to 0; no champion is touched. */
    DM1_V1_FoodWaterStatePc34 sNeg;
    memset(&sNeg, 0, sizeof(sNeg));
    DM1_V1_FoodWater_InitPc34Compat(&sNeg, -5);
    check_int("init.clamp_neg", sNeg.count, 0);

    /* NULL state is a no-op. */
    DM1_V1_FoodWater_InitPc34Compat(0, 4);
...
static void test_tick_decay(void)
{
     ...
     * after a tick; DM1_V1_FoodWater_TickPc34Compat clamps to 0. The decay rate in the
     * shim is DM1_V1_FOOD_DECAY_PER_TICK_PC34 (3) / DM1_V1_WATER_DECAY_PER_TICK_PC34 (3). */
    DM1_V1_FoodWaterStatePc34 s;
    DM1_V1_FoodWater_InitPc34Compat(&s, 2);
    s.champions[0].food = 1000;
    s.champions[0].water = 1000;
    s.champions[1].food = 5;
    s.champions[1].water = 4;

    DM1_V1_FoodWater_TickPc34Compat(&s, 1);
     ...
     * flags go high. */
    DM1_V1_FoodWater_TickPc34Compat(&s, 2);
    ...
    for (int i = 0; i < 50; ++i) {
        DM1_V1_FoodWater_TickPc34Compat(&s, 100 + i);
    }
    ...
    DM1_V1_FoodWater_TickPc34Compat(0, 1);
...
static void test_eat_drink(void)
{
     ...
     * for the DM1_V1_FoodWaterStatePc34 side, matching the existing API contract
     * used by dm1_v1_fountain_interaction_pc34_compat. */
    DM1_V1_FoodWaterStatePc34 s;
    DM1_V1_FoodWater_InitPc34Compat(&s, 1);
    ...
    /* Eat 400 food → 900. */
    int rc = DM1_V1_FoodWater_EatPc34Compat(&s, 0, 400, 1234);
    ...
    DM1_V1_FoodWater_EatPc34Compat(&s, 0, 300, 1500);
    check_int("eat.food_capped", s.champions[0].food, 1000);

    /* Drink 400 water → 900. */
    rc = DM1_V1_FoodWater_DrinkPc34Compat(&s, 0, 400, 2222);
    ...
    DM1_V1_FoodWater_DrinkPc34Compat(&s, 0, 300, 2500);
    check_int("drink.water_capped", s.champions[0].water, 1000);

    /* Eat/drink with NULL state return 0. */
    check_int("eat.null_state", DM1_V1_FoodWater_EatPc34Compat(0, 0, 100, 0), 0);
    check_int("drink.null_state", DM1_V1_FoodWater_DrinkPc34Compat(0, 0, 100, 0), 0);

    /* Eat/drink with out-of-range champion returns 0 and does not
     * mutate any champion. */
    check_int("eat.oor_neg", DM1_V1_FoodWater_EatPc34Compat(&s, -1, 100, 0), 0);
    check_int("eat.oor_high", DM1_V1_FoodWater_EatPc34Compat(&s, 5, 100, 0), 0);
    check_int("drink.oor_neg", DM1_V1_FoodWater_DrinkPc34Compat(&s, -1, 100, 0), 0);
    check_int("drink.oor_high", DM1_V1_FoodWater_DrinkPc34Compat(&s, 5, 100, 0), 0);
...
{
    /* Getter parity: DM1_V1_FoodWater_GetFoodPc34Compat/_get_water mirror champion[i].food
    ...
    check_int("get.food[0]", DM1_V1_FoodWater_GetFoodPc34Compat(&s, 0), 700);
    check_int("get.water[0]", DM1_V1_FoodWater_GetWaterPc34Compat(&s, 0), 800);
    check_int("get.food[1]", DM1_V1_FoodWater_GetFoodPc34Compat(&s, 1), 100);
    check_int("get.water[1]", DM1_V1_FoodWater_GetWaterPc34Compat(&s, 1), 200);

    /* Bounds. */
    check_int("get.food.oor_neg", DM1_V1_FoodWater_GetFoodPc34Compat(&s, -1), 0);
    check_int("get.food.oor_high", DM1_V1_FoodWater_GetFoodPc34Compat(&s, 99), 0);
    check_int("get.water.oor_neg", DM1_V1_FoodWater_GetWaterPc34Compat(&s, -1), 0);
    check_int("get.water.oor_high", DM1_V1_FoodWater_GetWaterPc34Compat(&s, 99), 0);

    /* NULL state. */
    check_int("get.food.null", DM1_V1_FoodWater_GetFoodPc34Compat(0, 0), 0);
    check_int("get.water.null", DM1_V1_FoodWater_GetWaterPc34Compat(0, 0), 0);

    /* Starved/thirsty getters track the same flag the tick sets. */
    check_int("is.starved[0]", DM1_V1_FoodWater_IsStarvedPc34Compat(&s, 0), 0);
    check_int("is.thirsty[0]", DM1_V1_FoodWater_IsThirstyPc34Compat(&s, 0), 0);
    s.champions[1].starved = 1;
    s.champions[1].thirsty = 1;
    check_int("is.starved[1]", DM1_V1_FoodWater_IsStarvedPc34Compat(&s, 1), 1);
    check_int("is.thirsty[1]", DM1_V1_FoodWater_IsThirstyPc34Compat(&s, 1), 1);
    check_int("is.starved.oor", DM1_V1_FoodWater_IsStarvedPc34Compat(&s, 99), 0);
    check_int("is.thirsty.oor", DM1_V1_FoodWater_IsThirstyPc34Compat(&s, 99), 0);
    check_int("is.starved.null", DM1_V1_FoodWater_IsStarvedPc34Compat(0, 0), 0);
    check_int("is.thirsty.null", DM1_V1_FoodWater_IsThirstyPc34Compat(0, 0), 0);
}
...
static void test_starvation_damage(void)
{
     ...
     * DM1_V1_FoodWater_StarvationDamagePc34Compat returns 2 per active starve/thirst flag
     * so callers can feed it back into the M11 combat HP delta path. */
    DM1_V1_FoodWaterStatePc34 s;
    DM1_V1_FoodWater_InitPc34Compat(&s, 4);
    /* All champions start full: no damage. */
    for (int i = 0; i < 4; ++i) {
        check_int("starve_dmg.normal", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, i), 0);
    }
    /* Champion 0 starving only → +2 damage. */
    s.champions[0].starved = 1;
    check_int("starve_dmg.food_only", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 0), 2);
    /* Champion 1 thirsty only → +2 damage. */
    s.champions[0].starved = 0;
    s.champions[1].thirsty = 1;
    check_int("starve_dmg.water_only", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 1), 2);
    /* Champion 2 both → +4 damage. */
    s.champions[2].starved = 1;
    s.champions[2].thirsty = 1;
    check_int("starve_dmg.both", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 2), 4);
    /* Champion 3 neither → 0. */
    check_int("starve_dmg.none", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 3), 0);
    /* Bounds: OOR / NULL → 0. */
    check_int("starve_dmg.oor_neg", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, -1), 0);
    check_int("starve_dmg.oor_high", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 99), 0);
    check_int("starve_dmg.null", DM1_V1_FoodWater_StarvationDamagePc34Compat(0, 0), 0);
}
...
static void test_inventory_handoff_lifecycle(void)
{
     ...
     * DM1_V1_FoodWater_EatPc34Compat / DM1_V1_FoodWater_DrinkPc34Compat. The 8-entry G0242 apple..dragon-steak
    ...
    DM1_V1_FoodWater_EatPc34Compat(&s, 0, appleAmt, 9001);
    ...
    DM1_V1_FoodWater_EatPc34Compat(&s, 0, steakAmt, 9002);
    ...
    DM1_V1_FoodWater_EatPc34Compat(&s, 0, junkAmt, 9003);
    ...
    DM1_V1_FoodWater_DrinkPc34Compat(&s, 0, 800, 9100);
    DM1_V1_FoodWater_DrinkPc34Compat(&s, 0, 800, 9101);
    ...
    s.champions[0].thirsty = 1;
    DM1_V1_FoodWater_DrinkPc34Compat(&s, 0, 800, 9200);
...
static void test_decay_then_consume_round_trip(void)
{
    ...
    for (int i = 0; i < 400; ++i) {
        DM1_V1_FoodWater_TickPc34Compat(&s, i);
    }
    ...
    check_int("rt.starve_dmg", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 0), 4);

    /* Consume an apple (500 food) and a water flask (1600 water,
     * clamped to 1000 in DM1_V1_FoodWater_DrinkPc34Compat). */
    DM1_V1_FoodWater_EatPc34Compat(&s, 0, 500, 12345);
    DM1_V1_FoodWater_DrinkPc34Compat(&s, 0, 1600, 12346);
    ...
    check_int("rt.starve_dmg_after_eat", DM1_V1_FoodWater_StarvationDamagePc34Compat(&s, 0), 0);

    /* Tick once after eat/drink: decay resumes. */
    DM1_V1_FoodWater_TickPc34Compat(&s, 99999);
