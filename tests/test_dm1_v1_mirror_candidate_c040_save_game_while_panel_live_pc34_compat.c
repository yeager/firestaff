...
static void test_source_evidence_is_pinned(void)
{
    ...
    CHECK(evidence != 0);
    CHECK(strstr(evidence, "F0380:2367-2369") != 0);
    CHECK(strstr(evidence, "F0433") != 0);
    CHECK(strstr(evidence, "F0280:124-132") != 0);
    CHECK(strstr(evidence, "F0282:744-806") != 0);
    CHECK(strstr(evidence, "C140") != 0);
}

static void test_spec_is_stable(void)
{
    const DM1_V1_MirrorCandidateC040SaveGameWhilePanelLiveSpecPc34 *spec =
    ...
    CHECK(spec->saveGameCommand == 140);
    CHECK(spec->nonOverlap != 0);
    CHECK(strstr(spec->nonOverlap, "save-game-while-c040-live") != 0);
    CHECK(strstr(spec->nonOverlap, "C111 action-area") != 0);
    CHECK(strstr(spec->nonOverlap, "Disjoint from pass788") != 0);
}
...
static void test_init_clears_observability(void)
{
    ...
    dm1_v1_mirror_candidate_c040_save_game_while_panel_live_init_pc34(&state);
    CHECK(state.contractOnly == 1);
    CHECK(state.f0282DispatchCount == 0);
    CHECK(state.f0433CallCount == 0);
    CHECK(state.rejectedWhileLiveCount == 0);
    CHECK(state.rejectedNoPartyCount == 0);
...
{
    DM1_V1_MirrorCandidateC040SaveGameWhilePanelLiveResultPc34 r;
    int ok =
        dm1_v1_mirror_candidate_c040_save_game_while_panel_live_run_pc34(&r);
    CHECK(ok == 1);
    CHECK(r.accepted == 1);
    ...
    CHECK(r.f0433CallsAfterClear == 1);
    CHECK(r.rejectedAfterClear == 3);
    CHECK(r.panelContentAfterClear == 0);
    CHECK(r.candidateOrdinalAfterClear == 0);
}

static void test_run_gates_save_game_while_panel_live(void)
{
    DM1_V1_MirrorCandidateC040SaveGameWhilePanelLiveResultPc34 r;
    int ok;
    dm1_v1_mirror_candidate_c040_save_game_while_panel_live_run_pc34(&r);
    ok = (r.f0433CallsWhileLive == 0 &&
          r.rejectedWhileLive == 3 &&
          r.panelContentAfterLiveToggles == 568);
    CHECK(ok == 1);
}

static void test_run_unlocks_after_cancel(void)
{
    DM1_V1_MirrorCandidateC040SaveGameWhilePanelLiveResultPc34 r;
    int ok;
    dm1_v1_mirror_candidate_c040_save_game_while_panel_live_run_pc34(&r);
    ok = (r.f0282Dispatched &&
          r.f0433CallsAfterClear == 1 &&
...
int main(void)
{
    ...
    printf("dm1_v1_mirror_candidate_c040_save_game_while_panel_live: "
