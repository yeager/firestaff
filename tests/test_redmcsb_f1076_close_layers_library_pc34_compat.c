...
int main(void)
{
    ...
    assert(strstr(evidence,
                  ...
                  "AU2G_AU3E") != NULL);
    assert(strstr(evidence, "AMIGINIT.C:145-152") != NULL);
    assert(strstr(evidence, "F1076_CloseLayersLibrary") != NULL);
    assert(strstr(evidence, "CloseLibrary(LayersBase)") != NULL);
    assert(strstr(evidence, "clears LayersBase") != NULL);
    assert(strstr(evidence, "AMIGINIT.C:363-378") != NULL);
    ...
    puts("ok: ReDMCSB F1076 Layers-library teardown boundary");
    return 0;
}
