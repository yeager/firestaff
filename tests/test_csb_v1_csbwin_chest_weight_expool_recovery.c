/* CSBWin Mouse.cpp EDBT_ObjectWeights read-only recovery regression. */
...
static void add_record(uint8_t *tail, uint32_t block_index,
                       uint32_t record_id, uint32_t weight)
{
    ...
    put_le32(tail, (size_t)(node + 2u) * 4u, weight);
...
int main(void)
{
    ...
    add_record(tail, 0u, record_id, 73u);
    prepare_profile(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_chest_base_weight(
              &profile, &weight) == 1 && weight == 73,
          "CSBWin recovers the raw EDBT_ObjectWeights chest base");

    profile.csbwin_appended_tail[0] ^= 1u;
    check(csb_v1_runtime_recover_csbwin_chest_base_weight(
              &profile, &weight) == 0 && weight == -1,
          "CSBWin rejects a drifted ObjectWeights receipt");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, 73u);
    add_record(tail, 1u, record_id, 73u);
    prepare_profile(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_chest_base_weight(
              &profile, &weight) == 0 && weight == -1,
          "CSBWin rejects duplicate live ObjectWeights owners");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    add_record(tail, 0u, record_id, 0x80000000u);
    prepare_profile(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_chest_base_weight(
              &profile, &weight) == 0 && weight == -1,
          "CSBWin rejects an out-of-range raw chest weight");
    csb_v1_runtime_cleanup(&profile);
    ...
    check(csb_v1_runtime_recover_csbwin_chest_base_weight(
              &profile, &weight) == 0 && weight == -1,
          "CSBWin rejects an absent ObjectWeights owner without defaulting");
    csb_v1_runtime_cleanup(&profile);
