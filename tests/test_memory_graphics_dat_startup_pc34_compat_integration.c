...
static int test_startup_chain_initializes_runtime_preloads_dialog_and_loads_viewport(void) {
    const char* path = "./test_graphics_dat_startup.bin";
    ...
    if (!startup.runtimeInitialized || !startup.dialogPreloaded || !startup.viewportLoaded) {
        F0479_MEMORY_FreeStartupGraphicsChain_Compat(&startup);
        return 0;
    }
    if (startup.runtimeState.graphicCount != 2 || startup.runtimeState.fileSize != 28) {
        F0479_MEMORY_FreeStartupGraphicsChain_Compat(&startup);
        return 0;
    }
    if (startup.specials.dialogBoxGraphic == 0 || startup.specials.dialogBoxAllocatedByteCount != 25 || startup.specials.dialogBoxLoadedByteCount != 5) {
        F0479_MEMORY_FreeStartupGraphicsChain_Compat(&startup);
        return 0;
    }
    if (!startup.viewportTransaction.graphicsOpened || !startup.viewportTransaction.graphicsClosed || !startup.viewportTransaction.session.usedViewportBuffer) {
        F0479_MEMORY_FreeStartupGraphicsChain_Compat(&startup);
        return 0;
    }
    if (startup.viewportSelection.offset != 20 || startup.viewportSelection.compressedByteCount != 3 || startup.viewportSelection.widthHeight.Width != 7 || startup.viewportSelection.widthHeight.Height != 8) {
        F0479_MEMORY_FreeStartupGraphicsChain_Compat(&startup);
        return 0;
    }
    if (gApplyCallCount != 1 || gLastLoadedByteCount != 3) {
        F0479_MEMORY_FreeStartupGraphicsChain_Compat(&startup);
        return 0;
    }
    for (i = 0; i < 3; i++) {
...
int main(void) {
    if (!test_startup_chain_initializes_runtime_preloads_dialog_and_loads_viewport()) {
        fprintf(stderr, "test_startup_chain_initializes_runtime_preloads_dialog_and_loads_viewport failed\n");
        return 1;
    }
    printf("ok\n");
    return 0;
