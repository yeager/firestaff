...
int main(void)
{
    ...
    if (dm2_v1_runtime_bind_weather_distant_environment(&slot, 1u) != 1 ||
        dm2_v1_runtime_bind_weather_distant_environment(NULL, 0u) != 1) {
        fputs("FAIL: runtime did not admit/clear source weather slots\n", stderr);
        return 1;
    }
    ...
    puts("PASS: runtime weather slot gate fails closed without source receipts");
    return 0;
}
