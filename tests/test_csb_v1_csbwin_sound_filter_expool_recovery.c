/* CSBWin Sound.cpp SoundFilter raw DB11 recovery regression. */
...
int main(void)
{
    enum { record_id = (3u << 24) | 8u,
           block_bytes = CSB_V1_CSBWIN_EXPOOL_BLOCK_BYTES };
    const uint32_t source_location = 0x00019a2du;
    ...
    check(csb_v1_runtime_recover_csbwin_sound_filter_location(&profile, &location) == 1 &&
              location == source_location && profile.csbwin_appended_tail_fnv1a == receipt,
          "CSBWin recovers the raw SoundFilter location without mutation");
    profile.csbwin_appended_tail[0] ^= 1u;
    check(csb_v1_runtime_recover_csbwin_sound_filter_location(&profile, &location) == 0,
          "CSBWin rejects a drifted SoundFilter receipt");
    ...
    prepare(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_sound_filter_location(&profile, &location) == 0,
          "CSBWin rejects duplicate SoundFilter records");
    csb_v1_runtime_cleanup(&profile);
    ...
    prepare(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_sound_filter_location(&profile, &location) == 0,
          "CSBWin rejects an overlong SoundFilter record");
    csb_v1_runtime_cleanup(&profile);

    memset(tail, 0, sizeof(tail));
    prepare(&profile, tail, sizeof(tail));
    check(csb_v1_runtime_recover_csbwin_sound_filter_location(&profile, &location) == 0,
          "CSBWin rejects an absent SoundFilter record without defaulting");
    csb_v1_runtime_cleanup(&profile);
