...
{
    struct DM1_Event_V1 timeline_event;

    memset(&timeline_event, 0, sizeof(timeline_event));
    timeline_event.type = DM1_EVENT_MOVE_PROJECTILE;
    timeline_event.map_time = DM1_MAP_TIME_MAKE(event->mapIndex,
                                                 event->fireAtTick);
    timeline_event.priority = (unsigned char)event->aux0;
    timeline_event.b_mapX = (unsigned char)event->mapX;
    timeline_event.b_mapY = (unsigned char)event->mapY;
    timeline_event.c_cell = (unsigned char)event->cell;
    timeline_event.c_effect = (unsigned char)event->aux3;
    CHECK(csb_v1_runtime_add_timeline_event(profile, &timeline_event) >= 0,
          "C49 move is queued through the live CSB timeline");
}
...
int main(void)
{
    ...
    put_le16(raw, 82, DM1_SENSOR_FLOOR_THERON_PARTY_CREATURE_OBJECT);
    put_le16(raw, 84, (unsigned short)(DM1_EFFECT_HOLD << 3));
    ...
    CHECK(F0810_PROJECTILE_Create_Compat(&input, &profile.projectiles,
          ...
          "C49 projectile owns a real-format associated object");
    queue_projectile_move(&profile, &first_move);
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "first C49 boundary advances the live projectile");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "second C49 boundary materializes the associated object");
    CHECK(profile.projectiles.count == 0,
          "wall impact consumes the projectile before object routing");
    CHECK(get_le16(raw, 66) == (unsigned short)(3u << 10) &&
              get_le16(raw, 80) == (unsigned short)(1u << 10) &&
              get_le16(raw, 72) == 0xfffeu,
          "C001 and C05 remain at the object source square");
    CHECK(get_le16(raw, 68) == (unsigned short)(5u << 10) &&
              get_le16(raw, 88) == 0xfffeu,
          "F0267 routes the object through the object-scope C05 target");
    CHECK(profile.timeline_queue.eventCount == 1 &&
          ...
          "F0276 C001 add SET then source-unlink CLEAR follows the source order");
