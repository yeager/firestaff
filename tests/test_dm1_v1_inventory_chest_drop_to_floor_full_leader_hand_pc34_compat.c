...
static int click_chest_slot(M11_GameViewState* state, int chestOrdinal)
{
    ...
    if (!dm1_v1_inventory_chest_slot_box_zone_xywh_pc34(chestOrdinal, &x, &y, &w, &h)) {
        return 0;
    }
    ...
    DM1_V1_InventoryChestDropToFloorFullLeaderHandProbePc34* out)
{
    ...
    if (!DM1_V1_M11Runtime_SetLeaderHandObjectPc34Compat(&state, leaderThing)) {
        return 0;
    }
    ...
    out->leaderHandBeforeEye = DM1_V1_M11Runtime_GetLeaderHandThingPc34Compat(&state);
    ...
    out->leaderHandAfterEye = DM1_V1_M11Runtime_GetLeaderHandThingPc34Compat(&state);
...
static void test_empty_hand_eye_does_not_populate_floor(void)
{
    ...
    expect_ulong("negative open chest",
                 (unsigned long)DM1_V1_M11Runtime_OpenActionHandChestPc34Compat(&state),
                 1, A_F0333);
    expect_ulong("negative leader hand starts empty",
                 (unsigned long)(DM1_V1_M11Runtime_GetLeaderHandThingPc34Compat(&state) ==
                                 THING_NONE),
                 1, A_F0302);
