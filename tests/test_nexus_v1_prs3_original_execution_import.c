...
static void fixture(Nexus_V1_Prs3Vdp1CaptureReceipt *t, Nexus_V1_Prs3Vdp1CaptureBindingReceipt *b, Nexus_V1_Prs3OriginalExecutionAuthentication *a, const uint8_t *raw, size_t size)
{
    ...
    a->valid=a->mednafen_saturn_debugger_export=a->independently_authenticated_original_saturn_execution=1; a->trace_export_fnv1a64=fnv(raw,size); if(firestaff_x68k_media_receipt_sha256_hex(raw,size,a->trace_export_sha256,sizeof(a->trace_export_sha256))!=0) a->valid=0;
}
int main(void)
{
    ...
    fixture(&t,&b,&a,raw,sizeof(raw)-1U);a.trace_export_sha256[0]='0';if(nexus_v1_prs3_original_execution_evidence_import(&in,&r))return 1;
