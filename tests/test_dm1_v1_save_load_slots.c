...
static DM1_V1_SaveLoadHeaderPc34 make_header(uint16_t level, int16_t x, int16_t y,
                                     uint32_t game_time) {
...
int main(void) {
    char dir[512];
    DM1_V1_SaveLoadStatePc34 state;
    DM1_V1_SaveLoadStatePc34 scanned;
    DM1_V1_SaveLoadStatePc34 uninformed;
    DM1_V1_SaveLoadHeaderPc34 header;
    DM1_V1_SaveLoadHeaderPc34 loaded_header;
    uint8_t data_a[] = { 1, 2, 3, 4 };
    uint8_t data_b[] = { 9, 8, 7 };
    uint8_t out[4] = { 0, 0, 0, 0 };
    size_t actual = 0;

    if (DM1_V1_SaveLoad_SourceRuntimeSlotCountPc34Compat() != 1 ||
        !DM1_V1_SaveLoad_SourceRuntimeSlotSupportedPc34Compat(0) ||
        DM1_V1_SaveLoad_SourceRuntimeSlotSupportedPc34Compat(1) ||
        DM1_V1_SaveLoad_SourceRuntimeSlotSupportedPc34Compat(DM1_SL_MAX_SLOTS - 1)) {
    ...
    DM1_V1_SaveLoad_InitPc34Compat(&state, dir);

    header = make_header(3, 10, 11, 1000);
    if (!DM1_V1_SaveLoad_SavePc34Compat(&state, 2, &header, data_a, sizeof(data_a))) {
        printf("FAIL: save slot 2 with data failed\n");
        cleanup_slot_dir(dir);
        return 1;
    }
    if (!expect_int("slot count after first save", state.slot_count, 1) ||
        !DM1_V1_SaveLoad_SlotOccupiedPc34Compat(&state, 2)) {
        cleanup_slot_dir(dir);
        return 1;
    }

    header = make_header(5, 12, 13, 2000);
    if (!DM1_V1_SaveLoad_SavePc34Compat(&state, 2, &header, data_b, sizeof(data_b))) {
    ...
    if (!DM1_V1_SaveLoad_LoadHeaderPc34Compat(&state, 2, &loaded_header) ||
        !expect_int("overwritten level", loaded_header.current_level, 5) ||
        !expect_int("overwritten data size", loaded_header.data_size, 3)) {
        cleanup_slot_dir(dir);
        return 1;
    }
    if (!DM1_V1_SaveLoad_LoadDataPc34Compat(&state, 2, out, sizeof(out), &actual) ||
    ...
    actual = 0;
    if (DM1_V1_SaveLoad_LoadDataPc34Compat(&state, 2, out, 2, &actual) ||
    ...
    if (!DM1_V1_SaveLoad_SavePc34Compat(&state, 5, &header, NULL, 0)) {
        printf("FAIL: header-only slot save failed\n");
        cleanup_slot_dir(dir);
        return 1;
    }
    if (!expect_int("slot count after header-only save", state.slot_count, 2) ||
        !DM1_V1_SaveLoad_LoadHeaderPc34Compat(&state, 5, &loaded_header) ||
        !expect_int("header-only data size", loaded_header.data_size, 0)) {
        cleanup_slot_dir(dir);
        return 1;
    }

    DM1_V1_SaveLoad_InitPc34Compat(&scanned, dir);
    if (!DM1_V1_SaveLoad_ScanSlotsPc34Compat(&scanned) ||
        !expect_int("scanned slot count", scanned.slot_count, 2) ||
        !DM1_V1_SaveLoad_SlotOccupiedPc34Compat(&scanned, 2) ||
        !DM1_V1_SaveLoad_SlotOccupiedPc34Compat(&scanned, 5)) {
    ...
    if (!DM1_V1_SaveLoad_SavePc34Compat(&state, 7, &header, data_a, sizeof(data_a))) {
        printf("FAIL: save slot 7 for uninformed delete failed\n");
        cleanup_slot_dir(dir);
        return 1;
    }
    DM1_V1_SaveLoad_InitPc34Compat(&uninformed, dir);
    if (!DM1_V1_SaveLoad_DeletePc34Compat(&uninformed, 7) ||
        !expect_int("uninformed delete slot count", uninformed.slot_count, 0)) {
        cleanup_slot_dir(dir);
        return 1;
    }

    if (!DM1_V1_SaveLoad_DeletePc34Compat(&state, 2) || DM1_V1_SaveLoad_SlotOccupiedPc34Compat(&state, 2)) {
