/* Source: skproject SKWIN/SkWinCore.cpp DRAW_ITEM_IN_HAND,
...
static int fetch_carried_item_map_chip(void *user,
                                       int gdat_index,
...
static int fetch_carried_item_local_palette(void *user,
                                            int gdat_index,
                                            ...
                                            uint32_t *out_hash)
{
    ...
    if (out_hash) *out_hash = 0x43415252u;
    return 0;
}
...
static void setup_carried_item(DM2_V1_ViewportState *viewport,
                               uint8_t *framebuffer,
                               FetchTrace *trace,
                               int bind_palette)
{
    ...
    viewport->carried_item.screen_x = 96;
    viewport->carried_item.screen_y = 88;
    dm2_v1_viewport_set_asset_provider(
        viewport, fetch_carried_item_map_chip, trace);
    if (bind_palette) {
        dm2_v1_viewport_set_asset_palette_provider(
            viewport, fetch_carried_item_local_palette, trace);
    }
...
int main(void)
{
    ...
    setup_carried_item(&viewport, framebuffer, &trace, 1);
    dm2_v1_render_carried_item(&viewport);
    CHECK("carried item consumes its matching source IMG3 palette",
          trace.asset_fetches == 1 && trace.palette_fetches == 1 &&
              ...
              framebuffer_contains(framebuffer, 0x61u) &&
              ...
              (viewport.blocked_material_mask &
               DM2_V1_VIEWPORT_BLOCKED_MATERIAL_CARRIED_ITEM) == 0u);

    memset(framebuffer, 0x7e, sizeof(framebuffer));
    memset(&trace, 0, sizeof(trace));
    setup_carried_item(&viewport, framebuffer, &trace, 0);
    dm2_v1_render_carried_item(&viewport);
    CHECK("source-required carried item fails closed without its palette",
    ...
    printf("DM2 carried-item local palette gate: %d/%d passed\n",
           passed, checks);
