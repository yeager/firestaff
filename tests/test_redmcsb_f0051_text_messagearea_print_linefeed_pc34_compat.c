...
int main(void)
{
    ...
    ok &= check(F0051_TEXT_MESSAGEAREA_PrintLineFeed_PC34(
                    capture_print_message, &capture),
                "accepts a print-message delegate");
    ok &= check(capture.calls == 1 && capture.color == 0 &&
                    strcmp(capture.message, "\n") == 0,
                "delegates one black line feed exactly as TEXT.C F0051");

    ok &= check(!F0051_TEXT_MESSAGEAREA_PrintLineFeed_PC34(NULL, &capture),
    ...
    puts("PASS redmcsb_f0051_text_messagearea_print_linefeed_pc34_compat");
    return 0;
}
