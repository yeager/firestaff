...
static int test_first_frame_mini_dispatches_once_prerequisites_are_ready(void) {
    const char* path = "./test_graphics_dat_first_frame.bin";
    struct MemoryGraphicsDatState_Compat fileState = {0};
    struct MemoryGraphicsDatFirstFrameResult_Compat frame;
    ...
    if (!frame.dispatchReady || !frame.firstFrameAttempted || frame.stage != MEMORY_GRAPHICS_DAT_FRAME_STAGE_FIRST_FRAME_ATTEMPTED) {
        F0479_MEMORY_FreeFirstFrameMini_Compat(&frame);
        return 0;
    }
    if (!frame.startup.firstFramePrerequisitesReady || frame.startup.boot.stage != MEMORY_GRAPHICS_DAT_BOOT_STAGE_COMPLETE) {
        F0479_MEMORY_FreeFirstFrameMini_Compat(&frame);
        return 0;
    }
...
int main(void) {
    if (!test_first_frame_mini_dispatches_once_prerequisites_are_ready()) {
        fprintf(stderr, "test_first_frame_mini_dispatches_once_prerequisites_are_ready failed\n");
        return 1;
    }
    printf("ok\n");
    return 0;
