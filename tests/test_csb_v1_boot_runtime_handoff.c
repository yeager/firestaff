...
static void test_utility_flow_load_game_requires_original_corpus(void)
{
    CSB_V1_UtilFlowContext ctx;
    ...
    CHECK(csb_v1_util_flow_step(&ctx) == 0 &&
          ...
          "Utility LOAD verifies its disk before choosing an action");
    CHECK(csb_v1_util_flow_step(&ctx) == 0 &&
              ctx.state == CSB_V1_UTIL_FLOW_DISK_OK,
          "Utility LOAD accepts verified Utility media");
    CHECK(csb_v1_util_flow_step(&ctx) == 0 &&
              ctx.state == CSB_V1_UTIL_FLOW_SELECT_ACTION,
          "Utility LOAD reaches the source action menu");
    csb_v1_util_flow_set_action(&ctx, CSB_V1_UTIL_ACTION_LOAD);
    CHECK(csb_v1_util_flow_step(&ctx) == 0 &&
              ctx.state == CSB_V1_UTIL_FLOW_LOAD_GAME,
          "Utility LOAD enters its file-selection state");
    CHECK(csb_v1_util_flow_step(&ctx) == 0 &&
              ctx.state == CSB_V1_UTIL_FLOW_ERROR &&
              ctx.last_error == -9 &&
              strstr(csb_v1_util_flow_last_error(&ctx), "Original CSB") != NULL,
          "Utility LOAD rejects private runtime saves without original corpus evidence");
}
...
int main(void)
{
    ...
    test_utility_flow_load_game_requires_original_corpus();
    printf("\nPASSED: %d\nFAILED: %d\n", passed, failed);
    if (failed == 0) {
