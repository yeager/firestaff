...
int main(void)
{
    ...
    put_le16(raw, 78, DM1_EFFECT_TOGGLE);
    ...
    CHECK(F0810_PROJECTILE_Create_Compat(&input, &profile.projectiles,
          ...
          "C49 projectile owns an object for C004 local-effect placement");
    queue_projectile_move(&profile, &first_move);
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "first C49 boundary advances the projectile");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "second C49 boundary materializes the object on local C004");
    CHECK(profile.timeline_queue.eventCount == 0,
          "C004 LocalEffect does not queue an F0268 remote event");
    CHECK(get_le16(raw, 66) == (unsigned short)((3u << 10) | 1u) &&
              get_le16(raw, 80) == (unsigned short)(3u << 10) &&
              get_le16(raw, 72) == (unsigned short)(5u << 10),
          "C004 LocalEffect TOGGLE rotates the complete source sensor run");
