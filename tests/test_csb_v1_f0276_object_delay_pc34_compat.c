...
int main(void)
{
    ...
    put_le16(raw, 76, (unsigned short)((DM1_EFFECT_SET << 3) | (3u << 7)));
    put_le16(raw, 78, sensor_target(0, 0, 0));
    ...
    CHECK(F0810_PROJECTILE_Create_Compat(&input, &profile.projectiles,
          ...
          "C49 projectile owns an object for delayed C004 placement");
    queue_projectile_move(&profile, &first_move);
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "first C49 boundary advances the projectile");
    CHECK(csb_v1_runtime_tick_v1(&profile) == 1,
          "second C49 boundary materializes the object on delayed C004");
    ...
    CHECK(profile.timeline_queue.eventCount == 1 &&
              ...
              profile.timeline_queue.events[0].map_time == DM1_MAP_TIME_MAKE(0, 4u),
          "C004 Value delays F0268 by three source ticks from game time one");
