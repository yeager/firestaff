...
static void test_source_evidence_is_pinned(void)
{
    ...
    CHECK(evidence != 0);
    CHECK(strstr(evidence, "F0380:2159-2161") != 0);
    CHECK(strstr(evidence, "F0367") != 0);
    CHECK(strstr(evidence, "F0280:124-132") != 0);
    CHECK(strstr(evidence, "F0282:744-806") != 0);
    CHECK(strstr(evidence, "C012") != 0);
    CHECK(strstr(evidence, "C015") != 0);
}

static void test_spec_is_stable(void)
{
    const DM1_V1_MirrorCandidateC040StatusBoxClickWhilePanelLiveSpecPc34 *spec =
    ...
    CHECK(spec->statusBoxClickChampion0 == 12);
    CHECK(spec->statusBoxClickChampion3 == 15);
    CHECK(spec->nonOverlap != 0);
    CHECK(strstr(spec->nonOverlap, "status-box-click-while-c040-live") != 0);
    CHECK(strstr(spec->nonOverlap, "C111 action-area") != 0);
    CHECK(strstr(spec->nonOverlap, "Disjoint from pass787") != 0);
}
...
static void test_init_clears_observability(void)
{
    ...
    CHECK(state.f0367CallCount == 0);
    CHECK(state.rejectedWhileLiveCount == 0);
    CHECK(state.rejectedOutOfRangeCount == 0);
...
int main(void)
{
    ...
    printf("dm1_v1_mirror_candidate_c040_status_box_click_while_panel_live: "
