...
static void test_source_evidence_is_pinned(void)
{
    ...
    CHECK(evidence != 0);
    CHECK(strstr(evidence, "F0380:2181-2183") != 0);
    CHECK(strstr(evidence, "F0355:2299-2318") != 0);
    CHECK(strstr(evidence, "F0280:124-132") != 0);
    CHECK(strstr(evidence, "F0282:744-806") != 0);
    CHECK(strstr(evidence, "C007") != 0);
    CHECK(strstr(evidence, "C011") != 0);
}

static void test_spec_is_stable(void)
{
    const DM1_V1_MirrorCandidateC040InventoryToggleWhilePanelLiveSpecPc34 *spec =
    ...
    CHECK(spec->inventoryToggleChampion0 == 7);
    CHECK(spec->inventoryToggleChampion3 == 10);
    CHECK(spec->closeInventory == 11);
    CHECK(spec->nonOverlap != 0);
    CHECK(strstr(spec->nonOverlap, "inventory-toggle-while-c040-live") != 0);
    CHECK(strstr(spec->nonOverlap, "C140 save") != 0);
}
...
{
    DM1_V1_MirrorCandidateC040InventoryToggleWhilePanelLiveResultPc34 r;
    int ok =
    ...
    CHECK(r.f0355CallsWhileLive == 0);
    CHECK(r.rejectedWhileLive == 5);
    CHECK(r.panelContentAfterLiveToggles == 568);
    CHECK(r.candidateOrdinalAfterLiveToggles == 3);
    CHECK(r.f0282Dispatched == 1);
    CHECK(r.f0355CallsAfterClear == 1);
    CHECK(r.rejectedAfterClear == 5);
    CHECK(r.panelContentAfterClear == 0);
    CHECK(r.candidateOrdinalAfterClear == 0);
}

static void test_run_gates_inventory_while_panel_live(void)
{
    DM1_V1_MirrorCandidateC040InventoryToggleWhilePanelLiveResultPc34 r;
    int ok;
    dm1_v1_mirror_candidate_c040_inventory_toggle_while_panel_live_run_pc34(
        &r);
    ok = (r.f0355CallsWhileLive == 0 &&
          r.rejectedWhileLive == 5 &&
...
int main(void)
{
    ...
    printf("dm1_v1_mirror_candidate_c040_inventory_toggle_while_panel_live: "
