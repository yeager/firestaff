...
static int test_defrag_entry_hit_bypasses_defrag_allocator(void) {
    const char* path = "./test_graphics_dat_defrag_entry.bin";
    struct MemoryGraphicsDatState_Compat fileState = {0};
    struct MemoryGraphicsDatRuntimeState_Compat runtimeState;
    struct MemoryGraphicsDatDefragEntryResult_Compat result;
    ...
    if (F0489_MEMORY_GetOrLoadBitmapWithDefragEntry_Compat(
            path,
            &runtimeState,
            &fileState,
            1,
    ...
    if (result.normalizedRequestedSize != 0 || result.defragApplied != 0 || allocatorState.cacheBytesAvailable != 50) {
        F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState);
        return 0;
    }
...
static int test_defrag_entry_miss_requests_defrag_then_loads(void) {
    const char* path = "./test_graphics_dat_defrag_entry.bin";
    struct MemoryGraphicsDatState_Compat fileState = {0};
    struct MemoryGraphicsDatRuntimeState_Compat runtimeState;
    struct MemoryGraphicsDatDefragEntryResult_Compat result;
    ...
    struct MemoryCacheUnusedBlock_Compat splitRemainder = {0};
    struct MemoryCacheDefragResultState_Compat defragState = {80, 0};
    ...
    if (result.normalizedRequestedSize != 26 || result.defragApplied != 1 || !result.allocator.defragmentRequested || !result.allocator.allocatedFromTop) {
        F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState);
        return 0;
    }
    if (allocatorState.cacheBytesAvailable != 74 || allocatorState.cacheMemoryTopOffset != 90 || firstUnusedBlock != 0) {
        F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState);
        return 0;
    }
    if (gBuildCallCount != 1 || gApplyCallCount != 0 || gPrepareCallCount != 1 || indices[1] != 0) {
        F0479_MEMORY_FreeGraphicsDatState_Compat(&runtimeState);
        return 0;
    }
    for (i = 0; i < 5; i++) {
...
int main(void) {
    if (!test_defrag_entry_hit_bypasses_defrag_allocator()) {
        fprintf(stderr, "test_defrag_entry_hit_bypasses_defrag_allocator failed\n");
        return 1;
    }
    if (!test_defrag_entry_miss_requests_defrag_then_loads()) {
        fprintf(stderr, "test_defrag_entry_miss_requests_defrag_then_loads failed\n");
        return 1;
    }
    printf("ok\n");
    return 0;
