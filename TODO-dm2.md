...
- Replace the optional FFmpeg-only Macintosh QuickTime/MooV pixel decoder
  with bounded native decoders for the authenticated retail streams:
  Cinepak (`cvid`) + `twos` PCM for `Title.MooV`, QuickTime Animation
  (`rle `) + `raw ` PCM for `Swoosh.MooV`, and Cinepak + `raw ` PCM for
  `Credits.MooV` and `Ending.MooV`. The native atom/sample-table reader now
  proves the exact spans in the retained original view; the normal build must
  decode those spans in memory with no host codec or runtime dependency before
