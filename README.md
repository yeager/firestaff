...
![Theron's Quest Track 02 authenticated VDC/VCE capture](verification-screens/theron-track02-dungeon-capture.png)

Theron's Quest: the first published real-data capture is a 256×224
screen-space frame reconstructed from the hash-verified Track 02 VRAM/VCE
snapshot. It is an original-media capture artifact, not a synthetic render;
the dungeon-square, perspective and HUD consumers remain explicitly marked as
bring-up work.
...
The README capture is generated by the source-bound
`firestaff_theron_v1_authenticated_capture_bmp` route from the authenticated
VRAM/VCE files; it does not claim full gameplay or perspective parity.
