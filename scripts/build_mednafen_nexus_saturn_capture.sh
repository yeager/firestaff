...
sh2_ram_read_patch_id='FIRESTAFF_NEXUS_SH2_RAM_READ_TRACE_V3_LIVE_REGISTERS'
...
sh2_ram_write_patch_id='FIRESTAFF_NEXUS_SH2_RAM_WRITE_TRACE_V3_FRAME_WINDOW'
...
vdp2_trace_patch_id='FIRESTAFF_NEXUS_VDP2_WRITE_TRACE_V14_REGISTER_PR'
if [[ ! -f "$vdp2_trace_marker" ]]; then
  ...
  patch -d "$source_dir" -p0 < "$repo_root/scripts/mednafen_1.32.1_nexus_vdp2_writer_register_call.patch"
...
smpc_read_trace_patch_id='FIRESTAFF_NEXUS_SMPC_READ_TRACE_V4_LIVE_REGISTERS'
...
elif [[ "$(cat "$smpc_read_trace_marker" 2>/dev/null)" != "$smpc_read_trace_patch_id" ]]; then
