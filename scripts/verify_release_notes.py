...
AUDIENCE_SECTIONS = ("User-facing changes", "Developer changes")
GENERIC_TERMS = (
    "various",
    "miscellaneous",
    "improvements",
...
def verify_audience_section(lines: list[str], heading: str) -> int:
    """Validate the release-note sections readers see as product audiences."""
    try:
        start = lines.index(heading) + 1
    except ValueError:
        return 0
    ...
    for line in lines[start:end]:
        ...
        if line.startswith("- "):
            ...
            fail(f"{heading!r} may contain only bullet entries")
    if not entries:
        fail(f"{heading!r} must contain at least one concrete delta")
    ...
    for entry in entries:
        ...
        if not re.match(r"`[^`]+`:\s+\S", body):
            fail(
                f"{heading!r} must name the affected feature or subsystem in backticks: "
                f"{entry!r}"
            )
        change_words = tuple(word for words in ACTION_WORDS.values() for word in words)
        if not any(word in lower for word in change_words):
            fail(f"{heading!r} must state what changed, not only its result: {entry!r}")
    return len(entries)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="require at least one explicit user-facing or developer-facing section",
    )
    ...
    audience_entry_count = 0
    for name in AUDIENCE_SECTIONS:
        audience_entry_count += verify_audience_section(section, f"## {name}")
    entry_count += audience_entry_count
    if entry_count == 0:
        fail("must contain at least one concrete functional delta")
    if args.require_audience_sections and audience_entry_count == 0:
        fail(
            "must include at least one of '## User-facing changes' or "
            "'## Developer changes' with concrete bullets"
        )
