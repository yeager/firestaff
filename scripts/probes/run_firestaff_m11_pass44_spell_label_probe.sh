...
OUT_DIR=${1:-$HERE/verification-m11/pass44-spell-labels}
GRAPHICS_DAT=${2:-$HOME/.firestaff/data/GRAPHICS.DAT}
mkdir -p "$OUT_DIR"

PROBE_BIN="$HERE/firestaff_m11_pass44_spell_label_probe_bin"
LOG="$OUT_DIR/pass44_spell_label_probe.log"
...
cc $CFLAGS_COMMON \
    -o "$PROBE_BIN" \
    "$HERE/firestaff_m11_pass44_spell_label_probe.c" \
...
echo "Pass 44 spell-label probe: $SUMMARY"
