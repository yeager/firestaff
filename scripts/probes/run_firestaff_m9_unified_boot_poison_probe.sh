...
cc -std=c99 -Wall -Wextra -pedantic -DCOMPILE_H '-DSTATICFUNCTION=static' '-DSEPARATOR=,' '-DFINAL_SEPARATOR=)' \
  -I$HERE \
  $HERE/firestaff_m9_unified_boot_poison_probe.c \
  ...
  -o $HERE/firestaff_m9_unified_boot_poison_probe
if [ $# -eq 0 ]; then
  echo "built: $HERE/firestaff_m9_unified_boot_poison_probe"
  exit 0
fi
$HERE/firestaff_m9_unified_boot_poison_probe "$@"
