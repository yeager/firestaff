...
OUT_DIR=${2:-$HERE/verification-m10/runtime-dynamics}
ROOT=$HERE

mkdir -p "$OUT_DIR"

PROBE_BIN="$ROOT/firestaff_m10_runtime_dynamics_probe_bin"

cc -Wall -Wextra -O1 \
    -I "$ROOT" \
    -o "$PROBE_BIN" \
    "$ROOT/firestaff_m10_runtime_dynamics_probe.c" \
...
echo "Runtime dynamics probe complete. Output: $OUT_DIR"
