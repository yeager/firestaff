...
OUT_DIR=${1:-$HERE/verification-m11/pass418-door-closing-obstruction}
mkdir -p "$OUT_DIR"
PROBE_BIN="$HERE/firestaff_m11_pass418_door_closing_obstruction_probe_bin"
LOG="$OUT_DIR/pass418_door_closing_obstruction_probe.log"
cc -std=c99 -Wall -Wextra -O1 \
    -I "$HERE" \
    -o "$PROBE_BIN" \
    "$HERE/firestaff_m11_pass418_door_closing_obstruction_probe.c" \
...
echo "Pass 418 door closing obstruction probe: $SUMMARY"
