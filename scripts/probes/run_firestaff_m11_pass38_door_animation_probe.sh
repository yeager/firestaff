...
OUT_DIR=${1:-$HERE/verification-m11/pass38-door-animation}
mkdir -p "$OUT_DIR"

PROBE_BIN="$HERE/firestaff_m11_pass38_door_animation_probe_bin"
LOG="$OUT_DIR/pass38_door_animation_probe.log"

cc -std=c99 -Wall -Wextra -O1 \
    -I "$HERE" \
    -o "$PROBE_BIN" \
    "$HERE/firestaff_m11_pass38_door_animation_probe.c" \
...
echo "Pass 38 door animation probe: $SUMMARY"
