...
[[ ${#plan_rows[@]} -eq 17 && ${plan_rows[0]} == THERON_TRACK02_DUNGEON_HANDOFF_CAPTURE_PLAN_V1 &&
   ${plan_rows[1]} == route=dungeon_handoff && ${plan_rows[14]} == payload_policy=opaque_only &&
   ${plan_rows[15]} == decoder_policy=forbidden && ${plan_rows[16]} == render_policy=no_draw ]] || {
...
[[ $track02_md5 =~ ^[0-9a-f]{32}$ && $artifact_path == "$artifact" ]] || {
    printf '%s\n' 'REJECTED: artifact path or Track 02 identity does not match the local plan' >&2; exit 1;
}
...
[[ ${rows[2]} == "mednafen_trace_md5=$trace_md5" && ${rows[3]} == campaign_route=2 ]] || {
