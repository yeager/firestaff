/*
 * csb_v1_csbgraphics_runtime_binding.h
 *
 * Data-free readiness contract for the future CSBgraphics.dat
 * decoded-payload -> runtime render handoff. This does not decode
 * CSBgraphics.dat LZW payloads and does not install a runtime
 * override. It only defines the narrow, bounds-checked shape a
 * trusted decoded override must satisfy before the runtime viewport or
 * HUD lanes may consume it. The companion apply helper copies an
 * already trusted indexed bitmap into the runtime 320x200 framebuffer
 * according to that binding decision. The decode-entry helper can
 * also bridge one caller-selected CSBgraphics.dat entry through the
 * existing LZW decoder when the caller supplies the expected indexed
 * bitmap dimensions; discovery and route selection still live
 * outside this module.
 */

#ifndef FIRESTAFF_CSB_V1_CSBGRAPHICS_RUNTIME_BINDING_READINESS_H
#define FIRESTAFF_CSB_V1_CSBGRAPHICS_RUNTIME_BINDING_READINESS_H

#include "csb_v1_csbgraphics_dat_classify.h"

#include <stddef.h>
#include <stdint.h>

#ifdef __cplusplus
extern "C" {
#endif

#define CSB_V1_CSBGRAPHICS_RUNTIME_SOURCE_W 320
#define CSB_V1_CSBGRAPHICS_RUNTIME_SOURCE_H 200
#define CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_X 0
#define CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_Y 33
#define CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_W 224
#define CSB_V1_CSBGRAPHICS_RUNTIME_VIEWPORT_H 136
#define CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_X 80
#define CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_Y 52
#define CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_W 144
#define CSB_V1_CSBGRAPHICS_RUNTIME_C040_PANEL_H 73
#define CSB_V1_CSBGRAPHICS_RUNTIME_PALETTE_MAX 15u

typedef enum {
    CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_NONE = 0,
    CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_VIEWPORT_DERIVED = 1,
    CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_INVENTORY = 2,
    CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_HUD_RESURRECT_PANEL = 3,
    CSB_V1_CSBGRAPHICS_RUNTIME_ROUTE_VIEWPORT_CUSTOM_BACKGROUND = 4
} CSB_V1_CSBGraphicsRuntimeRoute;

typedef enum {
    CSB_V1_CSBGRAPHICS_RUNTIME_DECISION_FALLBACK_ORIGINAL = 0,
    CSB_V1_CSBGRAPHICS_RUNTIME_DECISION_BIND_OVERRIDE = 1
} CSB_V1_CSBGraphicsRuntimeDecision;

typedef struct {
    uint32_t entry_index;
    uint16_t width;
    uint16_t height;
    uint8_t bits_per_pixel;
    uint8_t max_palette_index;
    int decoded_ok;
    int trusted;
    const uint8_t *indexed_pixels;
    size_t indexed_pixel_count;
} CSB_V1_CSBGraphicsDecodedBitmap;

typedef struct {
    CSB_V1_CSBGraphicsRuntimeDecision decision;
    CSB_V1_CSBGraphicsRuntimeRoute route;
    uint32_t entry_index;
    int destination_x;
    int destination_y;
    int destination_w;
    int destination_h;
    int decoded_w;
    int decoded_h;
    int needs_viewport_redraw;
    int needs_hud_redraw;
    int preserves_v1_palette_indices;
    const char *reason;
    const char *source_evidence;
} CSB_V1_CSBGraphicsRuntimeBinding;

int csb_v1_csbgraphics_runtime_prepare_binding(
    const CSB_V1_CSBGraphicsEntrySpan *span,
    const CSB_V1_CSBGraphicsDecodedBitmap *decoded,
    CSB_V1_CSBGraphicsRuntimeBinding *out_binding);

int csb_v1_csbgraphics_runtime_apply_binding(
    const CSB_V1_CSBGraphicsRuntimeBinding *binding,
    const CSB_V1_CSBGraphicsDecodedBitmap *decoded,
    uint8_t *framebuffer,
    int framebuffer_width,
    int framebuffer_height,
    int framebuffer_stride);

int csb_v1_csbgraphics_runtime_prepare_and_apply(
    const CSB_V1_CSBGraphicsEntrySpan *span,
    const CSB_V1_CSBGraphicsDecodedBitmap *decoded,
    uint8_t *framebuffer,
    int framebuffer_width,
    int framebuffer_height,
    int framebuffer_stride,
    CSB_V1_CSBGraphicsRuntimeBinding *out_binding);

/* ReDMCSB PANEL.C F0346 line 1632 draws C040 after F0346/F0352 has
 * expanded C017 into the viewport (line 2376).  Validate both trusted
 * original-data bindings before touching the framebuffer, then preserve that
 * source order so a rejected C040 can never leave a half-updated HUD. */
int csb_v1_csbgraphics_runtime_prepare_and_apply_hud_pair(
    const CSB_V1_CSBGraphicsEntrySpan *inventory_span,
    const CSB_V1_CSBGraphicsDecodedBitmap *inventory_decoded,
    const CSB_V1_CSBGraphicsEntrySpan *resurrect_span,
    const CSB_V1_CSBGraphicsDecodedBitmap *resurrect_decoded,
    uint8_t *framebuffer,
    int framebuffer_width,
    int framebuffer_height,
    int framebuffer_stride,
    CSB_V1_CSBGraphicsRuntimeBinding *out_inventory_binding,
    CSB_V1_CSBGraphicsRuntimeBinding *out_resurrect_binding);

int csb_v1_csbgraphics_runtime_decode_entry_and_apply(
    const uint8_t *csbgraphics_bytes,
    size_t csbgraphics_size,
    uint32_t entry_index,
    uint16_t decoded_width,
    uint16_t decoded_height,
    uint8_t *framebuffer,
    int framebuffer_width,
    int framebuffer_height,
    int framebuffer_stride,
    CSB_V1_CSBGraphicsRuntimeBinding *out_binding);

const char *csb_v1_csbgraphics_runtime_route_name(
    CSB_V1_CSBGraphicsRuntimeRoute route);

const char *csb_v1_csbgraphics_runtime_decision_name(
    CSB_V1_CSBGraphicsRuntimeDecision decision);

const char *csb_v1_csbgraphics_runtime_source_evidence(void);

#ifdef __cplusplus
}
#endif

#endif /* FIRESTAFF_CSB_V1_CSBGRAPHICS_RUNTIME_BINDING_READINESS_H */
