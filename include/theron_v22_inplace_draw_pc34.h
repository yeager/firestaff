/*
 * theron_v22_inplace_draw_pc34.h
 *
 * Theron V2.2 GPU render path: V22 modern-art IN-PLACE bitmap lookup.
 *
 ...
 *
 * Architecture:
 *   theron_v22_shape_cache_update -> per-cell V22 shape (params, variant)
 *     -> theron_v22_inplace_get_cell_bitmap(depth, lateral, &w, &h)
 *        -> variant -> asset_id in modern_asset_manifest.json
 *        -> asset_id -> file path (via theron_v22_get_shape_path)
 *        -> PNG decode to RGBA buffer (cached at init)
 *        -> return RGBA* + width + height
 *   theron_draw_* (9-square viewport) passes consult the bitmap and blit it instead of
 ...
 *
 * Source-lock: theron_v22_shape_cache_pc34.h (the cache),
 * theron_v22_modern_assets_pc34.c (manifest lookup),
 * theron_v22_render_overlay_pc34.c (sibling overlay path),
 * include/dm1_v2_shape_runtime_pc34.h (shape variant enum),
 * THQUEST.ASM T400/T520/T600 (Theron viewport).
 *
 * Module: src/dm1v2/theron_v22_inplace_draw_pc34.c
 * Test:   tests/test_theron_v22_inplace_draw_pc34.c
 */

#ifndef FIRESTAFF_Theron_V22_INPLACE_DRAW_PC34_H
#define FIRESTAFF_Theron_V22_INPLACE_DRAW_PC34_H
 ...
 * valid until theron_v22_inplace_draw_shutdown(). */
const uint32_t* theron_v22_inplace_get_cell_bitmap(int depth, int lateral,
                                                 int* out_w, int* out_h);
 ...
 * cut: walls all map to wall_dungeon_stone_01 (the most common carved
 * stone), floors map by tile pattern, creatures map by silhouette
 * tag. Per-cell refinement (e.g., mossy walls for slime zones) is
 * a follow-up. */
const char* theron_v22_inplace_get_cell_asset_id(int depth, int lateral);

/* theron_v22_inplace_render_pass — paints the cached V22 bitmaps into
 * the framebuffer at the Theron 4x3 cell rectangles (same coords as the
 ...
 *
 * This is the V22 in-place equivalent of theron_v22_render_overlay:
...
int theron_v22_inplace_render_pass(unsigned char* framebuffer, int fbW, int fbH);

/* Source evidence for tests/probes. */
const char* theron_v22_inplace_draw_source_evidence(void);

#ifdef __cplusplus
}
#endif

#endif /* FIRESTAFF_Theron_V22_INPLACE_DRAW_PC34_H */
