 ...
 *     -> nexus_v22_inplace_get_cell_bitmap(depth, lateral, &w, &h)
 *        -> variant -> asset_id in modern_asset_manifest.json
 *        -> asset_id -> file path (via nexus_v22_get_shape_path)
 *        -> PNG decode to RGBA buffer (cached at init)
 *        -> return RGBA* + width + height
 *   nexus_draw_* (9-square viewport) passes consult the bitmap and blit it instead of
 ...
 * nexus_v22_modern_assets_pc34.c (manifest lookup),
 * nexus_v22_render_overlay_pc34.c (sibling overlay path),
 * include/dm1_v2_shape_runtime_pc34.h (shape variant enum),
 * SATURN_DMDF T400/T600 (composition order).
 *
 * Module: src/dm1v2/nexus_v22_inplace_draw_pc34.c
 * Test:   tests/test_nexus_v22_inplace_draw_pc34.c
 */

#ifndef FIRESTAFF_NEXUS_V22_INPLACE_DRAW_PC34_H
#define FIRESTAFF_NEXUS_V22_INPLACE_DRAW_PC34_H
 ...
 * by ~/.firestaff/assets/nexus/modern/modern_asset_manifest.json into
 * RGBA buffers keyed by (category, asset_id). Call once at startup
 * after nexus_v22_set_manifest_path() and nexus_v22_validate_manifest().
 *
 ...
 * valid until nexus_v22_inplace_draw_shutdown(). */
 ...
 * the framebuffer at the Nexus 3x3 cell rectangles (same coords as the
 ...
 *
 * This is the V22 in-place equivalent of nexus_v22_render_overlay:
...
#endif /* FIRESTAFF_NEXUS_V22_INPLACE_DRAW_PC34_H */
