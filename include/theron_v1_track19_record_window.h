 ...
 * historical opaque window by one byte. The complete table is byte-identical
 * in the real US/JP Track 19 ISOs and is authenticated directly from the
 * selected media. The remaining window bytes stay opaque until their
 * consumer is proven by the original loader/disassembly. */
...
#define THERON_TRACK19_ITEM_PROPERTY_TABLE_FNV1A 0xB97787EFu
...
/* Return one record only after the complete real Track 19 table hash matches.
 * No inferred record is returned for a partial, mutated, or unrecognised ISO. */
