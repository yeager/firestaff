...
OUT_DIR=${1:-$HERE/verification-m11/pass54-song-runtime}
mkdir -p "$OUT_DIR"

PROBE_BIN="$OUT_DIR/firestaff_m11_pass54_song_runtime_probe_bin"
...
cc -std=c99 -Wall -Wextra -O2 -I "$HERE" \
    $SDL3_CFLAGS \
    -o "$PROBE_BIN" \
    "$HERE/probes/m11/firestaff_m11_pass54_song_runtime_probe.c" \
...
"$PROBE_BIN" | tee "$OUT_DIR/pass54_song_runtime_probe.log"
