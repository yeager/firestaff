...
static void m11_framebuffer_to_rgba_fade(const unsigned char* src,
                                         int logicalWidth,
                                         int logicalHeight,
                                         int specialPalette,
                                         int fadeStep,
    ...
    for (int i = 0; i < pixelCount; ++i) {
        ...
        unsigned char idx = raw & M11_FB_INDEX_MASK;
        int perPixelLevel = (raw & M11_FB_LEVEL_MASK) >> M11_FB_LEVEL_SHIFT;
        int level = perPixelLevel > 0 ? perPixelLevel : g_state.paletteLevel;
        const unsigned char* rgbSpecial;
        const unsigned char* rgbGame;
        int r, g, b;
        if (level >= M11_PALETTE_LEVELS) {
            level = M11_PALETTE_LEVELS - 1;
        }
        rgbSpecial = F9011_VGA_GetSpecialColorRgb_Compat(idx, (unsigned int)specialPalette);
        if (!rgbSpecial) {
            rgbSpecial = G9010_auc_VgaPaletteAll_Compat[level][idx];
        }
        rgbGame = G9010_auc_VgaPaletteAll_Compat[level][idx];
        /* Linear interpolation: step 0 = pure special, step N = pure game */
        r = rgbSpecial[0] + (rgbGame[0] - rgbSpecial[0]) * fadeStep / fadeSteps;
        g = rgbSpecial[1] + (rgbGame[1] - rgbSpecial[1]) * fadeStep / fadeSteps;
        b = rgbSpecial[2] + (rgbGame[2] - rgbSpecial[2]) * fadeStep / fadeSteps;
        dst[i * 4 + 0] = (unsigned char)(r < 0 ? 0 : r > 255 ? 255 : r);
        dst[i * 4 + 1] = (unsigned char)(g < 0 ? 0 : g > 255 ? 255 : g);
        dst[i * 4 + 2] = (unsigned char)(b < 0 ? 0 : b > 255 ? 255 : b);
        dst[i * 4 + 3] = 0xFF;
    }
}
...
int M11_Render_PresentIndexedWithPaletteFade(const unsigned char* framebuffer,
    ...
    g_state.contentW = logicalWidth;
    g_state.contentH = logicalHeight;
    m11_framebuffer_to_rgba_fade(framebuffer, logicalWidth, logicalHeight,
                                 specialPalette, fadeStep, fadeSteps);
    m11_compute_present_rect(&destX, &destY, &destW, &destH);
#if SDL_VERSION_ATLEAST(3, 0, 0)
    sourceRect.x = 0.0f;
    sourceRect.y = 0.0f;
