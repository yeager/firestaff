...
- `m11_apply_dm1_v1_pipeline_tick` tokens `EnqueueCommand` / `DecrementCooldowns` / `ProcessOneTick` / `world.gameTick++` / `viewportDirty-return` stay in order (current `m11_game_view.c:27255-27470`).
...
- Main-loop input redraw tokens `redrawWasAfterViewportDirty` / `lastDm1V1MovementPipelineResult.viewportDirty` / `M11_GameView_Draw` / `inputRedrawAfterViewportDirtyCount` stay in order (current `main_loop_m11.c:7056-7073`).
