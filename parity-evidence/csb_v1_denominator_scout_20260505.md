...
find <csb-source>/CSB/src -maxdepth 2 -type f
find <csbwin-source>/CSBWin -maxdepth 2 -type f
git -C <csb-source>/CSB/src log -1 --format="%H %ci %s"
git -C <csbwin-source>/CSBWin log -1 --format="%H %ci %s"

# Count/hash inventory
python3 - <<PY
from pathlib import Path
import hashlib
for root in [Path(<csb-source>/CSB/src), Path(<csbwin-source>/CSBWin)]:
